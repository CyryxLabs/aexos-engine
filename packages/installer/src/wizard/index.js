...
const { installSynapseDomains } = require('../installer/synapse-installer');
const {
  scaffoldCoreSquads,
...
async function installExtraCommands(projectRoot, transaction, copyOptions = {}) {
  try {
    const commandsResult = await copyExtraCommandFiles(
      projectRoot,
      copyOptions.sourceRoot,
      copyOptions,
    );
    transaction.newFiles.push(...(commandsResult.newFiles || []));
    transaction.overwrittenFiles.push(...(commandsResult.rollbackSnapshots || []));
    return commandsResult;
  } catch (error) {
    throw new Error(`Required Claude command installation failed: ${error.message}`);
  }
}
...
async function runWizard(options = {}) {
  ...
  try {
    ...
    if (nonInteractive) {
      ...
      // Interactive mode
      // Phase 1: select the language before constructing translated questions.
      // Reuse an existing Claude Code preference to keep reinstall idempotent.
      let languageAnswer;
      const existingLanguage = await getExistingLanguage();
      if (existingLanguage) {
        languageAnswer = { language: existingLanguage };
        setLanguage(existingLanguage);
        console.log(`\n✓ ${t('languageSkipped')}: ${existingLanguage}\n`);
      } else {
    ...
    // The package carries the contributor checkout's core-config.yaml so the
    // framework repository can intentionally run with protection disabled.
    // Remember whether the target already had its own config before that file
    // is copied; only a genuinely pre-existing file represents a user choice.
    const coreConfigExistedBeforeInstall = await fse.pathExists(
      path.join(process.cwd(), '.aexos-core', 'core-config.yaml'),
    );
    ...
    try {
      ...
      if (!cyryxCoreResult?.success) {
        throw new Error(`Required AEXOS core installation incomplete: ${(cyryxCoreResult?.errors || []).join('; ')}`);
      }
      if (cyryxCoreResult.success) {
    ...
    console.log('\n🧠 Installing SYNAPSE operational domains...');
    const synapseResult = await installSynapseDomains({
      targetDir: process.cwd(),
      transaction: installationRollback,
    });
    answers.synapseInstalled = true;
    answers.synapseResult = synapseResult;
    console.log(`✅ SYNAPSE: ${synapseResult.count} domains installed`);
    ...
    // generateIDEConfigs normally creates this surface. Keep an explicit
    // local-first pass here for install paths that defer IDE config writes.
    if ((answers.selectedIDEs || []).includes('grok') && !answers.grokSkillsGenerated) {
      console.log('\n🧠 Generating Grok Build surface...');
      const grokResult = generateGrokSkills(process.cwd());
      answers.grokSkillsGenerated = grokResult.files;
      answers.grokAgentsGenerated = grokResult.agents;
      answers.grokSkillsSkipped = false;
      console.log(`✅ Grok Build: ${grokResult.agents} agents → ${grokResult.files} files in .grok/`);
    }

    // Story INS-4.3: Copy extra commands (Gap #12)
    console.log('\n📋 Copying extra commands...');
    try {
      const commandsResult = await installExtraCommands(process.cwd(), installationRollback);
    ...
    if ((answers.selectedIDEs || []).includes('grok')) {
      ...
      try {
        ...
        const { validateGrok } = requireCyryxCoreModule(
          '.aexos-core', 'infrastructure', 'scripts', 'grok-skills-sync', 'validate',
        );
        const validation = validateGrok({
          projectRoot: targetProjectRoot,
          grokRoot: path.join(targetProjectRoot, '.grok'),
          strict: true,
          quiet: true,
    ...
    try {
      const envResult = await configureEnvironment({
        ...
        coreConfigExistedBeforeInstall,
      });
    ...
    // Generate boundary permissions only after environment setup has finalized
    // the installed core config. Fresh installs secure the package seed there;
    // existing projects retain their explicit frameworkProtection choice.
    ...
    try {
      ...
      const settingsContent = await fse.readFile(
        path.join(process.cwd(), '.claude', 'settings.json'),
        'utf8',
      ).catch(() => '{}');
      const settingsParsed = JSON.parse(settingsContent);
      const denyCount = settingsParsed.permissions?.deny?.length || 0;
    ...
    await rollbackInstallationArtifacts(installationRollback, error);
    if (error.isTtyError) {
...
async function rollbackInstallationArtifacts(transaction = {}, originalError) {
  const overwrittenFiles = Array.isArray(transaction.overwrittenFiles)
    ? transaction.overwrittenFiles
    : [];
  const newFiles = Array.isArray(transaction.newFiles) ? transaction.newFiles : [];
  const failures = [];

  for (const snapshot of [...overwrittenFiles].reverse()) {
    if (snapshot?.path && snapshot.content !== undefined) {
      try {
        await fse.outputFile(snapshot.path, snapshot.content);
      } catch (error) {
        failures.push({ operation: 'restore', path: snapshot.path, error });
      }
    }
  }
  ...
  for (const filePath of [...newFiles].reverse()) {
    try {
      ...
      failures.push({ operation: 'remove', path: filePath, error });
    }
  }

  if (failures.length) {
    const details = failures.map(item => `${item.operation} ${item.path}: ${item.error.message}`);
    const failure = new AggregateError(
      [...(originalError ? [originalError] : []), ...failures.map(item => item.error)],
      `${originalError ? originalError.message + '; ' : ''}Installation rollback incomplete: ${details.join('; ')}`,
      { cause: originalError },
    );
