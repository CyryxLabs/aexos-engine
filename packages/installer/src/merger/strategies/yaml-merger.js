...
class YamlMerger extends BaseMerger {
  ...
  async merge(sourceContent, targetContent, _options = {}) {
    ...
    // A reinstall that adds no configuration must not reserialize the user's
    // document: js-yaml would otherwise discard comments and formatting.
    const content = stats.added === 0 ? targetContent : yaml.dump(merged, {
      indent: 2,
      lineWidth: 120,
      noRefs: true,
      sortKeys: false,
