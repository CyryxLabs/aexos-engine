 ...
 *
 * @param {string} filePath - Relative file path from .aexos-core
 ...
 * Comprehensive validation of AEXOS Core installation with security hardening
 */
class PostInstallValidator {
  /**
   * Create a new PostInstallValidator instance
   *
   * @param {string} targetDir - Directory where AEXOS was installed (project root)
  ...
  constructor(targetDir, sourceDir = null, options = {}) {
    ...
    this.cyryxCoreTarget = path.join(this.targetDir, '.aexos-core');
    this.cyryxCoreSource = this.sourceDir ? path.join(this.sourceDir, '.aexos-core') : null;
  ...
  _getRealTargetDir() {
    if (this._realTargetDirCache === null) {
      try {
        this._realTargetDirCache = fs.realpathSync(this.cyryxCoreTarget);
      } catch {
  ...
  async loadManifest() {
    ...
    const sourceManifestPath = this.cyryxCoreSource
      ? path.join(this.cyryxCoreSource, 'install-manifest.yaml')
      : null;
    const targetManifestPath = path.join(this.cyryxCoreTarget, 'install-manifest.yaml');
  ...
  async validateFile(entry) {
    ...
    const absolutePath = path.resolve(this.cyryxCoreTarget, relativePath);
    ...
    if (!isPathContained(absolutePath, this.cyryxCoreTarget)) {
      ...
      if (error.code === 'ENOENT') {
        result.issue = {
          ...
          remediation: this.sourceDir
            ? "Run 'aexos validate --repair' to restore"
            : 'Re-run installation',
          relativePath,
        };
    ...
    // allowing path traversal attacks (e.g., /install/.aexos-core/symlinked-dir/../../../etc/passwd)
    //
    ...
    try {
      ...
      const expectedRelative = path.relative(this.cyryxCoreTarget, absolutePath);
  ...
  async detectExtraFiles() {
    ...
    await scanDir(this.cyryxCoreTarget, this.cyryxCoreTarget);
    return extraFiles;
  }
  ...
  async validate() {
    ...
    if (!fs.existsSync(this.cyryxCoreTarget)) {
      this.issues.push({
        type: IssueType.MISSING_FILE,
        severity: Severity.CRITICAL,
  ...
  generateRecommendations() {
    ...
    if (this.stats.missingFiles > 0) {
      if (this.stats.missingFiles > 50) {
        ...
        recommendations.push(
          `${this.stats.missingFiles} file(s) missing. Run 'aexos validate --repair'.`,
        );
      }
    }
    ...
    if (this.stats.corruptedFiles > 0) {
      recommendations.push(
        `${this.stats.corruptedFiles} file(s) corrupted. Run 'aexos validate --repair'.`,
      );
    }
  ...
  async repair(options = {}) {
    ...
    if (!this.sourceDir || !fs.existsSync(this.cyryxCoreSource)) {
      return {
        success: false,
    ...
    for (let i = 0; i < repairableIssues.length; i++) {
      ...
      const sourcePath = path.resolve(this.cyryxCoreSource, relativePath);
      const targetPath = path.resolve(this.cyryxCoreTarget, relativePath);

      onProgress(i + 1, repairableIssues.length, relativePath);

      // SECURITY: Path containment for source
      if (!isPathContained(sourcePath, this.cyryxCoreSource)) {
        result.skipped.push({ path: relativePath, reason: 'Source path traversal blocked' });
        continue;
      }

      // SECURITY: Path containment for target
      if (!isPathContained(targetPath, this.cyryxCoreTarget)) {
      ...
      try {
        ...
        // Walk each path component from cyryxCoreTarget to targetDir
        // and verify none are symlinks
        let currentPath = this.cyryxCoreTarget;
        const relativeParts = path.relative(this.cyryxCoreTarget, targetDir).split(path.sep);
        ...
        for (const part of relativeParts) {
          ...
          try {
            ...
            if (componentStat.isSymbolicLink()) {
              result.skipped.push({
                path: relativePath,
                reason: `Symlink detected in path component: ${path.relative(this.cyryxCoreTarget, currentPath)}`,
              });
              continue;
            }
          } catch (_statError) {
        ...
        // Final realpath verification: ensure resolved target stays within resolved cyryxCoreTarget
        ...
        if (fs.existsSync(targetDir)) {
          ...
          const realCyryxCoreTarget = fs.realpathSync(this.cyryxCoreTarget);

          if (!isPathContained(realTargetDir, realCyryxCoreTarget)) {
            result.skipped.push({
              path: relativePath,
              reason: `Realpath escapes target directory: ${realTargetDir} is outside ${realCyryxCoreTarget}`,
            });
            continue;
          }
        }
...
function formatReport(report, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}AEXOS Core Installation Validation Report${c.reset}`);
