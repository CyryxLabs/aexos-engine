...
/**
 * Prepare deterministic installed-manifest bytes before filesystem mutation.
 *
 ...
 * @param {Date} installedAt - Timestamp recorded in the manifest
 * @returns {{installedManifest: Object, content: string}}
 */
function prepareInstalledManifest(sourceManifest, sourcePackage, installedAt = new Date()) {
...
`;
  ...
  return { installedManifest, content: header + yamlContent };
}
...
function updateInstalledManifest(targetDir, sourceManifest, sourcePackage) {
  ...
  const prepared = prepareInstalledManifest(sourceManifest, sourcePackage);

  fs.ensureDirSync(path.dirname(installedManifestPath));
  fs.writeFileSync(installedManifestPath, prepared.content, 'utf8');
