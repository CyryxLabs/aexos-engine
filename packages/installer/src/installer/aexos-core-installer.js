/**
 * AEXOS Core Installer Module
 *
 * Story 1.4/1.6: IDE Selection & Environment Configuration
 * Handles copying .aexos-core content (agents, tasks, workflows, templates, etc.)
 * to the target project directory.
 *
 * @module installer/aexos-core-installer
 */
...
/**
 * Get the path to the source .aexos-core directory in the package
 * @returns {string} Absolute path to .aexos-core source
 */
function getCyryxCoreSourcePath() {
  return resolveCyryxCorePath('.aexos-core');
}

/**
 * Folders to copy from .aexos-core
...
/**
 * Root files to copy from .aexos-core
 * @constant {string[]}
 */
const ROOT_FILES_TO_COPY = [
  ...
  'constitution.md',    // AEXOS fundamental principles
  'user-guide.md',
...
async function copyManifestArtifacts(sourceDir, targetCyryxCore) {
  const manifestPath = path.join(sourceDir, 'install-manifest.yaml');
  const signaturePath = manifestPath + '.minisig';
  const targetSignaturePath = path.join(targetCyryxCore, 'install-manifest.yaml.minisig');

  if (await fs.pathExists(manifestPath)) {
    await fs.copy(manifestPath, path.join(targetCyryxCore, 'install-manifest.yaml'), {
      overwrite: true,
    });
  }
 ...
 * @param {string} rootPath - Replacement path (e.g., '.aexos-core')
 * @returns {string} Content with {root} replaced
 */
function replaceRootPlaceholder(content, rootPath = '.aexos-core') {
 ...
 *
 * @param {string} targetCyryxCore - Path to .aexos-core directory
 * @param {string[]} installedFiles - List of installed files (relative to .aexos-core)
 * @returns {Promise<Object>} Object mapping file paths to their sha256 hashes
 */
async function generateFileHashes(targetCyryxCore, installedFiles) {
  const fileHashes = {};

  for (const filePath of installedFiles) {
    const absolutePath = path.join(targetCyryxCore, filePath);
 ...
 * @param {string} options.targetCyryxCore - Path to .aexos-core directory
...
async function generateVersionJson(options) {
  ...
  const fileHashes = providedFileHashes || await generateFileHashes(targetCyryxCore, installedFiles);
  ...
  const versionJsonPath = path.join(targetCyryxCore, 'version.json');
...
async function copyFileWithRootReplacement(sourcePath, destPath, replaceRoot = true, options = {}) {
  try {
    ...
    if (needsReplacement) {
      ...
      const updatedContent = replaceRootPlaceholder(content, '.aexos-core');
...
/**
 * Install .aexos-core content to target directory
 *
 ...
 *
 * @example
 * const result = await installCyryxCore({ targetDir: '/path/to/project' });
 * console.log(result.installedFiles); // List of installed files
 */
async function installCyryxCore(options = {}) {
  const {
  ...
  const spinner = ora('Installing AEXOS core framework...').start();

  try {
    const sourceDir = providedSourceDir || getCyryxCoreSourcePath();
    const targetCyryxCore = path.join(targetDir, '.aexos-core');
    const preserveExisting = isBrownfieldProjectType(projectType);

    // Check if source exists
    if (!await fs.pathExists(sourceDir)) {
      throw new Error('.aexos-core source directory not found in package');
    }

    // Create target .aexos-core directory
    await fs.ensureDir(targetCyryxCore);

    // Copy each folder
    for (const folder of FOLDERS_TO_COPY) {
      const folderSource = path.join(sourceDir, folder);
      const folderDest = path.join(targetCyryxCore, folder);
    ...
    for (const file of ROOT_FILES_TO_COPY) {
      ...
      const fileDest = path.join(targetCyryxCore, file);
    ...
    const packageVersion = providedPackageVersion || getCyryxCoreVersion();

    spinner.text = 'Copying installation manifest...';
    await copyManifestArtifacts(sourceDir, targetCyryxCore);
    if (!sourceManifest) {
      const manifest = {
      ...
      await fs.writeFile(
        path.join(targetCyryxCore, 'install-manifest.yaml'),
    ...
    if (sourceManifest) {
      updateInstalledManifest(targetDir, sourceManifest, `aexos-core@${packageVersion}`);
    }

    // BUG-2 fix (INS-1): Install .aexos-core dependencies after copy
    // The copied .aexos-core/package.json has dependencies (js-yaml, execa, etc.)
    // that must be installed for the activation pipeline to work
    // INS-4.12: Track dep install success for bootstrap guard
    const cyryxCorePackageJson = path.join(targetCyryxCore, 'package.json');
    result.cyryxCoreDepsInstalled = false;
    if (await fs.pathExists(cyryxCorePackageJson)) {
      spinner.text = 'Installing .aexos-core dependencies (js-yaml, fast-glob, etc.)...';
      try {
        ...
        result.cyryxCoreDepsInstalled = true;
        spinner.succeed('Installed .aexos-core dependencies');
        spinner.start('Finishing installation...');
      } catch (depError) {
        spinner.warn(`Could not install .aexos-core dependencies: ${depError.message}`);
    ...
    result.success = true;
    spinner.succeed(`AEXOS core installed (${result.installedFiles.length} files)`);

  } catch (error) {
    spinner.fail('AEXOS core installation failed');
 ...
 * @param {string} [options.targetCyryxCore] - Installed .aexos-core directory
 * @returns {Promise<Object>} Link result
 */
async function ensureProjectNodeModulesLink(options = {}) {
  const {
    targetDir = process.cwd(),
    targetCyryxCore = path.join(targetDir, '.aexos-core'),
  } = options;

  const projectNodeModules = path.join(targetDir, 'node_modules');
  const frameworkNodeModules = path.join(targetCyryxCore, 'node_modules');
...
/**
 * Create a basic package.json for AEXOS projects
 * @param {Object} options - Options
 * @param {string} [options.targetDir=process.cwd()] - Target directory
...
async function createBasicPackageJson(options = {}) {
  ...
  const packageJson = {
    ...
    version: '0.1.0',
    description: `AEXOS-powered ${projectType} project`,
    private: true,
    scripts: {
