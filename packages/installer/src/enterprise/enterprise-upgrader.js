...
function ensureEnterpriseEntitlement(entitlement, env = process.env) {
  const keyName = entitlement.envVar || 'AEXOS_ENTERPRISE_KEY';
  const fixtureName = entitlement.testFixtureEnvVar || 'AEXOS_ENTERPRISE_TEST_FIXTURE';
  ...
  throw new EnterpriseUpgradeError(
    'AEXOS_ENTERPRISE_ENTITLEMENT_REQUIRED',
...
function createEmptyExecutionManifest(options) {
  return {
    schemaVersion: '1.0',
    kind: 'cyryx.enterprise.upgrade-execution-manifest',
    mode: 'apply',
...
function validateCopiedFile(relativePath, sourcePath, destPath, executionManifest) {
  ...
  if (sourceHash !== destHash) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_CHECKSUM_MISMATCH',
...
function runDoctorIfAvailable(targetDir) {
  const localBin = path.join(targetDir, 'bin', 'aexos.js');

  if (!fs.existsSync(localBin)) {
    return {
      skipped: true,
      reason: 'aexos doctor --json not available in target',
    };
  }
...
function applyEnterpriseUpgrade(options = {}) {
  ...
  const backupDir = path.join(targetDir, '.aexos', 'enterprise-upgrade-backups', options.timestamp || nowStamp());
  ...
  try {
    ...
    for (const candidate of plan.candidateOps) {
      ...
      if (candidate.required && files.length === 0) {
        throw new EnterpriseUpgradeError(
          'AEXOS_ENTERPRISE_REQUIRED_SOURCE_MISSING',
    ...
    executionManifest.errors.push({
      code: error.code || 'AEXOS_ENTERPRISE_APPLY_FAILED',
