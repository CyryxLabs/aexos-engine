...
function readExecutionManifest(manifestPath) {
  ...
  if (!fs.existsSync(absoluteManifestPath)) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_ROLLBACK_MANIFEST_NOT_FOUND',
  ...
  if (manifest.kind !== 'cyryx.enterprise.upgrade-execution-manifest') {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_ROLLBACK_MANIFEST_INVALID',
...
function rollbackEnterpriseUpgrade(manifestPath) {
  ...
  const rollbackManifest = {
    schemaVersion: '1.0',
    kind: 'cyryx.enterprise.rollback-manifest',
  ...
  const rollbackPath = path.join(targetDir, '.aexos', 'enterprise-upgrade-rollback.yaml');
  ...
  if (errors.length > 0) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_ROLLBACK_INCOMPLETE',
