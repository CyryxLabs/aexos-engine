...
function buildWarnings(context, options = {}) {
  ...
  if (!context.core.version) {
    warnings.push('AEXOS Core version could not be resolved from detected markers.');
  }

  if (!context.pro.version) {
    warnings.push('AEXOS Pro version could not be resolved from detected markers.');
  }
...
function validateContext(context) {
  if (!context.core.detected) {
    throw new EnterpriseUpgradeError(
      'AEXOS_CORE_NOT_DETECTED',
      'AEXOS Core installation not detected',
  ...
  if (!context.pro.detected) {
    throw new EnterpriseUpgradeError(
      'AEXOS_PRO_NOT_DETECTED',
      'AEXOS Pro activation not detected',
      { targetDir: context.targetDir },
    );
  }
  ...
  if (!context.enterprise.valid) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_SOURCE_NOT_DETECTED',
      'AEXOS Enterprise source not detected',
      {
...
function buildEnterpriseUpgradePlan(options = {}) {
  ...
  return {
    ...
    kind: 'cyryx.enterprise.upgrade-plan',
...
function writeEnterpriseUpgradePlan(plan, planPath, options = {}) {
  if (!planPath) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_PLAN_PATH_REQUIRED',
...
function readRequiredOptionValue(argv, index, optionName) {
  ...
  if (!value || value.startsWith('-')) {
    throw new EnterpriseUpgradeError(
      'AEXOS_ENTERPRISE_OPTION_VALUE_REQUIRED',
...
function parseEnterpriseUpgradeArgs(argv = []) {
  ...
  for (let index = startIndex; index < argv.length; index += 1) {
    ...
    if (arg === '--dry-run') {
      ...
      throw new EnterpriseUpgradeError(
        'AEXOS_ENTERPRISE_UNKNOWN_OPTION',
...
function enterpriseUpgradeHelp() {
  return `
Usage:
  aexos enterprise upgrade --target <project> --enterprise-source <path> --dry-run [--plan <file>]
  aexos enterprise upgrade --target <project> --enterprise-source <path> --apply
  aexos enterprise upgrade rollback --manifest <file>

Plan, apply, or rollback an Enterprise upgrade for an existing AEXOS Core + Pro project.

Options:
  --target <project>             Target project to inspect (default: current directory)
  --enterprise-source <path>     Local AEXOS Enterprise source checkout
  ...
  try {
    ...
    if (options.subcommand === 'rollback') {
      if (!options.manifestPath) {
        throw new EnterpriseUpgradeError(
          'AEXOS_ENTERPRISE_ROLLBACK_MANIFEST_REQUIRED',
    ...
    if (options.dryRun && options.apply) {
      throw new EnterpriseUpgradeError(
        'AEXOS_ENTERPRISE_MODE_CONFLICT',
    ...
    if (!options.dryRun && !options.apply) {
      throw new EnterpriseUpgradeError(
        'AEXOS_ENTERPRISE_MODE_REQUIRED',
    ...
    if (!options.enterpriseSource) {
      throw new EnterpriseUpgradeError(
        'AEXOS_ENTERPRISE_SOURCE_REQUIRED',
    ...
    if (options.format && !['yaml', 'json'].includes(options.format)) {
      throw new EnterpriseUpgradeError(
        'AEXOS_ENTERPRISE_UNSUPPORTED_FORMAT',
