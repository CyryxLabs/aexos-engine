 ...
 * 1. EXISTING_CYRYX - .aexos-core/ directory exists
 ...
 * @returns {string} 'GREENFIELD' | 'BROWNFIELD' | 'EXISTING_CYRYX' | 'UNKNOWN'
...
function detectProjectType(targetDir = process.cwd()) {
  try {
    ...
    // Check for AEXOS installation markers (use path.join for security)
    const hasCyryxCore = fs.existsSync(path.join(normalizedDir, '.aexos-core'));
    
    // If AEXOS already installed, return immediately (highest priority)
    if (hasCyryxCore) {
      return 'EXISTING_CYRYX';
    }
