...
/**
 * AEXOS Installer Performance Benchmark
...
/**
 * Get the .aexos-core directory for benchmarking
 */
function getCyryxCoreDir() {
  const projectRoot = path.resolve(__dirname, '../../../../');
  return path.join(projectRoot, '.aexos-core');
}
...
async function runBenchmarks() {
  const cyryxCoreDir = getCyryxCoreDir();

  if (!fs.existsSync(cyryxCoreDir)) {
    console.error(`Error: .aexos-core directory not found at ${cyryxCoreDir}`);
    process.exit(1);
  }

  log(`Starting benchmark with ${CONFIG.runs} runs`);
  log(`Using .aexos-core at: ${cyryxCoreDir}`);

  // Collect files for benchmarking
  log('Collecting files...');
  const allFiles = collectFiles(cyryxCoreDir, CONFIG.testProjectSize);
  ...
  for (let run = 1; run <= CONFIG.runs; run++) {
    ...
    const agentsDir = path.join(cyryxCoreDir, 'development', 'agents');
  ...
  console.error('\n' + '='.repeat(60));
  console.error('AEXOS Installer Performance Baseline');
