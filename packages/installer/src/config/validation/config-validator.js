...
function validateCoreConfigStructure(config) {
  ...
  if (config.project) {
    if (!config.project.type) {
      ...
      const validTypes = ['GREENFIELD', 'BROWNFIELD', 'EXISTING_CYRYX', 'greenfield', 'brownfield', 'existing_cyryx'];
      if (!validTypes.includes(config.project.type)) {
        errors.push(`Invalid project.type: ${config.project.type}. Expected greenfield, brownfield, or existing_cyryx`);
      }
    }
  }
