...
const { generate: generateSettings } = require('../../../../.aexos-core/infrastructure/scripts/generate-settings-json');
...
describe('Environment Configuration Integration', () => {
  ...
  describe('configureEnvironment', () => {
    ...
    it('replaces a package-seeded contributor config with protected install defaults', async () => {
      const configPath = path.join(testDir, '.aexos-core', 'core-config.yaml');
      const packagedCoreRoot = path.resolve(__dirname, '../../../../.aexos-core');
      const packagedConfig = yaml.load(await fs.readFile(
        path.join(packagedCoreRoot, 'core-config.yaml'),
        'utf8',
      ));
      packagedConfig.synapse.operationalSentinel = 'package-default-retained';
      await fs.outputFile(configPath, yaml.dump(packagedConfig));

      // The generator expands the installed core directory one level deep.
      // Mirror its real entry shape so this regression exercises the installed
      // deny-rule threshold without copying framework contents into the fixture.
      const targetCoreDir = path.join(testDir, '.aexos-core', 'core');
      await fs.ensureDir(targetCoreDir);
      for (const entry of await fs.readdir(path.join(packagedCoreRoot, 'core'), { withFileTypes: true })) {
        const targetEntry = path.join(targetCoreDir, entry.name);
        if (entry.isDirectory()) {
          await fs.ensureDir(targetEntry);
        } else {
          await fs.outputFile(targetEntry, '');
        }
      }
      ...
      await configureEnvironment({
        ...
        skipPrompts: true,
        projectType: 'BROWNFIELD',
        coreConfigExistedBeforeInstall: false,
      });

      const config = yaml.load(await fs.readFile(configPath, 'utf8'));
      expect(config.boundary.frameworkProtection).toBe(true);
      expect(config.synapse.operationalSentinel).toBe('package-default-retained');
      expect(config.synapse.pipelineTimeoutMs).toBe(packagedConfig.synapse.pipelineTimeoutMs);
      expect(config.models).toEqual(packagedConfig.models);
      expect(config.lazyLoading).toEqual(packagedConfig.lazyLoading);

      generateSettings(testDir);
      const settings = JSON.parse(await fs.readFile(
        path.join(testDir, '.claude', 'settings.json'),
        'utf8',
      ));
      expect(settings.permissions.deny.length).toBeGreaterThanOrEqual(40);
    });

    it('runs settings generation after environment configuration in the installer', () => {
      const wizardSource = fs.readFileSync(
        path.join(__dirname, '../../src/wizard/index.js'),
        'utf8',
      );
      const configureCall = wizardSource.indexOf('const envResult = await configureEnvironment({');
      const boundaryGeneration = wizardSource.indexOf("console.log('\\n🔒 Generating boundary rules...');");

      expect(configureCall).toBeGreaterThan(-1);
      expect(boundaryGeneration).toBeGreaterThan(configureCall);
    });

    it('preserves an explicit disabled-protection choice from an existing project', async () => {
      const configPath = path.join(testDir, '.aexos-core', 'core-config.yaml');
      await fs.outputFile(configPath, [
        'boundary:',
      ...
      await configureEnvironment({
        ...
        skipPrompts: true,
        projectType: 'BROWNFIELD',
        coreConfigExistedBeforeInstall: true,
      });
