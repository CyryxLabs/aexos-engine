...
describe('Wizard Integration with Project Type Detection', () => {
  ...
  describe('GREENFIELD Detection Flow', () => {
    test('wizard correctly detects and processes GREENFIELD project', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        // No markers exist
        if (checkPath.includes('.aexos-core')) return false;
  ...
  describe('BROWNFIELD Detection Flow', () => {
    test('wizard correctly detects and processes BROWNFIELD project with package.json', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === targetPath) return true;
        if (checkPath.includes('.aexos-core')) return false; // No AEXOS
  ...
  // ============================================================================
  // Task 1.3.5.4: Test full wizard flow with EXISTING_CYRYX detection
  // ============================================================================
  describe('EXISTING_CYRYX Detection Flow', () => {
    test('wizard correctly detects and processes EXISTING_CYRYX installation', async () => {
      // Setup: Directory with .aexos-core
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath.includes('test-existing')) return true;
        if (checkPath.endsWith('.aexos-core')) return true;
        return true; // Other files exist
      });
      fs.readdirSync.mockReturnValue(['.aexos-core', 'package.json', '.git']);

      const result = await runWizard({ targetDir: '/test-existing' });

      expect(result.projectType).toBe('EXISTING_CYRYX');
      expect(result.targetDir).toBe('/test-existing');
      // Output shows "brownfield" since EXISTING_CYRYX is treated as brownfield update
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('brownfield'));
    });

    test('confirmProjectType shows update/reinstall message for EXISTING_CYRYX', async () => {
      const confirmed = await confirmProjectType('EXISTING_CYRYX');

      expect(confirmed).toBe('EXISTING_CYRYX');
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('update or reinstall'),
      );
    });

    test('getProjectType helper returns EXISTING_CYRYX when .aexos-core exists', () => {
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath.includes('existing')) return true;
        if (checkPath.endsWith('.aexos-core')) return true;
        return false;
      });
      fs.readdirSync.mockReturnValue(['.aexos-core']);

      const type = getProjectType('/test/existing');

      expect(type).toBe('EXISTING_CYRYX');
    });
  });
  ...
  describe('User Override Functionality', () => {
    ...
    test('confirmProjectType shows appropriate message for each type', async () => {
      const types = ['GREENFIELD', 'BROWNFIELD', 'EXISTING_CYRYX', 'UNKNOWN'];
  ...
  describe('Default Wizard Behavior', () => {
    ...
    test('wizard displays welcome message', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Welcome to AEXOS Installer'),
      );
    });
