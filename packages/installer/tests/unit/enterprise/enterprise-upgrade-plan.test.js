...
function createCoreInstall(targetDir, version = '5.1.15') {
  ...
  writeJson(path.join(targetDir, '.aexos-core', 'version.json'), {
    version,
    mode: 'core',
  });
}
...
function createProInstall(targetDir, version = '2.0.0') {
  ...
  writeYaml(path.join(targetDir, '.aexos-core', 'pro-config.yaml'), {
    pro: {
      version,
      enabled: true,
    },
  });
  writeYaml(path.join(targetDir, '.aexos-core', 'feature-registry.yaml'), {
    features: {
      pro: true,
    },
  });
...
describe('Enterprise upgrade dry-run planning', () => {
  ...
  test('fails when AEXOS Core installation is not detected', () => {
    const targetDir = temp('aexos-no-core');
    ...
    expect(() => buildEnterpriseUpgradePlan({ targetDir, enterpriseSource }))
      .toThrow('AEXOS Core installation not detected');
  });

  test('fails when AEXOS Core exists without Pro activation', () => {
    const targetDir = temp('aexos-core-no-pro');
    const enterpriseSource = temp('aexos-enterprise-source');
    createCoreInstall(targetDir);
    createEnterpriseSource(enterpriseSource);

    expect(() => buildEnterpriseUpgradePlan({ targetDir, enterpriseSource }))
      .toThrow('AEXOS Pro activation not detected');
  });
  ...
  test('creates a parseable dry-run plan for Core plus Pro projects', () => {
    ...
    expect(parsed.enterprise.product).toBe('AEXOS Enterprise');
    expect(parsed.enterprise.version).toBe('1.2.0');
    expect(parsed.enterprise.source).toBe('CyryxLabs/aexos-enterprise');
  ...
  test('fails when the Enterprise source is invalid', () => {
    ...
    expect(() => buildEnterpriseUpgradePlan({ targetDir, enterpriseSource }))
      .toThrow('AEXOS Enterprise source not detected');
  });

  test('writes only the requested plan file during dry-run', () => {
    const targetDir = temp('aexos-dry-run-target');
    const enterpriseSource = temp('aexos-enterprise-source');
  ...
  test('CLI returns non-zero for missing prereqs and zero for valid dry-run', async () => {
    const invalidTarget = temp('aexos-cli-invalid-target');
    const validTarget = temp('aexos-cli-valid-target');
    const enterpriseSource = temp('aexos-enterprise-source');
    createEnterpriseSource(enterpriseSource);
    ...
    expect(badErr.getOutput()).toContain('AEXOS Core installation not detected');
