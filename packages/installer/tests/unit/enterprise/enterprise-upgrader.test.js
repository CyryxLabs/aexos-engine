...
describe('Enterprise transactional upgrader and rollback', () => {
  ...
  function createReadyTarget() {
    const targetDir = temp('aexos-apply-target');
  ...
  test('apply creates backup before overwriting a changed file and writes execution manifest', () => {
    ...
    const result = applyEnterpriseUpgrade({
      targetDir,
      enterpriseSource,
      env: { AEXOS_ENTERPRISE_TEST_FIXTURE: '1' },
    ...
    expect(fs.existsSync(path.join(targetDir, '.aexos', 'enterprise-upgrade-backups', '20260508070126', 'scripts', 'hub-sync.js'))).toBe(true);
  ...
  test('merge-yaml unions arrays while preserving target order', () => {
    ...
    const enterpriseSource = temp('aexos-enterprise-source');
    createEnterpriseSource(enterpriseSource);
    writeYaml(path.join(targetDir, '.aexos-sync.yaml'), {
      active_ides: ['codex'],
    });
    ...
    const merged = yaml.load(fs.readFileSync(path.join(targetDir, '.aexos-sync.yaml'), 'utf8'));
  ...
  test('rollback restores backed up files from execution manifest', () => {
    ...
    expect(fs.existsSync(path.join(targetDir, '.aexos', 'enterprise-upgrade-rollback.yaml'))).toBe(true);
  });
  ...
  test('apply keeps Pro markers and records doctor output when available', () => {
    ...
    createEnterpriseSource(enterpriseSource);
    fs.outputFileSync(
      path.join(targetDir, 'bin', 'aexos.js'),
  ...
  test('CLI apply and rollback return expected exit codes', async () => {
    ...
    const previousFixture = process.env.AEXOS_ENTERPRISE_TEST_FIXTURE;
    process.env.AEXOS_ENTERPRISE_TEST_FIXTURE = '1';
    ...
    try {
      ...
      const rollbackCode = await runEnterpriseUpgradeCli([
        ...
        'rollback',
        '--manifest',
        path.join(targetDir, '.aexos', 'enterprise-upgrade-manifest.yaml'),
      ...
      if (previousFixture === undefined) {
        delete process.env.AEXOS_ENTERPRISE_TEST_FIXTURE;
      } else {
        process.env.AEXOS_ENTERPRISE_TEST_FIXTURE = previousFixture;
      }
    }
  });
});
