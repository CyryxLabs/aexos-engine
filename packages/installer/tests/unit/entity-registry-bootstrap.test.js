 ...
 * handles failures gracefully, and that aexos doctor can verify the registry.
 */
...
const POPULATE_SCRIPT = path.join(
  __dirname, '..', '..', '..', '..', '.aexos-core', 'development', 'scripts', 'populate-entity-registry.js',
);
const DOCTOR_CHECK = path.join(
  __dirname, '..', '..', '..', '..', '.aexos-core', 'core', 'doctor', 'checks', 'entity-registry.js',
);
const REGISTRY_PATH = path.join(
  __dirname, '..', '..', '..', '..', '.aexos-core', 'data', 'entity-registry.yaml',
);
const PRE_PUSH_HOOK = path.join(__dirname, '..', '..', '..', '..', '.husky', 'pre-push');
const IDS_PRE_PUSH = path.join(
  __dirname, '..', '..', '..', '..', '.aexos-core', 'hooks', 'ids-pre-push.js',
);
...
describe('Entity Registry Bootstrap (Story INS-4.6)', () => {
  ...
  describe('AC1: Bootstrap called during install', () => {
    ...
    test('bootstrap runs after .aexos-core/ copy (injection order)', () => {
      const cyryxCoreIdx = wizardSource.indexOf('AEXOS core installed');
      ...
      // Bootstrap must come after aexos-core install
      expect(bootstrapIdx).toBeGreaterThan(cyryxCoreIdx);
    ...
    test('bootstrap has try/catch — failure does not abort install', () => {
      ...
      expect(wizardSource).toContain("run 'aexos doctor' post-install");
    });
  ...
  describe('AC2: Sanity check (relative threshold)', () => {
    test('aexos doctor entity-registry check exists', () => {
  ...
  describe('AC5: Post-bootstrap verification', () => {
    ...
    test('aexos doctor entity-registry check passes on current registry', async () => {
