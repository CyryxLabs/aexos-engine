...
describe('detectProjectType', () => {
  ...
  describe('AC #1: GREENFIELD detection', () => {
    test('detects GREENFIELD when directory is empty', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .aexos-core, package.json, or .git
      });
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('.aexos-core'));
  ...
  describe('AC #2: BROWNFIELD detection - package.json', () => {
    test('detects BROWNFIELD when package.json exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .aexos-core or .git
      });
    ...
    test('detects BROWNFIELD when only package.json exists (no git)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.aexos-core')) return false;
        return false;
      });
  ...
  describe('AC #3: BROWNFIELD detection - .git', () => {
    test('detects BROWNFIELD when .git exists', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        return false; // No .aexos-core or package.json
      });
  ...
  // ============================================================================
  // AC #4: .aexos-core Exists → EXISTING_CYRYX
  // ============================================================================
  describe('AC #4: EXISTING_CYRYX detection', () => {
    test('detects EXISTING_CYRYX when .aexos-core exists', () => {
      // Setup
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/existing')) return true;
        if (checkPath.endsWith('.aexos-core')) return true; // .aexos-core exists
        return true; // Other files may exist too
      });
      fs.readdirSync.mockReturnValue(['.aexos-core', 'package.json', '.git', 'src']);
      
      const result = detectProjectType('/test/existing');
      
      expect(result).toBe('EXISTING_CYRYX');
    });

    test('EXISTING_CYRYX takes priority over BROWNFIELD markers', () => {
      // Setup: All markers present, but EXISTING_CYRYX should win
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/priority-test')) return true;
        // All markers present
        if (checkPath.endsWith('.aexos-core')) return true;
        if (checkPath.endsWith('package.json')) return true;
        if (checkPath.endsWith('.git')) return true;
        return false;
      });
      ...
      // EXISTING_CYRYX has highest priority
      expect(result).toBe('EXISTING_CYRYX');
    });
  });
  ...
  describe('Edge Case: UNKNOWN detection', () => {
    test('returns UNKNOWN when directory has files but no recognized markers', () => {
      // Setup: Files exist but no package.json, .git, or .aexos-core
      fs.existsSync.mockImplementation((checkPath) => {
  ...
  describe('Error Handling', () => {
    ...
    test('throws error when directory cannot be accessed (EACCES)', () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        if (checkPath.endsWith('.aexos-core')) {
  ...
  describe('Detection Priority Order', () => {
    test('priority: EXISTING_CYRYX > GREENFIELD', () => {
      // Even if directory appears empty, .aexos-core presence should win
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === path.resolve('/test/priority1')) return true;
        if (checkPath.endsWith('.aexos-core')) return true;
        return false;
      });
      fs.readdirSync.mockReturnValue(['.aexos-core']); // Only .aexos-core
