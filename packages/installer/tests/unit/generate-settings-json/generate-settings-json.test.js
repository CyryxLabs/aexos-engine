...
const {
  ...
  generatePermissions,
  writeSettingsJson,
} = require('../../../../../.aexos-core/infrastructure/scripts/generate-settings-json');
...
function createTempProject(boundary, existingSettings) {
  ...
  fs.writeFileSync(path.join(tmpDir, '.aexos-core', 'core-config.yaml'), yamlContent, 'utf8');

  // Create directory structure for expansion tests
  if (boundary.protected.includes('.aexos-core/core/**')) {
    const coreDir = path.join(tmpDir, '.aexos-core', 'core');
...
describe('generate-settings-json', () => {
  describe('readBoundaryConfig', () => {
    test('reads boundary config from core-config.yaml', () => {
      const tmpDir = createTempProject({
        ...
        protected: ['.aexos-core/core/**', 'bin/aexos.js'],
        exceptions: ['.aexos-core/data/**'],
      });
      ...
      try {
        ...
        expect(config.protected).toContain('.aexos-core/core/**');
        expect(config.protected).toContain('bin/aexos.js');
        expect(config.exceptions).toContain('.aexos-core/data/**');
      } finally {
  ...
  describe('generatePermissions — frameworkProtection: true', () => {
    test('generates deny rules covering all protected paths', () => {
      const tmpDir = createTempProject({
        frameworkProtection: true,
        protected: ['.aexos-core/core/**', '.aexos-core/infrastructure/**', 'bin/aexos.js'],
        exceptions: ['.aexos-core/data/**'],
      });

      // Create infrastructure dir (no expansion for non-core paths)
      fs.mkdirSync(path.join(tmpDir, '.aexos-core', 'infrastructure'), { recursive: true });

      try {
        const boundary = readBoundaryConfig(tmpDir);
        const permissions = generatePermissions(boundary, tmpDir);

        // Should have deny rules for core subdirs (events/**, utils/**, index.js) + infrastructure/** + bin/aexos.js
  ...
  describe('generatePermissions — frameworkProtection: false', () => {
    test('produces no boundary deny rules', () => {
      const boundary = {
        ...
        protected: ['.aexos-core/core/**', '.aexos-core/infrastructure/**'],
        exceptions: ['.aexos-core/data/**'],
      };
