...
const AGENT_INFO = {
  'aexos-master': { icon: '🧠', role: 'Master Orchestrator' },
...
function commandNameForAgent(agentId) {
  if (agentId.startsWith('aexos-')) {
    return `/aexos-${agentId.replace(/^aexos-/, '')}`;
  }
  return `/aexos-${agentId}`;
}

function hasAgent(projectRoot, agentId) {
  const canonical = path.join(projectRoot, '.aexos-core', 'development', 'agents', `${agentId}.md`);
  const gemini = path.join(projectRoot, '.gemini', 'rules', 'AEXOS', 'agents', `${agentId}.md`);
  return fs.existsSync(canonical) || fs.existsSync(gemini);
}

function renderGreeting(projectRoot, agentId) {
  const scriptPath = path.join(projectRoot, '.aexos-core', 'development', 'scripts', 'generate-greeting.js');
...
function buildActivationPrompt(agentId) {
  return [
    `Ative o agente ${agentId} usando .aexos-core/development/agents/${agentId}.md`,
    `(fallback: .gemini/rules/AEXOS/agents/${agentId}.md),`,
    `renderize o greeting via node .aexos-core/development/scripts/generate-greeting.js ${agentId}`,
...
function runAgentLauncher(agentId, projectRoot = process.cwd()) {
  if (!agentId) {
    console.log('Uso: /aexos-agent <agent-id>');
    return 1;
  }
  ...
  console.log(`${info.icon} AEXOS Agent Selected: ${agentId}`);
...
function runAgentMenu(projectRoot = process.cwd()) {
  ...
  console.log('🤖 AEXOS Quick Agent Menu (Gemini)');
  console.log('');

  if (agents.length === 0) {
    console.log('No AEXOS agents found. Run: npm run sync:ide:gemini');
    return 1;
  }
  ...
  console.log('\nTip: run /aexos-<agent-id> to prepare activation prompt quickly.');
  return 0;
}
