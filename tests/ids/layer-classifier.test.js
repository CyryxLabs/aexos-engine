...
const { classifyLayer, LAYER_RULES } = require(
  path.resolve(__dirname, '../../.aexos-core/core/ids/layer-classifier'),
);

describe('classifyLayer', () => {
  // --- L1: Framework Core ---
  describe('L1 — Framework Core', () => {
    test('classifies .aexos-core/core/ modules as L1', () => {
      expect(classifyLayer('.aexos-core/core/ids/index.js')).toBe('L1');
    });

    test('classifies .aexos-core/core/ nested modules as L1', () => {
      expect(classifyLayer('.aexos-core/core/ids/registry-updater.js')).toBe('L1');
      expect(classifyLayer('.aexos-core/core/utils/helpers.js')).toBe('L1');
    });

    test('classifies bin/ executables as L1', () => {
      expect(classifyLayer('bin/aexos.js')).toBe('L1');
      expect(classifyLayer('bin/aexos-init.js')).toBe('L1');
    });

    test('classifies constitution.md as L1', () => {
      expect(classifyLayer('.aexos-core/constitution.md')).toBe('L1');
    });
  });

  // --- L2: Framework Templates ---
  describe('L2 — Framework Templates', () => {
    test('classifies .aexos-core/development/ as L2', () => {
      expect(classifyLayer('.aexos-core/development/tasks/create-next-story.md')).toBe('L2');
    });

    test('classifies .aexos-core/development/agents/ (non-MEMORY) as L2', () => {
      expect(classifyLayer('.aexos-core/development/agents/dev.md')).toBe('L2');
    });

    test('classifies .aexos-core/infrastructure/ as L2', () => {
      expect(classifyLayer('.aexos-core/infrastructure/scripts/deploy.sh')).toBe('L2');
    });

    test('classifies .aexos-core/product/ as L2', () => {
      expect(classifyLayer('.aexos-core/product/templates/story-tmpl.yaml')).toBe('L2');
    });
  });

  // --- L3: Project Config ---
  describe('L3 — Project Config', () => {
    test('classifies .aexos-core/data/ as L3', () => {
      expect(classifyLayer('.aexos-core/data/entity-registry.yaml')).toBe('L3');
    });

    test('classifies MEMORY.md inside agents as L3 (not L2)', () => {
      expect(classifyLayer('.aexos-core/development/agents/dev/MEMORY.md')).toBe('L3');
      expect(classifyLayer('.aexos-core/development/agents/qa/MEMORY.md')).toBe('L3');
    });
  ...
  describe('Edge cases', () => {
    ...
    test('non-MEMORY agent file inside agents/ classifies as L2', () => {
      expect(classifyLayer('.aexos-core/development/agents/dev/skills.yaml')).toBe('L2');
    });
