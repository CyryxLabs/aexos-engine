...
jest.mock('child_process', () => {
  const execSync = jest.fn(() => '.git');
  return { execSync, execFileSync: jest.fn((command, args, options) => execSync([command, ...args].join(' '), options)) };
});
...
jest.mock('../../.aexos-core/infrastructure/scripts/worktree-manager', () => {
    ...
    it('should return worktrees status object', async () => {
      WorktreeManager.mockImplementation(() => ({
        list: jest.fn().mockResolvedValue([
          {
            storyId: 'STORY-42',
            path: '/project/.aexos/worktrees/STORY-42',
    ...
    describe('_resolveCacheFilePath', () => {
      it('should use default path for main working tree', () => {
        ...
        expect(newLoader.cacheFile).toBe(
          path.join(projectRoot, '.aexos', 'project-status.yaml'),
        );
      });
