// Adapted from frozen upstream tests/tools/validators.test.js
// Upstream commit: 4ef6530ff03b83aea953e4a426f95e012b8b70c5
// SHA256: 4ec906fce1ad7541fa78f0cd4d951eac8b0265eeca4416872b809252bba3ecfe
...
describe('Complex Tool Validators - Integration Tests', () => {
  ...
  beforeAll(async () => {
    const toolsPath = path.join(__dirname, '../../.aexos-core/infrastructure/tools');
  ...
  describe('ClickUp Assignee Format Validation', () => {
    ...
    test('rejects object format for create_task (expects array)', async () => {
      ...
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toContain('array');
    });
    ...
    test('validates required fields for create_task', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/name.*required/i);
    });
  ...
  describe('Google Workspace Permission Validation', () => {
    ...
    test('requires either content or fileUrl for file creation', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/content.*fileUrl/i);
    });
    ...
    test('validates mimeType format', async () => {
      ...
      expect(invalidMime.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/mimeType/i);
    ...
    test('requires fileId for share_file', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/fileId.*required/i);
    });
    ...
    test('validates email format for sharing', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/email/i);
    });
    ...
    test('validates event time format', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/time|date/i);
    });
  ...
  describe('n8n Workflow Validation', () => {
    ...
    test('requires workflow_id for execution', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/workflow_id.*required/i);
    });
  ...
  describe('Supabase RLS Policy Validation', () => {
    ...
    test('requires project_id for SQL execution', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/project_id.*required/i);
    });
    ...
    test('requires query for SQL execution', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/query.*required/i);
    });
    ...
    test('blocks DDL operations in execute_sql', async () => {
      ...
      for (const query of ddlOperations) {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/ddl.*migration/i);
      }
    });
    ...
    test('validates project reference format', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.map(error => typeof error === 'string' ? error : error.message).join(' ')).toMatch(/project_id.*format/i);
    });
  });
