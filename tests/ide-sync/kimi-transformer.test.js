 ...
 * Covers:
 *  - skillId normalization (no double aexos- prefix)
...
const { syncIde } = require('../../.aexos-core/infrastructure/scripts/ide-sync/index');
...
function buildAgentData(overrides = {}) {
  return {
    ...
    persona_profile: {
      archetype: 'Builder',
      communication: {
        greeting_levels: { named: '💻 Vulcan (Builder) ready' },
        tone: 'pragmatic',
      },
...
describe('kimi transformer', () => {
  test('normalizes skill id without double aexos- prefix', () => {
    expect(kimi.getSkillId({ id: 'dev', agent: {} })).toBe('aexos-dev');
    expect(kimi.getSkillId({ id: 'aexos-master', agent: {} })).toBe('aexos-master');
    expect(kimi.getSkillId({ id: 'cyryx-master', agent: {} })).toBe('aexos-master');
  });
  ...
  test('respects preferredActivationAlias', () => {
    ...
    expect(skillId).toBe('aexos-davi-ribas');
  });
  ...
  test('sanitizes skill ids used as directories', () => {
    ...
    expect(skillId).toBe('aexos-team-danger-agent');
  ...
  test('does not duplicate AEXOS in already-branded titles', () => {
    const out = kimi.transform(buildAgentData({
      agent: { title: 'AEXOS Master Orchestrator' },
    }));
    expect(out).toContain('Activate the AEXOS Master Orchestrator agent');
    expect(out).not.toContain('AEXOS AEXOS');
  });
  ...
  test('produces nested layout: <skill-id>/SKILL.md', () => {
    ...
    expect(kimi.getDirname(data)).toBe('aexos-dev');
