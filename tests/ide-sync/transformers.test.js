...
const cursor = require('../../.aexos-core/infrastructure/scripts/ide-sync/transformers/cursor');
const antigravity = require('../../.aexos-core/infrastructure/scripts/ide-sync/transformers/antigravity');
const {
  generateRedirect,
  generateRedirectContent,
  getRedirectFilenames,
  sanitizeRedirectId,
} = require('../../.aexos-core/infrastructure/scripts/ide-sync/redirect-generator');
...
describe('IDE Transformers', () => {
  ...
  describe('claude-code transformer', () => {
    ...
    it('should add sync footer if not present', () => {
      const result = claudeCode.transform(sampleAgent);
      expect(result).toContain('Synced from .aexos-core/development/agents/dev.md');
    });
    ...
    it('should not duplicate sync footer', () => {
      const agentWithFooter = {
        ...
        raw:
          sampleAgent.raw +
          '\n---\n*CYRYX Agent - Synced from .aexos-core/development/agents/dev.md*',
      };
    ...
    it('should generate Claude skill sidecar content', () => {
      ...
      expect(result).toContain('name: aexos-dev');
      ...
      expect(result).toContain('Source: .aexos-core/development/agents/dev.md');
      expect(result).toContain('# dev');
    });
    ...
    it('should generate Claude legacy command shim content', () => {
      ...
      expect(result).toContain('Canonical Skill: .claude/skills/AEXOS/agents/dev/SKILL.md');
      expect(result).toContain('Source: .aexos-core/development/agents/dev.md');
    ...
    it('should return correct Claude skill relative path', () => {
      expect(claudeCode.getSkillRelativePath(sampleAgent)).toBe('AEXOS/agents/dev/SKILL.md');
    });
  ...
  describe('cursor transformer', () => {
    it('should generate condensed format', () => {
      ...
      expect(result).toMatch(/^---\ndescription: 'AEXOS agent @dev - Full Stack Developer'\nalwaysApply: false\n---/);
      expect(result).toContain('# Vulcan (@dev)');
      expect(result).toContain('💻 **Full Stack Developer**');
    ...
    it('should escape redirect frontmatter values for Cursor MDC files', () => {
      ...
      expect(result).toContain("description: 'AEXOS redirect from @old''agent name to @new''agent'");
      expect(result).toMatch(/^---\ndescription: 'AEXOS redirect/m);
    });
