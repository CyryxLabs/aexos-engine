...
} = require('../../.aexos-core/infrastructure/scripts/ide-sync/validator');
...
describe('validator', () => {
  ...
  describe('validateIdeSync', () => {
    ...
    it('should detect orphaned files in nested Kimi skill directories', () => {
      fs.ensureDirSync(path.join(targetDir, 'aexos-dev'));
      fs.ensureDirSync(path.join(targetDir, 'aexos-qa'));
      fs.writeFileSync(path.join(targetDir, 'aexos-dev', 'SKILL.md'), 'content');
      fs.writeFileSync(path.join(targetDir, 'aexos-qa', 'SKILL.md'), 'orphan');

      const expected = [{ filename: path.join('aexos-dev', 'SKILL.md'), content: 'content' }];
      const result = validateIdeSync(expected, targetDir, {}, 'kimi-skill');

      expect(result.orphaned).toHaveLength(1);
      expect(result.orphaned[0].filename).toBe(path.join('aexos-qa', 'SKILL.md'));
    });

    it('should not count redirect files as orphaned', () => {
      fs.writeFileSync(path.join(targetDir, 'agent.md'), 'content');
      fs.writeFileSync(path.join(targetDir, 'aexos-developer.md'), 'redirect');

      const expected = [{ filename: 'agent.md', content: 'content' }];
      const redirects = { 'aexos-developer': 'aexos-master' };
    ...
    it('should not count Cursor mdc redirect files as orphaned', () => {
      ...
      fs.writeFileSync(path.join(targetDir, 'aexos-developer.mdc'), 'redirect');
