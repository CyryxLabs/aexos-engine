...
const {
  ...
  syncGeminiCommands,
} = require('../../.aexos-core/infrastructure/scripts/ide-sync/gemini-commands');
...
describe('gemini command sync', () => {
  ...
  it('normalizes command slugs and menu names', () => {
    expect(commandSlugForAgent('aexos-master')).toBe('master');
    expect(commandSlugForAgent('dev')).toBe('dev');
    expect(menuCommandName('aexos-master')).toBe('/aexos-master');
    expect(menuCommandName('dev')).toBe('/aexos-dev');
  });
  ...
  it('builds menu + one command per agent', () => {
    ...
    expect(files.find((f) => f.filename === 'aexos-menu.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'aexos-dev.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'aexos-architect.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'aexos-qa.toml')).toBeDefined();
  ...
  it('derives command description from agent title and whenToUse', () => {
    ...
    const dev = files.find((f) => f.filename === 'aexos-dev.toml');
  ...
  it('falls back to generic description when metadata is missing', () => {
    ...
    const dev = files.find((f) => f.filename === 'aexos-dev.toml');
    expect(dev.content).toContain('description = "Ativar agente AEXOS dev"');
  });
  ...
  it('writes command files to .gemini/commands', () => {
    ...
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'aexos-menu.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'aexos-dev.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'aexos-qa.toml'))).toBe(true);
  });
});
