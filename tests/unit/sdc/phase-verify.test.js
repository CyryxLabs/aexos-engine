...
const { verifyPhase } = require('../../../.aexos-core/core/sdc/phase-verify');
const { extractQaEvidence } = require('../../../.aexos-core/core/sdc/story-meta');
const {
  initFullSdc,
  loadSdcState,
  markPhase,
...
describe('phase-verify + progress', () => {
  ...
  it('initFullSdc writes state under cwd .aexos/sdc', () => {
    const file = writeStory(dir, 's.md', '| Story ID | T.9 |\n| Status | Draft |\n');
    // chdir so relative .aexos lands in temp — use opts.cwd in progress
    ...
    try {
      ...
      expect(fs.existsSync(path.join(dir, '.aexos', 'sdc', 'T.9', 'state.json'))).toBe(true);
    } finally {
      process.chdir(prev);
    }
  });
  ...
  it('persists checkpoint JSON through an atomic rename without temp residue', () => {
    const file = path.join(dir, '.aexos', 'sdc', 'atomic', 'state.json');
  ...
  it('resolves complete provenance from a story-bound gate file fallback', () => {
    ...
    expect(result.meta.qaReviewer).toBe('Argus');
  ...
  it('uses qa.qaLocation from the canonical core config', () => {
    ...
    fs.mkdirSync(path.join(dir, '.aexos-core'), { recursive: true });
    fs.writeFileSync(
      path.join(dir, '.aexos-core', 'core-config.yaml'),
