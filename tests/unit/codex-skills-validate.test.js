...
describe('Codex Skills Validator', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cyryx-codex-validate-'));
  ...
  it('derives default source and skills directories from a supplied project root', () => {
    const tmpSourceDir = path.join(tmpRoot, '.aexos-core', 'development', 'agents');
  ...
  it('fails self-test when a skill source path cannot be resolved', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const target = path.join(skillsDir, 'aexos-dev', 'SKILL.md');
    const original = fs.readFileSync(target, 'utf8');
    fs.writeFileSync(
      target,
      original.replace('.aexos-core/development/agents/dev.md', '.aexos-core/development/agents/missing-dev.md'),
      'utf8',
    );
    ...
    const devSelfTest = result.selfTests.find(test => test.skillId === 'aexos-dev');
  ...
  it('does not cascade payload errors when a skill frontmatter name mismatches', () => {
    ...
    const target = path.join(skillsDir, 'aexos-dev', 'SKILL.md');
    const original = fs.readFileSync(target, 'utf8');
    fs.writeFileSync(target, original.replace('name: aexos-dev', 'name: aexos-not-dev'), 'utf8');
    ...
    expect(result.ok).toBe(false);
    expect(devSelfTest).toBeDefined();
    expect(devSelfTest.errors).toContain('self-test frontmatter name mismatch: expected "aexos-dev"');
  ...
  it('normalizes Skill tool invocation targets', () => {
    ...
    expect(normalizeSkillToolTarget('$aexos-qa')).toBe('aexos-qa');
  });

  it('fails when a generated skill is missing', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    fs.rmSync(path.join(skillsDir, 'aexos-architect', 'SKILL.md'), { force: true });
  ...
  it('fails in strict mode when orphaned cyryx-* skill dir exists', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const orphanPath = path.join(skillsDir, 'cyryx-legacy');
    ...
    expect(result.ok).toBe(false);
    expect(result.orphaned).toContain('cyryx-legacy');
  });

  it('fails in strict mode when a legacy cyryx-* alias dir exists for a core agent', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const legacyPath = path.join(skillsDir, 'cyryx-dev');
  ...
  it('ignores generated squad chief skills that point to an existing squad source', () => {
    ...
    const squadSkillPath = path.join(skillsDir, 'cyryx-demo-chief');
    fs.mkdirSync(squadSkillPath, { recursive: true });
    fs.writeFileSync(
      path.join(squadSkillPath, 'SKILL.md'),
      [
        '---',
        'name: cyryx-demo-chief',
    ...
    expect(result.ok).toBe(true);
    expect(result.ignored).toContain('cyryx-demo-chief');
    expect(result.orphaned).not.toContain('cyryx-demo-chief');
  });
