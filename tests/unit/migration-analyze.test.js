...
const {
  ...
  formatMigrationPlan,
  analyzeImports,
} = require('../../.aexos-core/cli/commands/migrate/analyze');

describe('Migration Analyze Module', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `aexos-analyze-test-${Date.now()}`);
  ...
  describe('detectV2Structure', () => {
    ...
    it('should detect v2.1 modular structure', async () => {
      ...
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'product'), { recursive: true });
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'infrastructure'), { recursive: true });
    ...
    it('should return error if no .aexos-core exists', async () => {
      const result = await detectV2Structure(testDir);
  ...
  describe('analyzeMigrationPlan', () => {
    it('should generate migration plan for v2.0 project', async () => {
      ...
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'tasks', 'build.md'), 'Task');
    ...
    it('should detect conflicts', async () => {
      ...
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'agents'), { recursive: true });
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'core'), { recursive: true }); // Conflict
  ...
  describe('analyzeImports', () => {
    it('should analyze importable files', async () => {
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'cli'), { recursive: true });
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'cli', 'index.js'), 'module.exports = {}');
