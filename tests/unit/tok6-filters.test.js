...
const { filterContent, stripHtml, truncateAtBoundary, extractFields, CHARS_PER_TOKEN } = require('../../.aexos-core/utils/filters/content-filter');
const { filterSchema, projectFields } = require('../../.aexos-core/utils/filters/schema-filter');
const { filterFields } = require('../../.aexos-core/utils/filters/field-filter');
const { applyFilter, loadFilterConfig } = require('../../.aexos-core/utils/filters/index');
...
describe('constants.js', () => {
  it('exports CHARS_PER_TOKEN as 4', () => {
    const { CHARS_PER_TOKEN } = require('../../.aexos-core/utils/filters/constants');
    expect(CHARS_PER_TOKEN).toBe(4);
  });

  it('is the same value used by content-filter and schema-filter', () => {
    const { CHARS_PER_TOKEN: contentCPT } = require('../../.aexos-core/utils/filters/content-filter');
    const { CHARS_PER_TOKEN: schemaCPT } = require('../../.aexos-core/utils/filters/schema-filter');
    const { CHARS_PER_TOKEN: sharedCPT } = require('../../.aexos-core/utils/filters/constants');
...
describe('index.js (filter dispatcher)', () => {
  const registryPath = path.join(__dirname, '..', '..', '.aexos-core', 'data', 'tool-registry.yaml');
