...
describe('IDE Config Generator', () => {
  ...
  describe('createCursorMdcFallbackContent', () => {
    it('should wrap raw agent markdown in valid Cursor MDC frontmatter', () => {
      ...
      expect(result).toMatch(/^---\ndescription: 'AEXOS agent @dev''ops lead'\nalwaysApply: false\n---/);
  ...
  describe('generateTemplateVariables', () => {
    it('should generate default variables', () => {
      ...
      expect(variables).toHaveProperty('cyryxVersion');
    });
    ...
    it('should include AEXOS version', () => {
      const wizardState = {};
      const variables = generateTemplateVariables(wizardState);

      expect(variables.cyryxVersion).toBeDefined();
      expect(typeof variables.cyryxVersion).toBe('string');
    });
  });
  ...
  describe('generateIDEConfigs', () => {
    ...
    it('should create config file for single IDE', async () => {
      ...
      const configPath = path.join(testDir, '.cursor', 'rules', 'aexos-global.mdc');
    ...
    it('should create config files for multiple IDEs', async () => {
      ...
      expect(await fs.pathExists(path.join(testDir, '.cursor', 'rules', 'aexos-global.mdc'))).toBe(true);
      ...
      expect(await fs.pathExists(path.join(testDir, '.gemini', 'rules', 'AEXOS', 'agents'))).toBe(true);
    });
    ...
    it('should install Claude Code native subagents and authority hook registration', async () => {
      ...
      expect(result.success).toBe(true);
      expect(await fs.pathExists(path.join(testDir, '.claude', 'agents', 'aexos-dev.md'))).toBe(true);
    ...
    it('should render template with variables', async () => {
      ...
      // Cursor uses .cursor/rules/aexos-global.mdc.
      const configPath = path.join(testDir, '.cursor', 'rules', 'aexos-global.mdc');
    ...
    it('should configure Gemini hooks and settings with active AEXOS hooks', async () => {
      ...
      const beforeAgentWrapper = settings.hooks.BeforeAgent.find(
        (w) => Array.isArray(w.hooks) && w.hooks.some((h) => h.name === 'aexos-context-inject'),
      );
      expect(beforeAgentWrapper).toBeDefined();
      const hook = beforeAgentWrapper.hooks.find((h) => h.name === 'aexos-context-inject');
  ...
  describe('generateCodexSkills', () => {
    ...
    beforeEach(async () => {
      await fs.ensureDir(path.join(testDir, '.aexos-core', 'development', 'agents'));
      await fs.copy(
        path.join(process.cwd(), '.aexos-core', 'development', 'agents'),
        path.join(testDir, '.aexos-core', 'development', 'agents'),
      );
    });
    ...
    it('should generate local Codex skills from canonical agents', async () => {
      ...
      const skillPath = path.join(testDir, '.codex', 'skills', 'aexos-architect', 'SKILL.md');
