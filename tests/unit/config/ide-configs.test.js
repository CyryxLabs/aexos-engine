 ...
 *
 * AEXOS supports 7 IDEs/CLIs, including Grok Build.
 */
...
describe('IDE Configs', () => {
  describe('IDE_CONFIGS', () => {
    it('should have 7 IDE configurations', () => {
    ...
    it('should have valid structure for each IDE', () => {
      Object.entries(IDE_CONFIGS).forEach(([key, config]) => {
        ...
        if (key !== 'grok') expect(typeof config.agentFolder).toBe('string');
      });
    });
    ...
    it('should have correct directory requirements', () => {
      ...
      expect(IDE_CONFIGS.grok.requiresDirectory).toBe(true);
    ...
    it('should have correct config file paths', () => {
      ...
      expect(IDE_CONFIGS.grok.configFile).toBe(path.join('.grok', 'rules', 'aexos-core.md'));
      expect(IDE_CONFIGS.gemini.configFile).toContain('.gemini');
      expect(IDE_CONFIGS.cursor.configFile).toBe(path.join('.cursor', 'rules', 'aexos-global.mdc'));
    ...
    it('should have Claude Code, Codex, and Grok as recommended', () => {
      expect(IDE_CONFIGS['claude-code'].recommended).toBe(true);
      expect(IDE_CONFIGS.codex.recommended).toBe(true);
      expect(IDE_CONFIGS.grok.recommended).toBe(true);
    });
  ...
  describe('getIDEConfig', () => {
    ...
    it('should return correct config for all IDEs', () => {
      const ides = ['claude-code', 'codex', 'grok', 'gemini', 'cursor', 'github-copilot', 'antigravity'];
