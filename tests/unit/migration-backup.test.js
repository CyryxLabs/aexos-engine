...
const {
  ...
  findLatestBackup,
  listBackups,
} = require('../../.aexos-core/cli/commands/migrate/backup');
...
describe('Migration Backup Module', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `aexos-backup-test-${testId}`);
  ...
  describe('createBackupDirName', () => {
    it('should create backup directory name with date format', () => {
      ...
      expect(name).toMatch(/^\.aexos-backup-\d{4}-\d{2}-\d{2}$/);
    });

    it('should use current date', () => {
      const name = createBackupDirName();
      const today = new Date().toISOString().split('T')[0];
      expect(name).toBe(`.aexos-backup-${today}`);
    });
  });
  ...
  describe('createBackup', () => {
    it('should create backup of .aexos-core directory', async () => {
      // Create mock .aexos-core structure
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(path.join(cyryxCoreDir, 'agents'), { recursive: true });
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'agents', 'test.md'), 'Agent content');
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'index.js'), 'module.exports = {}');
    ...
    it('should fail if .aexos-core does not exist', async () => {
      await expect(createBackup(testDir)).rejects.toThrow(/No .aexos-core directory/);
    });

    it('should include backup manifest', async () => {
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(cyryxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'test.js'), 'test');
  ...
  describe('verifyBackup', () => {
    it('should verify valid backup', async () => {
      ...
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(cyryxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'test.js'), 'test content');
    ...
    it('should detect corrupted files', async () => {
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      await fs.promises.mkdir(cyryxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(cyryxCoreDir, 'test.js'), 'original');

      const backupResult = await createBackup(testDir);

      // Corrupt a file in backup
      const backedUpFile = path.join(backupResult.backupDir, '.aexos-core', 'test.js');
  ...
  describe('findLatestBackup', () => {
    it('should find the most recent backup', async () => {
      ...
      expect(latest.name).toMatch(/^\.aexos-backup-/);
    });
