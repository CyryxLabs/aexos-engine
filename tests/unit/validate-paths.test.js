...
const { validatePaths } = require('../../.aexos-core/infrastructure/scripts/validate-paths');
...
describe('Path Validator', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-path-validate-'));
  ...
  it('passes with relative canonical paths', () => {
    ...
    write(path.join(tmpRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'), '# codex\n');
    write(
      path.join(skillsDir, 'aexos-dev', 'SKILL.md'),
      [
        '# Skill',
        'Load .aexos-core/development/agents/dev.md',
        'Run node .aexos-core/development/scripts/generate-greeting.js dev',
      ].join('\n'),
    );
    ...
    const result = validatePaths({
      ...
      requiredFiles: [
        ...
        path.join(tmpRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
      ],
    });
  ...
  it('fails when absolute user path is found', () => {
    write(path.join(tmpRoot, 'AGENTS.md'), 'Path /Users/alan/Code/aexos-core');
    write(path.join(tmpRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'), '# codex\n');
    write(
      path.join(skillsDir, 'aexos-dev', 'SKILL.md'),
      [
  ...
  it('fails when skill lacks canonical activation paths', () => {
    write(path.join(tmpRoot, 'AGENTS.md'), '# Agents\n');
    write(path.join(tmpRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'), '# codex\n');
    write(path.join(skillsDir, 'aexos-dev', 'SKILL.md'), '# Skill\nUse dev\n');
  ...
  it('accepts generated squad-chief skills with squad source paths', () => {
    ...
    write(path.join(tmpRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'), '# codex\n');
    write(
      path.join(skillsDir, 'aexos-demo-chief', 'SKILL.md'),
      [
        '# Demo Chief',
