...
const {
  ...
  formatNotFoundError,
  wrapText,
} = require('../../.aexos-core/cli/commands/workers/formatters/info-formatter');
const { findSuggestions, findRelatedWorkers } = require('../../.aexos-core/cli/commands/workers/info');
const { levenshteinDistance } = require('../../.aexos-core/cli/commands/workers/search-keyword');
...
describe('Info Formatter - Pretty Output', () => {
  ...
  test('formatInfoPretty includes usage example', () => {
    ...
    expect(output).toContain('cyryx task run json-csv-transformer');
  });
...
describe('Not Found Error Formatter', () => {
  ...
  test('formatNotFoundError includes search hint', () => {
    ...
    expect(output).toContain('aexos workers search invalid');
  });
});
