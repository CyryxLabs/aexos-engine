...
const SessionContextLoader = require('../../.aexos-core/scripts/session-context-loader');

describe('SessionContextLoader', () => {
  let loader;
  const testSessionPath = path.join(process.cwd(), '.aexos', 'session-state-test.json');
  ...
  describe('loadContext() - Session Detection', () => {
    ...
    test('detects existing session after agent activation', () => {
      ...
      loader.updateSession('po', 'Themis', 'validate-story-draft');
  ...
  describe('loadContext() - Agent Transition Tracking', () => {
    test('tracks agent transitions correctly', () => {
      ...
      loader.updateSession('po', 'Themis', 'validate-story-draft');
      loader.updateSession('dev', 'Vulcan', 'develop');

      // Load context for @qa
      const context = loader.loadContext('qa');

      expect(context.previousAgent.agentId).toBe('dev');
      expect(context.previousAgent.agentName).toBe('Vulcan');
    ...
    test('skips same agent in previous agent detection', () => {
      ...
      loader.updateSession('po', 'Themis', 'create-story');
      loader.updateSession('dev', 'Vulcan', 'develop');
      loader.updateSession('dev', 'Vulcan', 'run-tests');
    ...
    test('limits command history to 10 entries', () => {
      // Add 15 commands
      for (let i = 1; i <= 15; i++) {
        loader.updateSession('dev', 'Vulcan', `command-${i}`);
      }
  ...
  describe('updateSession() - Session State Management', () => {
    ...
    test('limits agent sequence to last 20 activations', () => {
      ...
      for (let i = 1; i <= 25; i++) {
        loader.updateSession('dev', 'Vulcan', `develop-${i}`);
      }
  ...
  describe('generateContextMessage() - Message Generation', () => {
    test('generates message with previous agent info', () => {
      ...
      expect(context.message).toContain('Themis');
  ...
  describe('getPreviousAgent() - Agent History', () => {
    ...
    test('returns most recent different agent', () => {
      const sessionState = {
        agentSequence: [
          { agentId: 'po', agentName: 'Themis', activatedAt: Date.now() - 10000, lastCommand: 'create-story' },
          { agentId: 'dev', agentName: 'Vulcan', activatedAt: Date.now() - 5000, lastCommand: 'develop' },
          { agentId: 'qa', agentName: 'Argus', activatedAt: Date.now(), lastCommand: 'review' },
        ],
      };
      ...
      expect(previous.agentName).toBe('Vulcan');
    });
  });
