...
const VALIDATOR_PATH = path.join(PROJECT_ROOT, 'scripts', 'validate-aexos-core-deps.js');

describe('validate-aexos-core-deps.js (INS-4.12)', () => {
  ...
  test('.aexos-core/package.json includes fast-glob', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fast-glob');
  });

  test('.aexos-core/package.json includes fs-extra', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fs-extra');
  });

  test('.aexos-core/package.json includes semver', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('semver');
  });

  test('.aexos-core/package.json includes ajv', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('ajv');
  });

  test('.aexos-core/package.json includes tar', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('tar');
  });

  test('.aexos-core/package.json includes proper-lockfile for IDS registry updater', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.aexos-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
describe('validate-publish.js dependency check (INS-4.12)', () => {
  test('validate-publish includes dependency completeness check', () => {
    ...
    expect(content).toContain('validate-aexos-core-deps');
  });
});
...
describe('wizard NODE_PATH fix (INS-4.12)', () => {
  test('wizard/index.js sets NODE_PATH for entity registry bootstrap', () => {
    ...
    expect(content).toContain('cyryxCoreNodeModules');
  });
...
describe('doctor npm-packages check (INS-4.12)', () => {
  test('doctor check validates .aexos-core/node_modules/', () => {
    const checkPath = path.join(PROJECT_ROOT, '.aexos-core', 'core', 'doctor', 'checks', 'npm-packages.js');
    const content = fs.readFileSync(checkPath, 'utf8');
    expect(content).toContain('cyryxCoreNodeModules');
    expect(content).toContain('.aexos-core');
    expect(content).toContain('INS-4.12');
  });
});
