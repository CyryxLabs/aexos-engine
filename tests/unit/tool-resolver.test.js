...
`;
  ...
  describe('Cache Mechanism', () => {
    test('should cache tool after first resolution', async () => {
      // Mock file system
      fg.sync.mockReturnValue(['aexos-core/tools/test-simple.yaml']);
  ...
  describe('Search Path Priority', () => {
    ...
    test('should fall back to core when squad tool not found', async () => {
      ...
      fg.sync.mockImplementation((pattern) => {
        ...
        if (pattern.includes('aexos-core/tools')) {
          return ['aexos-core/tools/test-simple.yaml'];
        }
        return [];
      });
  ...
  describe('Schema Validation', () => {
    test('should validate required fields', async () => {
      fg.sync.mockReturnValue(['aexos-core/tools/invalid.yaml']);
    ...
    test('should validate v1.0 tool schema', async () => {
      fg.sync.mockReturnValue(['aexos-core/tools/simple.yaml']);
      fs.readFile.mockResolvedValue(simpleToolYaml);
    ...
    test('should validate v2.0 tool schema with executable knowledge', async () => {
      fg.sync.mockReturnValue(['aexos-core/tools/complex.yaml']);
    ...
    test('should reject invalid YAML syntax', async () => {
      fg.sync.mockReturnValue(['aexos-core/tools/broken.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/auto.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/api.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/anti.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/health.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/http-health.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['aexos-core/tools/function-health.yaml']);
  ...
  describe('Additional Methods', () => {
    test('should list all available tools', async () => {
      fg.sync.mockReturnValue([
        'aexos-core/tools/tool1.yaml',
        'aexos-core/tools/tool2.yaml',
    ...
    test('should check if tool exists without loading', async () => {
      ...
      fg.sync.mockImplementation((pattern) => {
        if (pattern.includes('/exists.yaml')) {
          return ['aexos-core/tools/exists.yaml'];
        }
        return [];
      });
