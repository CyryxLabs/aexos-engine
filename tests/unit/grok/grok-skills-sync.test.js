// Adapted from AIOX 4ef6530ff03b83aea953e4a426f95e012b8b70c5.
// Source SHA256: a93b859c3dcffa8bc891fc21cb40f3e332b2473319c0d2a097fc68b48fac74ab
// Corrective fixture: stale owned bytes carry their actual hash; changed owner files must be preserved.
'use strict';
...
const {
  ...
  getSkillId,
} = require('../../../.aexos-core/infrastructure/scripts/grok-skills-sync/index');
...
describe('Grok skills sync + validate', () => {
  it('syncs agents, short aliases, hooks, and config into a temp .grok tree', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-grok-sync-'));
    ...
    const result = syncGrok({
      ...
      sourceDir: path.join(repoRoot, '.aexos-core', 'development', 'agents'),
      grokRoot,
      quiet: true,
    });
    ...
    expect(fs.existsSync(path.join(grokRoot, 'rules', 'aexos-core.md'))).toBe(true);
    expect(fs.existsSync(path.join(grokRoot, 'aexos-managed.json'))).toBe(true);

    // Activation protocol must register active-agent bridge
    const devopsSkill = fs.readFileSync(
      path.join(grokRoot, 'skills', 'aexos-devops', 'SKILL.md'),
      'utf8',
    );
    expect(devopsSkill).toContain('.aexos/active-agent');
  ...
  it('preserves brownfield rules outside AEXOS managed sections', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-grok-rules-'));
    const grokRoot = path.join(tmp, '.grok');
    const rulesPath = path.join(grokRoot, 'rules', 'aexos-core.md');
  ...
  it('detects managed content drift but permits custom rule content', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-grok-drift-'));
    const grokRoot = path.join(tmp, '.grok');
    syncGrok({ projectRoot: repoRoot, grokRoot, quiet: true });
    ...
    const agentPath = path.join(grokRoot, 'agents', 'aexos-dev.md');
    fs.appendFileSync(agentPath, '\nMANAGED-DRIFT\n', 'utf8');
    const result = validateGrok({ projectRoot: repoRoot, grokRoot, strict: true, quiet: true });
    expect(result.ok).toBe(false);
    expect(result.errors).toContain('Managed content drift: agents/aexos-dev.md');
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it('removes stale owned artifacts and preserves project extensions', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-grok-owned-'));
    const grokRoot = path.join(tmp, '.grok');
    syncGrok({ projectRoot: repoRoot, grokRoot, quiet: true });

    const stalePath = path.join(grokRoot, 'skills', 'aexos-obsolete', 'SKILL.md');
    ...
    const manifestPath = path.join(grokRoot, 'aexos-managed.json');
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    manifest.files.push({ path: 'skills/aexos-obsolete/SKILL.md', mode: 'full', sha256: require('crypto').createHash('sha256').update('stale').digest('hex') });
