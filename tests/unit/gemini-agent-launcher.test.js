...
const {
  ...
  commandNameForAgent,
} = require('../../packages/gemini-aexos-extension/commands/lib/agent-launcher');
...
describe('gemini agent launcher', () => {
  ...
  it('lists canonical agents from source-of-truth directory', () => {
    ...
    write(path.join(tmpRoot, '.aexos-core', 'development', 'agents', 'architect.md'), '# architect');
    write(path.join(tmpRoot, '.aexos-core', 'development', 'agents', '_README.md'), '# ignore');
  ...
  it('detects agent presence in canonical or gemini mirrored folders', () => {
    write(path.join(tmpRoot, '.gemini', 'rules', 'AEXOS', 'agents', 'qa.md'), '# qa');
  ...
  it('builds deterministic activation prompt', () => {
    ...
    expect(prompt).toContain('.aexos-core/development/agents/devops.md');
  ...
  it('maps command name correctly for master agent', () => {
    expect(commandNameForAgent('aexos-master')).toBe('/aexos-master');
    expect(commandNameForAgent('dev')).toBe('/aexos-dev');
  });
});
