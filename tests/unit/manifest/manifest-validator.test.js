...
const {
  ...
  parseCSV,
  parseCSVLine,
} = require('../../../.aexos-core/core/manifest/manifest-validator');
  ...
  describe('ManifestValidator', () => {
    ...
    describe('getAgentsSchema', () => {
      test('returns correct schema for agents', () => {
        ...
        expect(schema.sourceDir).toBe('.aexos-core/development/agents');
      });
    });
    ...
    describe('getTasksSchema', () => {
      test('returns correct schema for tasks', () => {
        ...
        expect(schema.required).toContain('id');
        expect(schema.required).toContain('name');
        expect(schema.sourceDir).toBe('.aexos-core/development/tasks');
      });
    });
    ...
    describe('validateAll', () => {
      test('validates all manifests', async () => {
        ...
        const { createManifestGenerator } = require('../../../.aexos-core/core/manifest/manifest-generator');
      ...
      test('reports valid manifests correctly', async () => {
        ...
        expect(results.summary).toMatchObject({ valid: 3, invalid: 0 });
      });
    });

    describe('validateManifest', () => {
      test('keeps duplicate worker IDs invalid', async () => {
        const manifestPath = `${basePath}/.aexos-core/manifests/workers.csv`;
        await fs.mkdir(`${basePath}/.aexos-core/manifests`, { recursive: true });
        await fs.writeFile(
          manifestPath,
          [
            'id,name,category,subcategory,executor_types,tags,file_path,status',
            'duplicate,First,script,,CLI,,.aexos-core/core/manifest/manifest-generator.js,active',
            'duplicate,Second,script,,CLI,,.aexos-core/core/manifest/manifest-validator.js,active',
          ].join('\n'),
        );

        const result = await validator.validateManifest('workers.csv', validator.getWorkersSchema());
        expect(result.valid).toBe(false);
        expect(result.errors).toContain("Duplicate ID 'duplicate' at line 3");
      });
    });
