...
const {
  ...
  buildResult,
  formatReport,
} = require('../../.aexos-core/infrastructure/scripts/framework-3way-diff');
...
describe('framework-3way-diff (Wave 0)', () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(rootA, '.aexos-core', 'core', 'errors'), {
      recursive: true,
    });
    fs.mkdirSync(path.join(rootA, '.aexos-core', 'core', 'foo'), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(rootA, '.aexos-core', 'core', 'errors', 'x.js'),
      'module.exports = 1;\n',
    );
    fs.writeFileSync(path.join(rootA, '.aexos-core', 'core', 'foo', 'a.js'), 'const a = 1;\n');

    fs.mkdirSync(path.join(rootB, '.aexos-core', 'core', 'foo'), {
      recursive: true,
    });
    fs.mkdirSync(path.join(rootB, '.aexos-core', 'core', 'bar'), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(rootB, '.aexos-core', 'core', 'foo', 'a.js'),
      'const a = 2;\nconst b = 3;\n',
    );
    fs.writeFileSync(path.join(rootB, '.aexos-core', 'core', 'bar', 'only.js'), 'only\n');

    fs.mkdirSync(path.join(rootC, '.aexos-core', 'core', 'foo'), {
      recursive: true,
    });
    fs.mkdirSync(path.join(rootC, '.aexos-core', 'development', 'skills', 'full-sdc'), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(rootC, '.aexos-core', 'core', 'foo', 'a.js'),
      'const a = 2;\nconst b = 3;\n',
    );
    fs.writeFileSync(
      path.join(rootC, '.aexos-core', 'development', 'skills', 'full-sdc', 'SKILL.md'),
  ...
  it('indexes relative paths under .aexos-core', () => {
  ...
  it('uses pair-specific labels and omits OSS-wins from hub comparisons', () => {
    fs.writeFileSync(path.join(rootC, '.aexos-core', 'core', 'foo', 'a.js'), 'const a = 99;\n');
  ...
  it('fails closed with the unreadable tree and path in the diagnostic', () => {
    const target = path.join(rootA, '.aexos-core', 'core', 'foo', 'a.js');
