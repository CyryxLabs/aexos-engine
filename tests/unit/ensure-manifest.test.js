...
describe('ensure-manifest', () => {
  describe('shouldCheckManifest', () => {
    ...
    it('returns false when only manifest file is staged', () => {
      expect(shouldCheckManifest(['.aexos-core/install-manifest.yaml'])).toBe(false);
    });

    it('returns true when any other .aexos-core file is staged', () => {
      expect(shouldCheckManifest(['.aexos-core/core-config.yaml'])).toBe(true);
      expect(
        shouldCheckManifest([
          'README.md',
    ...
    it('returns false when only non-.aexos-core files are staged', () => {
