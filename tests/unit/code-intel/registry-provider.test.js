...
const { RegistryProvider, LAYER_PRIORITY } = require('../../../.aexos-core/core/code-intel/providers/registry-provider');
const { CodeIntelProvider } = require('../../../.aexos-core/core/code-intel/providers/provider-interface');
const { CodeGraphProvider } = require('../../../.aexos-core/core/code-intel/providers/code-graph-provider');
const { CodeIntelClient, CB_CLOSED } = require('../../../.aexos-core/core/code-intel/code-intel-client');
...
entities:
  tasks:
    create-story:
      ...
      dependencies:
        - story-template
    validate-story:
      path: .aexos-core/development/tasks/validate-story.md
      layer: L2
      type: task
  ...
  agents:
    dev-agent:
      path: .aexos-core/development/agents/dev.md
      layer: L2
      type: agent
      ...
      dependencies:
        - create-story
    sm-agent:
      path: .aexos-core/development/agents/sm.md
      layer: L2
      type: agent
  ...
  templates:
    story-template:
      path: .aexos-core/development/templates/story-tmpl.yaml
      layer: L2
      type: template
  ...
  core:
    code-intel-client:
      path: .aexos-core/core/code-intel/code-intel-client.js
      layer: L1
      type: module
...
entities:
  tasks:
    yaml-validator:
      path: .aexos-core/development/tasks/yaml-validator.md
      layer: L2
      type: task
      ...
      dependencies: []
    index:
      path: .aexos-core/development/tasks/index.md
      layer: L2
      type: task
  ...
  scripts:
    yaml-validator:
      path: .aexos-core/development/scripts/yaml-validator.js
      layer: L2
      type: script
      ...
      dependencies: []
    backup-manager:
      path: .aexos-core/development/scripts/backup-manager.js
      layer: L2
      type: script
  ...
  core:
    yaml-validator:
      path: .aexos-core/core/yaml-validator.js
      layer: L1
      type: module
    ...
    index:
      ...
      dependencies: []
    backup-manager:
      path: .aexos-core/core/backup-manager.js
      layer: L1
      type: module
  ...
  templates:
    yaml-validator:
      path: .aexos-core/development/templates/yaml-validator.yaml
      layer: L3
      type: template
      ...
      dependencies: []
    index:
      path: .aexos-core/development/templates/index.yaml
      layer: L3
      type: template
      ...
      dependencies: []
    backup-manager:
      path: .aexos-core/development/templates/backup-manager.yaml
      layer: L3
      type: template
...
describe('RegistryProvider — 5 Implemented Primitives', () => {
  ...
  describe('findDefinition()', () => {
    test('finds entity by exact name', async () => {
      ...
      expect(result.file).toBe('.aexos-core/development/tasks/create-story.md');
  ...
  describe('findReferences()', () => {
    ...
    test('finds references via dependencies field', async () => {
      ...
      expect(paths).toContain('.aexos-core/development/tasks/create-story.md');
    });
...
describe('RegistryProvider — Entity Name Disambiguation', () => {
  ...
  test('yaml-validator has 4 entries, all preserved in index', async () => {
    ...
    expect(result.file).toBe('.aexos-core/core/yaml-validator.js');
  });
  ...
  test('index has 3 entries, all preserved', async () => {
    ...
    expect(result).not.toBeNull();
    // L1 core should win
    expect(result.file).toBe('.aexos-core/core/index.js');
  });

  test('backup-manager has 3 entries, L1 wins', async () => {
    const result = await provider.findDefinition('backup-manager');
    expect(result).not.toBeNull();
    expect(result.file).toBe('.aexos-core/core/backup-manager.js');
  });
  ...
  test('disambiguation scoring: L1 > L2 > L3', async () => {
    ...
    const result = await provider.findDefinition('yaml-validator');
    expect(result.file).toContain('.aexos-core/core/');
  });
});
...
`);
    ...
    // This contract is mtime invalidation. A 100ms sleep does not advance mtime
    // on filesystems with two-second timestamp resolution (including this
    // Windows workspace), so change the timestamp explicitly after writing.
    const firstMtime = fs.statSync(tempPath).mtimeMs;
    fs.writeFileSync(tempPath, `
entities:
  tasks:
    task-v2:
...
`);
    ...
    const changedTime = new Date(firstMtime + 4000);
    fs.utimesSync(tempPath, changedTime, changedTime);
    expect(fs.statSync(tempPath).mtimeMs).not.toBe(firstMtime);
