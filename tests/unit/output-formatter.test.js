...
const PersonalizedOutputFormatter = require('../../.aexos-core/infrastructure/scripts/output-formatter');
const OutputPatternValidator = require('../../.aexos-core/infrastructure/scripts/validate-output-pattern');
const fs = require('fs');
...
`;
  ...
  describe('Core Formatting', () => {
    test('generates valid output for Builder agent', () => {
      ...
      expect(output).toContain('**Agent:** Vulcan (Builder)');
    ...
    test('places Metrics section last', () => {
      ...
      expect(lines[lastSectionIndex]).toContain('— Vulcan');
    });
  });
  ...
  describe('Personality Injection', () => {
    ...
    test('injects signature closing correctly', () => {
      ...
      expect(signature).toBe('— Vulcan, sempre construindo 🔨');
    });
...
`;
    ...
    test('handles missing agent file gracefully', () => {
      ...
      expect(output).toContain('## 📊 Task Execution Report');
      expect(output).toContain('**Agent:** Vulcan (Agent)'); // Neutral fallback
    });
  ...
  describe('All 11 Agents', () => {
    const agents = [
      { id: 'dev', name: 'Vulcan', archetype: 'Builder', tone: 'pragmatic' },
      { id: 'qa', name: 'Argus', archetype: 'Guardian', tone: 'analytical' },
      { id: 'po', name: 'Themis', archetype: 'Balancer', tone: 'collaborative' },
      { id: 'pm', name: 'Janus', archetype: 'Visionary', tone: 'pragmatic' },
      { id: 'sm', name: 'Chronos', archetype: 'Flow Master', tone: 'empathetic' },
      { id: 'architect', name: 'Vega', archetype: 'Architect', tone: 'analytical' },
      { id: 'analyst', name: 'Sirius', archetype: 'Explorer', tone: 'analytical' },
      { id: 'ux-design-expert', name: 'Iris', archetype: 'Empathizer', tone: 'empathetic' },
      { id: 'data-engineer', name: 'Ceres', archetype: 'Engineer', tone: 'pragmatic' },
      { id: 'devops', name: 'Polaris', archetype: 'Operator', tone: 'pragmatic' },
      { id: 'aexos-master', name: 'Zeus', archetype: 'Orchestrator', tone: 'collaborative' },
    ];
