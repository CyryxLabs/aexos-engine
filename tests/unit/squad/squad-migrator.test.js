...
describe('SquadMigrator', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should detect missing cyryx.type field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);

      expect(analysis.needsMigration).toBe(true);
      expect(analysis.issues.some((i) => i.type === 'MISSING_AEXOS_TYPE')).toBe(true);
      expect(analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'cyryx.type')).toBe(
        true,
      );
    });

    it('should detect missing cyryx.minVersion field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);
      ...
      expect(
        analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'cyryx.minVersion'),
      ).toBe(true);
    });
    ...
    it('should return needsMigration: false for up-to-date squad', async () => {
      ...
      const manifest = {
        cyryx: { type: 'squad', minVersion: '2.1.0' },
  ...
  describe('migrate()', () => {
    ...
    it('should add missing cyryx.type and cyryx.minVersion fields (AC 5)', async () => {
      // Copy legacy squad to temp dir
      const srcPath = path.join(FIXTURES_PATH, 'legacy-squad');
      ...
      expect(manifest.cyryx?.type).toBe('squad');
      expect(manifest.cyryx?.minVersion).toBe('2.1.0');
    });
