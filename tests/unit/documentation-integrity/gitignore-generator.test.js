...
const {
  ...
  GitignoreTemplates,
  TechStack,
} = require('../../../.aexos-core/infrastructure/scripts/documentation-integrity/gitignore-generator');

describe('Gitignore Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-gitignore-test-'));
  });
  ...
  describe('loadGitignoreTemplate', () => {
    it('should load CYRYX base template', () => {
      const template = loadGitignoreTemplate(GitignoreTemplates.AEXOS_BASE);

      expect(template).toContain('.aexos-core/local/');
    ...
    it('should load brownfield merge template', () => {
      ...
      expect(template).toContain('AEXOS Integration Section');
    });
  ...
  describe('getTemplatesForStacks', () => {
    it('should always include CYRYX base', () => {
      const templates = getTemplatesForStacks([]);

      expect(templates).toContain(GitignoreTemplates.AEXOS_BASE);
    });
  ...
  describe('generateGitignore', () => {
    it('should generate gitignore for Node.js project', () => {
      ...
      expect(content).toContain('.aexos-core/local/');
    });
  ...
  describe('mergeGitignore', () => {
    it('should append CYRYX section to existing content', () => {
    ...
    it('should skip if CYRYX section already exists', () => {
      const existing = '# AEXOS Integration Section\n.aexos-core/\n';
      const merged = mergeGitignore(existing);
  ...
  describe('generateGitignoreFile', () => {
    ...
    it('should merge with existing .gitignore', () => {
      ...
      expect(result.content).toContain('AEXOS Integration Section');
    });
  ...
  describe('hasCyryxIntegration', () => {
    it('should return false for empty directory', () => {
      expect(hasCyryxIntegration(tempDir)).toBe(false);
    });

    it('should return true if CYRYX section exists', () => {
      fs.writeFileSync(
        path.join(tempDir, '.gitignore'),
        '# AEXOS Integration Section\n.aexos-core/\n',
      );

      expect(hasCyryxIntegration(tempDir)).toBe(true);
    });

    it('should return true if .aexos-core/ pattern exists', () => {
      fs.writeFileSync(path.join(tempDir, '.gitignore'), '.aexos-core/local/\n');
  ...
  describe('parseGitignore', () => {
    ...
    it('should identify CYRYX section', () => {
      const content = `# Header
node_modules/
# AEXOS Integration Section
.aexos-core/
# End of AEXOS Integration Section
`;
      const parsed = parseGitignore(content);

      expect(parsed.cyryxSection).toBeDefined();
      expect(parsed.cyryxSection.start).toBeGreaterThan(0);
    });

    it('should return null cyryxSection when not present', () => {
      const content = 'node_modules/\n';
      const parsed = parseGitignore(content);

      expect(parsed.cyryxSection).toBeNull();
    });
  });

  describe('GitignoreTemplates enum', () => {
    it('should have all required templates', () => {
      expect(GitignoreTemplates.AEXOS_BASE).toBe('gitignore-aexos-base.tmpl');
