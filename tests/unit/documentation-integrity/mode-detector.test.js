...
const {
  ...
  collectMarkers,
  isCyryxCoreRepository,
  mapLegacyTypeToMode,
  validateModeSelection,
  getModeOptions,
  InstallationMode,
  LegacyProjectType,
} = require('../../../.aexos-core/infrastructure/scripts/documentation-integrity/mode-detector');
...
describe('Mode Detector', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-test-'));
  });
  ...
  describe('detectInstallationMode', () => {
    ...
    it('should detect FRAMEWORK_DEV mode for aexos-core repository', () => {
      // Create .aexos-core directory
      fs.mkdirSync(path.join(tempDir, '.aexos-core'));

      // Create package.json with aexos-core name
      fs.writeFileSync(
      ...
      expect(result.legacyType).toBe(LegacyProjectType.EXISTING_CYRYX);
      expect(result.confidence).toBe(100);
      expect(result.markers.isCyryxCoreRepo).toBe(true);
    });

    it('should detect BROWNFIELD mode for user project with existing AEXOS', () => {
      // Create .aexos-core directory
      fs.mkdirSync(path.join(tempDir, '.aexos-core'));
  ...
  describe('isCyryxCoreRepository', () => {
    it('should return true for @cyryx/core package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: '@cyryx/core' }),
      );

      expect(isCyryxCoreRepository(tempDir)).toBe(true);
    });

    it('should return true for aexos-core package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'aexos-core' }),
      );
    ...
    it('should return false for generic monorepo with workspaces pattern only', () => {
      // Generic monorepos should NOT be detected as aexos-core
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );

      expect(isCyryxCoreRepository(tempDir)).toBe(false);
    });

    it('should return true for workspaces pattern with cyryx marker', () => {
      // Workspaces + .aexos-core/infrastructure marker = aexos-core repo
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );
      fs.mkdirSync(path.join(tempDir, '.aexos-core', 'infrastructure'), { recursive: true });
  ...
  describe('mapLegacyTypeToMode', () => {
    it('should map EXISTING_CYRYX to BROWNFIELD by default (safer)', () => {
      // Without context, EXISTING_CYRYX defaults to BROWNFIELD (won't skip project setup)
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_CYRYX)).toBe(
        InstallationMode.BROWNFIELD,
      );
    });

    it('should map EXISTING_CYRYX to FRAMEWORK_DEV with isCyryxCoreRepo context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_CYRYX, { isCyryxCoreRepo: true })).toBe(
        InstallationMode.FRAMEWORK_DEV,
      );
    });

    it('should map EXISTING_CYRYX to BROWNFIELD with non-aexos-core context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_CYRYX, { isCyryxCoreRepo: false })).toBe(
        InstallationMode.BROWNFIELD,
      );
    });
  ...
  describe('validateModeSelection', () => {
    ...
    it('should warn when selecting framework-dev for non-aexos-core repo', () => {
      const detected = {
        mode: InstallationMode.BROWNFIELD,
        markers: { isCyryxCoreRepo: false },
      };

      const result = validateModeSelection(InstallationMode.FRAMEWORK_DEV, detected);

      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('aexos-core repository');
    });
  ...
  describe('LegacyProjectType enum', () => {
    it('should have all legacy types', () => {
      expect(LegacyProjectType.EXISTING_CYRYX).toBe('EXISTING_CYRYX');
