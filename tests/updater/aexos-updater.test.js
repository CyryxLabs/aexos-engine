...
describe('AEXOSUpdater', () => {
  let tempDir;
  let updater;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-updater-test-'));

    // Create minimal .aexos-core structure
    await fs.ensureDir(path.join(tempDir, '.aexos-core'));
    await fs.ensureDir(path.join(tempDir, '.aexos'));

    // Create version.json
    await fs.writeJson(path.join(tempDir, '.aexos-core', 'version.json'), {
      version: '1.0.0',
    ...
    updater = new CYRYXUpdater(tempDir, {
      verbose: false,
  ...
  describe('constructor', () => {
    it('should initialize with default options', () => {
      const u = new CYRYXUpdater(tempDir);
      ...
      expect(u.options.reconcileDependencies).toBe(false);
    });
  ...
  describe('getInstalledVersion', () => {
    ...
    it('should keep legacy package fallback for brownfield installs', async () => {
      await fs.remove(path.join(tempDir, '.aexos-core', 'version.json'));
      await fs.ensureDir(path.join(tempDir, 'node_modules', 'aexos-core'));
      await fs.writeJson(path.join(tempDir, 'node_modules', 'aexos-core', 'package.json'), {
        name: 'aexos-core',
        version: '5.0.7',
      });
    ...
    it('should handle corrupted version.json', async () => {
      await fs.writeFile(path.join(tempDir, '.aexos-core', 'version.json'), 'invalid json');
      const version = await updater.getInstalledVersion();
      expect(version).toBeNull();
    });
  });
  ...
  describe('detectCustomizations', () => {
    it('should detect unchanged files', async () => {
      ...
      const testFile = path.join(tempDir, '.aexos-core', 'test-file.md');
  ...
  describe('createBackup', () => {
    ...
    it('should backup version.json', async () => {
      await updater.createBackup();
      const backupVersionJson = path.join(updater.backupDir, 'framework', 'version.json');
      expect(fs.existsSync(backupVersionJson)).toBe(true);
    });

    it('should backup manifest signature when present', async () => {
      await fs.writeFile(
        path.join(tempDir, '.aexos-core', 'install-manifest.yaml.minisig'),
        'signature',
        'utf8',
      );
      ...
      expect(
        fs.existsSync(
          path.join(updater.backupDir, 'framework', 'install-manifest.yaml.minisig'),
        ),
      ).toBe(true);
    });
  });
  ...
  describe('rollback', () => {
    ...
    it('should restore files from backup', async () => {
      ...
      // Verify restored
      const restored = await fs.readJson(path.join(tempDir, '.aexos-core', 'version.json'));
  ...
  describe('updateVersionInfo', () => {
    it('should update version.json with new version', async () => {
      ...
      const versionJson = await fs.readJson(path.join(tempDir, '.aexos-core', 'version.json'));
  ...
  describe('update execution', () => {
    it('should validate against the updated package source and persist manifest hashes', async () => {
      ...
      jest.spyOn(updater, 'applyUpdate').mockResolvedValue({
        ...
        sourcePackageRoot: '/tmp/aexos-package',
        sourceManifest,
      });
      ...
      jest.spyOn(updater, 'updateBackupState').mockResolvedValue();
