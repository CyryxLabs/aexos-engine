 ...
 *
 * Before the fix: `aexos install --ci --yes --merge --ide claude-code` would
 ...
 * then process.env.CI / AEXOS_NON_INTERACTIVE, then TTY presence. When ANY
...
describe('isNonInteractive() — signal precedence (#739 Bug 1)', () => {
  ...
  it('returns true when AEXOS_NON_INTERACTIVE is "true"', () => {
    process.env.AEXOS_NON_INTERACTIVE = 'true';
    expect(isNonInteractive()).toBe(true);
  });
