...
describe('brownfield-upgrader', () => {
  ...
  describe('generateUpgradeReport', () => {
    ...
    it('should preserve local files missing from installed manifest when they already exist on disk', () => {
      ...
      fs.ensureDirSync(path.join(cyryxCoreDir, 'development', 'agents', 'dev'));
      fs.writeFileSync(
        path.join(cyryxCoreDir, 'development', 'agents', 'dev', 'MEMORY.md'),
    ...
    it('should treat untracked local files as unchanged when they already match source', () => {
      const cyryxCoreDir = path.join(targetDir, '.aexos-core');
      fs.ensureDirSync(cyryxCoreDir);
      const filePath = path.join(cyryxCoreDir, 'already-there.md');
    ...
    it('should identify modified files', () => {
      const cyryxCoreDir = path.join(targetDir, '.aexos-core');
      fs.ensureDirSync(cyryxCoreDir);
      fs.writeFileSync(path.join(cyryxCoreDir, 'changed.md'), 'original');
      const originalHash = `sha256:${hashFile(path.join(cyryxCoreDir, 'changed.md'))}`;
    ...
    it('should identify user-modified files', () => {
      const cyryxCoreDir = path.join(targetDir, '.aexos-core');
      fs.ensureDirSync(cyryxCoreDir);
      fs.writeFileSync(path.join(cyryxCoreDir, 'user-changed.md'), 'user modified content');
  ...
  describe('applyUpgrade', () => {
    ...
    it('should install new files', async () => {
      ...
      expect(fs.existsSync(path.join(targetDir, '.aexos-core', 'new-file.md'))).toBe(true);
    });
    ...
    it('should perform dry run without modifying files', async () => {
      ...
      expect(result.filesInstalled).toHaveLength(1);
      expect(fs.existsSync(path.join(targetDir, '.aexos-core', 'new-file.md'))).toBe(false);
    });
  });

  describe('updateInstalledManifest', () => {
    it('prepares deterministic installed-manifest bytes before writing', () => {
      const sourceManifest = {
        version: '2.1.0',
      ...
      expect(prepared.content).toContain('# AEXOS Core Installed Manifest');
      expect(yaml.load(prepared.content)).toMatchObject({
        installed_at: '2026-01-02T03:04:05.000Z',
        installed_from: 'aexos-core@2.1.0',
    ...
    it('should create installed manifest file', () => {
      ...
      fs.ensureDirSync(path.join(targetDir, '.aexos-core'));
      updateInstalledManifest(targetDir, sourceManifest, 'aexos-core@2.1.0');

      const installedPath = path.join(targetDir, '.aexos-core', '.installed-manifest.yaml');
      expect(fs.existsSync(installedPath)).toBe(true);

      const content = yaml.load(fs.readFileSync(installedPath, 'utf8'));
      expect(content.installed_version).toBe('2.1.0');
      expect(content.installed_from).toBe('aexos-core@2.1.0');
    });

    it('should load installed manifest from version.json when explicit manifest is missing', () => {
      fs.ensureDirSync(path.join(targetDir, '.aexos-core'));
      fs.writeJsonSync(path.join(targetDir, '.aexos-core', 'version.json'), {
        version: '5.0.4',
        fileHashes: {
