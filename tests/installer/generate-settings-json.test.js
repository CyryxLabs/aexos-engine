...
const { writeSettingsJson } = require('../../.aexos-core/infrastructure/scripts/generate-settings-json');

describe('generate-settings-json', () => {
  let tempDir;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-settings-json-'));
  ...
  it('should merge permissions without dropping existing security rules', async () => {
    ...
    expect(settings.permissions.allow).toEqual(
      expect.arrayContaining(['Bash(git status:*)', 'Read(./.aexos-core/**)']),
    );
