...
describe('v2.1 Module Structure Tests', () => {
  describe('INS-01: Source Path Configuration', () => {
    it('should point getCyryxCorePath to .aexos-core directory', () => {
      const cyryxCorePath = resourceLocator.getCyryxCorePath();
      expect(cyryxCorePath).toContain('.aexos-core');
      expect(cyryxCorePath).not.toMatch(/[^.]aexos-core$/);
    });

    it('should have .aexos-core as source for config-loader', () => {
      const cyryxCorePath = configLoader.getCyryxCorePath();
      expect(cyryxCorePath).toContain('.aexos-core');
    });
  });
  ...
  describe('INS-02: Agent Path Resolution', () => {
    ...
    it('should list agents from development/agents/', async () => {
      ...
      for (const agent of agents) {
        expect(agent.file).toContain('.aexos-core/development/agents/');
      }
    });
  });
  ...
  describe('INS-04: Resource Locator v2.1 Paths', () => {
    it('should resolve agent dependencies with v2.1 module mapping', async () => {
      ...
      for (const depPath of deps.all) {
        // Should not have flat structure paths like .aexos-core/tasks/
        expect(depPath).not.toMatch(/\.aexos-core\/(tasks|templates|checklists|workflows|utils|data)\//);
  ...
  describe('INS-05: Manifest Files Location', () => {
    it('should have manifests directory in .aexos-core', async () => {
      const manifestsPath = path.join(resourceLocator.getCyryxCorePath(), 'manifests');
      const exists = await fs.pathExists(manifestsPath);
      expect(exists).toBe(true);
    });

    it('should have agents.csv manifest', async () => {
      const agentsCsvPath = path.join(resourceLocator.getCyryxCorePath(), 'manifests', 'agents.csv');
      const exists = await fs.pathExists(agentsCsvPath);
      expect(exists).toBe(true);
    });

    it('should have tasks.csv manifest', async () => {
      const tasksCsvPath = path.join(resourceLocator.getCyryxCorePath(), 'manifests', 'tasks.csv');
      const exists = await fs.pathExists(tasksCsvPath);
      expect(exists).toBe(true);
    });

    it('should have workers.csv manifest', async () => {
      const workersCsvPath = path.join(resourceLocator.getCyryxCorePath(), 'manifests', 'workers.csv');
