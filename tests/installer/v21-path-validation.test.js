 ...
 * 4. No {root} placeholders remain (should be replaced with .aexos-core)
 *
...
// Path to .aexos-core directory
const AEXOS_CORE_PATH = path.join(__dirname, '..', '..', '.aexos-core');
 ...
 * @param {string|string[]} ext - File extension filter
...
function resolveDependencyPath(type, filename) {
  const candidateMappings = {
    templates: ['development/templates', 'product/templates'],
    checklists: ['development/checklists', 'product/checklists'],
    data: ['data', 'development/data', 'product/data'],
  };
  if (candidateMappings[type]) {
    const candidates = candidateMappings[type].map(modulePath =>
      path.join(AEXOS_CORE_PATH, modulePath, filename));
    return candidates.find(candidate => fs.existsSync(candidate)) || candidates[0];
  }
  const modulePath = MODULE_MAPPING[type];
  if (!modulePath) {
    // Fallback to development/{type}
    return path.join(AEXOS_CORE_PATH, 'development', type, filename);
  }
  return path.join(AEXOS_CORE_PATH, modulePath, filename);
}
...
describe('v2.1 Path Validation', () => {
  ...
  before(async () => {
    // Load all agents
    const agentDir = path.join(AEXOS_CORE_PATH, 'development', 'agents');
    if (await fs.pathExists(agentDir)) {
      ...
      for (const file of agentFiles) {
        ...
        agents.push({
          file: path.relative(AEXOS_CORE_PATH, file),
    ...
    // Load all tasks
    const taskDir = path.join(AEXOS_CORE_PATH, 'development', 'tasks');
    ...
    // Load all workflows
    const workflowDir = path.join(AEXOS_CORE_PATH, 'development', 'workflows');
    if (await fs.pathExists(workflowDir)) {
      const workflowFiles = await getFilesRecursive(workflowDir, ['.yaml', '.yml']);
    ...
    allFiles = await getFilesRecursive(AEXOS_CORE_PATH, ['.md', '.yaml', '.yml']);
  });
        ...
        for (const task of taskDeps) {
          ...
          if (!await fs.pathExists(taskPath)) {
            missingDeps.push({
              ...
              type: 'tasks',
              dependency: task,
              expectedPath: path.relative(AEXOS_CORE_PATH, taskPath),
            });
          }
        }
      }
        ...
        for (const checklist of checklistDeps) {
          ...
          if (!await fs.pathExists(checklistPath)) {
            missingDeps.push({
              ...
              expectedPath: path.relative(AEXOS_CORE_PATH, checklistPath),
            });
          }
        }
      }
        ...
        for (const template of templateDeps) {
          ...
          if (!await fs.pathExists(templatePath)) {
            missingDeps.push({
              ...
              type: 'templates',
              dependency: template,
              expectedPath: path.relative(AEXOS_CORE_PATH, templatePath),
            });
          }
        }
      }
          ...
          for (const dep of deps) {
            ...
            if (!await fs.pathExists(depPath)) {
              missingDeps.push({
                ...
                type,
                dependency: dep,
                expectedPath: path.relative(AEXOS_CORE_PATH, depPath),
              });
            }
          }
        }
  ...
  describe('Workflow Validation', () => {
    it('should load workflows (if any exist)', () => {
      assert.ok(workflows.length > 0, 'No YAML workflows found in development/workflows/');
      assert.ok(workflows.every(workflow => workflow.yaml && workflow.yaml.workflow),
        'Every workflow YAML file must define a workflow object');
    ...
    it('should have valid workflow step references', async () => {
      ...
      for (const workflow of workflows) {
        if (!workflow.yaml || !workflow.yaml.workflow) continue;
        ...
        const visit = (value) => {
          if (Array.isArray(value)) {
            ...
            if (value.agent || value.task) steps.push(value);
            Object.values(value).forEach(visit);
          }
        };
        visit(workflow.yaml.workflow);
        ...
        for (const step of steps) {
          ...
          const agentName = typeof step.agent === 'string'
            ? step.agent.replace(/^@/, '')
            : null;
          if (agentName && agentName !== 'system' && !agentName.includes('${')) {
            const agentPath = resolveDependencyPath('agents', `${agentName}.md`);
            if (!await fs.pathExists(agentPath)) {
              invalidRefs.push({
                workflow: workflow.name,
                stepType: 'agent',
                reference: agentName,
                expectedPath: path.relative(AEXOS_CORE_PATH, agentPath),
              });
            }
          }

          // Check task references
          if (typeof step.task === 'string' && step.task.endsWith('.md')) {
            const taskPath = resolveDependencyPath('tasks', step.task);
            if (!await fs.pathExists(taskPath)) {
              invalidRefs.push({
                workflow: workflow.name,
                stepType: 'task',
  ...
  describe('v2.1 Module Structure Validation', () => {
    it('should have core module directory', async () => {
      const coreDir = path.join(AEXOS_CORE_PATH, 'core');
    ...
    it('should have development module directory', async () => {
      const devDir = path.join(AEXOS_CORE_PATH, 'development');
    ...
    it('should have product module directory', async () => {
      const productDir = path.join(AEXOS_CORE_PATH, 'product');
    ...
    it('should have infrastructure module directory', async () => {
      const infraDir = path.join(AEXOS_CORE_PATH, 'infrastructure');
    ...
    it('should have agents in development/agents/', async () => {
      const agentsDir = path.join(AEXOS_CORE_PATH, 'development', 'agents');
    ...
    it('should have tasks in development/tasks/', async () => {
      const tasksDir = path.join(AEXOS_CORE_PATH, 'development', 'tasks');
    ...
    it('should have templates in product/templates/', async () => {
      const templatesDir = path.join(AEXOS_CORE_PATH, 'product', 'templates');
    ...
    it('should have checklists in product/checklists/', async () => {
      const checklistsDir = path.join(AEXOS_CORE_PATH, 'product', 'checklists');
...
async function generateValidationReport() {
  ...
  // Check modules exist
  report.modules.core = await fs.pathExists(path.join(AEXOS_CORE_PATH, 'core'));
  report.modules.development = await fs.pathExists(path.join(AEXOS_CORE_PATH, 'development'));
  report.modules.product = await fs.pathExists(path.join(AEXOS_CORE_PATH, 'product'));
  report.modules.infrastructure = await fs.pathExists(path.join(AEXOS_CORE_PATH, 'infrastructure'));
