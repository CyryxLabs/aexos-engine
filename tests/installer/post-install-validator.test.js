...
describe('PostInstallValidator Security Tests', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `aexos-validator-test-${Date.now()}`);
  ...
  describe('Symlink Rejection', () => {
    test('should reject symlinks during validation', async () => {
      ...
      const realFile = path.join(targetDir, '.aexos-core', 'real.txt');
      const symlink = path.join(targetDir, '.aexos-core', 'link.txt');
      ...
      await fs.writeFile(
        path.join(targetDir, '.aexos-core', 'install-manifest.yaml'),
        'version: "1.0.0"\nfiles:\n  - path: link.txt',
      );
  ...
  describe('Signature Verification', () => {
    ...
    test('should allow validation without signature in dev mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'test.txt'), 'test');
  ...
  describe('Quick Mode Safety (H2)', () => {
    test('should fail when size is missing in quick mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'test.txt'), 'content');
    ...
    test('should fail on size mismatch in quick mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'test.txt'), 'small');
...
files:
  ...
  - path: data/entity-registry.yaml
    ...
    size: 10`;
      ...
      await fs.ensureDir(path.join(targetDir, '.aexos-core', 'data'));
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'install-manifest.yaml'), manifest);
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'core-config.yaml'), 'project:\n  name: generated\n');
      await fs.writeFile(
        path.join(targetDir, '.aexos-core', 'data', 'entity-registry.yaml'),
        'metadata:\n  entityCount: 757\n  updatedAt: now\n',
      );
...
files:
  - path: data/capability-detection.js
    ...
    size: 10`;
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'install-manifest.yaml'), manifest);
      await fs.writeFile(
        path.join(targetDir, '.aexos-core', 'data', 'capability-detection.js'),
        'module.exports = {};\n',
      );
  ...
  describe('Secure Repair (C4)', () => {
    ...
    test('should verify source hash before copying', async () => {
      ...
      const sourceFile = path.join(sourceDir, '.aexos-core', 'test.txt');
...
files:
  - path: test.txt
    ...
    size: 13`;
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'install-manifest.yaml'), manifest);
      await fs.writeFile(path.join(sourceDir, '.aexos-core', 'install-manifest.yaml'), manifest);
  ...
  describe('Hash Error Handling (H3)', () => {
    test('should treat hash errors as failures', async () => {
      ...
      const dirPath = path.join(targetDir, '.aexos-core', 'notafile');
  ...
  describe('DoS Protection (H6)', () => {
    ...
    test('should enforce max manifest size', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'install-manifest.yaml'), bigContent);
    ...
    test('should use byte length not character length for size check (DOS-4)', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.aexos-core', 'install-manifest.yaml'), emojiContent);
