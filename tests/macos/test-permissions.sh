...
LOG_FILE="/tmp/aexos-test-perms-$(date +%Y%m%d-%H%M%S).log"
...
test_script_permissions() {
    ...
    # Find AEXOS scripts
    if [[ -d "$HOME/.aexos/bin" ]]; then
        SCRIPTS=$(find "$HOME/.aexos/bin" -type f -name "*.sh" 2>/dev/null || true)
...
test_directory_permissions() {
    ...
    if [[ -d "$HOME/.cyryx" ]]; then
        DIR_PERMS=$(stat -f "%Lp" "$HOME/.cyryx" 2>/dev/null || stat -c "%a" "$HOME/.cyryx" 2>/dev/null)
...
test_no_sudo_required() {
    log_info "Test 4: Verifying no sudo required for AEXOS operations..."

    # Check if current user owns .aexos directory
    if [[ -d "$HOME/.cyryx" ]]; then
        OWNER=$(stat -f "%Su" "$HOME/.cyryx" 2>/dev/null || stat -c "%U" "$HOME/.cyryx" 2>/dev/null)

        if [[ "$OWNER" == "$(whoami)" ]]; then
            pass_test "User owns .aexos directory (no sudo needed)"
        else
            fail_test ".aexos owned by: $OWNER (expected: $(whoami))"
        fi
    fi
}
