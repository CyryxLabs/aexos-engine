...
This guide provides step-by-step instructions for manually testing AEXOS on macOS (Intel and Apple Silicon).
...
```bash
# Backup existing AEXOS installation (if any)
if [ -d "$HOME/.cyryx" ]; then
    mv "$HOME/.cyryx" "$HOME/.aexos.backup.$(date +%Y%m%d-%H%M%S)"
fi

# Verify backup
ls -la "$HOME/.aexos.backup"*
```

### 2. Navigate to Test Directory

```bash
cd path/to/aexos-core/tests/macos
chmod +x *.sh
```
...
```bash
# Check AEXOS command availability
which cyryx
...
```bash
# Check AEXOS installation path
which cyryx
# Should use forward slashes: /usr/local/bin/cyryx
# NOT backslashes: \usr\local\bin\cyryx
```
...
cd ~/.aexos
pwd
# Should show full path: /Users/username/.aexos
```

#### 3. Test Symlinks

```bash
# Check if cyryx command is a symlink
ls -la $(which cyryx)
...
```bash
# Check line endings of AEXOS config
file ~/.aexos/config.json
# Should show: ASCII text (LF line endings)
# NOT: ASCII text, with CRLF line endings

# Check for carriage returns
grep -r $'\r' ~/.aexos/ || echo "No CRLF found - PASS"
```
...
ls -la ~/.aexos/bin/*.sh 2>/dev/null
# Should show: -rwxr-xr-x (executable)

# Try running a script without chmod
~/.aexos/bin/some-script.sh
...
- [ ] No "unidentified developer" warnings for AEXOS
...
```bash
# Create backup
cp -r ~/.aexos ~/.aexos.backup

# Simulate corruption
echo "corrupted" > ~/.aexos/config.json

# Try to recover
mv ~/.aexos ~/.cyryx.broken
mv ~/.aexos.backup ~/.aexos
...
- Permission denied: `chmod 000 ~/.cyryx`
...
npx @aexos/core init --verbose
```

#### Command Not Found

**Symptom:** `bash: cyryx: command not found`
...
# Check PATH
echo $PATH | grep -o '/[^:]*cyryx[^:]*'

# Manually add to PATH if needed
export PATH="$HOME/.aexos/bin:$PATH"
```
...
chmod +x ~/.aexos/bin/*.sh

# Fix ownership
sudo chown -R $(whoami) ~/.aexos
...
**Symptom:** `aexos health` shows unhealthy MCPs

**Solution:**

```bash
# Reinstall specific MCP
aexos mcp reinstall <mcp-name>
...
2. **Test Logs:**
   - All files from `/tmp/aexos-test-*.log`
...
**Story:** 1.10b - macOS Testing & Validation
**Reviewed by:** Argus (QA Guardian) 🛡️
