...
const {
  ...
  satisfiesPeer,
} = require('../../.aexos-core/core/pro/pro-updater');
...
describe('pro-updater', () => {
  ...
  describe('getCoreVersion()', () => {
    it('should prefer .aexos-core/version.json when available', () => {
      const projectRoot = '/tmp/aexos-project';
      const versionJsonPath = projectFile(projectRoot, '.aexos-core', 'version.json');
    ...
    it('should read declared aexos-core dependency from the project manifest', () => {
      const projectRoot = '/tmp/aexos-project';
    ...
    it('should read declared scoped aexos-core dependency from the project manifest', () => {
      const projectRoot = '/tmp/aexos-project';
      const packageJsonPath = projectFile(projectRoot, 'package.json');
  ...
  describe('updatePro()', () => {
    ...
    it('should honor scoped aexos-core peer dependencies when checking compatibility', async () => {
      const projectRoot = '/tmp/aexos-project';
      const versionJsonPath = projectFile(projectRoot, '.aexos-core', 'version.json');
      ...
      expect(result.error).toContain('requires aexos-core >=6.0.0');
      expect(result.actions).toEqual(expect.arrayContaining([
        expect.objectContaining({
          action: 'compat',
          status: 'incompatible',
