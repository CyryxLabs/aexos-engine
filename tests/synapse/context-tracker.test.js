...
} = require('../../.aexos-core/core/synapse/context/context-tracker');
...
describe('getModelConfig', () => {
  ...
  function createProjectConfig(contents) {
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-context-tracker-'));
    const configDir = path.join(projectDir, '.aexos-core');
...
describe('AC8: dynamic configuration', () => {
  test('module source should expose config loading and cache reset helpers', () => {
    const source = fs.readFileSync(
      path.join(__dirname, '../../.aexos-core/core/synapse/context/context-tracker.js'),
      'utf8',
    );
