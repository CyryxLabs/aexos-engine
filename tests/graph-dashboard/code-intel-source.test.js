...
jest.mock('../../.aexos-core/core/code-intel', () => ({
  getClient: () => mockClient,
  isCodeIntelAvailable: () => mockIsAvailable,
}));

jest.mock('../../.aexos-core/core/ids/registry-loader', () => ({
  RegistryLoader: jest.fn().mockImplementation(() => ({
    load: () => mockRegistryData,
  })),
}));

const { CodeIntelSource, _classifyScript, _detectCategory } = require('../../.aexos-core/core/graph-dashboard/data-sources/code-intel-source');
...
describe('CodeIntelSource', () => {
  ...
  describe('_classifyScript', () => {
    it('should classify development scripts as scripts/task', () => {
      expect(_classifyScript('.aexos-core/development/scripts/build.js')).toBe('scripts/task');
    });

    it('should classify core scripts as scripts/engine', () => {
      expect(_classifyScript('.aexos-core/core/graph-dashboard/cli.js')).toBe('scripts/engine');
    });

    it('should classify infrastructure scripts as scripts/infra', () => {
      expect(_classifyScript('.aexos-core/infrastructure/ci/deploy.js')).toBe('scripts/infra');
    });
  ...
  describe('_detectCategory', () => {
    it('should detect checklists from path', () => {
      expect(_detectCategory('other', '.aexos-core/development/checklists/pre-push.md')).toBe('checklists');
    });

    it('should detect workflows from path', () => {
      expect(_detectCategory('other', '.aexos-core/development/workflows/deploy.yaml')).toBe('workflows');
    });

    it('should detect utils from path', () => {
      expect(_detectCategory('other', '.aexos-core/utils/helper.js')).toBe('utils');
    });

    it('should detect data from path', () => {
      expect(_detectCategory('other', '.aexos-core/data/entity-registry.yaml')).toBe('data');
    });

    it('should detect tools from path', () => {
      expect(_detectCategory('other', '.aexos-core/development/tools/coderabbit.md')).toBe('tools');
    });

    it('should subcategorize scripts by directory', () => {
      expect(_detectCategory('scripts', '.aexos-core/development/scripts/build.js')).toBe('scripts/task');
      expect(_detectCategory('scripts', '.aexos-core/core/code-intel/index.js')).toBe('scripts/engine');
      expect(_detectCategory('scripts', '.aexos-core/infrastructure/ci/lint.js')).toBe('scripts/infra');
    });

    it('should return base category when no path match', () => {
      expect(_detectCategory('agents', '.aexos-core/development/agents/dev.md')).toBe('agents');
      expect(_detectCategory('tasks', '.aexos-core/development/tasks/build.md')).toBe('tasks');
    });
  ...
  describe('getData - category detection in registry', () => {
    it('should apply _detectCategory to registry entities', async () => {
      mockRegistryData = {
        ...
        entities: {
          scripts: {
            'build-script': { path: '.aexos-core/development/scripts/build.js', type: 'script', dependencies: [] },
            'engine-script': { path: '.aexos-core/core/graph-dashboard/cli.js', type: 'script', dependencies: [] },
          },
          other: {
            'my-checklist': { path: '.aexos-core/development/checklists/pre-push.md', type: 'checklist', dependencies: [] },
          },
        },
      };
  ...
  describe('getData - lifecycle field in registry nodes', () => {
    it('should include lifecycle from registry entity data', async () => {
      mockRegistryData = {
        ...
        entities: {
          agents: {
            'dev-agent': { path: '.aexos-core/agents/dev.md', type: 'agent', lifecycle: 'production', dependencies: [], usedBy: [] },
            'old-agent': { path: '.aexos-core/agents/old.md', type: 'agent', lifecycle: 'deprecated', dependencies: [], usedBy: [] },
          },
          tasks: {
            'orphan-task': { path: '.aexos-core/tasks/orphan.md', type: 'task', lifecycle: 'orphan', dependencies: [], usedBy: [] },
          },
        },
      };
    ...
    it('should default lifecycle to production when missing', async () => {
      mockRegistryData = {
        ...
        entities: {
          agents: {
            'no-lifecycle': { path: '.aexos-core/agents/test.md', type: 'agent', dependencies: [], usedBy: [] },
          },
        },
      };
