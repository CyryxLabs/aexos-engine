...
const { parseArgs, handleWatch, WATCH_FORMAT_MAP } = require('../../.aexos-core/core/graph-dashboard/cli');

describe('watch-mode', () => {
  let consoleLogSpy;
  let consoleErrorSpy;
  let watchState;
  const testOutputDir = path.resolve(process.cwd(), '.aexos');
  ...
  describe('handleWatch lifecycle', () => {
    ...
    it('should create .aexos directory if it does not exist', async () => {
      // .aexos should be created by handleWatch
      watchState = await handleWatch({ format: 'dot', interval: 5 });
