...
const {
  ...
  USER_FIELDS,
  PROJECT_FIELDS,
} = require('../../../.aexos-core/core/config/migrate-config');
...
describe('extractProjectFields', () => {
  ...
  it('should extract project.type from nested project object', () => {
    // Given
    const legacyConfig = {
      project: { type: 'EXISTING_CYRYX', version: '2.1.0' },
    };
    ...
    // Then
    expect(projectFields.project_type).toBe('EXISTING_CYRYX');
  });
...
describe('migrateConfig', () => {
  ...
  it('should be idempotent: re-running does not duplicate', () => {
    ...
    const userConfigPath = path.join(FAKE_HOME, '.aexos', 'user-config.yaml');
    setupFileSystem({
      'core-config.yaml': LEGACY_CONFIG_YAML,
...
describe('ensureUserConfigDir', () => {
  it('should create ~/.aexos/ directory if it does not exist', () => {
    // Given
    fs.existsSync.mockReturnValue(false);
