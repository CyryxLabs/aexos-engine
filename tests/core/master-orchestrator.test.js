...
const MasterOrchestrator = require('../../.aexos-core/core/orchestration/master-orchestrator');
...
describe('MasterOrchestrator', () => {
  ...
  function installSuccessfulExecutors(target) {
    for (const epicNum of [3, 4, 5, 6]) {
      target.executors[epicNum] = {
    ...
    target.gateEvaluator.evaluate = jest.fn(async () => ({
      verdict: 'approved',
      score: 100,
      issues: [],
    }));
  ...
  beforeEach(async () => {
    ...
    // Create .aexos/dashboard directory for dashboard integration
    await fs.ensureDir(path.join(tempDir, '.aexos', 'dashboard'));
    ...
    installSuccessfulExecutors(orchestrator);
  });
  ...
  describe('executeEpic (AC4)', () => {
    ...
    it('should propagate a blocked executor result without marking the epic complete', async () => {
      orchestrator.executors[3] = {
        execute: jest.fn(async () => ({
          success: false,
          blocked: true,
          error: 'A configured executor is required',
          pendingRequirements: ['invokeAgent'],
        })),
      };
      ...
      expect(orchestrator.executionState.epics[3].status).toBe(EpicStatus.BLOCKED);
      expect(orchestrator.executionState.epics[3].result.success).toBe(false);
    });

    it('should reject a legacy stub result as blocked', async () => {
      orchestrator.executors[3] = {
        execute: jest.fn(async () => ({ status: 'stub', epicNum: 3 })),
      };
      ...
      expect(result.error).toContain('stub result');
      expect(orchestrator.executionState.epics[3].status).toBe(EpicStatus.BLOCKED);
    });

    it('should persist a blocked gate without emitting epicComplete', async () => {
      const completed = jest.fn();
      const failed = jest.fn();
      orchestrator.on('epicComplete', completed);
      orchestrator.on('epicFailed', failed);
      orchestrator._inFullPipeline = true;
      orchestrator.executors[3] = {
        execute: jest.fn(async () => ({ success: true, specPath: '/real/spec.md' })),
      };
      orchestrator.gateEvaluator.evaluate = jest.fn(async () => ({
        gate: 'epic3_to_epic4',
        verdict: 'blocked',
        score: 0,
        issues: [{ message: 'missing evidence' }],
      }));
      orchestrator.gateEvaluator.shouldBlock = jest.fn(() => true);

      const result = await orchestrator.executeEpic(3);
      const persisted = await orchestrator.loadState();

      expect(result).toMatchObject({ success: false, blocked: true });
      expect(orchestrator.executionState.epics[3].status).toBe(EpicStatus.BLOCKED);
      expect(orchestrator.executionState.epics[3].completedAt).toBeNull();
      expect(persisted.epics[3].status).toBe(EpicStatus.BLOCKED);
  ...
  describe('State Persistence (Story 0.2)', () => {
    describe('saveState (AC1, AC3)', () => {
      it('should save state to correct path', async () => {
        ...
        expect(normalizedPath).toContain('.aexos/master-orchestrator/TEST-001.json');
...
describe('EpicStatus Enum', () => {
  it('should have all required status values', () => {
    ...
    expect(EpicStatus.BLOCKED).toBe('blocked');
