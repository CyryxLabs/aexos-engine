 ...
 * - aexos-master has *update-source-tree command
 */
...
describe('Story ACT-8: Agent Config Enrichment', () => {
  ...
  beforeAll(() => {
    agentConfig = loadYaml('.aexos-core/data/agent-config-requirements.yaml');
  });
  ...
  describe('YAML Validity', () => {
    ...
    test('all expected agents have entries', () => {
      const expectedAgents = [
        'aexos-master', 'dev', 'qa', 'devops', 'github-devops',
        'architect', 'po', 'sm', 'data-engineer', 'db-sage',
        'pm', 'analyst', 'ux-design-expert', 'squad-creator',
        'aexos-developer', 'aexos-orchestrator', 'default',
      ];
  ...
  describe('7 Required Data Files Exist on Disk', () => {
    const requiredFiles = [
      ...
      { path: '.aexos-core/data/technical-preferences.md', owner: '@architect' },
      { path: '.aexos-core/product/data/test-levels-framework.md', owner: '@qa' },
      { path: '.aexos-core/product/data/test-priorities-matrix.md', owner: '@qa' },
      { path: '.aexos-core/product/data/brainstorming-techniques.md', owner: '@analyst' },
      { path: '.aexos-core/product/data/elicitation-methods.md', owner: '@po' },
    ];
  ...
  describe('Enriched Agent: @analyst', () => {
    test('analyst now loads brainstorming-techniques, tech-stack, and source-tree', () => {
      ...
      expect(paths).toContain('.aexos-core/product/data/brainstorming-techniques.md');
      expect(paths).toContain('docs/framework/tech-stack.md');
  ...
  describe('Enriched Agent: @sm', () => {
    test('sm now loads mode-selection, workflow-patterns, and coding-standards', () => {
      ...
      expect(paths).toContain('.aexos-core/product/data/mode-selection-best-practices.md');
      expect(paths).toContain('.aexos-core/data/workflow-patterns.yaml');
      expect(paths).toContain('docs/framework/coding-standards.md');
    });
...
describe('Story ACT-8: Document Governance', () => {
  ...
  describe('Governance Task', () => {
    test('update-source-tree.md task file exists', () => {
      expect(fileExists('.aexos-core/development/tasks/update-source-tree.md')).toBe(true);
    });

    test('task file contains expected sections', () => {
      const content = readFile('.aexos-core/development/tasks/update-source-tree.md');
  ...
  describe('aexos-master Command', () => {
    test('aexos-master.md contains *update-source-tree command', () => {
      const content = readFile('.aexos-core/development/agents/aexos-master.md');
      expect(content).toContain('update-source-tree');
      expect(content).toContain('Validate data file governance');
    });

    test('aexos-master dependencies include update-source-tree.md', () => {
      const content = readFile('.aexos-core/development/agents/aexos-master.md');
