...
const {
  ...
  ConflictSeverity,
  MergeDecision,
} = require('../../.aexos-core/core/execution/semantic-merge-engine');
...
describe('SemanticMergeEngine Module', () => {
  ...
  describe('CustomRulesLoader', () => {
    ...
    test('constructor sets paths', () => {
      ...
      expect(loader.rulesPath).toContain('.aexos');
    });
  ...
  describe('SemanticMergeEngine', () => {
    ...
    test('saveReport writes JSON and MD files', async () => {
      const engine = new SemanticMergeEngine({
        rootPath: tmpDir,
        storageDir: path.join(tmpDir, '.aexos', 'merge'),
      });
      ...
      const mergeDir = path.join(tmpDir, '.aexos', 'merge');
