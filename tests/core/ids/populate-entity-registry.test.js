...
const {
  ...
  EXTERNAL_TOOLS,
  DEPRECATED_PATTERNS,
} = require('../../../.aexos-core/development/scripts/populate-entity-registry');
...
describe('populate-entity-registry (AC: 3, 4, 12)', () => {
  ...
  describe('resolveEntityId()', () => {
    ...
    it('keeps basename ID when there is no collision', () => {
      const filePath = '/repo/.aexos-core/core/errors/constants.js';
    ...
    it('keeps basename ID when the existing entity points to the same path', () => {
      const filePath = '/repo/.aexos-core/core/index.js';
      const existing = {
        index: {
    ...
    it('uses a path-scoped ID when basename belongs to a different path', () => {
      const filePath = '/repo/.aexos-core/core/errors/index.js';
      const existing = {
        index: {
          path: '.aexos-core/core/index.js',
        },
    ...
    it('adds a numeric suffix when the scoped ID is also occupied by another path', () => {
      ...
      const existing = {
        ...
        'errors-index': {
          path: '.aexos-core/core/other/index.js',
        },
      };
  ...
  describe('toScopedEntityId()', () => {
    it('derives deterministic path-scoped IDs from the category base path', () => {
      ...
      expect(toScopedEntityId('/repo/.aexos-core/core/errors/index.js', config, '/repo')).toBe('errors-index');
      expect(toScopedEntityId('/repo/.aexos-core/core/code-intel/helpers/creation-helper.js', config, '/repo')).toBe(
  ...
  describe('findScanConfigForPath()', () => {
    it('matches scan config only on directory boundaries', () => {
      ...
      expect(findScanConfigForPath('/repo/.aexos-core/core/errors/index.js', repoRoot)?.category).toBe('modules');
      expect(findScanConfigForPath('/repo/.aexos-core/corex/errors/index.js', repoRoot)).toBeNull();
    });
  });
  ...
  describe('syncSelfRegistryEntry()', () => {
    const SELF_PATH = '.aexos-core/data/entity-registry.yaml';
        ...
        entities: {
          data: {
            'entity-registry': {
              ...
              purpose: 'Entity at .aexos-core/data/entity-registry.yaml',
            },
          },
        },
      };
    ...
    it('does NOT mutate entries whose path is different (other entity-registry pointing to .js module)', () => {
      const registry = {
        entities: {
          data: {
            'entity-registry': {
              path: '.aexos-core/core/doctor/checks/entity-registry.js',
    ...
    it('is a no-op when self-entry is missing', () => {
      const registry = {
        entities: {
          data: {
            'some-other-data': { path: '.aexos-core/data/something.yaml', checksum: 'sha256:keep' },
          },
        },
      };
  ...
  describe('detectDependencies()', () => {
    ...
    it('resolves relative directory index dependencies to scoped entity ids', () => {
      ...
      const currentFilePath = path.resolve(__dirname, '../../../.aexos-core/core/synapse/engine.js');
  ...
  describe('resolveUsedBy enhanced (NOG-15 AC4)', () => {
    it('creates correct reverse references via name index', () => {
      const entities = {
        tasks: {
          'task-a': {
            path: '.aexos-core/development/tasks/task-a.md',
            type: 'task',
            dependencies: ['util-x'],
            usedBy: [],
          },
        ...
        utils: {
          'util-x': {
            path: '.aexos-core/core/utils/util-x.js',
            type: 'util',
            dependencies: [],
            usedBy: [],
          },
    ...
    it('resolves by filename when ID does not match', () => {
      const entities = {
        tasks: {
          'my-task': {
            path: '.aexos-core/development/tasks/my-task.md',
            type: 'task',
        ...
        scripts: {
          helper: {
            path: '.aexos-core/development/scripts/helper.js',
            type: 'script',
            dependencies: [],
            usedBy: [],
          },
  ...
  describe('buildNameIndex() (NOG-16A AC5)', () => {
    it('builds index from entity IDs, filenames, and basenames', () => {
      const entities = {
        tasks: {
          'task-a': { path: '.aexos-core/development/tasks/task-a.md', dependencies: [], usedBy: [] },
        },
        scripts: {
          helper: { path: '.aexos-core/development/scripts/helper.js', dependencies: [], usedBy: [] },
        },
      };
  ...
  describe('regression: real deps preserved (NOG-16A AC6)', () => {
    it('dev agent still has 40+ dependencies after filtering (all extractors combined)', () => {
      const devAgentPath = path.resolve(__dirname, '../../../.aexos-core/development/agents/dev.md');
