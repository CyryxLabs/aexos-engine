...
const {
  ...
  ReincarnationQueue,
  StateCommitLog,
} = require('../../../.aexos-core/core/resilience');
...
describe('Agent Immortality Protocol', () => {
  ...
  test('records a compact autopsy and reincarnation context for context overflow', () => {
    ...
    expect(report.reincarnationContext).toContain('[AEXOS REINCARNATION CONTEXT]');
  ...
  test('quarantines corrupt persistence files and continues with an empty list', () => {
    const dataDir = path.join(tempDir, '.aexos', 'immortality');
