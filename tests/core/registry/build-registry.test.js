...
const { buildRegistry, saveRegistry, REGISTRY_VERSION } = require('../../../.aexos-core/core/registry/build-registry');
...
describe('buildRegistry', () => {
  ...
  describe('deduplicação de IDs', () => {
    test('qualifica todos os IDs duplicados pelo caminho sem alterar IDs únicos', async () => {
      // Dois arquivos em categorias diferentes com mesmo basename
      glob.mockImplementation(async (pattern) => {
        if (pattern.includes('tasks')) {
          return [
            '.aexos-core/development/tasks/setup-env.md',
            '.aexos-core/development/tasks/unique-task.md',
          ];
        }
        if (pattern.includes('checklists')) {
          return ['.aexos-core/product/checklists/setup-env.md'];
        }
        return [];
      });
      fs.readFile.mockResolvedValue('Desc');
      ...
      expect(ids).toContain('setup-env-development-tasks');
      expect(ids).toContain('setup-env-product-checklists');
      expect(ids).toContain('unique-task');
      expect(new Set(ids).size).toBe(ids.length);
    });

    test('qualifica colisões dentro da mesma categoria de forma independente da ordem', async () => {
      let reverse = false;
      glob.mockImplementation(async (pattern) => {
        if (!pattern.includes('scripts')) return [];
        const files = [
          '.aexos-core/infrastructure/scripts/zeta/index.js',
          '.aexos-core/infrastructure/scripts/alpha/index.js',
        ];
        return reverse ? files.reverse() : files;
      });
      fs.readFile.mockResolvedValue('/** utility */');

      const first = await buildRegistry('/fake');
      reverse = true;
      const second = await buildRegistry('/fake');
      const expectedIds = [
        'index-infrastructure-scripts-alpha',
        'index-infrastructure-scripts-zeta',
      ];
      expect(first.workers.map((worker) => worker.id).sort()).toEqual(expectedIds);
      expect(second.workers.map((worker) => worker.id).sort()).toEqual(expectedIds);
    });
  });
