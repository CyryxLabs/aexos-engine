...
const {
  ...
  PhaseStatus,
  CheckpointDecision,
} = require('../../.aexos-core/core/orchestration/workflow-executor');
...
describe('WorkflowExecutor', () => {
  ...
  describe('State Management', () => {
    ...
    test('should generate correct state file path', () => {
      ...
      expect(stateFile).toMatch(/11\.3-[a-f0-9]{16}-state\.yaml$/);
    });
  });
...
\`\`\`
  ...
  describe('Checkpoint Decisions', () => {
    test('should return checkpoint with options', async () => {
      ...
      expect(result.status).toBe(PhaseStatus.WAITING);
    ...
    test('GO decision should finish the current story instead of looping it', async () => {
      const executor = new WorkflowExecutor(projectRoot);
      await executor.loadWorkflow();
...
describe('development-cycle.yaml', () => {
  ...
  test('should be valid YAML', async () => {
    const content = await fs.readFile(
      path.join(projectRoot, '.aexos-core/development/workflows/development-cycle.yaml'),
      'utf8',
    );
