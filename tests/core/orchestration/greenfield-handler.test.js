...
const {
  ...
  PHASE_1_SEQUENCE,
} = require('../../../.aexos-core/core/orchestration/greenfield-handler');
...
jest.mock('../../../.aexos-core/core/orchestration/terminal-spawner', () => mockTerminalSpawner);
...
let existingPaths;
let outputMetadata;
const mockDispatchSubagent = jest.fn(async ({ context }) => {
  for (const outputPath of context.creates || []) {
    const target = path.resolve(TEST_PROJECT_ROOT, outputPath);
    existingPaths.add(target);
    if (outputPath.endsWith('/') || outputPath.endsWith(path.sep)) {
      outputMetadata.set(target, { type: 'directory' });
      const generatedFile = path.join(target, 'generated.md');
      existingPaths.add(generatedFile);
      outputMetadata.set(generatedFile, { type: 'file', size: 9 });
    } else {
      outputMetadata.set(target, { type: 'file', size: 9 });
    }
  }
...
describe('GreenfieldHandler', () => {
  ...
  beforeEach(() => {
    ...
    fs.existsSync.mockImplementation((candidate) => existingPaths.has(candidate));
    fs.realpathSync.mockImplementation((candidate) => candidate);
    fs.statSync.mockImplementation((candidate) => {
      const metadata = outputMetadata.get(candidate);
      if (!metadata) throw new Error(`Missing mocked path: ${candidate}`);
      return {
    ...
    fs.readdirSync.mockImplementation((candidate) => {
      const prefix = `${candidate}${path.sep}`;
      return [...outputMetadata.keys()]
    ...
    handler = new GreenfieldHandler(TEST_PROJECT_ROOT, {
      ...
      dispatchSubagent: mockDispatchSubagent,
    });
  });
  ...
  describe('handle (main entry)', () => {
    test('should start from Phase 0 when nothing exists (not skippable)', async () => {
      ...
      // Should surface only after the configured dispatcher creates the required outputs.
    ...
    test('should skip Phase 0 when package.json + .git exist (AC16)', async () => {
      fs.existsSync.mockImplementation((p) => {
        return p.endsWith('package.json') || p.endsWith('.git') || existingPaths.has(p);
      });
  ...
  describe('Phase 0: Bootstrap (AC2)', () => {
    ...
    test('should return pending when no dispatcher or terminal spawner is available', async () => {
      handler.options.dispatchSubagent = null;
      mockTerminalSpawner.isSpawnerAvailable.mockReturnValue(false);
      ...
      expect(result.pendingRequirements).toHaveLength(1);
      expect(handler.phaseProgress[GreenfieldPhase.BOOTSTRAP].status).toBe('pending_dispatch');
    });

    test('should reject declared success when required outputs do not exist', async () => {
      handler.options.dispatchSubagent = jest.fn(async () => ({ success: true, status: 'success' }));

      const result = await handler._executePhase0({});

      expect(result.action).toBe('greenfield_phase_failure');
      expect(handler.phaseProgress[GreenfieldPhase.BOOTSTRAP].result.status).toBe('failed_validation');
      expect(handler.phaseProgress[GreenfieldPhase.BOOTSTRAP].result.missingOutputs).toContain(
        '.aexos/config.yaml',
      );
    });

    test('should reject inconsistent completion status combinations', () => {
      expect(handler._normalizeSpawnResult(
    ...
    test('should require nonempty regular output content inside the project root', () => {
      const emptyFile = path.resolve(TEST_PROJECT_ROOT, 'empty.md');
      const emptyDirectory = path.resolve(TEST_PROJECT_ROOT, 'empty-dir');
      existingPaths.add(emptyFile);
      outputMetadata.set(emptyFile, { type: 'file', size: 0 });
      existingPaths.add(emptyDirectory);
      outputMetadata.set(emptyDirectory, { type: 'directory' });

      const emptyFileResult = handler._normalizeSpawnResult(
        { success: true, status: 'completed' },
        '@devops',
        'bootstrap',
        { creates: 'empty.md' },
      );
      const emptyDirectoryResult = handler._normalizeSpawnResult(
        { success: true, status: 'completed' },
        '@devops',
        'bootstrap',
        { creates: ['empty-dir'] },
      );
      const outsideRootResult = handler._normalizeSpawnResult(
        { success: true, status: 'completed' },
        '@devops',
        'bootstrap',
        { creates: '../outside.md' },
      ...
      expect(emptyFileResult).toMatchObject({ success: false, status: 'failed_validation' });
      expect(emptyDirectoryResult).toMatchObject({ success: false, status: 'failed_validation' });
      expect(outsideRootResult).toMatchObject({ success: false, status: 'failed_validation' });
    });
  });
  ...
  describe('Phase 3: Dev Cycle (AC5)', () => {
    test('should return an explicit pending handoff without a story', async () => {
      ...
      expect(handler.phaseProgress[GreenfieldPhase.DEV_CYCLE].status).toBe('pending_handoff');
    });

    test('should complete only when WorkflowExecutor completes the supplied story', async () => {
      mockWorkflowExecutor.execute.mockResolvedValue({
        success: true,
        status: 'completed',
        phaseResults: { '6_checkpoint': { status: 'completed' } },
      });

      const result = await handler._executePhase3({ storyPath: '/tmp/story.md' });

      expect(mockWorkflowExecutor.execute).toHaveBeenCalledWith('/tmp/story.md', {});
      expect(result).toMatchObject({ success: true, status: 'completed' });
      expect(handler.phaseProgress[GreenfieldPhase.DEV_CYCLE].status).toBe('complete');
    });

    test('should preserve waiting workflow state without completing Phase 3', async () => {
      mockWorkflowExecutor.execute.mockResolvedValue({
        success: false,
      ...
      expect(handler.phaseProgress[GreenfieldPhase.DEV_CYCLE].status).toBe('waiting_for_input');
    });
  });
