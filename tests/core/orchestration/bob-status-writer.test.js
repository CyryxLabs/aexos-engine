...
} = require('../../../.aexos-core/core/orchestration/bob-status-writer');
...
describe('BobStatusWriter', () => {
  ...
  describe('constructor', () => {
    ...
    it('should set correct paths', () => {
      ...
      expect(writer.dashboardDir).toBe(path.join(tempDir, '.aexos', 'dashboard'));
      expect(writer.statusPath).toBe(path.join(tempDir, '.aexos', 'dashboard', 'bob-status.json'));
    });
  });
  ...
  describe('updateAgent', () => {
    it('should update current_agent fields', async () => {
      await writer.initialize();
      await writer.updateAgent('@dev', 'Vulcan', 'development', 'Story type: code_general');

      const status = await fs.readJson(writer.statusPath);
      expect(status.current_agent.id).toBe('@dev');
      expect(status.current_agent.name).toBe('Vulcan');
  ...
  describe('clearAgent', () => {
    it('should clear current_agent fields', async () => {
      await writer.initialize();
      await writer.updateAgent('@dev', 'Vulcan', 'development', 'reason');
