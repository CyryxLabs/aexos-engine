...
const {
  ...
  PostDiscoveryChoice,
  PhaseFailureAction,
} = require('../../../.aexos-core/core/orchestration/brownfield-handler');
...
describe('BrownfieldHandler', () => {
  ...
  describe('constructor', () => {
    ...
    test('should set correct workflow path', () => {
      expect(handler.workflowPath).toBe(
        path.join(TEST_PROJECT_ROOT, '.aexos-core/development/workflows/brownfield-discovery.yaml'),
      );
    });
  });
  ...
  describe('AC3: Workflow execution', () => {
    ...
    test('should forward a configured dispatcher to the workflow adapter', async () => {
      const dispatchSubagent = jest.fn();
      const configuredHandler = new BrownfieldHandler(TEST_PROJECT_ROOT, {
        workflowExecutor: mockWorkflowExecutor,
        surfaceChecker: mockSurfaceChecker,
        sessionState: mockSessionState,
        dispatchSubagent,
      ...
      await configuredHandler.handleUserDecision(true, {});

      expect(mockWorkflowExecutor.executeWorkflow).toHaveBeenCalledWith(
        configuredHandler.workflowPath,
        expect.objectContaining({ dispatchSubagent }),
      );
    });

    test('should preserve a waiting workflow result', async () => {
      ...
      expect(result).toMatchObject({
        action: 'brownfield_waiting',
        success: false,
        status: 'waiting_for_input',
        awaiting_input: true,
      });
      expect(result.data.result.pendingPhases).toEqual([2]);
    });

    test('should reject workflow success when required outputs are absent', async () => {
      fs.existsSync.mockImplementation((candidate) => candidate === handler.workflowPath);
      mockWorkflowExecutor.executeWorkflow.mockResolvedValue({ success: true, status: 'completed' });
      ...
      expect(result.data.missingOutputs).toEqual([
  ...
  describe('Task 6.6: Edge cases', () => {
    ...
    test('should normalize workflow phase objects to distinct step identifiers', async () => {
      const phaseStartHandler = jest.fn();
      handler.on('phaseStart', phaseStartHandler);

      const systemPhase = { step: 'system_documentation', phase: 1 };
      const databasePhase = { step: 'database_documentation', phase: 2 };
      await handler._onPhaseStart(systemPhase, {});
      await handler._onPhaseStart(databasePhase, {});
      await handler._onPhaseComplete(systemPhase, { status: 'success' }, {});

      expect(Object.keys(handler.phaseProgress)).toEqual([
        'system_documentation',
        'database_documentation',
      ]);
      expect(handler.phaseProgress.system_documentation.status).toBe('complete');
      expect(handler.phaseProgress.database_documentation.status).toBe('in_progress');
      expect(handler.phaseProgress['[object Object]']).toBeUndefined();
      expect(phaseStartHandler).toHaveBeenNthCalledWith(
        1,
        expect.objectContaining({
          phase: 'system_documentation',
          phaseDefinition: systemPhase,
        }),
      );
    });
  });
