...
const {
  ...
  POLL_INTERVAL_MS,
  MAX_RETRIES,
} = require('../../../.aexos-core/core/orchestration/terminal-spawner');

// Test fixtures
const TEST_OUTPUT_DIR = path.join(os.tmpdir(), 'aexos-terminal-spawner-test');
...
describe('Terminal Spawner (Story 12.10)', () => {
  ...
  describe('detectEnvironment() (Task 1)', () => {
    describe('Explicit inline and test-runner detection', () => {
      it('should honor AEXOS_INLINE_MODE before considering a visual terminal', () => {
        process.env.AEXOS_INLINE_MODE = 'true';
      ...
      it('should honor AEXOS_NO_VISUAL_TERMINAL before considering a visual terminal', () => {
        process.env.AEXOS_NO_VISUAL_TERMINAL = 'true';
      ...
      it('should disable visual side effects under a local test runner', () => {
        ...
        delete process.env.AEXOS_INLINE_MODE;
        delete process.env.AEXOS_NO_VISUAL_TERMINAL;
  ...
  describe('Timeout handling (Task 3.2)', () => {
    it('should timeout if lock file persists', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'aexos-output-timeout-test.md');
    ...
    it('should return output when lock is removed', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'aexos-output-success-test.md');
      const lockFile = outputFile.replace('output', 'lock');
  ...
  describe('Lock cleanup (Task 3.3, 3.4)', () => {
    ...
    it('should cleanup old files', async () => {
      ...
      const oldFile = path.join(TEST_OUTPUT_DIR, 'aexos-output-old.md');
