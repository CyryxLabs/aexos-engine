...
const {
  BobOrchestrator,
  ProjectState,
} = require('../../../.aexos-core/core/orchestration/bob-orchestrator');
...
jest.mock('../../../.aexos-core/core/config/config-resolver', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/message-formatter', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/brownfield-handler', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/executor-assignment', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/terminal-spawner', () => ({
  spawnAgent: jest.fn().mockResolvedValue({ success: true, output: 'done' }),
  isSpawnerAvailable: jest.fn().mockReturnValue(true),
}));

jest.mock('../../../.aexos-core/core/orchestration/workflow-executor', () => {
  return {
...
jest.mock('../../../.aexos-core/core/ui/observability-panel', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/bob-status-writer', () => ({
...
jest.mock('../../../.aexos-core/core/events/dashboard-emitter', () => ({
...
jest.mock('../../../.aexos-core/core/orchestration/surface-checker', () => {
...
jest.mock('../../../.aexos-core/core/orchestration/session-state', () => {
...
jest.mock('../../../.aexos-core/core/orchestration/lock-manager', () => {
...
jest.mock('../../../.aexos-core/core/orchestration/data-lifecycle-manager', () => {
...
const { resolveConfig } = require('../../../.aexos-core/core/config/config-resolver');
...
describe('BobOrchestrator', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(TEST_PROJECT_ROOT, '.aexos/locks'), { recursive: true });
  ...
  describe('orchestrate', () => {
    it('should return lock_failed when lock cannot be acquired', async () => {
      // Given
      const LockManager = require('../../../.aexos-core/core/orchestration/lock-manager');
    ...
    it('should route to greenfield for GREENFIELD state (AC6)', async () => {
      ...
      await fs.mkdir(emptyRoot, { recursive: true });
      const dispatchSubagent = jest.fn(async ({ context: dispatchContext }) => {
        for (const outputPath of dispatchContext.creates || []) {
          const target = path.join(emptyRoot, outputPath);
          if (outputPath.endsWith('/')) {
            await fs.mkdir(target, { recursive: true });
            await fs.writeFile(path.join(target, 'generated.md'), 'generated content');
          } else {
            await fs.mkdir(path.dirname(target), { recursive: true });
            await fs.writeFile(target, 'created by configured test dispatcher');
          }
        }
        return { success: true, status: 'success' };
      });
      const greenOrch = new BobOrchestrator(emptyRoot, { dispatchSubagent });
      ...
      expect(dispatchSubagent).toHaveBeenCalled();
    });
    ...
    it('should preserve waiting story status and not invent later phase transitions', async () => {
      const storyPath = path.join(TEST_PROJECT_ROOT, 'docs/stories/waiting.story.md');
      await fs.mkdir(path.dirname(storyPath), { recursive: true });
      await fs.writeFile(storyPath, '# Waiting Story\n');
      ...
      orchestrator.workflowExecutor.execute.mockResolvedValue({
        ...
        phaseResults: {
          ...
          '2_development': { status: 'waiting_for_input' },
        },
      });

      const result = await orchestrator._executeStory(storyPath);
      ...
      expect(orchestrator.sessionState.recordPhaseChange).toHaveBeenCalledTimes(1);
      expect(orchestrator.sessionState.recordPhaseChange).toHaveBeenCalledWith(
        'validation',
        'waiting',
        '@dev',
      );
      expect(orchestrator.sessionState.recordPhaseChange).not.toHaveBeenCalledWith(
        'quality_gate',
        expect.anything(),
        expect.anything(),
      );
    ...
    it('should not invent phase transitions from success without phase evidence', async () => {
      const storyPath = path.join(TEST_PROJECT_ROOT, 'docs/stories/no-evidence.story.md');
      await fs.mkdir(path.dirname(storyPath), { recursive: true });
      await fs.writeFile(storyPath, '# No Phase Evidence\n');
      orchestrator.sessionState.exists = jest.fn().mockResolvedValue(true);
      orchestrator.sessionState.state = { session_state: {} };
      orchestrator.sessionState.recordPhaseChange = jest.fn().mockResolvedValue({});
      orchestrator.workflowExecutor.execute.mockResolvedValue({ success: true, status: 'completed' });

      const result = await orchestrator._executeStory(storyPath);

      expect(result).toMatchObject({ success: true, status: 'completed' });
      expect(orchestrator.sessionState.recordPhaseChange).not.toHaveBeenCalled();
    });
  });
  ...
  describe('constraint: router not god class (AC13)', () => {
    it('should have less than 50 lines of other-agent-specific logic', async () => {
      ...
      const sourcePath = path.join(
        __dirname,
        '../../../.aexos-core/core/orchestration/bob-orchestrator.js',
      );
  ...
  describe('Educational Mode (Story 12.7)', () => {
    ...
    describe('handleEducationalModeToggle (AC6)', () => {
      ...
      it('should persist to user config for permanent type', async () => {
        // Given
        const { setUserConfigValue } = require('../../../.aexos-core/core/config/config-resolver');
