...
const { MessageFormatter, createMessageFormatter } = require('../../../.aexos-core/core/orchestration/message-formatter');
...
describe('MessageFormatter', () => {
    ...
    describe('educational mode ON', () => {
      ...
      it('should include agents when provided', () => {
        const result = formatter.formatActionResult('Database Migration', {
          agents: [
            { id: '@data-engineer', name: 'Ceres', task: 'Create migration' },
            { id: '@dev', name: 'Vulcan', task: 'Update models' },
          ],
        });
        expect(result).toContain('👥 Agentes envolvidos:');
        expect(result).toContain('@data-engineer (Ceres): Create migration');
        expect(result).toContain('@dev (Vulcan): Update models');
      });
  ...
  describe('formatAgentAssignment', () => {
    it('should return empty string when educational mode is OFF', () => {
      const formatter = new MessageFormatter({ educationalMode: false });
      const result = formatter.formatAgentAssignment('@dev', 'Vulcan', 'Implement feature');
      expect(result).toBe('');
    });

    it('should return explanation when educational mode is ON', () => {
      const formatter = new MessageFormatter({ educationalMode: true });
      const result = formatter.formatAgentAssignment('@dev', 'Vulcan', 'Implement feature', 'Best for code implementation');
      expect(result).toContain('🤖 @dev (Vulcan) assumindo: Implement feature');
      expect(result).toContain('Por quê: Best for code implementation');
    });

    it('should work without reason', () => {
      const formatter = new MessageFormatter({ educationalMode: true });
      const result = formatter.formatAgentAssignment('@qa', 'Argus', 'Run tests');
      expect(result).toContain('🤖 @qa (Argus) assumindo: Run tests');
