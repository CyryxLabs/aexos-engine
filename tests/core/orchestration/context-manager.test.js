...
jest.mock('../../../.aexos-core/core/synapse/utils/atomic-write', () => ({
  atomicWriteSync: (filePath, contents) => mockWriteSnapshot(filePath, JSON.parse(contents), { spaces: 2 }),
}));
...
describe('ContextManager', () => {
  ...
  async function bootstrapManager() {
    ...
    mockWriteSnapshot.mockReturnValue(undefined);
    fs.pathExists.mockResolvedValue(false);
  }
  ...
  describe('constructor', () => {
    ...
    test('calcula stateDir como .aexos/workflow-state dentro do projectRoot', () => {
      expect(manager.stateDir).toBe(path.join(PROJECT_ROOT, '.aexos', 'workflow-state'));
    });

    test('calcula statePath com o workflowId no nome do arquivo', () => {
      expect(manager.statePath).toBe(
        path.join(PROJECT_ROOT, '.aexos', 'workflow-state', `${WORKFLOW_ID}.json`),
      );
    });

    test('calcula handoffDir dentro do stateDir', () => {
      expect(manager.handoffDir).toBe(
        path.join(PROJECT_ROOT, '.aexos', 'workflow-state', 'handoffs'),
      );
    });

    test('calcula confidenceDir dentro do stateDir', () => {
      expect(manager.confidenceDir).toBe(
        path.join(PROJECT_ROOT, '.aexos', 'workflow-state', 'confidence'),
      );
    });
  ...
  describe('initialize', () => {
    ...
    test('propaga erro quando mockWriteSnapshot falha ao criar estado inicial', async () => {
      fs.pathExists.mockResolvedValue(false);
      mockWriteSnapshot.mockImplementation(() => { throw new Error('disk write failed'); });
  ...
  describe('_saveState', () => {
    ...
    test('propaga erro quando mockWriteSnapshot falha', async () => {
      manager._stateCache = manager._createInitialState();
      mockWriteSnapshot.mockImplementation(() => { throw new Error('disk write failed'); });
  ...
  describe('_saveHandoffFile', () => {
    ...
    test('propaga erro quando mockWriteSnapshot falha', async () => {
      mockWriteSnapshot.mockImplementation(() => { throw new Error('handoff write failed'); });
  ...
  describe('_saveConfidenceFile', () => {
    ...
    test('propaga erro quando mockWriteSnapshot falha', async () => {
      mockWriteSnapshot.mockImplementation(() => { throw new Error('confidence write failed'); });
  ...
  describe('_resolveConfidenceThreshold', () => {
    const originalEnv = process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD;

    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD = originalEnv;
      } else {
        delete process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD;
      }
    });
    ...
    test('retorna valor numérico da env quando é um número válido', () => {
      process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD = '85';

      expect(manager._resolveConfidenceThreshold()).toBe(85);
    });

    test('retorna 70 quando env contém valor não numérico', () => {
      process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD = 'invalid';

      expect(manager._resolveConfidenceThreshold()).toBe(70);
    });

    test('retorna 0 quando env é string vazia (Number("") === 0)', () => {
      process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD = '';
    ...
    test('aceita valores decimais', () => {
      process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD = '75.5';
