...
describe('TerminalSpawner', () => {
  ...
  describe('createContextFile', () => {
    ...
    test('should create valid JSON file', async () => {
      ...
      try {
        expect(contextPath).toContain('aexos-context-');
  ...
  describe('pollForOutput', () => {
    test('should return output when lock file is missing', async () => {
      ...
      const outputPath = path.join(tmpDir, `aexos-output-${timestamp}.md`);
    ...
    test('should return "No output captured" if file does not exist and no lock', async () => {
      const fakePath = path.join(tmpDir, `aexos-output-nonexistent-${Date.now()}.md`);
    ...
    test('should timeout when lock file persists', async () => {
      ...
      const lockPath = path.join(tmpDir, `aexos-lock-${timestamp}.md`);
    ...
    test('should return output after lock is removed', async () => {
      const timestamp = Date.now();
      const outputPath = path.join(tmpDir, `aexos-output-${timestamp}.md`);
      const lockPath = path.join(tmpDir, `aexos-lock-${timestamp}.lock`);
  ...
  describe('cleanupOldFiles', () => {
    test('should clean old AEXOS temp files', async () => {
      // Create old files (mock old timestamp by naming)
      const oldTimestamp = Date.now() - 7200000; // 2 hours ago
      const oldOutputPath = path.join(tmpDir, `aexos-output-${oldTimestamp}.md`);
      const oldLockPath = path.join(tmpDir, `aexos-lock-${oldTimestamp}.lock`);
      const oldContextPath = path.join(tmpDir, `aexos-context-${oldTimestamp}.json`);
    ...
    test('should not clean recent files', async () => {
      ...
      const recentPath = path.join(tmpDir, `aexos-output-${recentTimestamp}.md`);
  ...
  describe('Index Integration', () => {
    test('should be exported from orchestration index', () => {
      const orchestration = require('../../.aexos-core/core/orchestration');
...
describe('pm.sh Script', () => {
  ...
  test('should display help with --help flag', () => {
    const result = execFileSync(bashExecutable, [scriptPath, '--help'], { encoding: 'utf8' });
    expect(result).toContain('AEXOS Multi-Modal Orchestration Script');
  ...
  test('should display version with --version flag', () => {
    const result = execFileSync(bashExecutable, [scriptPath, '--version'], { encoding: 'utf8' });
  ...
  test('should fail with missing arguments', () => {
    try {
      execFileSync(bashExecutable, [scriptPath], { encoding: 'utf8', stdio: 'pipe' });
  ...
  test('should fail with only agent argument', () => {
    try {
      execFileSync(bashExecutable, [scriptPath, 'dev'], { encoding: 'utf8', stdio: 'pipe' });
      fail('Should have thrown an error');
    } catch (error) {
      expect(error.status).toBe(1);
    }
  ...
  test('should fail with non-existent context file', () => {
    try {
      execFileSync(bashExecutable, [scriptPath, 'dev', 'develop', '--context', '/nonexistent/file.json'], {
        encoding: 'utf8',
        stdio: 'pipe',
      });
      fail('Should have thrown an error');
