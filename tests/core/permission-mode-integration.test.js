...
const { PermissionMode } = require('../../.aexos-core/core/permissions/permission-mode');
const { OperationGuard } = require('../../.aexos-core/core/permissions/operation-guard');
const {
  createGuard,
  checkOperation,
  getModeBadge,
  ...
  cycleMode,
  enforcePermission,
} = require('../../.aexos-core/core/permissions');
...
describe('Permission Mode Integration (Story ACT-4)', () => {
  ...
  beforeEach(() => {
    ...
    const cyryxDir = path.join(tempDir, '.aexos');
    fs.mkdirSync(cyryxDir, { recursive: true });
    configPath = path.join(cyryxDir, 'config.yaml');
  });
