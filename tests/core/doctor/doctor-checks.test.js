 ...
 *
 * @see .aexos-core/core/doctor/checks/
 * @issue #52
 */
...
describe('Doctor Check: node-version', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/node-version');
...
describe('Doctor Check: core-config', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/core-config');
  let tmpDir;
  ...
  it('deve retornar PASS quando todas as secoes obrigatorias estao presentes', async () => {
    const configDir = path.join(tmpDir, '.aexos-core');
...
describe('Doctor Check: rules-files', () => {
  const { run, name, EXPECTED_RULES } = requireFromProjectRoot('.aexos-core/core/doctor/checks/rules-files');
  let tmpDir;
...
describe('Doctor Check: agent-memory', () => {
  const { run, name, EXPECTED_AGENTS } = requireFromProjectRoot('.aexos-core/core/doctor/checks/agent-memory');
  let tmpDir;
  ...
  it('deve retornar PASS quando todos os MEMORY.md existem', async () => {
    for (const agent of EXPECTED_AGENTS) {
      const agentDir = path.join(tmpDir, '.aexos-core', 'development', 'agents', agent);
  ...
  it('deve retornar WARN quando faltam alguns MEMORY.md', async () => {
    ...
    const baseDir = path.join(tmpDir, '.aexos-core', 'development', 'agents');
...
describe('Doctor Check: entity-registry', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/entity-registry');
  let tmpDir;
  ...
  it('deve retornar PASS quando entity-registry.yaml existe e e recente', async () => {
    const dataDir = path.join(tmpDir, '.aexos-core', 'data');
...
describe('Doctor Check: git-hooks', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/git-hooks');
  let tmpDir;
...
describe('Doctor Check: skills-count', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/skills-count');
  let tmpDir;
...
describe('Doctor Check: commands-count', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/commands-count');
  let tmpDir;
...
describe('Doctor Check: hooks-claude-count', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/hooks-claude-count');
  let tmpDir;
...
describe('Doctor Check: settings-json', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/settings-json');
  let tmpDir;
...
describe('Doctor Check: ide-sync', () => {
  const { run, name } = requireFromProjectRoot('.aexos-core/core/doctor/checks/ide-sync');
  let tmpDir;
  ...
  it('deve retornar WARN quando diretorio IDE nao existe', async () => {
    const sourceDir = path.join(tmpDir, '.aexos-core', 'development', 'agents');
  ...
  it('deve retornar PASS quando skills e commands coincidem', async () => {
    const sourceDir = path.join(tmpDir, '.aexos-core', 'development', 'agents');
    const ideDir = path.join(tmpDir, '.claude', 'commands', 'AEXOS', 'agents');
    const skillDir = path.join(tmpDir, '.claude', 'skills', 'AEXOS', 'agents');
    fs.mkdirSync(sourceDir, { recursive: true });
