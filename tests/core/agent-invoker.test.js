...
describe('Agent Invoker (Story 0.7)', () => {
  ...
  beforeEach(async () => {
    ...
    invoker = new AgentInvoker({
      projectRoot: tempDir,
      maxRetries: 2,
      executor: async (_agent, _task, context) => ({ success: true, context }),
    });
  });
  ...
  describe('invokeAgent (AC1)', () => {
    it('fails without an executor instead of reporting simulated completion', async () => {
      const inv = new AgentInvoker({ projectRoot: tempDir });
      const result = await inv.invokeAgent('dev', 'sample-task');
      expect(result.success).toBe(false);
      expect(result.error).toContain('not configured');
      expect(inv.invocations[0].status).toBe(InvocationStatus.FAILED);
    });

    it.each([{ success: false }, { status: 'simulated' }, { status: 'blocked' }, { status: 'running' },
      { status: 'pending' }, { status: 'cancelled' }, {}, null])(
      'propagates unsuccessful callback output: %p', async output => {
        const inv = new AgentInvoker({ projectRoot: tempDir, executor: async () => output });
        expect((await inv.invokeAgent('dev', 'sample-task')).success).toBe(false);
        expect(inv.invocations[0].status).toBe(InvocationStatus.FAILED);
      },
    );

    it('does not dispatch when the real agent definition is missing', async () => {
      await fs.remove(path.join(tempDir, '.aexos-core/development/agents/dev.md'));
      const execute = jest.fn(async () => ({ success: true }));
      const result = await new AgentInvoker({ projectRoot: tempDir, executor: execute }).invokeAgent('dev', 'sample-task');
      expect(result.success).toBe(false);
  ...
  describe('Context Building (AC3)', () => {
    it('should pass inputs to task', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.result.context.inputs).toEqual(inputs);
    });
  });
...
`,
      ...
      const inv = new AgentInvoker({
        projectRoot: tempDir,
        validateOutput: true,
        executor: async () => ({ success: true, result: 'success' }),
      });
...
describe('Integration with MasterOrchestrator', () => {
  ...
  it('should integrate AgentInvoker with MasterOrchestrator', async () => {
    const { MasterOrchestrator } = require('../../.aexos-core/core/orchestration');
  ...
  it('should invoke agent through orchestrator', async () => {
    ...
    orchestrator.agentInvoker.executor = async (_agent, _task, context) => ({ success: true, inputs: context.inputs });
