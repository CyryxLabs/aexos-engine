...
const { resolveCodeIntel, formatAsXml, escapeXml, _resetForTesting } = require(
  path.join(__dirname, '..', '..', '.aexos-core', 'core', 'code-intel', 'hook-runtime.js'),
);
...
describe('code-intel hook-runtime', () => {
  ...
  describe('resolveCodeIntel', () => {
    ...
    it('completes within 1000ms', async () => {
      ...
      await resolveCodeIntel('.aexos-core/development/tasks/create-next-story.md', PROJECT_ROOT);
  ...
  describe('formatAsXml', () => {
    ...
    it('generates valid XML with entity data', () => {
      const intel = {
        entity: {
          file: '.aexos-core/core/some-module.js',
      ...
      expect(xml).toContain('<path>.aexos-core/core/some-module.js</path>');
