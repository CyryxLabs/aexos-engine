...
describe('Core Super Update CLI integration', () => {
  ...
  it('executes the complete SDC CLI lifecycle through FAIL, fixes, PASS, and close', async () => {
    ...
    await program.parseAsync(['node', 'cyryx', 'sdc', 'plan', storyPath, '--mode', 'yolo']);
    await program.parseAsync(['node', 'cyryx', 'sdc', 'next', storyPath]);
    await program.parseAsync(['node', 'cyryx', 'sdc', 'verify', storyPath, 'validate', '--mark']);

    fs.writeFileSync(
      storyPath,
      '| Story ID | T.1 |\n| Status | InReview |\n\n## File List\n\n- `src/a.js`\n',
    );
    await program.parseAsync(['node', 'cyryx', 'sdc', 'verify', storyPath, 'develop', '--mark']);

    fs.writeFileSync(
      storyPath,
      '| Story ID | T.1 |\n| Status | InProgress |\n\n## File List\n\n- `src/a.js`\n\n## QA Results\n\nReviewer: Argus\nreviewed_revision: working-tree:first\nGate: FAIL\n',
    );
    await program.parseAsync(['node', 'cyryx', 'sdc', 'verify', storyPath, 'review', '--mark']);
    ...
    await program.parseAsync(['node', 'cyryx', 'sdc', 'verify', storyPath, 'review', '--mark']);
    await program.parseAsync(['node', 'cyryx', 'sdc', 'verify', storyPath, 'close', '--mark']);
    await program.parseAsync(['node', 'cyryx', 'sdc', 'status', 'T.1']);
    const state = JSON.parse(
      fs.readFileSync(path.join(dir, '.aexos', 'sdc', 'T.1', 'state.json'), 'utf8'),
    );
  ...
  it('executes complete wave plan and status commands', async () => {
    ...
    await program.parseAsync(['node', 'cyryx', 'wave', 'status', 'W1']);
    expect(fs.existsSync(path.join(dir, '.aexos', 'waves', 'W1', 'state.json'))).toBe(true);
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Wave W1');
...
describe('pm.sh governed execution integration', () => {
  ...
  const script = path.join(repoRoot, '.aexos-core', 'scripts', 'pm.sh');
  let dir;
  let fakeCli;
  let storyPath;
  let contextPath;
  ...
  function run(params, overrides = {}) {
    return spawnSync(
      ...
      [script, 'dev', 'develop', params, '--context', contextPath, '--output', outputPath],
      {
        ...
        env: {
          ...process.env,
          AEXOS_INLINE_MODE: 'true',
          AEXOS_OUTPUT_DIR: dir,
          AEXOS_MODEL_BUDGET_CEILING_USD: '2.50',
          AEXOS_PROJECT_ROOT: dir,
          CLAUDE_CMD: fakeCli,
          ...overrides,
        },
      },
  ...
  it('blocks missing budget before the fake model is invoked', () => {
    const result = run('safe params', { AEXOS_MODEL_BUDGET_CEILING_USD: '' });
