...
} = require('../../.aexos-core/development/scripts/unified-activation-pipeline');
...
describe('UnifiedActivationPipeline Memory Integration (MIS-6)', () => {
  ...
  const originalPipelineTimeout = process.env.AEXOS_PIPELINE_TIMEOUT;
  const originalCriticalLoaderTimeout = process.env.AEXOS_LOADER_TIMEOUT_CRITICAL;
  const originalHighLoaderTimeout = process.env.AEXOS_LOADER_TIMEOUT_HIGH;
  const originalBestEffortLoaderTimeout = process.env.AEXOS_LOADER_TIMEOUT_BEST_EFFORT;

  beforeEach(() => {
    // Increase activation budgets so full-suite IO contention does not turn no-Pro into fallback.
    process.env.AEXOS_PIPELINE_TIMEOUT = '5000';
    process.env.AEXOS_LOADER_TIMEOUT_CRITICAL = '5000';
    process.env.AEXOS_LOADER_TIMEOUT_HIGH = '5000';
    process.env.AEXOS_LOADER_TIMEOUT_BEST_EFFORT = '5000';
  ...
  afterEach(async () => {
    ...
    if (originalPipelineTimeout !== undefined) {
      process.env.AEXOS_PIPELINE_TIMEOUT = originalPipelineTimeout;
    } else {
      delete process.env.AEXOS_PIPELINE_TIMEOUT;
    }
    if (originalCriticalLoaderTimeout !== undefined) {
      process.env.AEXOS_LOADER_TIMEOUT_CRITICAL = originalCriticalLoaderTimeout;
    } else {
      delete process.env.AEXOS_LOADER_TIMEOUT_CRITICAL;
    }
    if (originalHighLoaderTimeout !== undefined) {
      process.env.AEXOS_LOADER_TIMEOUT_HIGH = originalHighLoaderTimeout;
    } else {
      delete process.env.AEXOS_LOADER_TIMEOUT_HIGH;
    }
    if (originalBestEffortLoaderTimeout !== undefined) {
      process.env.AEXOS_LOADER_TIMEOUT_BEST_EFFORT = originalBestEffortLoaderTimeout;
    } else {
      delete process.env.AEXOS_LOADER_TIMEOUT_BEST_EFFORT;
    }

    // Cleanup test data
    try {
      const digestsPath = path.join(testProjectRoot, '.aexos', 'session-digests');
