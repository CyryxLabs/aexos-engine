/**
 * Story CYRYX-DIFF-4.0.5: Onboarding smoke tests in clean environment.
 *
 * Objective:
 * - Validate that "Start Here" onboarding flow remains executable.
...
describe('Onboarding smoke flow (CYRYX-DIFF-4.0.5)', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-onboarding-smoke-'));
  });
  ...
  it('validates Start Here commands are discoverable and executable from clean env', async () => {
    expect(await fs.pathExists(path.join(tempDir, '.aexos-core'))).toBe(false);
  ...
  it('validates onboarding docs keep an objective first-value path', async () => {
    ...
    const releaseCommand = 'npx --yes --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos';
    for (const document of [readme, gettingStarted]) {
      expect(document).toContain(`${releaseCommand} init`);
      expect(document).toContain(`${releaseCommand} install`);
      expect(document).not.toContain('npx @aexos/core');
    }
  ...
  it('validates first-value activation signal and timing budget', () => {
    ...
    expect(greeting).toMatch(
      /Agent dev loaded|dev Agent ready|Vulcan (?:the Builder|\(Builder\)) ready/,
    );
