...
describe('Formatter Integration', () => {
  ...
  beforeEach(() => {
    ...
    // Setup dev agent (Vulcan - Builder)
    mockAgent = {
      id: 'dev',
      name: 'Vulcan',
...
- .aexos-core/scripts/output-formatter.js
- .aexos-core/scripts/validate-output-pattern.js
- .aexos-core/templates/task-execution-report.md
...
`;
  ...
  test('develop-story task uses formatter successfully', () => {
    // 1. Load dev agent (Vulcan - Builder)
    ...
    expect(formattedOutput).toContain('Vulcan (Builder)');
    expect(formattedOutput).toContain('Tá pronto!'); // Pragmatic tone
    expect(formattedOutput).toContain('— Vulcan, sempre construindo 🔨'); // Signature

    // 6. Verify metrics section last
    const lines = formattedOutput.split('\n');
    const metricsIndex = lines.findIndex((line) => line === '### Metrics');
    const signatureIndex = lines.findIndex((line) => line.includes('— Vulcan'));
  ...
  test('formatter injects personality correctly', () => {
    ...
    // Check signature
    expect(output).toContain('— Vulcan, sempre construindo 🔨');
  });
