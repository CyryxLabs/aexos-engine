...
const { createBackup, verifyBackup } = require('../../.aexos-core/cli/commands/migrate/backup');
const { detectV2Structure, analyzeMigrationPlan } = require('../../.aexos-core/cli/commands/migrate/analyze');
const { executeMigration } = require('../../.aexos-core/cli/commands/migrate/execute');
const { updateAllImports, verifyImports } = require('../../.aexos-core/cli/commands/migrate/update-imports');
const { validateStructure } = require('../../.aexos-core/cli/commands/migrate/validate');
const { executeRollback, canRollback } = require('../../.aexos-core/cli/commands/migrate/rollback');

describe('Full Migration Integration', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `aexos-full-migration-test-${Date.now()}`);
  ...
  async function createMockV20Project(dir) {
    const cyryxCoreDir = path.join(dir, '.aexos-core');

    // Create development directories
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'agents'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'tasks'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'templates'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'checklists'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'scripts'), { recursive: true });

    // Create core directories
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'registry'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'utils'), { recursive: true });
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'config'), { recursive: true });

    // Create product directories
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'cli', 'commands'), { recursive: true });

    // Create infrastructure directories
    await fs.promises.mkdir(path.join(cyryxCoreDir, 'hooks'), { recursive: true });

    // Create sample files
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'agents', 'dev.md'),
      '# Dev Agent\nDeveloper persona',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'agents', 'qa.md'),
      '# QA Agent\nQuality assurance',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'tasks', 'build.md'),
      '# Build Task\nBuild workflow',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'registry', 'index.js'),
      'const fs = require(\'fs\');\nconst utils = require(\'../utils\');\nmodule.exports = {};',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'utils', 'helpers.js'),
      'module.exports = { helper: () => true };',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'cli', 'index.js'),
      'const registry = require(\'../registry\');\nconst { Command } = require(\'commander\');\nmodule.exports = {};',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'cli', 'commands', 'run.js'),
      'module.exports = { run: () => {} };',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'hooks', 'pre-commit.js'),
      'module.exports = { hook: () => {} };',
    );
    await fs.promises.writeFile(
      path.join(cyryxCoreDir, 'index.js'),
      'module.exports = require("./registry");',
    );

    // Create config file
    await fs.promises.writeFile(
      path.join(dir, 'cyryx.config.js'),
  ...
  describe('MIG-04: Core Migrated', () => {
    it('should migrate core module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'core', 'registry', 'index.js'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'core', 'utils', 'helpers.js'))).toBe(true);
    });
  });
  ...
  describe('MIG-05: Dev Migrated', () => {
    it('should migrate development module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'development', 'agents', 'dev.md'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'development', 'tasks', 'build.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-06: Imports Updated', () => {
    it('should verify no broken imports after migration', async () => {
      ...
      const importResult = await verifyImports(cyryxCoreDir);
  ...
  describe('MIG-07: Validation Pass', () => {
    it('should validate migrated structure', async () => {
      ...
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      const validation = await validateStructure(cyryxCoreDir);
  ...
  describe('MIG-08: Rollback Works', () => {
    it('should rollback to original v2.0 state', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'development'))).toBe(true);
      ...
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'agents', 'dev.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-09: Dry Run', () => {
    it('should show plan without making changes in dry run mode', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'development'))).toBe(false);
      expect(fs.existsSync(path.join(testDir, '.aexos-core', 'core'))).toBe(false);
    });
  });
  ...
  describe('MIG-11: Conflict Detection', () => {
    it('should detect existing v2.1 directories as conflicts', async () => {
      ...
      // Add a conflict
      await fs.promises.mkdir(path.join(testDir, '.aexos-core', 'core'), { recursive: true });
  ...
  describe('MIG-12: Permissions', () => {
    it('should preserve file permissions during migration', async () => {
      ...
      const originalFile = path.join(testDir, '.aexos-core', 'cli', 'index.js');
      const originalStats = await fs.promises.stat(originalFile);

      const plan = await analyzeMigrationPlan(testDir);
      await executeMigration(plan, { cleanupOriginals: false });

      const migratedFile = path.join(testDir, '.aexos-core', 'product', 'cli', 'index.js');
  ...
  describe('Full Migration Flow', () => {
    it('should complete entire migration workflow', async () => {
      ...
      const cyryxCoreDir = path.join(testDir, '.aexos-core');
      const importResult = await updateAllImports(cyryxCoreDir, plan);
      expect(importResult.totalFiles).toBeGreaterThan(0);

      // Step 7: Validate structure
      const structureValidation = await validateStructure(cyryxCoreDir);
