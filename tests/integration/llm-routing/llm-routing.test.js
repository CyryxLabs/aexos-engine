...
const {
  ...
  LLM_ROUTING_VERSION,
} = require('../../../.aexos-core/infrastructure/scripts/llm-routing/install-llm-routing');

describe('LLM Routing Module', () => {
  const isWindows = os.platform() === 'win32';
  const testTemplatesDir = path.join(
    __dirname,
    '../../../.aexos-core/infrastructure/scripts/llm-routing/templates',
  );
  ...
  describe('Cross-Platform Path Handling', () => {
    test('should use path.join for cross-platform compatibility', () => {
      ...
      const installScript = fs.readFileSync(
        path.join(
          __dirname,
          '../../../.aexos-core/infrastructure/scripts/llm-routing/install-llm-routing.js',
        ),
        'utf8',
      );
