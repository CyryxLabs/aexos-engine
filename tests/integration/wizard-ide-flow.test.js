...
describe('Wizard IDE Flow Integration', () => {
  ...
  describe('Full flow: select -> generate -> verify', () => {
    ...
    it('should complete flow for all 7 IDEs', async () => {
      for (const relative of [
        ...
        await fs.copy(path.join(__dirname, '..', '..', relative), path.join(testDir, relative));
      }
      const wizardState = {
        projectType: 'greenfield',
      ...
      for (const ideKey of getIDEKeys()) {
        ...
        if (config.agentFolder) {
          ...
          expect(await fs.pathExists(path.join(testDir, '.grok', 'skills', 'aexos-dev', 'SKILL.md'))).toBe(true);
        }
      }
    });
  });
