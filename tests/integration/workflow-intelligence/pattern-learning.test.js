...
describe('Pattern Learning Integration', () => {
  ...
  beforeAll(() => {
    learningModule = require('../../../.aexos-core/workflow-intelligence/learning');
  });
  ...
  describe('Capture Hook Integration', () => {
    ...
    beforeAll(() => {
      captureHook = require('../../../.aexos-core/workflow-intelligence/learning/capture-hook');
    });
    ...
    it('should handle disabled state gracefully', async () => {
      // Save original env
      const originalEnv = process.env.AEXOS_PATTERN_CAPTURE;

      try {
        process.env.AEXOS_PATTERN_CAPTURE = 'false';
        captureHook.reset();
        ...
        if (originalEnv !== undefined) {
          process.env.AEXOS_PATTERN_CAPTURE = originalEnv;
        } else {
          delete process.env.AEXOS_PATTERN_CAPTURE;
        }
        captureHook.reset();
      }
    });
  ...
  describe('WIS Integration', () => {
    it('should be accessible from main WIS module', () => {
      const wis = require('../../../.aexos-core/workflow-intelligence');
