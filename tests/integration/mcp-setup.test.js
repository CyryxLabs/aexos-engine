...
describe('MCP Setup Integration', () => {
  const cliIndexPath = path.join(__dirname, '../../.aexos-core/cli/index.js');
  let tempDir;
  let originalHome;
  ...
  describe('CLI Module Loading', () => {
    it('should load mcp command module without errors', () => {
      expect(() => {
        require('../../.aexos-core/cli/commands/mcp');
      }).not.toThrow();
    });

    it('should export createMcpCommand function', () => {
      const mcpModule = require('../../.aexos-core/cli/commands/mcp');
      expect(typeof mcpModule.createMcpCommand).toBe('function');
    });

    it('should load all mcp subcommand modules', () => {
      expect(() => {
        require('../../.aexos-core/cli/commands/mcp/setup');
        require('../../.aexos-core/cli/commands/mcp/link');
        require('../../.aexos-core/cli/commands/mcp/status');
        require('../../.aexos-core/cli/commands/mcp/add');
      }).not.toThrow();
    });
  });
  ...
  describe('Core Module Loading', () => {
    it('should load os-detector module', () => {
      ...
      expect(osDetector.getGlobalCyryxDir).toBeDefined();
      expect(osDetector.getGlobalMcpDir).toBeDefined();
    });

    it('should load global-config-manager module', () => {
      const configManager = require('../../.aexos-core/core/mcp/global-config-manager');
    ...
    it('should load symlink-manager module', () => {
      const symlinkManager = require('../../.aexos-core/core/mcp/symlink-manager');
    ...
    it('should load config-migrator module', () => {
      const configMigrator = require('../../.aexos-core/core/mcp/config-migrator');
    ...
    it('should load mcp index module with all exports', () => {
      const mcp = require('../../.aexos-core/core/mcp');
  ...
  describe('OS Detection', () => {
    const { detectOS, isWindows, isMacOS, isLinux, getOSInfo } = require('../../.aexos-core/core/mcp/os-detector');
  ...
  describe('Server Templates', () => {
    const { getAvailableTemplates, getServerTemplate, SERVER_TEMPLATES } = require('../../.aexos-core/core/mcp/global-config-manager');
  ...
  describe('Path Generation', () => {
    ...
    it('should generate correct global AEXOS directory path', () => {
      const cyryxDir = getGlobalCyryxDir();

      expect(cyryxDir).toContain('.aexos');
      expect(path.isAbsolute(cyryxDir)).toBe(true);
    });

    it('should generate correct MCP directory path', () => {
      const mcpDir = getGlobalMcpDir();

      expect(mcpDir).toContain('.aexos');
  ...
  describe('Link Status Constants', () => {
    const { LINK_STATUS } = require('../../.aexos-core/core/mcp/symlink-manager');
  ...
  describe('Migration Options Constants', () => {
    const { MIGRATION_OPTION } = require('../../.aexos-core/core/mcp/config-migrator');
  ...
  describe('Default Config Structure', () => {
    const { DEFAULT_CONFIG } = require('../../.aexos-core/core/mcp/global-config-manager');
  ...
  describe('CLI Help Output', () => {
    const { createMcpCommand } = require('../../.aexos-core/cli/commands/mcp');
...
describe('MCP CLI Command Registration', () => {
  it('should have mcp command in main CLI', () => {
    const { createProgram } = require('../../.aexos-core/cli/index');
