/**
 * Unit tests for `aexos pro buyer` subcommand module (Story 123.8 — Wave 1).
 *
 * Covers:
...
const BUYER_MODULE = require('../../.aexos-core/cli/commands/pro/buyer');
const {
  isValidEmail,
  classifyError,
  parseEmailsFile,
...
const AEXOS_BIN = path.resolve(__dirname, '..', '..', 'bin', 'aexos.js');
...
//
// Buyer lookups call into pro/license/license-api.js. Public argument rejection
// and the unavailable-registration response do not require that private module.
...
describe('Story 123.8 — buyer CLI subprocess E2E', () => {
  const testWithPro = isProSubmoduleAvailable ? it : it.skip;
  let server;
  let serverUrl;
  ...
  testWithPro('validate --email <buyer> --json prints stable JSON and exits 0', async () => {
    ...
    const result = await runAsync(
      [AEXOS_BIN, 'pro', 'buyer', 'validate', '--email', 'buyer@example.com', '--json'],
      { ...process.env, AEXOS_LICENSE_API_URL: serverUrl },
    );
  ...
  testWithPro('validate --email <non-buyer> --json exits 1 with isBuyer=false', async () => {
    await createMockServer((req, res) => {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({
        isBuyer: false,
    ...
    const result = await runAsync(
      [AEXOS_BIN, 'pro', 'buyer', 'validate', '--email', 'stranger@example.com', '--json'],
      { ...process.env, AEXOS_LICENSE_API_URL: serverUrl },
    );
  ...
  it('validate with invalid email format exits 2 before hitting server', async () => {
    const result = await runAsync(
      [AEXOS_BIN, 'pro', 'buyer', 'validate', '--email', 'not-an-email', '--json'],
      process.env,
      10000,
    );
  ...
  it('register shows pending-Wave-2 message and exits 2', async () => {
    const result = await runAsync(
      [AEXOS_BIN, 'pro', 'buyer', 'register', '--email', 'new@example.com', '--name', 'Novo Comprador', '--yes'],
      process.env,
      10000,
    );
