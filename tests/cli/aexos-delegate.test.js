...
const {
  ...
  sanitizeSlug,
  DelegateCliError,
} = require('@aexos-core/core/external-executors/delegate-cli');

describe('aexos-delegate external executor CLI', () => {
  ...
  test('creates a codex plan with workspace-write sandbox by default', () => {
    ...
    expect(plan.runDir).toBe(path.join(workdir, '.aexos', 'external-runs', '20260508-023045-story-4.3'));
  ...
  test('rejects mutually exclusive prompt inputs', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-delegate-'));
  ...
  test('wraps missing prompt files in a CLI input error', () => {
    const missingPath = path.join(os.tmpdir(), `aexos-missing-prompt-${process.pid}-${Date.now()}.md`);
  ...
  test('dry-run prints machine-readable run metadata without creating run directory', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-delegate-'));
    const binPath = path.join(__dirname, '../../bin/aexos-delegate.js');
      ...
      await new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
          try {
            ...
            expect(stdout).toContain(`RUN_DIR=${path.join(tmpDir, '.aexos/external-runs')}`);
            expect(stdout).toContain('COMMAND=codex -a never -s workspace-write exec');
            expect(fs.existsSync(path.join(tmpDir, '.aexos'))).toBe(false);
            resolve();
          } catch (error) {
            reject(error);
          }
