...
describe('license-cache', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-license-test-'));
  });
  ...
  describe('writeLicenseCache', () => {
    ...
    it('should create .aexos directory if not exists', () => {
      const data = createTestCacheData();
      const cyryxDir = getCyryxDir(testDir);

      expect(fs.existsSync(cyryxDir)).toBe(false);

      writeLicenseCache(data, testDir);

      expect(fs.existsSync(cyryxDir)).toBe(true);
    });
    ...
    it('should use atomic write (no temp file left)', () => {
      ...
      const cyryxDir = getCyryxDir(testDir);
      const files = fs.readdirSync(cyryxDir);
  ...
  describe('readLicenseCache', () => {
    ...
    it('should return null for corrupted JSON', () => {
      ...
      fs.mkdirSync(getCyryxDir(testDir), { recursive: true });
  ...
  describe('Pending Deactivation (AC-7b)', () => {
    ...
    describe('markPendingDeactivationSynced', () => {
      ...
      it('should add syncedAt timestamp', () => {
        ...
        const filePath = path.join(getCyryxDir(testDir), 'pending-deactivation.json');
