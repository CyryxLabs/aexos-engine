...
function writeLegacyCache(cacheModule, cryptoModule, baseDir, data) {
  fs.mkdirSync(cacheModule.getCyryxDir(baseDir), { recursive: true });
...
describe('machine-id stability', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-machine-id-test-'));
  });
  ...
  it('migrates a legacy hostname CPU MAC cache to the native machine ID key', () => {
    ...
    expect(consoleInfoSpy).toHaveBeenCalledWith('[aexos-pro] Cache migrated to new machine_id format');
