...
describe('Performance Tests (AC-13)', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-perf-test-'));
  ...
  describe('Cache Read Performance', () => {
    ...
    it('should handle cache miss quickly (< 5ms)', () => {
      const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-perf-empty-'));
      try {
        const result = measureTime(() => {
    ...
    it('should write cache in reasonable time (< 100ms)', () => {
      const data = createTestCacheData();
      const writeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-perf-write-'));
  ...
  describe('Full Activation Flow Performance (< 3s with mocked API)', () => {
    ...
    it('should complete activation flow in < 3s (mocked)', async () => {
      ...
      const activationDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-perf-act-'));
  ...
  describe('Memory Efficiency', () => {
    it('should not leak memory across multiple cache operations', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `aexos-mem-${i}-`));
        try {
