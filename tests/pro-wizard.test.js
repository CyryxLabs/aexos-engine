...
beforeEach(() => {
  ...
  delete process.env.AEXOS_PRO_KEY;
...
describe('showProHeader', () => {
  test('outputs branded header', () => {
    ...
    expect(output).toContain('AEXOS Pro');
...
describe('Pro command hints', () => {
  ...
  test.each(['en', 'pt', 'es'])('uses published scoped npx commands in %s', (language) => {
    ...
    expect(wizardI18n.t('proNeedHelp')).not.toContain('npx aexos-pro');
...
describe('stepLicenseGate', () => {
  test('CI mode: fails when AEXOS_PRO_KEY not set', async () => {
    process.env.CI = 'true';
    delete process.env.AEXOS_PRO_KEY;
    ...
    expect(result.error).toContain('AEXOS_PRO_KEY');
  });
  ...
  test('CI mode routes a formatted key through the client and preserves API denial', async () => {
    process.env.CI = 'true';
    process.env.AEXOS_PRO_KEY = 'PRO-ABCD-EFGH-IJKL-MNOP';
    // This unit contract controls the provider response. Live entitlement
    // validation is a separate integration obligation, never a timeout oracle.
    const originalLoadLicenseApi = proSetup._testing.loadLicenseApi;
    const online = jest.fn().mockResolvedValue(true);
    const activate = jest.fn().mockRejectedValue(Object.assign(new Error('Denied'), { code: 'INVALID_KEY' }));
    proSetup._testing.loadLicenseApi = () => ({
      LicenseApiClient: class {
        isOnline() { return online(); }
        activate(...args) { return activate(...args); }
      },
    });
    try {
      const result = await proSetup.stepLicenseGate();
      expect(online).toHaveBeenCalledTimes(1);
      expect(activate).toHaveBeenCalledWith(process.env.AEXOS_PRO_KEY, expect.any(String), expect.any(String));
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
      expect(result.error).not.toContain(process.env.AEXOS_PRO_KEY);
      expect(JSON.stringify([...console.log.mock.calls, ...console.error.mock.calls])).not.toContain(process.env.AEXOS_PRO_KEY);
    } finally {
...
describe('stepInstallScaffold', () => {
  test('scaffolds from an explicit pro source fixture', async () => {
    ...
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-pro-test-'));
  ...
  test('acquires Pro artifact source after license validation when no local source exists', async () => {
    ...
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-pro-artifact-test-'));
    const targetDir = path.join(tmpRoot, 'target');
...
describe('stepVerify', () => {
  ...
  const path = require('path');
  let targetDir;
  beforeEach(async () => { targetDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-pro-verify-')); });
  afterEach(async () => { await fs.remove(targetDir); });
  test('shows summary of scaffolded content', async () => {
    const mockScaffoldResult = {
      copiedFiles: [
        ...
        '.aexos-core/pro-config.yaml',
        'pro-version.json',
      ],
    };

    for (const file of mockScaffoldResult.copiedFiles) await fs.outputFile(path.join(targetDir, file), 'fixture');
    const result = await proSetup.stepVerify(mockScaffoldResult, { targetDir });
    ...
    // .yaml and .json files
    expect(result.configs).toEqual(['.aexos-core/pro-config.yaml', 'pro-version.json']);
  });

  test('handles null scaffoldResult', async () => {
    const result = await proSetup.stepVerify(null, { targetDir });
  ...
  test('handles empty copiedFiles', async () => {
    const result = await proSetup.stepVerify({ copiedFiles: [] }, { targetDir });

    expect(result.success).toBe(false);
    expect(result.squads).toEqual([]);
  });
  test('rejects missing files and paths escaping the installation', async () => {
    for (const file of ['missing.md', '../outside.md']) {
      const result = await proSetup.stepVerify({ copiedFiles: [file] }, { targetDir });
      expect(result.success).toBe(false);
      expect(result.error).toContain('verification failed');
    }
  });
  test('verifies preserved custom content on reinstall', async () => {
    await fs.outputFile(path.join(targetDir, 'squads/custom/agent.md'), 'custom content');
    const result = await proSetup.stepVerify({ copiedFiles: [], skippedFiles: ['squads/custom/agent.md'] }, { targetDir });
    expect(result.success).toBe(true);
    expect(result.squads).toEqual(['squads/custom/agent.md']);
    expect(await fs.readFile(path.join(targetDir, 'squads/custom/agent.md'), 'utf8')).toBe('custom content');
  });
});
...
describe('runProWizard', () => {
  test('fails in CI mode without AEXOS_PRO_KEY', async () => {
    process.env.CI = 'true';
    delete process.env.AEXOS_PRO_KEY;
  ...
  test('fails with invalid key format in CI', async () => {
    process.env.CI = 'true';
    process.env.AEXOS_PRO_KEY = 'INVALID';
  ...
  test('does not show branding in CI mode', async () => {
    process.env.CI = 'true';
    process.env.AEXOS_PRO_KEY = 'PRO-AAAA-BBBB-CCCC-DDDD';
...
describe('Key Masking Security', () => {
  test('full key never appears in console output during wizard', async () => {
    process.env.CI = 'true';
    const fullKey = 'PRO-ABCD-EFGH-IJKL-MNOP';
    process.env.AEXOS_PRO_KEY = fullKey;
