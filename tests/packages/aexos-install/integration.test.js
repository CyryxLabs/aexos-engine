...
describe('Integration - Task 8.3: Local NPX Execution', () => {
  ...
  describe('Package Structure Validation', () => {
    ...
    it('should have executable shebang in bin files', () => {
      // Given
      const binFiles = ['bin/aexos-install.js', 'bin/edmcp.js'];
  ...
  describe('CLI Execution Tests', () => {
    it('should execute aexos-install --version via node', () => {
      // Given
    ...
    it('should show help for aexos-install', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/aexos-install.js');
      ...
      // Then
      expect(result).toContain('aexos-install');
  ...
  (runNpxIntegration ? describe : describe.skip)('NPX Local Execution Simulation', () => {
    it('should execute via npm exec (simulates npx .)', () => {
      ...
      // When
      const result = execSync('npm exec -- aexos-install --version', {
        cwd: PKG_DIR,
        encoding: 'utf8',
        timeout: 90000,
