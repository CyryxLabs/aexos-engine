...
- [Squad Discussions](https://github.com/CyryxLabs/AEXOS/discussions/categories/ideas) - Share ideas
...
- Security concerns
- CYRYX-specific patterns (agents, tasks, workflows)
- Performance issues
...
CYRYX implements a **Defense in Depth** strategy with 3 validation layers:
...
| Path | Why |
| ------ | ----- |
| `.aexos-core/core/orchestration/` | Orchestration layer (MasterOrchestrator, GateEvaluator) |
| `.aexos-core/core/execution/` | Execution engine (WaveExecutor, ParallelExecutor) |
...
- **Discussions:** [Start a discussion](https://github.com/CyryxLabs/AEXOS/discussions)
...
CYRYX uses an Open Core model with a private `pro/` git submodule (see [ADR-PRO-001](docs/architecture/adr/adr-pro-001-repository-strategy.md)).
...
git clone https://github.com/<your-fork>/AEXOS.git
cd AEXOS
...
**Push order:** Always push `pro/` changes first, then `aexos-core`.
...
**Thank you for contributing to AEXOS (Cyryx)!**
