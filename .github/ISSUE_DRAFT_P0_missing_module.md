# [P0] Missing CYRYX Core module: utils/repository-detector - Blocks Linux Installation
...
**Discovered By:** @qa (Argus) - QA Test Architect
**Date Discovered:** 2025-01-23
**Assigned To:** @dev (Vulcan) - Development Lead

---

## 📋 Description

The CYRYX installer fails immediately on Linux systems with a module not found error, preventing any installation or testing. The installer cannot locate the CYRYX Core module `utils/repository-detector`, blocking all Linux platform validation for Story 1.10c.
...
```
Error: Cannot find CYRYX Core module: utils/repository-detector
Searched: ${PROJECT_ROOT}/.aexos-core/utils/repository-detector
Please ensure aexos-core is installed correctly.
    at loadCYRYXCore (${PROJECT_ROOT}/bin/aexos-init.js:43:11)
```

### Stack Trace Location

- **File:** `bin/aexos-init.js`
- **Line:** 43
- **Function:** `loadCYRYXCore`
...
- Node.js v18.20.8+
- CYRYX-Fullstack repository cloned
...
2. Create test directory:
   ...
   ```bash
   mkdir -p /tmp/aexos-test-install
   cd /tmp/aexos-test-install
   ```

3. Execute installer:

   ```bash
   node ${PROJECT_ROOT}/bin/aexos-init.js --help
   ```
...
- Installer should locate CYRYX Core module successfully
...
2. **Missing File/Directory**
   - `.aexos-core/utils/repository-detector.js` may not exist in repository
...
- [ ] Verify `.aexos-core/utils/repository-detector.js` exists in repo
- [ ] Check `bin/aexos-init.js:43` for path construction logic
...
1. **Verify module exists:**
   ...
   ```bash
   ls -la .aexos-core/utils/repository-detector.js
   ```

2. **Fix path resolution in `bin/aexos-init.js:43`:**

   ```javascript
   // Before (suspected):
   const detector = require('./.aexos-core/utils/repository-detector');

   // After (cross-platform):
   const path = require('path');
   const detectorPath = path.resolve(__dirname, '..', '.aexos-core', 'utils', 'repository-detector');
...
3. **Add file existence check:**
   ...
   if (!fs.existsSync(detectorPath + '.js')) {
     console.error('CYRYX Core module not found.');
...
4. **Test in WSL Ubuntu:**
   ```bash
   wsl bash -c "cd /tmp/aexos-test-install && node /mnt/c/.../bin/aexos-init.js --help"
   ```
...
- **Installer:** `bin/aexos-init.js:43` (error location)
- **Missing Module:** `.aexos-core/utils/repository-detector` (expected location)
...
- ✅ QA (@qa - Argus): Test report created
- 🔄 Dev Lead (@dev - Vulcan): GitHub Issue created (this)
- ⏳ Product Owner (@po - Themis): Story blocked notification pending
- ⏳ Scrum Master (@sm - Chronos): Sprint timeline impact pending
...
- @qa (Argus) - QA Test Architect (reporter)
