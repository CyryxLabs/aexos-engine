...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.0.0-19C7C0?style=flat-square&labelColor=11161A" alt="Version 1.0.0" />
...
> [!IMPORTANT]
> **Distribution status.** AEXOS `1.0.0` is distributed from the tagged GitHub release. Publishing
> the package to npm is a separate, explicitly authorised release operation and is not claimed as
> complete here. The licence shipped with each artifact governs that exact copy.
...
```bash
aexos office --host 127.0.0.1 --port 4011
```
...
### 1. Install the tagged GitHub release

**Into a new project** — `init` creates the directory:

```bash
npx --yes --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos init my-project
cd my-project
```
...
npx --yes --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos install
```
...
Verify the tagged distribution before installation:

```bash
npx --yes --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos --version
```

The command should report `1.0.0`.

> **Why the command names the package and binary explicitly.**
> The release is currently sourced from GitHub, so `--package` selects the verified release
> tarball and `aexos` selects the intended binary without depending on npm registry publication.
...
| ----------------------------------------------- | ---------------------------------------- |
| Starting a new project                          | `npx --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos init my-project`     |
| Adding AEXOS to a directory that already exists | `cd there && npx --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos install` |
| Contributing to AEXOS itself                    | `git clone` → `npm install` → `npm link` |

AEXOS `1.0.0` is installed from the public GitHub release tag shown above. The manifests retain
public npm package metadata so a separately authorised npm publication remains possible, but this
README does not present npm availability as complete. See [LICENSE](LICENSE) for the terms that
govern the distributed artifact.
...
`npx --yes --package=https://github.com/CyryxLabs/aexos-engine/releases/download/v1.0.0/aexos-core-1.0.0.tgz aexos <command>`. Restart the IDE after
a successful install because agent and skill projections are loaded when the IDE session starts.
...
The licence in this repository and in each distributed artifact governs that exact copy. A future
commercial release remains subject to legal review, signed artifact delivery and release
certification. Redistributing AEXOS itself — forks, mirrors, or hosted products whose value is
AEXOS's own functionality — is not permitted under the current licence; see Section 3.
