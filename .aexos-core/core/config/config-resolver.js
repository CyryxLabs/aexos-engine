...
const { mergeLocalConfig, MIGRATION_KEY } = require('./migration-compat');
 ...
 */
const CONFIG_FILES = {
  framework: '.aexos-core/framework-config.yaml',
  project: '.aexos-core/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.aexos-core/local-config.yaml',
  legacy: '.aexos-core/core-config.yaml',
  user: path.join(os.homedir(), '.aexos', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, 'aexos-app.config.yaml');
  ...
  if (l4.data) {
    config = mergeLocalConfig(config, l4.data);
    if (options.debug) {
      const sourceData = { ...l4.data };
      delete sourceData[MIGRATION_KEY];
      trackSources(sources, sourceData, LEVELS.local, CONFIG_FILES.local);
    }
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.aexos/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation = process.env.AEXOS_SUPPRESS_DEPRECATION === 'true'
    || process.env.AEXOS_SUPPRESS_DEPRECATION === '1';

  if (!suppressDeprecation) {
    warnings.push(
      '[DEPRECATION] Monolithic core-config.yaml detected. '
      + 'Run `aexos config migrate` to split into layered config files. '
      + 'Monolithic format will be removed in v4.0.0. '
      + 'Set AEXOS_SUPPRESS_DEPRECATION=true to silence this warning.',
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case 'app': case '3': case 'L3':
      ...
      relativePath = path.join(options.appDir, 'aexos-app.config.yaml');
      break;
...
/**
 * Ensure the ~/.aexos/ directory exists with secure permissions.
 *
 * @returns {string} Path to ~/.aexos/ directory
 */
...
/**
 * Set a value in the user config file (~/.aexos/user-config.yaml).
