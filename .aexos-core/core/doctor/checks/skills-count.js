 ...
 * PASS: >=7 total skills and all AEXOS agent skills present.
 * WARN: skills exist but AEXOS agent skills are incomplete.
 * FAIL: 0 or directory missing.
 *
 * @module aexos-core/doctor/checks/skills-count
 * @story INS-4.8
 */
...
function countAgentSkillFiles(agentSkillsDir) {
  ...
  try {
    ...
    if (error && error.code === 'ENOENT') return 0;
    throw new Error(`Cannot read AEXOS agent skills directory "${agentSkillsDir}": ${error.message}`);
  }
...
async function run(context) {
  ...
  const agentSkillsDir = path.join(skillsDir, 'AEXOS', 'agents');
  ...
  if (count >= 7) {
    return {
      check: name,
      status: 'PASS',
      message: `${count} skills found (${agentSkillCount}/${sourceAgentCount} AEXOS agent skills)`,
      fixCommand: null,
    };
  }
