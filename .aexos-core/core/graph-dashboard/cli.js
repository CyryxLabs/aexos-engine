...
/**
 * Write HTML graph to .aexos/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), '.aexos');
 ...
 * Writes to .aexos/graph.dot, .aexos/graph.mmd, or .aexos/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), '.aexos-core/data/entity-registry.yaml');
...
function handleHelp() {
  const usage = `
Usage: aexos graph [command] [options] (or aexos-graph)
...
Examples:
  aexos graph --deps                        Show dependency tree
  aexos graph --deps --format=json          Output as JSON
  aexos graph --deps --format=html          Interactive HTML graph (opens browser)
  aexos graph --deps --watch                Live DOT file for VS Code preview
  aexos graph --deps --watch --format=html  Live HTML with auto-refresh
  aexos graph --deps --watch --format=mermaid  Live Mermaid file
  aexos graph --deps --watch --interval=10  Refresh every 10 seconds
  aexos graph --stats                       Show entity stats and cache metrics
`.trim();
...
async function handleSummary() {
  ...
  sections.push('AEXOS Graph Dashboard');
