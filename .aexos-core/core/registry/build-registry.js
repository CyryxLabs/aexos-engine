 ...
 *
 * Scans AEXOS modules and builds the service registry JSON file.
...
const SCAN_SOURCES = [
  {
    pattern: '.aexos-core/development/tasks/**/*.md',
    category: 'task',
  ...
  {
    pattern: '.aexos-core/product/templates/**/*.md',
    category: 'template',
    taskFormat: 'TEMPLATE',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.aexos-core/infrastructure/scripts/**/*.js',
    category: 'script',
    taskFormat: 'SCRIPT',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.aexos-core/product/checklists/**/*.md',
    category: 'checklist',
    taskFormat: 'CHECKLIST',
    subcategoryExtractor: () => 'quality',
  },
  {
    pattern: '.aexos-core/development/workflows/**/*.yaml',
    category: 'workflow',
    taskFormat: 'WORKFLOW',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.aexos-core/core/data/**/*.md',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
    subcategoryExtractor: () => 'knowledge',
  },
  {
    pattern: '.aexos-core/core/data/**/*.yaml',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
...
/**
 * Qualify only colliding worker IDs with their canonical source directory.
 * Every member of a collision group is qualified so results do not depend on
 * glob or scan order. Non-colliding IDs remain unchanged for compatibility.
 *
 * @param {object[]} workers - Worker entries before identity resolution
 * @returns {object[]} The same entries with deterministic unique IDs
 */
function qualifyCollidingWorkerIds(workers) {
  const groups = new Map();
  for (const worker of workers) {
    if (!groups.has(worker.id)) groups.set(worker.id, []);
    groups.get(worker.id).push(worker);
  }
  ...
  for (const [baseId, group] of groups) {
    ...
    for (const worker of group) {
      const normalizedPath = worker.path.replace(/\\/g, '/');
      const relativePath = normalizedPath.replace(/^\.aexos-core\//, '');
      const directoryQualifier = toKebabId(path.posix.dirname(relativePath));
      const qualifiedId = `${baseId}-${directoryQualifier}`;
      if (!directoryQualifier || reserved.has(qualifiedId)) {
        throw new Error(
          `Cannot derive a unique path-qualified worker ID for ${baseId}: ${worker.path}`,
        );
      }
      worker.id = qualifiedId;
      reserved.add(qualifiedId);
    }
  }
  return workers;
}
...
async function buildRegistry(baseDir = process.cwd()) {
  ...
  qualifyCollidingWorkerIds(workers);
...
async function main() {
  ...
  const outputPath = process.argv[3] || path.join(baseDir, '.aexos-core/core/registry/service-registry.json');
