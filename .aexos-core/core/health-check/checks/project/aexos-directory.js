 ...
 *
 * @class AexosDirectoryCheck
 * @extends BaseCheck
 */
class AexosDirectoryCheck extends BaseCheck {
  constructor() {
    super({
      ...
      tags: ['aexos', 'directory', 'structure'],
    });
  }
  ...
  async execute(context) {
    ...
    const aexosPath = path.join(projectRoot, '.aexos');
    const issues = [];
    const found = [];

    // Check if .aexos exists at all
    try {
      const stats = await fs.stat(aexosPath);
    ...
    try {
      const testFile = path.join(aexosPath, '.write-test');
...
module.exports = AexosDirectoryCheck;
