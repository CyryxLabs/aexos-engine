 ...
 *
 * @module aexos-core/health-check/checks/services/gemini-cli
 * @version 1.0.0
...
class GeminiCliCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Check for rules.md (AEXOS rules)
      try {
      ...
      // Check for AEXOS agents
      try {
        const agentsDir = path.join(geminiDir, 'rules', 'AEXOS', 'agents');
        const agentFiles = await fs.readdir(agentsDir);
        details.cyryxAgents = agentFiles.filter((f) => f.endsWith('.md')).length;
      } catch {
        details.cyryxAgents = 0;
      }
    } catch {
      // No project config
    }
    ...
    if (details.projectConfig && details.cyryxAgents === 0) {
      warnings.push('No AEXOS agents installed for Gemini CLI');
    }
    ...
    if (details.cyryxAgents > 0) parts.push(`${details.cyryxAgents} AEXOS agents`);
