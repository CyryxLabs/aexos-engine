...
class GreenfieldHandler extends EventEmitter {
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.workflowPath = path.join(
      projectRoot,
      '.aexos-core/development/workflows/greenfield-fullstack.yaml',
    );
    ...
    if (!this._workflowExecutor) {
      try {
        ...
        this._workflowExecutor = new WorkflowExecutor(this.projectRoot, {
          ...
          dispatchSubagent: this.options.dispatchSubagent,
          checkpointHandler: this.options.checkpointHandler,
          checkTool: this.options.checkTool,
          executionProfile: this.options.executionProfile,
          executionContext: this.options.executionContext,
          confidenceThreshold: this.options.confidenceThreshold,
          enableConfidenceGate: this.options.enableConfidenceGate,
        });
      } catch (error) {
  ...
  async _executePhase0(context) {
    ...
    const spawnResult = await this._spawnAgent('@devops', 'environment-bootstrap', {
      ...
      creates: ['.aexos/config.yaml', '.aexos/environment-report.json', '.gitignore', 'README.md', 'package.json'],
    });

    this.phaseProgress[GreenfieldPhase.BOOTSTRAP] = {
      status: spawnResult.success ? 'complete' : spawnResult.pending ? 'pending_dispatch' : 'failed',
      endTime: Date.now(),
      result: spawnResult,
    };
    ...
    if (!spawnResult.success) {
      if (spawnResult.pending) {
        return this._handlePhasePending(GreenfieldPhase.BOOTSTRAP, spawnResult, context);
      }
    ...
    for (const step of PHASE_1_SEQUENCE) {
      ...
      if (!spawnResult.success) {
        this.phaseProgress[GreenfieldPhase.DISCOVERY] = {
          status: spawnResult.pending ? 'pending_dispatch' : 'failed',
          endTime: Date.now(),
          stepResults,
        };
        if (spawnResult.pending) {
          return this._handlePhasePending(
  ...
  async _executePhase2(context) {
    ...
    if (!spawnResult.success) {
      if (spawnResult.pending) {
        return this._handlePhasePending(GreenfieldPhase.SHARDING, spawnResult, context);
      }
  ...
  async _executePhase3(context) {
    ...
    // AC5: Transition to Development Cycle via WorkflowExecutor when a story is supplied.
    ...
    if (!context.storyPath) {
      const pending = {
        ...
        pendingRequirements: ['context.storyPath'],
        data: {
          message: 'Development cycle is ready and requires a story handoff.',
          nextStep: 'development_cycle',
          workflowExecutorAvailable: true,
          handoff: 'Use @sm → *create-story, then resume with context.storyPath',
          context,
        },
      };
      ...
      this.emit('phasePending', { phase: GreenfieldPhase.DEV_CYCLE, result: pending, context });
      return pending;
    }

    const workflowResult = await workflowExecutor.execute(context.storyPath, context.epicContext || {});
    const status = workflowResult?.success === true
      ? 'complete'
      : workflowResult?.awaiting_input === true || workflowResult?.status === 'waiting_for_input'
        ? 'waiting_for_input'
        : 'failed';
    this.phaseProgress[GreenfieldPhase.DEV_CYCLE] = {
      status,
    ...
    if (status === 'failed') {
      return this._handlePhaseFailure(
        GreenfieldPhase.DEV_CYCLE,
        workflowResult?.error || 'Development workflow failed',
        context,
      );
    }
    ...
    if (status === 'waiting_for_input') {
      const waiting = {
        ...
        awaiting_input: true,
        phase: GreenfieldPhase.DEV_CYCLE,
        data: { result: workflowResult, context },
      };
      this.emit('phasePending', { phase: GreenfieldPhase.DEV_CYCLE, result: waiting, context });
      return waiting;
    }
  ...
  async _spawnAgent(agent, task, spawnContext = {}) {
    ...
    try {
      if (typeof this.options.dispatchSubagent === 'function') {
        const result = await this.options.dispatchSubagent({
          agentId: agent,
          task,
          context: spawnContext,
          projectRoot: this.projectRoot,
        });
        return this._normalizeSpawnResult(result, agent, task, spawnContext);
      }
      ...
      if (TerminalSpawner.isSpawnerAvailable()) {
        ...
        return this._normalizeSpawnResult({
          success: result.success === true,
          status: result.status,
          pid: result.pid,
          output: result.output,
          outputFile: result.outputFile,
        }, agent, task, spawnContext);
      }
      ...
      return {
        ...
        pending: true,
        pendingRequirements: [`Configured dispatch for ${agent}:${task}`],
        manual: true,
        instructions: `Spawn ${agent} manually and execute: *${task}`,
        error: `No agent dispatcher is available for ${agent}:${task}`,
      };
    } catch (error) {
  ...
  _normalizeSpawnResult(result, agent, task, spawnContext) {
    const completionStatuses = ['success', 'completed', 'APPROVED'];
    const hasStatus = typeof result?.status === 'string';
    const statusCompleted = completionStatuses.includes(result?.status);
    const explicitlyCompleted = result?.success === true || statusCompleted;
    const inconsistent = (result?.success === false && statusCompleted) ||
      (hasStatus && !statusCompleted);
    if (!explicitlyCompleted || inconsistent || result?.simulated === true) {
      return {
        ...result,
        success: false,
        status: result?.status || 'failed',
        error: result?.error || `Agent ${agent} did not complete ${task}`,
      };
    }

    const declaredOutputs = Array.isArray(spawnContext.creates)
      ? spawnContext.creates
      : typeof spawnContext.creates === 'string'
        ? [spawnContext.creates]
        : [];
    const missingOutputs = declaredOutputs.filter(
      (outputPath) => !this._isUsableOutputPath(outputPath),
    );
    if (missingOutputs.length > 0) {
      return {
        ...
        error: `Agent ${agent} did not create required outputs: ${missingOutputs.join(', ')}`,
        missingOutputs,
      };
    }

    return { ...result, success: true, status: result.status || 'completed' };
  }

  _isUsableOutputPath(outputPath) {
    if (typeof outputPath !== 'string' || outputPath.trim() === '') return false;
    const rootPath = path.resolve(this.projectRoot);
    const candidatePath = path.resolve(rootPath, outputPath);
    if (!this._isPathWithinRoot(candidatePath, rootPath) || !fs.existsSync(candidatePath)) {
      return false;
    }

    try {
      return this._containsNonEmptyRegularFile(candidatePath, rootPath, new Set());
    } catch {
      return false;
    }
  }

  _containsNonEmptyRegularFile(candidatePath, rootPath, visited) {
    const realPath = fs.realpathSync(candidatePath);
    if (!this._isPathWithinRoot(realPath, rootPath) || visited.has(realPath)) return false;
    ...
    return fs.readdirSync(realPath).some((entry) =>
      this._containsNonEmptyRegularFile(path.join(realPath, entry), rootPath, visited),
    );
  }

  _isPathWithinRoot(candidatePath, rootPath) {
  ...
  _handlePhasePending(phase, result, context) {
    this.emit('phasePending', { phase, result, context });
    return {
      action: 'greenfield_phase_pending',
      success: false,
      status: result.status || 'pending_dispatch',
