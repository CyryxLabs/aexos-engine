...
class ContextManager {
  ...
  constructor(workflowId, projectRoot) {
    ...
    // State file path
    this.stateDir = path.join(projectRoot, '.aexos', 'workflow-state');
  ...
  async _saveState() {
    ...
    atomicWriteSync(this.statePath, JSON.stringify(this._stateCache, null, 2));
  }
  ...
  async _saveHandoffFile(handoff) {
    ...
    await fs.ensureDir(this.handoffDir);
    atomicWriteSync(filePath, JSON.stringify(handoff, null, 2));
  }
  ...
  async _saveConfidenceFile(confidence) {
    ...
    await fs.ensureDir(this.confidenceDir);
    atomicWriteSync(filePath, JSON.stringify(confidence, null, 2));
  }
  ...
  _resolveConfidenceThreshold() {
    const raw = process.env.AEXOS_DELIVERY_CONFIDENCE_THRESHOLD;
