...
class BobOrchestrator {
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.workflowExecutor = new WorkflowExecutor(projectRoot, workflowOptions);
    ...
    try {
      ...
      if (!lockAcquired) {
        return {
          ...
          projectState: null,
          action: 'lock_failed',
          error: 'Another Bob orchestration is already running. Wait or check .aexos/locks/',
        };
      }
  ...
  detectProjectState(projectRoot = this.projectRoot) {
    ...
    // Check 3: Does AEXOS documentation exist? (AC4, AC5)
  ...
  async _handleNoConfig(_context) {
    ...
    return {
      action: 'onboarding',
      data: {
        message: 'Projeto sem configuração AEXOS detectado. Iniciando onboarding...',
        nextStep: 'run_cyryx_init',
      },
    };
  }
  ...
  async _executeStory(storyPath) {
    ...
    const phaseDefinitions = [
      ['1_validation', 'validation', assignment.executor],
      ['2_development', 'development', assignment.executor],
      ['3_self_healing', 'self_healing', assignment.executor],
      ['4_quality_gate', 'quality_gate', assignment.quality_gate],
      ['5_push', 'push', '@devops'],
      ['6_checkpoint', 'checkpoint', assignment.executor],
    ];
    const completedStatuses = new Set(['completed', 'skipped']);
    for (const [phaseId, phaseName, executor] of phaseDefinitions) {
      const phaseResult = result?.phaseResults?.[phaseId];
      const explicitlyCompleted = phaseResult && completedStatuses.has(phaseResult.status);
      if (explicitlyCompleted) {
        await this._updatePhase(phaseName, storyId, executor);
      }
    }

    const status = result?.success === true
      ? 'completed'
      : result?.awaiting_input === true || result?.status === 'waiting_for_input'
        ? 'waiting_for_input'
    ...
    return {
      ...
      status,
      awaiting_input: status === 'waiting_for_input',
      data: {
        assignment,
        result,
        storyPath,
