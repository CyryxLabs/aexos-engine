 ...
 *
 * Provides async API for spawning AEXOS agents in separate terminals
...
const DEFAULT_OPTIONS = {
  ...
  retries: MAX_RETRIES,
  debug: false,
  budgetCeilingUsd: process.env.AEXOS_MODEL_BUDGET_CEILING_USD,
};
...
async function createContextFile(context, outputDir = os.tmpdir()) {
  ...
  const contextPath = path.join(outputDir, `aexos-context-${Date.now()}.json`);
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(bashExecutable, [scriptPath, ...args], {
      env: {
        ...
        AEXOS_OUTPUT_DIR: opts.outputDir,
        AEXOS_INLINE_MODE: 'true', // Signal to pm.sh that we're running inline
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const cyryxFiles = files.filter(
      (f) => f.startsWith('aexos-output-') || f.startsWith('aexos-lock-') || f.startsWith('aexos-context-'),
    );

    for (const file of cyryxFiles) {
...
function formatCompatibilityReport(report) {
  const lines = [
    '═══════════════════════════════════════════════════════════════',
    '                 AEXOS Terminal Spawner Compatibility Report     ',
    '═══════════════════════════════════════════════════════════════',
    '',
