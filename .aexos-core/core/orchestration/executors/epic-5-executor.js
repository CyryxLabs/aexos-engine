...
class Epic5Executor extends EpicExecutor {
  ...
  async execute(context) {
    ...
    try {
      this._validateContext(context, ['storyId', 'failedEpic', 'error']);
      if (context.storyId === null || context.failedEpic === null || context.error === null) {
        throw new Error('storyId, failedEpic, and error must be non-null');
      }
      const { failedEpic, error, attempts = 0 } = context;
      ...
      if (StuckDetector && Array.isArray(context.attemptHistory)) {
        const detector = new StuckDetector({
          maxAttempts: this.orchestrator?.maxRetries || 3,
        });

        const stuckStatus = detector.check(context.attemptHistory, context.currentApproach);
        isStuck = stuckStatus.stuck;
      ...
      if (recoveryResult.blocked || recoveryResult.success === false) {
        return {
          ...this._failExecution(recoveryResult.error || recoveryResult.message || 'Recovery was blocked'),
          blocked: true,
          pendingRequirements: recoveryResult.pendingRequirements || [],
          recoveryResult,
          strategy,
        };
      }
  ...
  async _executeRollbackAndRetry(context) {
    ...
    const subtaskId = context.subtaskId || context.failedSubtask;
    if (!subtaskId) {
      return {
        success: false,
        blocked: true,
        shouldRetry: false,
        error: 'Rollback requires the failed subtask identifier',
        pendingRequirements: ['context.subtaskId or context.failedSubtask'],
      };
    }
    if (RollbackManager) {
      try {
        ...
        const rollback = await manager.rollback(subtaskId, {
          hard: true,
          reason: context.error?.message || String(context.error || 'automatic recovery'),
        });
        if (!rollback?.success) {
          return {
            success: false,
            blocked: true,
            shouldRetry: false,
            error: rollback?.error || 'Rollback did not complete',
            pendingRequirements: [`Recoverable checkpoint for subtask ${subtaskId}`],
          };
        }
      ...
      return {
        ...
        blocked: true,
        shouldRetry: false,
        error: 'RollbackManager is unavailable',
        pendingRequirements: ['RollbackManager runtime module'],
      };
    }
  ...
  async _executeEscalateToHuman(context) {
    ...
    const reportPath = this._getPath('.aexos', 'escalations', `${context.storyId}-${Date.now()}.md`);
