 ...
 *
 * Handles first execution behavior for existing projects without AEXOS documentation.
 ...
 * projects that have code but do not yet have AEXOS documentation.
 */
class BrownfieldHandler extends EventEmitter {
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.workflowPath = path.join(
      projectRoot,
      '.aexos-core/development/workflows/brownfield-discovery.yaml',
    );
  ...
  async handle(context = {}) {
    this._log('🔍 First execution detected — project has code but no AEXOS docs');
...
Quer que eu comece?`;
  ...
  async _executeDiscovery(context) {
    ...
    try {
      ...
      if (result.status === 'waiting_for_input' || result.awaiting_input === true) {
        return {
          action: 'brownfield_waiting',
          success: false,
          status: 'waiting_for_input',
          awaiting_input: true,
          data: { result, message: 'Brownfield discovery is waiting for input' },
        };
      }
      ...
      if (result.success) {
        ...
        const missingOutputs = requiredOutputs.filter((outputPath) => {
          const absolutePath = path.join(this.projectRoot, outputPath);
          if (!fs.existsSync(absolutePath)) return true;
          try {
            return fs.readFileSync(absolutePath, 'utf8').trim().length === 0;
          } catch {
            return true;
          }
        });
        if (missingOutputs.length > 0) {
          return {
            ...
            data: {
              result,
              message: 'Brownfield discovery completed without required output artifacts',
              missingOutputs,
              canRetry: true,
            },
          };
  ...
  async _onPhaseStart(phase, context) {
    const phaseId = this._getPhaseId(phase);
    this._log(`Phase started: ${phaseId}`);
    this.phaseProgress[phaseId] = { status: 'in_progress', startTime: Date.now() };

    await this._recordPhase(phaseId, context);

    this.emit('phaseStart', { phase: phaseId, phaseDefinition: phase, context });
  }
  ...
  async _onPhaseComplete(phase, output, context) {
    const phaseId = this._getPhaseId(phase);
    this._log(`Phase completed: ${phaseId}`);
    ...
    this.emit('phaseComplete', { phase: phaseId, phaseDefinition: phase, output, context });
  }
  ...
  async _onPhaseError(phase, error, context) {
    const phaseId = this._getPhaseId(phase);
    this._log(`Phase failed: ${phaseId} - ${error.message}`, 'error');
    this.phaseProgress[phaseId] = {
      ...this.phaseProgress[phaseId],
      status: 'failed',
      endTime: Date.now(),
    ...
    this.emit('phaseError', { phase: phaseId, phaseDefinition: phase, error, context });
  ...
  _getPhaseId(phase) {
    if (phase && typeof phase === 'object') {
      return phase.step ?? phase.phase ?? phase.id ?? phase.phase_name ?? 'unknown_phase';
    }
    return phase;
  }
      ...
      case PostDiscoveryChoice.RESOLVE_DEBTS:
        ...
        return {
          ...
          data: {
            ...
            taskPath: '.aexos-core/development/tasks/brownfield-create-epic.md',
            context,
          },
        };
