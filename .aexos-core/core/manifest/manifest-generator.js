 ...
 * Scans the .aexos-core directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.cyryxCoreDir = path.join(this.basePath, '.aexos-core');
    this.manifestDir = path.join(this.cyryxCoreDir, 'manifests');
    this.version = '2.1.0';
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.cyryxCoreDir, 'development', 'agents');
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: 'active',
              file_path: `.aexos-core/development/agents/${file}`,
  ...
  async generateWorkersManifest() {
    const registryPath = path.join(this.cyryxCoreDir, 'core', 'registry', 'service-registry.json');
    ...
    try {
      ...
      if (!registry || !Array.isArray(registry.workers)) {
        throw new Error('Service registry must contain a workers array');
      }
      const ids = new Set();
      const duplicateIds = new Set();
      for (const worker of registry.workers) {
        if (ids.has(worker.id)) duplicateIds.add(worker.id);
        ids.add(worker.id);
      }
      if (duplicateIds.size) {
        throw new Error(
          `Service registry contains ambiguous worker IDs: ${[...duplicateIds].sort().join(', ')}`,
        );
      }
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.cyryxCoreDir, 'development', 'tasks');
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.aexos-core/development/tasks/${file}`,
            status: 'active',
          });
