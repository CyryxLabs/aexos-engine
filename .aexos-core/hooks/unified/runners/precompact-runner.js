 ...
 * - aexos-core: Hook runner (this file) + pro detection
 * - aexos-pro: Digest extraction logic (memory/session-digest/)
 *
 * @module .aexos-core/hooks/unified/runners/precompact-runner
 ...
 *
 * Execution flow:
 * 1. Detect aexos-pro availability via pro-detector
...
async function onPreCompact(context) {
  try {
    // 1. Detect aexos-pro availability
    const proAvailable = isProAvailable();

    if (!proAvailable) {
      console.log('[PreCompact] aexos-pro not available, skipping session digest');
    ...
    setImmediate(async () => {
      try {
        ...
        if (!digestModule || typeof digestModule.extractSessionDigest !== 'function') {
          console.error('[PreCompact] Digest extractor not found or invalid in aexos-pro');
          return;
        }
