...
**Module:** `.aexos-core/hooks/unified`
...
Hook Runners (runners/*.js)
         ↓
Application Logic (aexos-core, aexos-pro)
```
...
- **`precompact-runner.js`** (Story MIS-3)
  ...
  - Open Core architecture (delegates to aexos-pro)
...
pro/memory/session-digest/extractor.js
         ↓
.aexos/session-digests/{session-id}-{timestamp}.yaml
```
...
- If aexos-pro not available: no-op (log and return)
...
```javascript
// .aexos-core/hooks/unified/runners/my-runner.js
...
async function main() {
  ...
  const { onMyEvent } = require('../../.aexos-core/hooks/unified/runners/my-runner');
...
if (!isProAvailable()) {
  console.log('[Hook] aexos-pro not available, skipping');
  return;
}
...
- **Story PRO-5:** aexos-pro Repository Bootstrap (pro-detector pattern)

---

*Unified Hooks System - AEXOS Core*
