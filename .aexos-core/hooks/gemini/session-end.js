#!/usr/bin/env node
/**
 * AEXOS Session End Hook for Gemini CLI
 * Story GEMINI-INT.6 - AEXOS Hooks System
 *
...
async function sessionEnd() {
  ...
  try {
    const sessionDir = path.join(projectDir, '.aexos', 'sessions');
    ...
    let modifiedFiles = [];
    const sessionFilesPath = path.join(projectDir, '.aexos', 'session-files.json');
    ...
    let toolCount = 0;
    const toolLogPath = path.join(projectDir, '.aexos', 'logs', 'tool-results.jsonl');
