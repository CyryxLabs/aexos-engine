#!/usr/bin/env node
/**
 * AEXOS Rewind Handler
 * Story GEMINI-INT.14 - /rewind Integration
 *
 * Syncs Gemini's /rewind command with AEXOS state.
 */
...
async function handleRewind() {
  ...
  // Sync with AEXOS memory layer
  try {
    const memoryDir = path.join(projectDir, '.aexos', 'memory');
    ...
    const logDir = path.join(projectDir, '.aexos', 'logs');
