 ...
 *
 * Usage: node .aexos-core/hooks/ids-post-commit.js
 */
...
async function main() {
  ...
  if (process.env.AEXOS_IDS_FORCE !== '1' && isDocsOnlyCommit(changes)) {
    console.log('[IDS-Hook] Docs-only commit detected, skipping registry update.');
    return;
  }

  try {
    const { RegistryUpdater } = require(path.resolve(REPO_ROOT, '.aexos-core/core/ids/registry-updater.js'));
