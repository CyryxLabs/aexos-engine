...
function loadContextFile(contextFile) {
  if (!contextFile) return {};

  const resolvedPath = path.resolve(process.cwd(), contextFile);
  let content;
  try {
    content = fs.readFileSync(resolvedPath, 'utf8');
  } catch (error) {
    throw new Error(`Unable to read context file '${contextFile}': ${error.message}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid JSON in context file '${contextFile}': ${error.message}`);
  }

  if (!context || typeof context !== 'object' || Array.isArray(context)) {
    throw new Error(`Context file '${contextFile}' must contain a plain JSON object`);
  }
  return context;
}

function actionOptions(options, command) {
  return command && typeof command.optsWithGlobals === 'function'
 ...
 */
async function generateDocument(templateType, options, command) {
  options = actionOptions(options, command);
  ...
  try {
    ...
    const context = loadContextFile(options.context);
    ...
    } else if (options.save && result.savedTo) {
    ...
    if (!options.json && result.validation && !result.validation.isValid) {
 ...
 * @param {Object} options - Command options
 */
async function listTemplates(options, command) {
  options = actionOptions(options, command);
  const Engine = getTemplateEngine();
 ...
 * @param {Object} options - Command options
 */
async function showTemplateInfo(templateType, options, command) {
  options = actionOptions(options, command);
  const Engine = getTemplateEngine();
  const engine = new Engine({ interactive: false });
...
function createGenerateCommand() {
  const generate = new Command('generate')
    ...
    .option('--context <json-file>', 'Load template variables from a JSON object')
