 ...
 *
 * CLI command for validating AEXOS-Core installation integrity.
 ...
 *
 * Usage:
 *   aexos validate                    # Validate current installation
 *   aexos validate --repair           # Repair missing/corrupted files
 *   aexos validate --repair --dry-run # Preview repairs without applying
 *   aexos validate --detailed         # Show detailed file list
 *   aexos validate --require-signature # Enforce signed manifests
 *   aexos validate --json             # Output as JSON
 */
...
function createValidateCommand() {
  ...
  validate
    .description('Validate AEXOS-Core installation integrity')
    ...
    .option(
      '--require-signature',
      'Fail when the manifest signature is missing (also enabled by AEXOS_REQUIRE_SIGNATURE=1)',
    )
...
async function runValidation(options) {
  ...
  // Check if AEXOS-Core is installed
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, '.aexos-core', 'install-manifest.yaml');
  ...
  if (!sourceDir && options.repair) {
    ...
    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, '.aexos-core', 'install-manifest.yaml'))) {
        sourceDir = src;
        break;
      }
    }
 ...
 * --require-signature or AEXOS_REQUIRE_SIGNATURE=1.
 *
 * @param {Object} params - Resolution params
 * @param {Object} params.options - Commander options
 * @param {string} params.cyryxCoreDir - Installed .aexos-core directory
 ...
 */
function resolveSignatureRequirement({ options = {}, cyryxCoreDir, sourceDir = null, env = process.env }) {
  if (options.signature === false) {
    return false;
  }

  if (options.requireSignature === true || isTruthyEnv(env.AEXOS_REQUIRE_SIGNATURE)) {
    return true;
  }

  const targetSignaturePath = path.join(cyryxCoreDir, 'install-manifest.yaml.minisig');
  if (fs.existsSync(targetSignaturePath)) {
    return true;
  }

  if (sourceDir) {
    const sourceSignaturePath = path.join(sourceDir, '.aexos-core', 'install-manifest.yaml.minisig');
