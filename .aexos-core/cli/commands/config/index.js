/**
 * Config Command Module
 *
 * CLI commands for the layered configuration system (ADR-PRO-002).
 *
 * Subcommands:
 *   aexos config show [--level <n>] [--app <name>] [--debug]
 *   aexos config diff --levels <a>,<b>
 *   aexos config migrate [--dry-run] [--force]
 *   aexos config validate [--level <n>]
 *   aexos config init-local
 *
 * @module cli/commands/config
 * @version 1.0.0
 * @story PRO-4 — Core-Config Split Implementation
 */

'use strict';

const { Command } = require('commander');
const path = require('path');
const fs = require('fs');
const yaml = require('js-yaml');
const { spawnSync } = require('child_process');
const { withMigrationCompatibility } = require('../../../core/config/migration-compat');
const frameworkConfigSchema = require('../../../core/config/schemas/framework-config.schema.json');
const projectConfigSchema = require('../../../core/config/schemas/project-config.schema.json');

const UNSAFE_CONFIG_KEYS = new Set(['__proto__', 'prototype', 'constructor']);

// Resolve core config modules (relative from .aexos-core/cli/commands/config/)
const configResolverPath = path.resolve(__dirname, '..', '..', '..', 'core', 'config', 'config-resolver');
const mergeUtilsPath = path.resolve(__dirname, '..', '..', '..', 'core', 'config', 'merge-utils');
const envInterpolatorPath = path.resolve(__dirname, '..', '..', '..', 'core', 'config', 'env-interpolator');

/**
 * Resolve the canonical framework version.
 *
 * Single source of truth is the root package.json shipped with the framework
 * (`@aexos/core`). Never hardcode a literal here — a frozen literal is
 * exactly how `framework_version` drifted to 3.12.0 while the package was 5.3.0
 * (AEX-0.6).
 *
 * @returns {string} Semantic version, or 'unknown' if package.json is unreadable
 */
function resolveFrameworkVersion() {
  try {
    // .aexos-core/cli/commands/config/ → repo root
    const pkgPath = path.resolve(__dirname, '..', '..', '..', '..', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    return pkg.version || 'unknown';
  } catch {
    return 'unknown';
  }
}

/**
 * Lazy-load config modules (avoids failing at import time if yaml not installed)
 */
function loadModules() {
  const { resolveConfig, isLegacyMode, getConfigAtLevel, CONFIG_FILES } = require(configResolverPath);
  const { deepMerge } = require(mergeUtilsPath);
  const { lintEnvPatterns } = require(envInterpolatorPath);
  return { resolveConfig, isLegacyMode, getConfigAtLevel, CONFIG_FILES, deepMerge, lintEnvPatterns };
}

/**
 * Get project root (cwd)
 */
function getProjectRoot() {
  return process.cwd();
}

// ---------------------------------------------------------------------------
// aexos config show
// ---------------------------------------------------------------------------

function showAction(options) {
  const { resolveConfig, getConfigAtLevel } = loadModules();
  const root = getProjectRoot();

  try {
    if (options.level) {
      // Show a specific level (raw, no merge)
      const data = getConfigAtLevel(root, options.level, { appDir: options.app });
      if (!data) {
        console.error(`No config found for level: ${options.level}`);
        process.exit(1);
      }
      console.log(yaml.dump(data, { lineWidth: 120, noRefs: true }));
      return;
    }

    // Show full resolved config
    const result = resolveConfig(root, {
      appDir: options.app,
      debug: options.debug,
      skipCache: true,
    });

    if (options.debug && result.sources) {
      // Debug mode: show each value with source annotation
      printDebugConfig(result.config, result.sources);
    } else {
      console.log(yaml.dump(result.config, { lineWidth: 120, noRefs: true }));
    }

    // Print warnings
    if (result.warnings && result.warnings.length > 0) {
      console.error('');
      for (const w of result.warnings) {
        console.error(`  WARNING: ${w}`);
      }
    }
  } catch (error) {
    console.error(`Error: ${error.message}`);
    process.exit(1);
  }
}

/**
 * Print config with source annotations for --debug mode.
 */
function printDebugConfig(config, sources, prefix = '') {
  for (const [key, value] of Object.entries(config)) {
    const fullKey = prefix ? `${prefix}.${key}` : key;
    const source = sources[fullKey];
    const tag = source ? `[${source.level}: ${path.basename(source.file)}]` : '[unknown]';

    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
      console.log(`${fullKey}: ${tag}`);
      printDebugConfig(value, sources, fullKey);
    } else {
      const display = Array.isArray(value)
        ? JSON.stringify(value)
        : String(value);
      console.log(`  ${fullKey} = ${display}  ${tag}`);
    }
  }
}

// ---------------------------------------------------------------------------
// aexos config diff
// ---------------------------------------------------------------------------

function diffAction(options) {
  const { getConfigAtLevel } = loadModules();
  const root = getProjectRoot();

  if (!options.levels) {
    console.error('Error: --levels is required (e.g., --levels 1,2)');
    process.exit(1);
  }

  const [levelA, levelB] = options.levels.split(',').map(l => l.trim());
  if (!levelA || !levelB) {
    console.error('Error: --levels requires two levels separated by comma (e.g., --levels 1,2)');
    process.exit(1);
  }

  try {
    const configA = getConfigAtLevel(root, levelA, { appDir: options.app }) || {};
    const configB = getConfigAtLevel(root, levelB, { appDir: options.app }) || {};

    const diff = computeDiff(configA, configB, levelA, levelB);

    if (diff.length === 0) {
      console.log(`No differences between level ${levelA} and level ${levelB}.`);
      return;
    }

    console.log(`Differences: ${levelA} vs ${levelB}`);
    console.log('='.repeat(60));
    for (const entry of diff) {
      switch (entry.type) {
        case 'added':
          console.log(`  + ${entry.key}: ${formatValue(entry.valueB)}  [only in ${levelB}]`);
          break;
        case 'removed':
          console.log(`  - ${entry.key}: ${formatValue(entry.valueA)}  [only in ${levelA}]`);
          break;
        case 'changed':
          console.log(`  ~ ${entry.key}:`);
          console.log(`      ${levelA}: ${formatValue(entry.valueA)}`);
          console.log(`      ${levelB}: ${formatValue(entry.valueB)}`);
          break;
      }
    }
    console.log(`\nTotal: ${diff.length} difference(s)`);
  } catch (error) {
    console.error(`Error: ${error.message}`);
    process.exit(1);
  }
}

function computeDiff(objA, objB, _labelA, _labelB, prefix = '') {
  const diff = [];
  const allKeys = new Set([...Object.keys(objA), ...Object.keys(objB)]);

  for (const key of allKeys) {
    const fullKey = prefix ? `${prefix}.${key}` : key;
    const inA = key in objA;
    const inB = key in objB;

    if (!inA) {
      diff.push({ type: 'added', key: fullKey, valueB: objB[key] });
    } else if (!inB) {
      diff.push({ type: 'removed', key: fullKey, valueA: objA[key] });
    } else if (isObj(objA[key]) && isObj(objB[key])) {
      diff.push(...computeDiff(objA[key], objB[key], _labelA, _labelB, fullKey));
    } else if (JSON.stringify(objA[key]) !== JSON.stringify(objB[key])) {
      diff.push({ type: 'changed', key: fullKey, valueA: objA[key], valueB: objB[key] });
    }
  }

  return diff;
}

function isObj(v) { return v !== null && typeof v === 'object' && !Array.isArray(v); }

function formatValue(v) {
  if (v === null || v === undefined) return 'null';
  if (typeof v === 'object') return JSON.stringify(v);
  return String(v);
}

// ---------------------------------------------------------------------------
// aexos config migrate
// ---------------------------------------------------------------------------

function migrateAction(options) {
  const { isLegacyMode, resolveConfig, CONFIG_FILES } = loadModules();
  const root = getProjectRoot();
  let preMigrationState = null;

  try {
    if (!isLegacyMode(root)) {
      console.log('Project already uses layered config (framework-config.yaml found).');
      console.log('Nothing to migrate.');
      return;
    }

    const legacyPath = path.join(root, CONFIG_FILES.legacy);
    const legacyContent = fs.readFileSync(legacyPath, 'utf8');
    const legacyConfig = yaml.load(legacyContent);
    validateLegacyConfigRoot(legacyConfig);
    const preMigrationResolved = resolveConfig(root, { skipCache: true }).config;

    // Section-to-level mapping per ADR-PRO-002
    const l1Sections = splitL1(legacyConfig);
    const l2Sections = splitL2(legacyConfig);
    const privateNormalized = mergeCompatibilityObjects(
      splitL1(legacyConfig, { allowAdditionalProperties: true }),
      splitL2(legacyConfig, { allowAdditionalProperties: true }),
    );
    // L4 is gitignored and has the highest project-local precedence. Retain
    // the complete legacy payload here as a compatibility layer, then let the
    // normalized L1/L2 keys coexist for new consumers. This keeps unknown or
    // private extensions out of the team-shared project config.
    const l4Sections = withMigrationCompatibility(mergeCompatibilityObjects(
      privateNormalized,
      mergeCompatibilityObjects(splitL4(legacyConfig), legacyConfig),
    ));

    if (options.dryRun) {
      console.log('=== DRY RUN — no files will be written ===\n');
      console.log(`framework-config.yaml (L1): ${summarizeConfigKeys(l1Sections)}`);
      console.log(`project-config.yaml (L2): ${summarizeConfigKeys(l2Sections)}`);
      console.log(
        'local-config.yaml (L4): complete legacy compatibility payload; '
        + `${Object.keys(legacyConfig).length} top-level key(s); names and values omitted`,
      );
      return;
    }

    // Check existing split files
    const fwPath = path.join(root, CONFIG_FILES.framework);
    const projPath = path.join(root, CONFIG_FILES.project);
    const localPath = path.join(root, CONFIG_FILES.local);
    const backupPath = legacyPath + '.backup';
    const gitignorePath = path.join(root, '.gitignore');
    const nestedGitignorePath = path.join(root, '.aexos-core', '.gitignore');

    if (!options.force) {
      const exists = [fwPath, projPath, localPath].filter(p => fs.existsSync(p));
      if (exists.length > 0) {
        console.error('Split config files already exist:');
        exists.forEach((p) => { console.error(`  ${p}`); });
        console.error('Use --force to overwrite.');
        process.exit(1);
      }
    }

    preMigrationState = captureFileState([
      fwPath,
      projPath,
      localPath,
      backupPath,
      gitignorePath,
      nestedGitignorePath,
    ]);

    // Create backup
    fs.copyFileSync(legacyPath, backupPath);
    console.log(`Backup created: ${backupPath}`);

    // Establish and verify privacy before writing the complete legacy payload,
    // which can contain secrets and machine-specific extensions.
    ensureGitignore(root, '.aexos-core/local-config.yaml');

    // Write split files
    const header1 = '# AEXOS Framework Configuration (Level 1)\n# DO NOT EDIT — Part of the AEXOS framework.\n# Override in project-config.yaml or local-config.yaml.\n\n';
    fs.writeFileSync(fwPath, header1 + yaml.dump(l1Sections, { lineWidth: 120 }));
    console.log(`Created: ${CONFIG_FILES.framework}`);

    const header2 = '# AEXOS Project Configuration (Level 2)\n# Project-specific settings shared across the team.\n\n';
    fs.writeFileSync(projPath, header2 + yaml.dump(l2Sections, { lineWidth: 120 }));
    console.log(`Created: ${CONFIG_FILES.project}`);

    const header4 = '# AEXOS Local Configuration (Level 4)\n# Machine-specific overrides. DO NOT commit to git.\n\n';
    fs.writeFileSync(localPath, header4 + yaml.dump(l4Sections, { lineWidth: 120 }));
    console.log(`Created: ${CONFIG_FILES.local}`);

    if (isGitTracked(root, '.aexos-core/local-config.yaml')) {
      console.error(
        'WARNING: .aexos-core/local-config.yaml is already tracked by git; '
        + 'the new ignore rule cannot untrack it.',
      );
    }

    // Validate both contracts independently: the raw compatibility payload
    // must retain placeholders verbatim, while resolved behavior must match
    // the legacy resolver after interpolation.
    const rawLocalConfig = yaml.load(fs.readFileSync(localPath, 'utf8')) || {};
    const rawLosses = findRetainedValueLosses(legacyConfig, rawLocalConfig);
    const resolved = resolveConfig(root, { skipCache: true });
    const resolvedLosses = findRetainedValueLosses(preMigrationResolved, resolved.config);
    if (rawLosses.length > 0 || resolvedLosses.length > 0) {
      const losses = [
        ...rawLosses.map(key => `raw:${key}`),
        ...resolvedLosses.map(key => `resolved:${key}`),
      ];
      throw new Error(
        `Migration validation lost ${losses.length} legacy value(s): ${losses.join(', ')}`,
      );
    }

    console.log('\nValidation: PASS — All legacy keys and values remain resolved.');

    console.log('\nMigration complete. Original preserved at core-config.yaml.backup');
  } catch (error) {
    if (preMigrationState) {
      const rollbackErrors = restoreFileState(preMigrationState);
      if (rollbackErrors.length === 0) {
        console.error('Migration rolled back: restored pre-migration files.');
      } else {
        console.error(
          `Migration rollback incomplete (${rollbackErrors.length} error(s)): `
          + rollbackErrors.join('; '),
        );
      }
    }
    console.error(`Migration error: ${error.message}`);
    process.exit(1);
  }
}

function captureFileState(files) {
  return files.map(file => ({
    file,
    existed: fs.existsSync(file),
    content: fs.existsSync(file) ? fs.readFileSync(file) : null,
  }));
}

function restoreFileState(snapshot) {
  const errors = [];
  for (const entry of snapshot) {
    try {
      if (entry.existed) {
        fs.mkdirSync(path.dirname(entry.file), { recursive: true });
        fs.writeFileSync(entry.file, entry.content);
      } else if (fs.existsSync(entry.file)) {
        fs.rmSync(entry.file, { force: true });
      }
    } catch (error) {
      errors.push(`${entry.file}: ${error.message}`);
    }
  }
  return errors;
}

function isPlainRecord(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return false;
  const prototype = Object.getPrototypeOf(value);
  return prototype === Object.prototype || prototype === null;
}

function validateLegacyConfigRoot(config) {
  if (!isPlainRecord(config)) {
    throw new Error('Legacy core-config.yaml must contain a top-level mapping/object');
  }
  const visit = (value, prefix = '') => {
    if (Array.isArray(value)) {
      value.forEach((child, index) => visit(child, `${prefix}[${index}]`));
      return;
    }
    if (!isPlainRecord(value)) return;
    for (const [key, child] of Object.entries(value)) {
      const keyPath = prefix ? `${prefix}.${key}` : key;
      if (UNSAFE_CONFIG_KEYS.has(key)) {
        throw new Error(`Legacy core-config.yaml contains unsafe key: ${keyPath}`);
      }
      visit(child, keyPath);
    }
  };
  visit(config);
}

function summarizeConfigKeys(config) {
  const keys = Object.keys(config);
  return keys.length > 0 ? `${keys.length} section(s): ${keys.join(', ')}` : 'no normalized sections';
}

const OMIT_SCHEMA_VALUE = Symbol('omit-schema-value');

function projectSchemaValue(value, schema, options = {}) {
  if (!schema || Object.keys(schema).length === 0) return value;
  if (Array.isArray(schema.oneOf)) {
    for (const candidate of schema.oneOf) {
      const projected = projectSchemaValue(value, candidate, options);
      if (projected !== OMIT_SCHEMA_VALUE) return projected;
    }
    return OMIT_SCHEMA_VALUE;
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'const') && value !== schema.const) {
    return OMIT_SCHEMA_VALUE;
  }
  if (Array.isArray(schema.enum) && !schema.enum.includes(value)) return OMIT_SCHEMA_VALUE;
  if (schema.type === 'null') return value === null ? null : OMIT_SCHEMA_VALUE;
  if (schema.type === 'string') return typeof value === 'string' ? value : OMIT_SCHEMA_VALUE;
  if (schema.type === 'boolean') return typeof value === 'boolean' ? value : OMIT_SCHEMA_VALUE;
  if (schema.type === 'integer') return Number.isInteger(value) ? value : OMIT_SCHEMA_VALUE;
  if (schema.type === 'number') {
    return typeof value === 'number' && Number.isFinite(value) ? value : OMIT_SCHEMA_VALUE;
  }
  if (schema.type === 'array') {
    if (!Array.isArray(value)) return OMIT_SCHEMA_VALUE;
    if (!schema.items) return [...value];
    const projected = value.map(item => projectSchemaValue(item, schema.items, options));
    return projected.includes(OMIT_SCHEMA_VALUE) ? OMIT_SCHEMA_VALUE : projected;
  }
  if (schema.type !== 'object') return value;
  if (!isPlainRecord(value)) return OMIT_SCHEMA_VALUE;
  const result = {};
  const properties = schema.properties || {};
  for (const [key, child] of Object.entries(value)) {
    const normalizedKey = key.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`);
    const outputKey = Object.prototype.hasOwnProperty.call(properties, key)
      ? key
      : Object.prototype.hasOwnProperty.call(properties, normalizedKey)
        ? normalizedKey
        : key;
    let childSchema = properties[outputKey];
    if (!childSchema) {
      if (!options.allowAdditionalProperties) continue;
      if (isPlainRecord(schema.additionalProperties)) childSchema = schema.additionalProperties;
      else if (schema.additionalProperties === true) childSchema = {};
      else continue;
    }
    const projected = projectSchemaValue(child, childSchema, options);
    if (projected === OMIT_SCHEMA_VALUE) continue;
    Object.defineProperty(result, outputKey, {
      value: projected,
      enumerable: true,
      configurable: true,
      writable: true,
    });
  }
  return result;
}

function assignProjected(target, key, value, schema, options = {}) {
  const projected = projectSchemaValue(value, schema, options);
  if (projected !== OMIT_SCHEMA_VALUE) target[key] = projected;
}

function mergeCompatibilityObjects(base, override) {
  const result = { ...base };
  for (const [key, value] of Object.entries(override)) {
    result[key] = isPlainRecord(value) && isPlainRecord(result[key])
      ? mergeCompatibilityObjects(result[key], value)
      : value;
  }
  return result;
}

/**
 * Find original legacy paths whose resolved value changed or disappeared.
 * Extra normalized paths are intentionally allowed.
 */
function findRetainedValueLosses(expected, actual, prefix = '') {
  const losses = [];

  for (const [key, expectedValue] of Object.entries(expected)) {
    const keyPath = prefix ? `${prefix}.${key}` : key;
    if (!actual || !Object.prototype.hasOwnProperty.call(actual, key)) {
      losses.push(keyPath);
      continue;
    }

    const actualValue = actual[key];
    if (isPlainRecord(expectedValue)) {
      if (!isPlainRecord(actualValue)) {
        losses.push(keyPath);
      } else {
        losses.push(...findRetainedValueLosses(expectedValue, actualValue, keyPath));
      }
      continue;
    }

    if (JSON.stringify(expectedValue) !== JSON.stringify(actualValue)) {
      losses.push(keyPath);
    }
  }

  return losses;
}

/**
 * Extract L1 (Framework) sections from monolithic config.
 */
function splitL1(config, projectionOptions = {}) {
  const l1 = {};

  // metadata (framework portion)
  if (config.markdownExploder !== undefined) {
    assignProjected(
      l1,
      'markdownExploder',
      config.markdownExploder,
      frameworkConfigSchema.properties.markdownExploder,
      projectionOptions,
    );
  }
  l1.metadata = { name: 'AEXOS (Cyryx)', framework_version: resolveFrameworkVersion() };

  // resource_locations (Section 3)
  const resourceSchema = frameworkConfigSchema.properties.resource_locations;
  const resources = {};
  if (config.toolsLocation) {
    assignProjected(
      resources,
      'tools_dir',
      config.toolsLocation,
      resourceSchema.properties.tools_dir,
      projectionOptions,
    );
  }
  if (config.scriptsLocation) {
    assignProjected(
      resources,
      'scripts',
      config.scriptsLocation,
      resourceSchema.properties.scripts,
      projectionOptions,
    );
  }
  if (config.dataLocation) {
    assignProjected(
      resources,
      'data_dir',
      config.dataLocation,
      resourceSchema.properties.data_dir,
      projectionOptions,
    );
  }
  if (config.elicitationLocation) {
    assignProjected(
      resources,
      'elicitation_dir',
      config.elicitationLocation,
      resourceSchema.properties.elicitation_dir,
      projectionOptions,
    );
  }
  if (config.squadsTemplateLocation) {
    assignProjected(
      resources,
      'squads_template_dir',
      config.squadsTemplateLocation,
      resourceSchema.properties.squads_template_dir,
      projectionOptions,
    );
  }
  if (config.mindsLocation) {
    assignProjected(
      resources,
      'minds_dir',
      config.mindsLocation,
      resourceSchema.properties.minds_dir,
      projectionOptions,
    );
  }
  if (Object.keys(resources).length > 0) l1.resource_locations = resources;

  // performance_defaults (Section 6 - defaults)
  if (config.lazyLoading) {
    l1.performance_defaults = {};
    assignProjected(
      l1.performance_defaults,
      'lazy_loading',
      config.lazyLoading,
      frameworkConfigSchema.properties.performance_defaults.properties.lazy_loading,
      projectionOptions,
    );
  }
  if (config.git) {
    l1.performance_defaults = l1.performance_defaults || {};
    assignProjected(
      l1.performance_defaults,
      'git',
      config.git,
      frameworkConfigSchema.properties.performance_defaults.properties.git,
      projectionOptions,
    );
  }
  if (l1.performance_defaults && Object.keys(l1.performance_defaults).length === 0) {
    delete l1.performance_defaults;
  }

  // utility_scripts_registry (Section 11)
  if (config.utils) {
    assignProjected(
      l1,
      'utility_scripts_registry',
      config.utils,
      frameworkConfigSchema.properties.utility_scripts_registry,
      projectionOptions,
    );
  }

  // ide_sync_system (Section 12)
  if (config.ideSync) {
    assignProjected(
      l1,
      'ide_sync_system',
      config.ideSync,
      frameworkConfigSchema.properties.ide_sync_system,
      projectionOptions,
    );
  }

  return l1;
}

/**
 * Extract L2 (Project) sections from monolithic config.
 */
function splitL2(config, projectionOptions = {}) {
  const l2 = {};

  // project metadata (Section 1 - project portion)
  if (config.project) {
    assignProjected(
      l2,
      'project',
      config.project,
      projectConfigSchema.properties.project,
      projectionOptions,
    );
  }

  // documentation_paths (Section 2)
  const docs = {};
  if (config.qa) docs.qa_dir = config.qa.qaLocation;
  if (config.prd) {
    docs.prd_file = config.prd.prdFile;
    docs.prd_version = config.prd.prdVersion;
    docs.prd_sharded = config.prd.prdSharded;
    docs.prd_sharded_location = config.prd.prdShardedLocation;
  }
  if (config.architecture) {
    docs.architecture_file = config.architecture.architectureFile;
    docs.architecture_version = config.architecture.architectureVersion;
    docs.architecture_sharded = config.architecture.architectureSharded;
    docs.architecture_sharded_location = config.architecture.architectureShardedLocation;
  }
  if (config.devStoryLocation) docs.stories_dir = config.devStoryLocation;
  if (config.devDebugLog) docs.dev_debug_log = config.devDebugLog;
  if (config.slashPrefix) docs.slash_prefix = config.slashPrefix;
  if (config.customTechnicalDocuments !== undefined) docs.custom_technical_documents = config.customTechnicalDocuments;
  if (config.devLoadAlwaysFiles) docs.dev_load_always_files = config.devLoadAlwaysFiles;
  if (config.devLoadAlwaysFilesFallback) docs.dev_load_always_files_fallback = config.devLoadAlwaysFilesFallback;
  if (Object.keys(docs).length > 0) {
    assignProjected(
      l2,
      'documentation_paths',
      docs,
      projectConfigSchema.properties.documentation_paths,
      projectionOptions,
    );
  }

  // github_integration (Section 8)
  if (config.github) {
    assignProjected(
      l2,
      'github_integration',
      config.github,
      projectConfigSchema.properties.github_integration,
      projectionOptions,
    );
  }

  // coderabbit_integration (Section 9 - non-secret portion)
  if (config.coderabbit_integration) {
    const cr = { ...config.coderabbit_integration };
    // Remove machine-specific keys (they go to L4)
    delete cr.installation_mode;
    delete cr.wsl_config;
    delete cr.commands;
    assignProjected(
      l2,
      'coderabbit_integration',
      cr,
      projectConfigSchema.properties.coderabbit_integration,
      projectionOptions,
    );
  }

  // squads (Section 10)
  if (config.squads) {
    assignProjected(
      l2,
      'squads',
      config.squads,
      projectConfigSchema.properties.squads,
      projectionOptions,
    );
  }

  // logging (Section 7)
  const logging = {};
  if (config.decisionLogging) {
    assignProjected(
      logging,
      'decision_logging',
      config.decisionLogging,
      projectConfigSchema.properties.logging.properties.decision_logging,
      projectionOptions,
    );
  }
  if (config.projectStatus) {
    assignProjected(
      logging,
      'project_status',
      config.projectStatus,
      projectConfigSchema.properties.logging.properties.project_status,
      projectionOptions,
    );
  }
  if (config.agentIdentity) {
    assignProjected(
      logging,
      'agent_identity',
      config.agentIdentity,
      projectConfigSchema.properties.logging.properties.agent_identity,
      projectionOptions,
    );
  }
  if (Object.keys(logging).length > 0) l2.logging = logging;

  // pvMindContext
  if (config.pvMindContext) {
    assignProjected(
      l2,
      'pv_mind_context',
      config.pvMindContext,
      projectConfigSchema.properties.pv_mind_context,
      projectionOptions,
    );
  }

  // storyBacklog
  if (config.storyBacklog) {
    assignProjected(
      l2,
      'story_backlog',
      config.storyBacklog,
      projectConfigSchema.properties.story_backlog,
      projectionOptions,
    );
  }

  // auto_claude (Section 13)
  if (config.autoClaude) {
    assignProjected(
      l2,
      'auto_claude',
      config.autoClaude,
      projectConfigSchema.properties.auto_claude,
      projectionOptions,
    );
  }

  return l2;
}

/**
 * Extract L4 (Local) sections from monolithic config.
 */
function splitL4(config) {
  const l4 = {};

  // ide_configuration (Section 4)
  if (config.ide) l4.ide = config.ide;

  // mcp_configuration (Section 5)
  if (config.mcp) l4.mcp = config.mcp;

  // coderabbit secret and machine config (Section 9 - machine portion)
  if (config.coderabbit_integration) {
    const cr = config.coderabbit_integration;
    const l4cr = {};
    if (cr.installation_mode) l4cr.installation_mode = cr.installation_mode;
    if (cr.wsl_config) l4cr.wsl_config = cr.wsl_config;
    if (cr.commands) l4cr.commands = cr.commands;
    if (Object.keys(l4cr).length > 0) l4.coderabbit_integration = l4cr;
  }

  return l4;
}

/**
 * Ensure a path is in .gitignore
 */
function ensureGitignore(root, entry) {
  const gitignorePath = path.join(root, '.gitignore');
  const anchoredEntry = `/${entry.replace(/^\/+/, '')}`;
  appendFinalIgnoreRule(gitignorePath, anchoredEntry, true);
  console.log(`.gitignore updated: added ${anchoredEntry}`);

  const nestedGitignorePath = path.join(root, '.aexos-core', '.gitignore');
  if (fs.existsSync(nestedGitignorePath)) {
    appendFinalIgnoreRule(nestedGitignorePath, 'local-config.yaml');
  }

  if (!isInsideGitWorkTree(root)) return;

  let check = checkGitIgnored(root, entry);
  if (check.status === 1) {
    // A deeper rule can override the root rule. Add the narrowest possible
    // positive rule at that level, then require Git itself to confirm it.
    appendFinalIgnoreRule(nestedGitignorePath, 'local-config.yaml');
    check = checkGitIgnored(root, entry);
  }

  if (check.status !== 0) {
    const detail = check.error?.message || check.stderr?.trim() || `exit ${check.status}`;
    throw new Error(`Cannot verify local-config privacy with git check-ignore: ${detail}`);
  }
}

function appendFinalIgnoreRule(file, rule, includeComment = false) {
  const content = fs.existsSync(file) ? fs.readFileSync(file, 'utf8') : '';
  const separator = content.length > 0 && !content.endsWith('\n') ? '\n' : '';
  const comment = includeComment ? '# Local config (machine-specific secrets)\n' : '';
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.appendFileSync(file, `${separator}${comment}${rule}\n`);
}

function checkGitIgnored(root, entry) {
  return spawnSync(
    'git',
    ['-C', root, 'check-ignore', '--no-index', '--quiet', '--', entry],
    { encoding: 'utf8' },
  );
}

function isGitTracked(root, entry) {
  if (!isInsideGitWorkTree(root)) return false;
  const result = spawnSync('git', ['-C', root, 'ls-files', '--error-unmatch', '--', entry], {
    encoding: 'utf8',
    stdio: 'ignore',
  });
  if (result.status === 0) return true;
  if (result.status === 1) return false;
  throw new Error(
    `Cannot verify whether local-config is tracked: ${result.error?.message || `exit ${result.status}`}`,
  );
}

function hasAncestorGitMarker(root) {
  let current = path.resolve(root);
  while (true) {
    if (fs.existsSync(path.join(current, '.git'))) return true;
    const parent = path.dirname(current);
    if (parent === current) return false;
    current = parent;
  }
}

function isInsideGitWorkTree(root) {
  const result = spawnSync(
    'git',
    ['-C', root, 'rev-parse', '--is-inside-work-tree'],
    { encoding: 'utf8' },
  );
  if (result.status === 0) return result.stdout.trim() === 'true';
  if (!result.error && result.status === 128 && !hasAncestorGitMarker(root)) return false;
  if (result.error && !hasAncestorGitMarker(root)) return false;
  const detail = result.error?.message || result.stderr?.trim() || `exit ${result.status}`;
  throw new Error(`Cannot determine Git worktree for local-config privacy: ${detail}`);
}

// ---------------------------------------------------------------------------
// aexos config validate
// ---------------------------------------------------------------------------

function validateAction(options) {
  const { getConfigAtLevel, CONFIG_FILES } = loadModules();
  const { lintEnvPatterns } = require(envInterpolatorPath);
  const root = getProjectRoot();

  try {
    const issues = [];

    if (options.level) {
      // Validate specific level
      const data = getConfigAtLevel(root, options.level);
      if (!data) {
        console.error(`No config found for level: ${options.level}`);
        process.exit(1);
      }
      validateYamlSyntax(root, options.level, issues);
    } else {
      // Validate all levels
      const levels = ['framework', 'project', 'pro', 'local'];
      for (const level of levels) {
        validateYamlSyntax(root, level, issues);
      }

      // Check env patterns in L1/L2
      const l1 = getConfigAtLevel(root, 'framework');
      if (l1) {
        const l1Lint = lintEnvPatterns(l1, CONFIG_FILES.framework);
        issues.push(...l1Lint.map(w => `[LINT WARNING] ${w}`));
      }

      const l2 = getConfigAtLevel(root, 'project');
      if (l2) {
        const l2Lint = lintEnvPatterns(l2, CONFIG_FILES.project);
        issues.push(...l2Lint.map(w => `[LINT WARNING] ${w}`));
      }
    }

    if (issues.length === 0) {
      console.log('Config validation: PASS');
    } else {
      console.log(`Config validation: ${issues.length} issue(s) found`);
      for (const issue of issues) {
        console.log(`  ${issue}`);
      }
      process.exit(1);
    }
  } catch (error) {
    console.error(`Validation error: ${error.message}`);
    process.exit(1);
  }
}

function validateYamlSyntax(root, level, issues) {
  const { CONFIG_FILES } = loadModules();
  const fileMap = {
    framework: CONFIG_FILES.framework,
    project: CONFIG_FILES.project,
    pro: CONFIG_FILES.pro,
    local: CONFIG_FILES.local,
  };

  const relativePath = fileMap[level];
  if (!relativePath) return;

  const fullPath = path.join(root, relativePath);
  if (!fs.existsSync(fullPath)) return; // Optional files are OK to be missing

  try {
    const content = fs.readFileSync(fullPath, 'utf8');
    yaml.load(content);
    console.log(`  ${level}: OK (${relativePath})`);
  } catch (error) {
    issues.push(`[YAML ERROR] ${relativePath}: ${error.message}`);
  }
}

// ---------------------------------------------------------------------------
// aexos config init-local
// ---------------------------------------------------------------------------

function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, '.aexos-core', 'local-config.yaml.template');
  const targetPath = path.join(root, '.aexos-core', 'local-config.yaml');

  if (!fs.existsSync(templatePath)) {
    console.error('Template not found: .aexos-core/local-config.yaml.template');
    process.exit(1);
  }

  if (fs.existsSync(targetPath)) {
    console.error('local-config.yaml already exists. Remove it first or edit directly.');
    process.exit(1);
  }

  fs.copyFileSync(templatePath, targetPath);
  console.log('Created: .aexos-core/local-config.yaml (from template)');
  console.log('Edit this file to customize your machine-specific settings.');

  // Ensure gitignore
  ensureGitignore(root, '.aexos-core/local-config.yaml');
}

// ---------------------------------------------------------------------------
// Command builder
// ---------------------------------------------------------------------------

/**
 * Create the `aexos config` command with all subcommands.
 * @returns {Command}
 */
function createConfigCommand() {
  const configCmd = new Command('config')
    .description('Manage layered configuration (ADR-PRO-002)');

  // aexos config show
  configCmd
    .command('show')
    .description('Show resolved configuration')
    .option('-l, --level <level>', 'Show specific level (1, 2, pro, 3, 4)')
    .option('-a, --app <name>', 'Include app-specific config (L3)')
    .option('-d, --debug', 'Show source annotations for each value')
    .action(showAction);

  // aexos config diff
  configCmd
    .command('diff')
    .description('Compare configuration between two levels')
    .requiredOption('--levels <a,b>', 'Two levels to compare (e.g., 1,2)')
    .option('-a, --app <name>', 'Include app-specific context')
    .action(diffAction);

  // aexos config migrate
  configCmd
    .command('migrate')
    .description('Migrate monolithic core-config.yaml to layered files')
    .option('--dry-run', 'Preview without writing files')
    .option('--force', 'Overwrite existing split files')
    .action(migrateAction);

  // aexos config validate
  configCmd
    .command('validate')
    .description('Validate YAML syntax and lint config files')
    .option('-l, --level <level>', 'Validate specific level only')
    .action(validateAction);

  // aexos config init-local
  configCmd
    .command('init-local')
    .description('Create local-config.yaml from template')
    .action(initLocalAction);

  return configCmd;
}

module.exports = {
  createConfigCommand,
};
