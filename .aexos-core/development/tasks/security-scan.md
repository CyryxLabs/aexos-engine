...
```yaml
**Scripts:**
- host-guided-security-scan:
    description: Run the substantive scan steps with the project's configured security tools and retain their real reports.
    unavailable_reference: .aexos-core/scripts/security-scan.js
    failure_contract: Missing scanners, configuration, target access, or report evidence blocks completion.
```
...
const utils_registry = config.utils?.registry || config['utils.registry'] || '.aexos-core/utils';
```
...
**Report Generator**: @qa (Argus - Test Architect)
```
