...
```javascript
const { isCodeIntelAvailable } = require(
  require('path').resolve(process.cwd(), '.aexos-core/core/code-intel'),
);
const { getImplementationImpact } = require(
  require('path').resolve(process.cwd(), '.aexos-core/core/code-intel/helpers/planning-helper.js'),
);
