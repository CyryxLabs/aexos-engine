...
- host-guided-execution:
    description: Active agent follows this task document; no standalone execute-task wrapper is distributed.
    unavailable_reference: .aexos-core/scripts/execute-task.js
    failure_contract: Propagate tool and validation failures; do not infer completion.

- documentation-synchronizer.js:
    description: Core documentation synchronization engine
    language: JavaScript
    location: .aexos-core/infrastructure/scripts/documentation-synchronizer.js
```
...
*sync-documentation --component aexos-core/scripts/pattern-learner.js
...
class SyncDocumentationTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📚 AEXOS Documentation Synchronization'));
