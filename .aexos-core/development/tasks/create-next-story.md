...
- **Tool:** component-generator
  ...
  - **Source:** .aexos-core/infrastructure/scripts/component-generator.js
...
- **Execution:** Host-guided component creation using the substantive steps and, where its supported component types apply, `.aexos-core/infrastructure/scripts/component-generator.js`.
  - **Unavailable wrapper:** `.aexos-core/scripts/create-component.js` is not distributed.
  - **Requirement:** Do not claim automated generation unless the real generator accepts the requested component type and its transaction/validation result succeeds.
...
- Load `aexos-core/core-config.yaml` from the project root
- If the file does not exist, HALT and inform the user: "core-config.yaml not found. This file is required for story creation. You can either: 1) Copy it from GITHUB aexos-core/core-config.yaml and configure it for your project OR 2) Run the AEXOS installer against your project to upgrade and add the file automatically. Please add and configure core-config.yaml before proceeding."
...
- **Check code intelligence availability:** Call `isCodeIntelAvailable()` from `.aexos-core/core/code-intel`
- **If available:**
  - Call `detectDuplicateStory(storyDescription)` from `.aexos-core/core/code-intel/helpers/story-helper.js`
  ...
  - Call `suggestRelevantFiles(storyDescription)` from `.aexos-core/core/code-intel/helpers/story-helper.js`
...
- Execute `.aexos-core/development/tasks/execute-checklist` `.aexos-core/product/checklists/story-draft-checklist`
- Provide summary to user including:
  ...
  - Checklist Results
  - Next steps: For Complex stories, suggest the user carefully review the story draft and also optionally have the PO run the task `.aexos-core/development/tasks/validate-next-story`
