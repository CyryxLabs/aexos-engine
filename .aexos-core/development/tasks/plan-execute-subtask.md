...
autoClaude:
  ...
  inputs:
    ...
    - name: projectContext
      type: file
      path: .aexos/project-context.yaml
      required: false
...
step_11:
  ...
  actions:
    ...
    - action: update_dashboard_status
      script: |
        const { PlanTracker } = require(require('path').resolve(process.cwd(), '.aexos-core/infrastructure/scripts/plan-tracker.js'));
...
recovery:
  ...
  attempt_tracking:
    storage: .aexos/recovery/{storyId}/{subtaskId}.json
    schema:
      subtaskId: string
      attempts: array
