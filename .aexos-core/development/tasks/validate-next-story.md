...
- **Validation binding:** active QA host and project test/schema tools
  - **Purpose:** Apply the task's declared rules and produce evidence from the real target.
  - **Availability:** `.aexos-core/utils/validation-engine.js` is not distributed; required project validators and schemas must be present and configured.
...
- **Execution:** Host-guided with the project's actual test, schema, and review tools.
  - **Unavailable wrapper:** `.aexos-core/scripts/run-validation.js` is not distributed.
  - **Requirement:** Retain this task's acceptance criteria and fail when required validators, target evidence, or dependencies are unavailable.
...
- Load `.aexos-core/core-config.yaml`
...
- Identify and load the following inputs:
  ...
  - **Story template**: `.aexos-core/product/templates/story-tmpl.yaml` for completeness validation

### 1. Template Completeness Validation

- Load `.aexos-core/product/templates/story-tmpl.yaml` and extract all section headings from the template
...
**PRD Reference:** AEXOS v2.0 "Projeto Bob" - Section 5 (Dynamic Executor Assignment)
...
- **If available:**
  - Call `validateNoDuplicates(storyDescription)` from `.aexos-core/core/code-intel/helpers/story-helper.js`
