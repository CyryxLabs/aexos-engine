...
- Repository context detected (run `aexos init` if needed)
...
if (!context) {
  ...
  console.error('Run "aexos init" to configure installation mode');
  process.exit(1);
}
...
```javascript
const { assessPrePushImpact, classifyRiskLevel } = require(require('path').resolve(process.cwd(), '.aexos-core/core/code-intel/helpers/devops-helper.js'));
