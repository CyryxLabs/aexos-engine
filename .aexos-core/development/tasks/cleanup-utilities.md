...
const configPath = path.join(__dirname, '../../.aexos-core/core-config.yaml');
...
**1.1 Create Backup**
```bash
# Create timestamped backup of the active infrastructure scripts directory
mkdir -p .backups
cp -r .aexos-core/infrastructure/scripts ".backups/infrastructure-scripts.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .aexos-core/development/agents .aexos-core/development/tasks .aexos-core/development/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .aexos-core/development/agents/ .aexos-core/development/tasks/ .aexos-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .aexos-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .aexos-core/utils-archive
```

**3.2 Create Archive README**

Create `.aexos-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back to the original relative path recorded in the manifest**:
   ```bash
   source=.aexos-core/infrastructure/scripts/utility-name.js
   archived=.aexos-core/utils-archive/infrastructure/scripts/utility-name.js
...
4. **Test thoroughly**:
   ```bash
   node .aexos-core/infrastructure/scripts/test-utilities.js
   ```
...
**Manifest-Guided Rollback** (restore owned files without deleting unrelated content):
```bash
while IFS=$'\t' read -r source archived; do
  mkdir -p "$(dirname "$source")"
  cp "$archived" "$source"
done < .aexos-core/utils-archive/ARCHIVE-MANIFEST.tsv
```

**Selective Restoration** (use the exact source/archive pair recorded in `ARCHIVE-MANIFEST.tsv`):
```bash
source=.aexos-core/infrastructure/scripts/specific-utility.js
archived=.aexos-core/utils-archive/infrastructure/scripts/specific-utility.js
mkdir -p "$(dirname "$source")"
cp "$archived" "$source"
```
...
- **Execution binding:** active agent host
  - **Purpose:** Follow this task's substantive steps, checkpoints, validations, and error handling with the host's available tools.
  - **Availability:** No standalone `.aexos-core/core/task-runner.js` or `.aexos-core/utils/logger.js` ships with AEXOS; these inherited references are unsupported prerequisites.
  - **Failure contract:** Surface tool and validation failures to the caller; do not report completion from an unavailable runner or logger.
...
- **Execution:** Host-guided; the active agent executes the instructions in this task document.
  - **Unavailable wrapper:** `.aexos-core/scripts/execute-task.js` is not distributed.
  - **Requirement:** Use only tools concretely required by the substantive steps and preserve all preconditions, outputs, checkpoints, validations, and recovery behavior.
...
For each utility in the deprecated list with 0 dependencies, preserve its original path in both the archive tree and the manifest:

```bash
archive_root=.aexos-core/utils-archive
manifest="$archive_root/ARCHIVE-MANIFEST.tsv"
source=.aexos-core/infrastructure/scripts/utility-name.js
relative="${source#.aexos-core/}"
archived="$archive_root/$relative"
mkdir -p "$(dirname "$archived")"
git mv "$source" "$archived"
printf '%s\t%s\n' "$source" "$archived" >> "$manifest"
```
...
git mv .aexos-core/infrastructure/scripts/aexos-validator.test.js tests/utils/
```

**4.3 Update Archive README**

Add the complete list of archived files to ARCHIVE-README.md. Each entry must retain the original source path and archived path from `ARCHIVE-MANIFEST.tsv`:

```markdown
## Archived Utilities

### Category A: Duplicate/Redundant (9 files)
1. `.aexos-core/infrastructure/scripts/aexos-validator-fixed.js` → `.aexos-core/utils-archive/infrastructure/scripts/aexos-validator-fixed.js` - Duplicate of aexos-validator.js
2. `.aexos-core/infrastructure/scripts/aexos-validator-refactored.js` → `.aexos-core/utils-archive/infrastructure/scripts/aexos-validator-refactored.js` - Duplicate of aexos-validator.js
...
...
Update utility count in `.aexos-core/core-config.yaml`:
...
framework:
  entities:
    utils:
      ...
      location: .aexos-core/infrastructure/scripts/
```
...
```bash
node .aexos-core/infrastructure/scripts/aexos-validator.js
```
...
while IFS= read -r util; do
  ...
  refs=$(grep -r "require.*$name" .aexos-core/development/agents .aexos-core/development/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
    echo "⚠️ Found $refs references to archived utility: $name"
  fi
done < <(find .aexos-core/utils-archive -type f -name '*.js' -print)
```
...
**Manifest-Guided Restoration** (restore only files owned by this cleanup; do not recursively delete the destination):
```bash
while IFS=$'\t' read -r source archived; do
  mkdir -p "$(dirname "$source")"
  cp "$archived" "$source"
done < .aexos-core/utils-archive/ARCHIVE-MANIFEST.tsv
git checkout .aexos-core/core-config.yaml
```
...
1. `.aexos-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.aexos-core/utils-archive/ARCHIVE-README.md` and `ARCHIVE-MANIFEST.tsv` - Archive documentation with exact original/archive paths
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.aexos-core/core-config.yaml` - Corrected utility count
...
- ✅ Framework validation (aexos-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
