...
} = require('./.aexos-core/development/scripts/decision-recorder.js');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.aexos-core/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.aexos-core/core/code-intel/helpers/dev-helper.js`
...
7. Execute `.aexos-core/product/checklists/story-dod-checklist.md`
...
- `.aexos-core/product/checklists/story-dod-checklist.md` - Definition of Done checklist
