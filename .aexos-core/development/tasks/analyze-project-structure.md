...
**Purpose:** Analyze an existing AEXOS project to understand its structure, services, patterns, and provide recommendations for implementing new features. This is Phase 1 of the Incremental Feature Workflow.
...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .aexos-core/
...
```yaml
pre-conditions:
  - [ ] Project has .aexos-core/ directory
    type: pre-condition
    blocker: true
    validation: |
      Check .aexos-core/ directory exists in project root
    error_message: "Pre-condition failed: Not an AEXOS project (.aexos-core/ not found)"
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .aexos-core/ configuration analyzed
...
1. **Error:** No .aexos-core/ Directory
   - **Cause:** Not an AEXOS project
   - **Resolution:** Initialize AEXOS first or check directory
...
```javascript
// Core AEXOS structure
const scanLocations = {
  cyryxCore: '.aexos-core/',
  services: '.aexos-core/infrastructure/services/',
  squads: '.aexos-core/squads/',
  agents: '.aexos-core/development/agents/',
  tasks: '.aexos-core/development/tasks/',
  data: '.aexos-core/data/'
};
```
...
const configPatterns = {
  ...
  configFile: exists('cyryx.config.js') || exists('.aexos-core/core-config.yaml'),
...
```
.aexos-core/infrastructure/services/{service-name}/
...
```
.aexos-core/infrastructure/services/{service_name}/
├── README.md
├── index.ts
...
```javascript
const { isCodeIntelAvailable } = require(require('path').resolve(process.cwd(), '.aexos-core/core/code-intel'));
const { getDependencyGraph, getComplexityAnalysis } = require(require('path').resolve(process.cwd(), '.aexos-core/core/code-intel/helpers/planning-helper.js'));
