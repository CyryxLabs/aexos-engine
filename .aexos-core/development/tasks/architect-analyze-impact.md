...
alyze Impact - AEXOS Developer Task

## Purpose
Analyze the potential impact of proposed component modifications on the broader AEXOS (Cyryx) framework.
...
*analyze-impact modify .aexos-core/development/agents/weather-agent.md --depth deep --include-tests

# Analyze deprecation impact with visual output
*analyze-impact deprecate aexos-core/scripts/old-helper.js --output-format visual --save-report reports/deprecation-impact.html

# Quick impact check for refactoring
*analyze-impact refactor .aexos-core/development/tasks/process-data.md --depth shallow --risk-threshold medium

# Analyze removal with approval workflow
*analyze-impact remove .aexos-core/development/workflows/legacy-workflow.yaml --depth deep --save-report reports/removal-impact.json
```
...
class AnalyzeImpactTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔍 AEXOS Impact Analysis'));
...
{
  "success": true,
  "targetComponent": ".aexos-core/development/agents/weather-agent.md",
  "riskLevel": "medium",
