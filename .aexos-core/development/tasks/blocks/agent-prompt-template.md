...
Standardized template for spawning AEXOS agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the AEXOS {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/AEXOS/agents/architect.md,
...
async function buildAgentPrompt(loadBlockFromHost, renderTemplateFromHost) {
  const template = await loadBlockFromHost('agent-prompt-template');
  return renderTemplateFromHost(template, {
    agent_name: 'Aria',
    agent_role: 'Architect',
    agent_file_path: '.claude/commands/AEXOS/agents/architect.md',
    context_category: 'Architecture',
