...
Load AEXOS project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .aexos-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .aexos-core/core-config.yaml
...
```javascript
// Caller-provided adapter. AEXOS does not distribute a compatible
// .aexos-core/utils/context-loader programmatic API.
async function loadTaskContext(loadContextFromHost) {
  const context = await loadContextFromHost({
    category: 'supabase',
    include_git: true,
  ...
  // The host preserves the optional-source behavior in Error Handling below.
...
| File | Purpose |
|------|---------|
| `.aexos/gotchas.json` | Known issues and workarounds |
| `.aexos-core/data/technical-preferences.md` | User-defined patterns |
| `.aexos-core/core-config.yaml` | Project configuration |
