...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the AEXOS framework.

## Directory Structure

```
.aexos-core/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
// Caller-provided adapters. AEXOS does not distribute
// .aexos-core/utils/block-loader.
async function runContextBlock(loadBlockFromHost, executeBlockFromHost) {
  const block = await loadBlockFromHost('context-loading');
  return executeBlockFromHost(block, {
    category: 'auth',
    include_git: true
  });
}
...
| `agent-prompt-template` | Standardized template for spawning AEXOS agents | Any skill/task that invokes agents via Task tool |
...
*AEXOS Task Blocks System v1.1.0*
