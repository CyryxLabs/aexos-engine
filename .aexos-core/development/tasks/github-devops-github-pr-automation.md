...
if (!context) {
  throw new Error('Unable to detect repository. Run "aexos init" first.');
}
```
...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.aexos-core', 'core-config.yaml');
...
```javascript
const { generateImpactSummary } = require(require('path').resolve(process.cwd(), '.aexos-core/core/code-intel/helpers/devops-helper.js'));
...
### NPM Package with Semantic-Release (aexos-core)
```yaml
github:
  pr:
