...
- **topic**: `string`
  ...
  - **Example**: "How can we improve user onboarding for AEXOS?"
...
```bash
cyryx activate Maestro
cyryx brainstorm "How can we improve AEXOS user onboarding for non-technical users?"
```
...
```bash
cyryx brainstorm "How to reduce API latency in database queries?" \
...
```bash
cyryx brainstorm "What should be our open-source expansion strategy for Q1 2026?" \
