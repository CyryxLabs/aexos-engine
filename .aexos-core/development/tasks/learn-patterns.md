...
# Learn Patterns - AEXOS Developer Task
...
*learn-patterns --component aexos-core/agents/developer.md --analyze
...
class LearnPatternsTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🧠 AEXOS Pattern Learning'));
  ...
  async loadModification(modificationId) {
    ...
    const sources = [
      path.join(this.rootPath, '.aexos', 'modifications', `${modificationId}.json`),
      path.join(this.rootPath, '.aexos', 'history', `${modificationId}.json`),
      path.join(this.rootPath, '.aexos', 'proposals', `${modificationId}.json`)
    ];
