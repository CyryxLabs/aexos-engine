...
```javascript
const { GotchasDocumenter } = require(require('path').resolve(process.cwd(), '.aexos-core/infrastructure/scripts/gotchas-documenter.js'));
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require(require('path').resolve(process.cwd(), '.aexos-core/infrastructure/scripts/gotchas-documenter.js'));
...
const { getGotchasForSelfCritique } = require(require('path').resolve(process.cwd(), '.aexos-core/infrastructure/scripts/gotchas-documenter.js'));
