...
```bash
node bin/aexos-ids.js ids:health
```
...
```bash
node bin/aexos-ids.js ids:health --fix
```
...
- Log all healing actions to `.aexos-core/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/aexos-ids.js ids:health --json
node bin/aexos-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require(require('path').resolve(process.cwd(), '.aexos-core/core/ids/registry-healer.js'));
