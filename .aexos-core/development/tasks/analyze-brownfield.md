...
pre-conditions:
  ...
  - [ ] Brownfield Analyzer module is available
    ...
    blocker: true
    validation: |
      Verify .aexos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js exists
...
- **Tool:** brownfield-analyzer
  ...
  - **Source:** .aexos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js

- **Tool:** mode-detector
  - **Purpose:** Collect project markers for analysis
  - **Source:** .aexos-core/infrastructure/scripts/documentation-integrity/mode-detector.js
...
- **Script:** brownfield-analyzer.js
  ...
  - **Language:** JavaScript
  - **Location:** .aexos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js
...
Analyze an existing project to understand its structure, tech stack, coding standards, and CI/CD workflows before AEXOS integration. This task provides recommendations for safe integration and identifies potential conflicts.
...
```javascript
const { analyzeProject, formatMigrationReport } = require('./.aexos-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js');
...
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  • Preserve existing ESLint configuration - AEXOS will adapt          ║
║  • Keep existing Prettier settings - AEXOS coding-standards.md will d ║
║  • Review existing CI/CD before adding AEXOS workflows                ║
║  • AEXOS will use existing tsconfig.json settings                     ║
...
|----------|---------|---------|
| `parallel` | Safe to proceed with standard AEXOS setup | Use `*setup-project-docs` directly |
...
1. **Review GitHub Workflows:**
   ...
   # Check for potential conflicts with AEXOS workflows
...
1. **docs/architecture/ exists:**
   - Decide: Keep existing or merge with AEXOS docs
   - Option A: Rename existing to `docs/legacy-architecture/`
   - Option B: Configure AEXOS to use alternate path
...
- Run this task BEFORE any AEXOS integration
