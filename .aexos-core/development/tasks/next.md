...
AEXOS 4.0.4 runtime-first mode adds deterministic next-step recommendation from
...
belongs in `.aexos-core/core/orchestration/session-state.js`.
...
const { SuggestionEngine } = require(path.join(projectRoot, '.aexos-core/workflow-intelligence/engine/suggestion-engine.js'));
...
```javascript
const { WorkflowStateManager } = require(path.join(projectRoot, '.aexos-core/development/scripts/workflow-state-manager.js'));
...
```javascript
const formatter = require(path.join(projectRoot, '.aexos-core/workflow-intelligence/engine/output-formatter.js'));
...
const mergedSuggestions = [runtimeSuggestion, ...(result.suggestions || [])].filter(
  (suggestion) => {
    const key = `${suggestion.command || ''}\u0000${suggestion.args || ''}`;
