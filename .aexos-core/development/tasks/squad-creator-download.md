...
Downloads public squads from the aexos-squads GitHub repository to use in your project.
...
Available Squads (from aexos-squads):
...
Downloading: etl-squad@1.0.0
  Source: github.com/CyryxLabs/aexos-squads/packages/etl-squad
...
```
1. Fetch registry.json from aexos-squads
...
3. Download via GitHub API
   ...
   ├── Download each file recursively to an isolated staging directory
   └── Reject untrusted URLs, unsafe paths, and symbolic links

4. Validate downloaded squad
   ├── Run SquadValidator
   ├── Require a readable manifest matching the catalog name and version
   └── Preserve the existing squad when any check fails

5. Load manifest
   └── Atomically promote the validated staging directory
```

### Programmatic catalog overrides

Tests and self-hosted catalogs can configure both metadata and content sources.
The registry never controls the local destination or the content endpoint:

```javascript
const downloader = new SquadDownloader({
  registryUrl: 'https://catalog.example.test/registry.json',
  contentApiBase: 'https://catalog.example.test/contents/packages',
  contentDownloadOrigins: ['https://files.example.test'],
});
```

All override values must use HTTPS. `contentDownloadOrigins` is an explicit
allowlist of origins for file bodies. API authentication is sent only to the
configured `contentApiBase` origin and is dropped for other origins. Redirects
are bounded and must remain within the applicable allowlist.

## Registry Structure

The registry.json in aexos-squads contains:
...
| `VERSION_NOT_FOUND` | Requested version is not in the catalog | Download the listed version |
| `REGISTRY_FETCH_ERROR` | Network issue | Check connection |
| `VALIDATION_ERROR` | Downloaded manifest/content is invalid or mismatched | Fix the catalog package before retrying |
