...
```bash
# Basic query
node bin/aexos-ids.js ids:query "validate story drafts"

# With JSON output
node bin/aexos-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/aexos-ids.js ids:query "database migration" --type script

# Filter by category
node bin/aexos-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.aexos-core/core/ids/registry-loader.js'));
const { IncrementalDecisionEngine } = require(path.resolve('.aexos-core/core/ids/incremental-decision-engine.js'));
...
- `aexos ids:create-review` — Review CREATE decisions for 30-day assessment
