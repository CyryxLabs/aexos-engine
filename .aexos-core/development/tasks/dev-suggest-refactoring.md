...
# Suggest Refactoring - AEXOS Developer Task
...
```bash
# Analyze single file
*suggest-refactoring aexos-core/scripts/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring aexos-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring aexos-core/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring aexos-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 AEXOS Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.aexos', 'refactoring', `${suggestionId}.json`);
...
6. **Code Intelligence Blast Radius** (if available):
   - Call `assessRefactoringImpact(candidateFiles)` from `.aexos-core/core/code-intel/helpers/dev-helper.js`
