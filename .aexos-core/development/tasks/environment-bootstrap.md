...
Complete environment bootstrap for new AEXOS projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
- **Host binding:** cli-checker
  - **Purpose:** Verify CLI installations and versions
  - **Availability:** `.aexos-core/infrastructure/scripts/cli-checker.js` is not distributed; use a concrete host or project tool that satisfies the substantive task steps, or treat the capability as blocked.

- **Tool:** github-cli
  - **Purpose:** Repository creation and authentication
  - **Source:** .aexos-core/infrastructure/tools/cli/github-cli.yaml
...
- Cache detection results in .aexos/environment-report.yaml
  ...
  quality:
    coderabbit:
      ...
      note: |
        ...
        - The aexos-core runtime auto-detects the host (`process.platform`) and
...
# AEXOS generated files
.aexos/project-status.yaml
.aexos/environment-report.yaml
...
---
*Generated by AEXOS Environment Bootstrap*
...
- Initialize AEXOS (Cyryx) project structure
...
**Action:** Create AEXOS-compliant project structure
...
# Create .aexos/config.yaml
@"
# AEXOS Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".aexos/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with AEXOS (Cyryx)",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.aexos/user-config.yaml`
...
```bash
# Create ~/.aexos/ with secure permissions
mkdir -p ~/.aexos
chmod 700 ~/.aexos

# Write user-config.yaml with selected profile
cat > ~/.aexos/user-config.yaml << EOF
# AEXOS User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require(
  require('path').resolve(process.cwd(), '.aexos-core/core/config/config-resolver.js'),
);
...
- `~/.aexos/` directory must have permissions 700
- `~/.aexos/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".aexos/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .aexos/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ AEXOS ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
║                                                                            ║
║  Full workflow: .aexos-core/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .aexos/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • AEXOS help: @aexos-master *help                                           ║
...
— Polaris, environment configured with confidence 🚀
```
...
- [AEXOS Greenfield Workflow](.aexos-core/development/workflows/greenfield-fullstack.yaml)
