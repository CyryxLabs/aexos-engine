...
- **Host binding:** code-analyzer
  - **Purpose:** Static code analysis and metrics
  - **Availability:** `.aexos-core/utils/code-analyzer.js` is not distributed; use a concrete host or project tool that satisfies the substantive task steps, or treat the capability as blocked.
...
- **Execution:** Host-guided — codebase analysis and reporting.
  - **Unavailable wrapper:** `.aexos-core/scripts/analyze-codebase.js` is not distributed.
  - **Requirement:** Preserve the task's declared inputs, validation, output, and failure behavior; do not report this wrapper as executed.
...
Systematically audit all utilities in `.aexos-core/infrastructure/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .aexos-core/infrastructure/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .aexos-core/development/agents .aexos-core/development/tasks Squads 2>/dev/null | wc -l)
