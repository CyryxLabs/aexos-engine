...
To help users contribute to the AEXOS open-source project (`aexos-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `aexos-core` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `".aexos-core/development/agents/improved-agent.md"`
...
- **pr_url**: `string`
  ...
  - **Example**: `"https://github.com/CyryxLabs/AEXOS/pull/123"`
...
2. **Validate Repository State**
   - Check if `aexos-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `.aexos-core/development/agents/*` → "agent"
     - `.aexos-core/development/tasks/*` → "task"
     - `aexos-core/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using AEXOS PR Automation.
      ```
...
18. **Provide Next Steps**
    - Display to user:
      ...
      Thank you for contributing to AEXOS! 🚀
      ```
...
- [ ] Fork of aexos-core exists
  - **Validation**: `gh repo view {user}/aexos-core` succeeds
...
```bash
cyryx activate Otto  # github-devops agent
cyryx pr create \
  --type="Squad" \
...
cyryx pr create \
  --type="agent" \
  --path=".aexos-core/development/agents/improved-po.md" \
...
cyryx pr create \
  --type="bug-fix" \
  --path=".aexos-core/development/tasks/create-next-story.md" \
