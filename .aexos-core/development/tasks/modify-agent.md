...
- **Host binding:** ast-parser
  - **Purpose:** Parse and modify code safely
  - **Availability:** `.aexos-core/utils/ast-parser.js` is not distributed; use a concrete host or project tool that satisfies the substantive task steps, or treat the capability as blocked.
...
- **Execution:** Host-guided — safe file modification with backup.
  - **Unavailable wrapper:** `.aexos-core/scripts/modify-file.js` is not distributed.
...
- Target agent must exist in `.aexos-core/development/agents/`
...
- Load target agent from `.aexos-core/development/agents/{agent-name}.md`
- Parse YAML header and markdown content separately
- Create timestamped backup: `.aexos-core/development/agents/.backups/{agent-name}.md.{timestamp}`
