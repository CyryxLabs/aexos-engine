...
class GreetingPreferenceManager {
  ...
  setPreference(preference) {
    ...
    try {
      ...
      // Atomic replacement keeps the original intact on failure. A shared old
      // backup must never replace current data after lock/backup acquisition fails.
   ...
   *
   * @returns {Function} Release function; throws if locking is unavailable
   * @private
   */
  _acquireLock() {
    let lastError;
    ...
    for (let attempt = 0; attempt <= LOCK_RETRY_COUNT; attempt++) {
      try {
        return lockfile.lockSync(this.configPath, {
          stale: LOCK_STALE_MS,
    ...
    throw new Error('Could not acquire config lock: ' + (lastError && lastError.message));
  }
  ...
  _saveConfig(config) {
    ...
    atomicWriteSync(this.configPath, content, 'utf8');
  }
}
