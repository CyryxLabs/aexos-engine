 ...
 *
 * Publishes squads to the aexos-squads GitHub repository via Pull Request.
...
/**
 * Repository for aexos-squads
 * @constant {string}
 */
const AEXOS_SQUADS_REPO = 'CyryxLabs/aexos-squads';
...
/**
 * Squad Publisher class for publishing squads to aexos-squads repository
 */
class SquadPublisher {
   ...
   * @param {string} [options.repo] - Target repository (default: CyryxLabs/aexos-squads)
   */
  constructor(options = {}) {
    this.verbose = options.verbose || false;
    this.dryRun = options.dryRun || false;
    this.repo = options.repo || AEXOS_SQUADS_REPO;
  }
  ...
  /**
   * Publish squad to aexos-squads repository
   *
  ...
  async publish(squadPath, options = {}) {
    ...
    if (!['official', 'community'].includes(category)) {
      throw new SquadPublisherError(PublisherErrorCodes.MANIFEST_ERROR, 'Category must be official or community');
    }
    ...
    // 7. Create actual PR
    const prUrl = await this._createPR(squadPath, manifest, branchName, prTitle, prBody, category);
...
- [x] Squad follows AEXOS task-first architecture
...
---
*Submitted via \`*publish-squad\` from AEXOS-FullStack*`;
  }
   ...
   */
  async _createPR(squadPath, manifest, branchName, title, body, category = 'community') {
    const squadName = manifest.name;
    ...
    let bodyDirectory;
    try {
      if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(this.repo) || !isValidName(squadName)) {
        throw new Error('Invalid repository or squad identity');
      }
      const loginResult = spawnSync('gh', ['api', 'user', '--jq', '.login'], {
        encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
      });
      if (loginResult.error || loginResult.status !== 0) throw new Error(loginResult.stderr || 'Cannot determine authenticated GitHub account');
      const login = (loginResult.stdout || '').trim();
      if (!/^[A-Za-z0-9][A-Za-z0-9-]*$/.test(login)) throw new Error('Invalid authenticated GitHub account');
      const [repositoryOwner, repositoryName] = this.repo.split('/');
      const forkRepo = `${login}/${repositoryName}`;
      ...
      if (repositoryOwner.toLowerCase() !== login.toLowerCase()) {
        ...
        if (forkResult.error || forkResult.status !== 0) throw new Error(forkResult.stderr || 'Fork failed');
        const parentResult = spawnSync('gh', ['api', `repos/${forkRepo}`, '--jq', '.parent.full_name'], {
          encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
        });
        if (parentResult.error || parentResult.status !== 0 ||
          (parentResult.stdout || '').trim().toLowerCase() !== this.repo.toLowerCase()) {
          throw new Error('Authenticated fork does not belong to the requested upstream repository');
        }
      }

      // Step 2: Clone the fork to a temp directory
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-squad-publish-'));

      this._log(`Cloning fork to: ${tempDir}`);
      // Use spawnSync with array args
      const cloneResult = spawnSync('gh', ['repo', 'clone', forkRepo, tempDir, '--', '--depth', '1'], {
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      ...
      const packagesPath = path.join(tempDir, 'packages');
      try {
        const packagesState = await fs.lstat(packagesPath);
        if (packagesState.isSymbolicLink() || !packagesState.isDirectory()) throw new Error('Repository packages path is not a real directory');
      ...
      await this._updateRegistry(registryPath, manifest, category);

      // Step 6: Commit changes
      this._log('Committing changes...');
      const addResult = spawnSync('git', ['add', '--', `packages/${squadName}`, 'registry.json'], {
        cwd: tempDir,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      if (addResult.error || addResult.status !== 0) throw new Error(addResult.stderr || 'Staging failed');
      ...
      // A cloned repository may contain a tracked symlink with this name.
      // Keep generated PR content outside that untrusted tree.
      bodyDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'aexos-squad-pr-body-'));
      const prBodyFile = path.join(bodyDirectory, 'pr-body.md');
      await fs.writeFile(prBodyFile, body, { flag: 'wx' });

      // Use spawnSync with array args - title contains validated squadName
      const prResult = spawnSync(
        'gh',
        ['pr', 'create', '--repo', this.repo, '--head', `${login}:${branchName}`, '--title', title, '--body-file', prBodyFile, '--base', 'main'],
        {
          cwd: tempDir,
          encoding: 'utf-8',
          stdio: ['pipe', 'pipe', 'pipe'],
      ...
      let parsedPr;
      try { parsedPr = new URL(prUrl); } catch { throw new Error('PR command did not return a valid pull request URL'); }
      const expectedPath = `/${this.repo}/pull/`;
      const expectedHost = process.env.GH_HOST || 'github.com';
      if (parsedPr.protocol !== 'https:' || parsedPr.host.toLowerCase() !== expectedHost.toLowerCase() ||
        !parsedPr.pathname.toLowerCase().startsWith(expectedPath.toLowerCase()) ||
        !/^\d+$/.test(parsedPr.pathname.slice(expectedPath.length)) || parsedPr.username || parsedPr.password) {
        throw new Error('PR command returned a pull request for the wrong repository');
      }
      ...
      // Only this invocation's exclusive temporary directory is owned here.
      if (bodyDirectory) await this._cleanupTemp(bodyDirectory);
      if (tempDir) await this._cleanupTemp(tempDir);
    }
  }
   ...
   * @param {Object} manifest - Squad manifest
   */
  async _updateRegistry(registryPath, manifest, category = 'community') {
    if (!['official', 'community'].includes(category)) throw new Error('Invalid squad category');
    let registry;

    try {
      const state = await fs.lstat(registryPath);
      if (state.isSymbolicLink() || !state.isFile()) throw new Error('Existing registry must be a regular file');
    ...
    // Reject corrupt existing catalogs instead of replacing their contents.
    if (!registry || typeof registry !== 'object' || Array.isArray(registry) ||
      !registry.squads || typeof registry.squads !== 'object' || Array.isArray(registry.squads)) {
      throw new Error('Invalid existing squad registry');
    }
    for (const group of ['official', 'community']) {
      if (registry.squads[group] === undefined) registry.squads[group] = [];
      if (!Array.isArray(registry.squads[group]) || registry.squads[group].some(entry =>
        !entry || typeof entry.name !== 'string')) throw new Error(`Invalid registry group ${group}`);
    }

    // Check if squad already exists
    const exists = registry.squads[category].some((s) => s.name === manifest.name);
    if (exists) {
      // Update existing entry
      registry.squads[category] = registry.squads[category].map((s) =>
    ...
    // Sort alphabetically
    registry.squads[category].sort((a, b) => a.name.localeCompare(b.name));
  ...
  async _copyDir(src, dest) {
    const sourceState = await fs.lstat(src);
    if (sourceState.isSymbolicLink() || !sourceState.isDirectory()) throw new Error('Squad source must be a real directory');
    await fs.mkdir(dest, { recursive: true });
    const destinationState = await fs.lstat(dest);
    if (destinationState.isSymbolicLink() || !destinationState.isDirectory()) throw new Error('Squad destination must be a real directory');
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Refusing to publish symbolic link: ${entry.name}`);
      try {
        if ((await fs.lstat(destPath)).isSymbolicLink()) throw new Error(`Repository target is a symbolic link: ${entry.name}`);
      } catch (error) { if (error.code !== 'ENOENT') throw error; }
