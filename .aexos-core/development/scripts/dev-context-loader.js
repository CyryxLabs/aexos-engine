...
class DevContextLoader {
  ...
  async loadFiles(fileList, options = {}) {
    ...
    for (const filePath of fileList) {
      ...
      try {
        ...
        // Load from disk
        const sourceBeforeRead = await fs.stat(absolutePath);
        ...
        // Save to cache
        await this.saveToCache(filePath, fileData, fullLoad, sourceBeforeRead);
  ...
  async loadFromCache(filePath, fullLoad) {
    ...
    try {
      const source = await fs.stat(path.resolve(filePath));
      const sourceIdentity = `${source.size}:${source.mtimeMs}:${source.ctimeMs}`;
      const memory = this.summaryCache.get(cachePath);
      if (memory && memory.sourceIdentity === sourceIdentity && Date.now() - memory.savedAt <= CACHE_TTL) {
        return memory.data;
      }
      this.summaryCache.delete(cachePath);
      ...
      if (cached.sourceIdentity !== sourceIdentity || !cached.data) return null;
      this.summaryCache.set(cachePath, cached);
      return cached.data;
    } catch (_error) {
      // Cache miss
      return null;
   ...
   */
  async saveToCache(filePath, data, fullLoad, sourceBeforeRead) {
    const cacheKey = this.getCacheKey(filePath, fullLoad);
    const cachePath = path.join(this.cacheDir, `${cacheKey}.json`);
    ...
    try {
      ...
      const source = await fs.stat(path.resolve(filePath));
      if (sourceBeforeRead && ['size', 'mtimeMs', 'ctimeMs'].some(key => source[key] !== sourceBeforeRead[key])) return;
      const cached = { data, sourceIdentity: `${source.size}:${source.mtimeMs}:${source.ctimeMs}`, savedAt: Date.now() };
  ...
  getCacheKey(filePath, fullLoad) {
    const normalized = createHash('sha256').update(path.resolve(filePath)).digest('hex');
