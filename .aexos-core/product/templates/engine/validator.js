...
class TemplateValidator {
  ...
  constructor(options = {}) {
    this.schemasDir = options.schemasDir ||
      path.join(process.cwd(), '.aexos-core', 'product', 'templates', 'engine', 'schemas');
    this.fallbackSchemasDir = options.fallbackSchemasDir || null;
  ...
  async loadSchema(templateType) {
    ...
    const primaryPath = path.join(this.schemasDir, schemaFile);
    const fallbackPath = this.fallbackSchemasDir
      ? path.join(this.fallbackSchemasDir, schemaFile)
      : null;
    const schemaPath = isDeclaredPath(primaryPath) || !fallbackPath
      ? primaryPath
      : fallbackPath;
   ...
   */
  extractDataFromMarkdown(content, _template) {
    const data = {};
