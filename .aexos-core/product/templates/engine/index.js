 ...
 * Main entry point for the AEXOS Template Engine
 *
...
class TemplateEngine {
  ...
  constructor(options = {}) {
    ...
    const hasExplicitTemplatesDir = Object.prototype.hasOwnProperty.call(options, 'templatesDir');
    const hasExplicitSchemasDir = Object.prototype.hasOwnProperty.call(options, 'schemasDir');
    const packageTemplatesDir = path.resolve(__dirname, '..');
    const packageSchemasDir = path.join(packageTemplatesDir, 'engine', 'schemas');

    this.templatesDir = options.templatesDir ||
      path.join(this.baseDir, '.aexos-core', 'product', 'templates');

    this.fallbackTemplatesDir = hasExplicitTemplatesDir || this.templatesDir === packageTemplatesDir
      ? null
      : packageTemplatesDir;

    const projectSchemasDir = path.join(this.templatesDir, 'engine', 'schemas');
    this.schemasDir = options.schemasDir || projectSchemasDir;
    this.fallbackSchemasDir = hasExplicitTemplatesDir || hasExplicitSchemasDir ||
      this.schemasDir === packageSchemasDir
      ? null
      : packageSchemasDir;
    ...
    this.loader = new TemplateLoader({
      ...
      fallbackTemplatesDir: this.fallbackTemplatesDir,
    });
    ...
    this.validator = new TemplateValidator({
      schemasDir: this.schemasDir,
      fallbackSchemasDir: this.fallbackSchemasDir,
    });
  }
