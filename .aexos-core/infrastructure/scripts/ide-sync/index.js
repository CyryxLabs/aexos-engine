...
function loadConfig(projectRoot) {
  ...
  const defaultConfig = {
    ...
    redirects: {
      'aexos-developer': 'aexos-master',
      'aexos-orchestrator': 'aexos-master',
...
async function commandValidate(options) {
  ...
  for (const [ideName, ideConfig] of targetIdes) {
    ...
    if (ideName === 'claude-code' && typeof transformer.transformSkill === 'function') {
      const skillNamespace = path.posix.join('AEXOS', 'agents') + '/';
      ...
      ideConfigs['claude-code-skills'] = {
        ...
        targetDir: path.join(projectRoot, ideConfig.skillsPath || '.claude/skills', 'AEXOS', 'agents'),
      };
    }
  }
...
function showHelp() {
  console.log(`
${colors.bright}IDE Sync${colors.reset} - Sync AEXOS agents to IDE command files
