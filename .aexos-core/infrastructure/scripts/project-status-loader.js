 ...
 * for display in agent greetings across all 11 AEXOS agents.
 *
...
class ProjectStatusLoader {
  ...
  loadConfig() {
    try {
      const configPath = path.join(this.rootPath, '.aexos-core', 'core-config.yaml');
  ...
  _resolveCacheFilePath() {
    try {
      ...
      if (normalizedGitDir !== normalizedCommonDir) {
        ...
        return path.join(this.rootPath, '.aexos', `project-status-${worktreeHash}.yaml`);
      }
    } catch (_error) {
      // Not a git repo or git not available - use default path
    }

    return path.join(this.rootPath, '.aexos', 'project-status.yaml');
  }
 ...
 * @property {string} path - Relative path to worktree (.aexos/worktrees/{story-id})
