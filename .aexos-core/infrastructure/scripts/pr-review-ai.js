...
class DiffAnalyzer {
  ...
  parseDiff(diff) {
    const files = [];
    const _fileRegex = /^diff --git a\/(.+) b\/(.+)$/gm;
    const _hunkRegex = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@(.*)$/gm;
...
class RedundancyAnalyzer {
   ...
   */
  analyze(parsedDiff, _codebaseContext = {}) {
    const findings = [];
...
class PRReviewAI extends EventEmitter {
  ...
  formatReviewBody(review) {
    ...
    lines.push('---');
    lines.push('*Generated by AEXOS PR Review AI*');
  ...
  async saveReport(prNumber, review) {
    const reportDir = path.join(this.rootPath, '.aexos', 'reviews');
   ...
   */
  async reviewLocal(baseBranch = 'main', options = {}) {
    const gitOptions = { cwd: this.rootPath, encoding: 'utf8', windowsHide: true };
    const base = execFileSync('git', ['rev-parse', '--verify', '--end-of-options', baseBranch + '^{commit}'], gitOptions).trim();
    const diff = execFileSync('git', ['diff', '--no-ext-diff', base + '...HEAD', '--'], gitOptions);
    ...
    return {
      ...
      findings,
      verdict:
        findings.some((f) => f.severity === Severity.CRITICAL || f.severity === Severity.HIGH)
          ? Verdict.REQUEST_CHANGES
          : Verdict.APPROVE,
    };
  }
...
Options:
  ...
  --security-only Limit local analysis to security findings
  --help          Show this help
...
`);
  ...
  if (args.includes('--local')) {
    ...
    reviewer
      .reviewLocal(baseBranch, { securityOnly: args.includes('--security-only') })
      .then((result) => {
        console.log('\n📊 Local Review Results:');
        console.log(`   Files changed: ${result.stats.filesChanged}`);
        console.log(`   Findings: ${result.findings.length}`);
        console.log(`   Verdict: ${result.verdict}`);
        process.exitCode = result.verdict === Verdict.REQUEST_CHANGES ? 1 : 0;
