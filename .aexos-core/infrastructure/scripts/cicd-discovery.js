 ...
 * integration points for AEXOS workflows.
 */
...
/**
 * IntegrationSuggester - Suggests AEXOS integration points
 */
class IntegrationSuggester {
  ...
  suggest(analysis, provider) {
    ...
    // Add AEXOS build step
    suggestions.push({
      type: 'add_step',
      priority: 'high',
      title: 'Run Project Build',
      description: 'Run the project build script after dependency installation; package.json must define scripts.build',
      code: this.getCYRYXBuildStep(provider),
    });
    ...
    if (!analysis.hasTests) {
      suggestions.push({
        ...
        description: 'Use AEXOS test discovery to automatically find and run tests',
    ...
    if (!analysis.hasSecurityScan) {
      suggestions.push({
        ...
        description: 'Run the shipped static security analyzer; checkout history must contain HEAD~1',
  ...
  /**
   * Get AEXOS build step for provider
   */
  getCYRYXBuildStep(provider) {
    const steps = {
      'github-actions': `- name: Project Build
  run: npm run build
  env:
    AEXOS_CI: true`,

      'gitlab-ci': `cyryx_build:
  stage: build
  script:
    - npm run build
  variables:
  ...
  run: node .aexos-core/infrastructure/scripts/test-discovery.js --run --coverage`,

      'gitlab-ci': `test:
  stage: test
  script:
    - node .aexos-core/infrastructure/scripts/test-discovery.js --run --coverage
  coverage: '/Coverage: \\d+\\.\\d+%/'`,

      jenkins: `stage('Test') {
  steps {
    sh 'node .aexos-core/infrastructure/scripts/test-discovery.js --run --coverage'
  }
}`,

      circleci: `- run:
    name: Run Tests
    command: node .aexos-core/infrastructure/scripts/test-discovery.js --run --coverage`,
    };
  ...
  run: node .aexos-core/infrastructure/scripts/pr-review-ai.js --local HEAD~1 --no-ai --security-only
  continue-on-error: false`,

      'gitlab-ci': `security_scan:
  stage: test
  script:
    - node .aexos-core/infrastructure/scripts/pr-review-ai.js --local HEAD~1 --no-ai --security-only
  allow_failure: false`,

      jenkins: `stage('Security Scan') {
  steps {
    sh 'node .aexos-core/infrastructure/scripts/pr-review-ai.js --local HEAD~1 --no-ai --security-only'
  }
  post {
    failure {
      ...
      circleci: `- run:
    name: Security Scan
    command: node .aexos-core/infrastructure/scripts/pr-review-ai.js --local HEAD~1 --no-ai --security-only
    when: always`,
    };
