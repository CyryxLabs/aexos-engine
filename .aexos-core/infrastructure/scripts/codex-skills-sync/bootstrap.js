 ...
 *   node .aexos-core/infrastructure/scripts/codex-skills-sync/bootstrap.js --help
 *
...
const MARKER = '<!-- CYRYX-CODEX-LOCAL-SKILLS: generated -->';
...
Usage:
  node .aexos-core/infrastructure/scripts/codex-skills-sync/bootstrap.js [options]
  npm run setup:codex-skills [-- options]

Options:
  --project-root <path>  Project root. Defaults to current directory or nearest parent with .aexos-core/squads.
...
function findProjectRoot(start) {
  ...
  for (let depth = 0; depth < 10; depth += 1) {
    const hasCyryxCore = fs.existsSync(path.join(current, '.aexos-core'));
    const hasSquads = fs.existsSync(path.join(current, 'squads'));
    if (hasCyryxCore || hasSquads) {
      return current;
    }
...
function loadYaml(projectRoot) {
  const candidates = [
    ...
    path.join(projectRoot, '.aexos-core', 'node_modules', 'js-yaml'),
    path.join(__dirname, 'node_modules', 'js-yaml'),
    path.join(__dirname, '.aexos-core', 'node_modules', 'js-yaml'),
  ];
...
function squadSkillId(squadName, entryAgent, squadAlias) {
  const alias = slug(squadAlias || '').replace(/^cyryx-/, '');
  const squadBase = slug(squadName).replace(/^cyryx-/, '');
  const entry = slug(entryAgent).replace(/^cyryx-/, '');
  ...
  if (entry === squadBase || entry.startsWith(`${squadBase}-`)) return `cyryx-${entry}`;
  if (alias && (entry === alias || entry.startsWith(`${alias}-`))) return `cyryx-${entry}`;
  ...
  return `cyryx-${squadBase || alias}-${entry}`;
}
  ...
  return files.map((filePath) => {
    ...
    return {
      ...
      content: buildSkillContent({
        skillId,
        title: `AEXOS ${agent.title}`,
        whenToUse: agent.whenToUse,
        sourcePath,
        scopePath: '.aexos-core/development',
...
function buildSquadPlans(projectRoot, yaml) {
  ...
  for (const squadDir of squadDirs) {
    ...
    // The framework disagreed with itself here: `*create-squad` scaffolds
    // squad.yaml and `*validate-squad` calls config.yaml deprecated, while this
    // discovery read config.yaml only — so a squad built the recommended way was
    // silently skipped and never became a skill. Accept both, canonical first,
    // so new squads are found and the two existing ones keep working untouched.
    const configPath = [
      path.join(squadDir, 'squad.yaml'),
      path.join(squadDir, 'config.yaml'),
    ].find((candidate) => fs.existsSync(candidate));
    if (!configPath) {
      warnings.push(`Skipped ${squadName}: no squad.yaml or config.yaml`);
      continue;
    }
...
function main() {
  ...
  if (!plans.length) {
    throw new Error('No skills were discovered. Run this from the project root that contains .aexos-core/ and squads/.');
  }
  ...
  if (!args.quiet) {
    ...
    console.log('- Use the aexos-* skill names to avoid matching unrelated global cyryx-* skills.');
