# Diagnóstico de problema com npx aexos-core
# Execute este script no PC com problema

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "   Diagnóstico NPX aexos-core" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4] Baixando aexos-core manualmente..." -ForegroundColor Yellow
$tempDir = "$env:TEMP\aexos-test-$(Get-Random)"
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
Set-Location $tempDir

Write-Host "    Diretório: $tempDir"
npm pack aexos-core@2.2.0 2>&1 | Out-Null

if (Test-Path "aexos-core-2.2.0.tgz") {
    Write-Host "    Download: OK" -ForegroundColor Green

    # Extrair
    tar -xzf aexos-core-2.2.0.tgz 2>&1 | Out-Null

    if (Test-Path "package/bin/aexos.js") {
        Write-Host "    Extração: OK" -ForegroundColor Green

        # Tentar executar
        Write-Host ""
        Write-Host "[5] Executando bin/aexos.js --version:" -ForegroundColor Yellow
        try {
            $result = node package/bin/aexos.js --version 2>&1
        ...
        Write-Host "    Executando: node package/bin/aexos.js --help"
        node package/bin/aexos.js --help 2>&1

    } else {
        Write-Host "    Extração: FALHOU - bin/aexos.js não encontrado" -ForegroundColor Red
...
Write-Host "4. Tente novamente: npx aexos-core@latest"
Write-Host ""
