...
$allTasks = Get-ChildItem .aexos-core/tasks/*.md | Where-Object { $_.Name -notlike '*v1-backup*' } | ForEach-Object { $_.Name }
...
foreach ($task in $allTasks) {
  $result = node .aexos-core/scripts/validate-task-v2.js ".aexos-core/tasks/$task" 2>&1
...
foreach ($task in $nonCompliantTasks) {
  ...
  node .aexos-core/scripts/migrate-task-to-v2.js ".aexos-core/tasks/$task" 2>&1 | Out-Null
