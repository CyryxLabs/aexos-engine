# Guia do Usuário AEXOS (Cyryx)

## Visão Geral

O AEXOS (Cyryx) é um framework de desenvolvimento full stack orientado por agentes de IA que implementa uma metodologia ágil única. Este guia explica como usar o AEXOS (Cyryx) de forma eficaz através de suas duas fases principais: **Planejamento** e **Desenvolvimento**.
...
### Instalando AEXOS-FullStack

O AEXOS-FullStack oferece instalação simplificada através de um instalador interativo completo.
...
```
🚀 AEXOS-FullStack Installation Wizard
...
📦 Select Squads to Install:
  ...
  ◯ aexos-infrastructure-devops (DevOps utilities)
...
Se você está contribuindo para o AEXOS-FullStack em si:

```bash
# Clone o repositório
git clone https://github.com/CyryxLabs/AEXOS
cd aexos-core
...
```
your-project/
├── .aexos-core/                    # ✅ Framework core instalado
...
Se você já tem AEXOS instalado e quer fazer upgrade:
...
npm view aexos-core@latest version
# Output: 1.0.0-rc.10
...
O futuro comando `aexos init` permitirá escolher explicitamente entre dois modos:

#### Framework Development Mode
- Para desenvolvedores contribuindo ao AEXOS-FullStack
- `.aexos-core/` é código fonte (commitado)
- Mudanças afetam o framework
- Quality gates testam integridade do framework

#### Project Development Mode
- Para desenvolvedores usando AEXOS em seus projetos
- `.aexos-core/` é dependência (gitignored)
...
# Cria .aexos-installation-config.yaml
...
O AEXOS (Cyryx) usa uma abordagem de duas fases que separa planejamento estratégico de implementação tática:
...
O agente **qa** do AEXOS (Cyryx) inclui capacidades avançadas de arquitetura de testes:
...
2. **Recursos especiais**:
   - Reconhecimento automático de comandos AEXOS
   - Integração com workflows e tasks
   - Rastreamento automático de checkboxes

## Sistema de Preferências Técnicas

O AEXOS (Cyryx) permite configurar preferências técnicas que influenciam decisões dos agentes:
...
O AEXOS-FullStack impõe integridade de repositório através de um agente centralizado de DevOps.
...
```bash
# No repositório aexos-core
@github-devops
*detect-repo
# Output:
# Repository: github.com/CyryxLabs/AEXOS
# Mode: framework-development
# Quality Gates: Testes do framework AEXOS
```
...
O AEXOS-FullStack oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
Ao integrar AEXOS (Cyryx) em projetos existentes:
...
O AEXOS (Cyryx) inclui meta-agentes para orquestração:

### aexos-master

O **aexos-master** é o agente de orquestração principal:
...
O **aexos-orchestrator** gerencia fluxos de trabalho:
...
O **aexos-developer** é o meta-agente para o próprio AEXOS:
...
[aexos-developer guiará você na criação de um novo agente]
```
...
O AEXOS (Cyryx) suporta squads para domínios específicos:
...
- **aexos-infrastructure-devops** - Utilities de DevOps e infraestrutura
...
Use o **aexos-developer** para criar agentes customizados:
...
Adicione templates próprios em `.aexos-core/product/templates/`:
...
- [ ] IDE configurado com regras AEXOS
...
- [Arquitetura do AEXOS](../docs/architecture.md)
