<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/api/squads-api.md
...
> **Interface pública de Squads — availability boundary**
>
> O AEXOS não publica neste repositório um endpoint hospedado de Squads nem um emissor verificado de chaves de API.
>
> **Portable content retained:** A interface pública disponível é local: ela carrega e valida manifestos de squads dentro do projeto.
>
> **Unsupported prerequisite:** O endpoint privado e o fluxo de chaves descritos pelo documento de origem não estão disponíveis e não devem ser reutilizados.
<!-- Tradução: PT-BR | Original: /docs/en/api/squads-api.md | Sincronização: 2026-01-26 -->

# Referência da API de Squads

API REST para sincronizar squads com Cyryx Labs e descobrir squads do marketplace.

## Visão Geral

A API de Squads permite:

- **Sync**: Enviar squads locais para a nuvem Cyryx Labs
- **Marketplace**: Descobrir e navegar squads públicos
- **Gerenciamento**: Atualizar visibilidade, excluir squads

**URL Base**: `AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE`
...
Obtenha sua chave de API em: AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE

## Endpoints

### Sincronizar Squad

Enviar uma definição de squad para Cyryx Labs.
...
```bash
curl -X POST AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE \
...
curl AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE
...
```bash
curl -X PATCH AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE \
  -H "Authorization: Bearer sk_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{"isPublic": true}'
```
...
Remover um squad do Cyryx Labs.
...
```bash
curl -X DELETE AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE \
  -H "Authorization: Bearer sk_your_api_key"
```
...
```json
{
  ...
  "data": {
    "valid": true,
    "errors": [],
    "warnings": ["Missing recommended field: description", "Missing aexos.minVersion field"]
  }
}
```
...
{
  "info": {
    "name": "Cyryx Labs Squads API",
    "description": "API REST para o Marketplace de Squads Cyryx Labs",
    "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  "variable": [
    {
      "key": "baseUrl",
      "value": "AEXOS_HOSTED_SQUADS_ENDPOINT_UNAVAILABLE"
    },
    {
      "key": "apiKey",
