<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/ENVIRONMENT.md
...
Este documento lista todas as variáveis de ambiente usadas pelo AEXOS (Cyryx) e seus componentes.

## Visão Geral

O AEXOS (Cyryx) usa variáveis de ambiente para configuração, chaves de API e informações sensíveis. **Nunca faça commit de variáveis de ambiente no repositório.**

## Variáveis de Ambiente Obrigatórias

### Core do Framework

Atualmente, o AEXOS (Cyryx) não requer nenhuma variável de ambiente obrigatória para operação básica. Toda a configuração é feita através do `core-config.yaml` e arquivos de configuração dos Squads.
...
Squads privados (no repositório `aexos-Squads`) podem requerer variáveis de ambiente adicionais. Consulte a documentação de cada Squad.
...
O AEXOS (Cyryx) usa `dotenv` (se instalado) ou o suporte nativo a variáveis de ambiente do Node.js. As variáveis de ambiente são carregadas automaticamente dos arquivos `.env` na raiz do projeto.
