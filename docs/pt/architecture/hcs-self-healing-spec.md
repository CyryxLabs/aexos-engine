<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/architecture/hcs-self-healing-spec.md
...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do AEXOS (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.aexos/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.aexos/cache/`     | Sim           |
...
- id: PC-001
  description: '.aexos/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .aexos/config.yaml (backup: .aexos/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .aexos/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx aexos task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx aexos task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .aexos/)
...
safeToAutoFix:
  paths:
    - '.aexos/**' # Arquivos de workspace do AEXOS
    - '.aexos-core/**/*.yaml' # YAML do framework (cuidado)
