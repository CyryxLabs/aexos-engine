<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/guides/mcp-global-setup.md
...
# Guia de Configuração Global MCP do AEXOS

> 🌐 [EN](../../guides/mcp-global-setup.md) | **PT** | [ES](../../es/guides/mcp-global-setup.md)

---

> Configure servidores MCP (Model Context Protocol) globais para AEXOS (Cyryx).
...
O Sistema Global MCP permite que você configure servidores MCP uma única vez e os compartilhe entre todos os projetos AEXOS. Isso elimina a necessidade de configurar os mesmos servidores em cada projeto.
...
- `~/.aexos/` - Diretório global do AEXOS
- `~/.aexos/mcp/` - Diretório de configuração MCP
- `~/.aexos/mcp/global-config.json` - Arquivo de configuração principal
- `~/.aexos/mcp/servers/` - Configurações individuais de servidores
- `~/.aexos/mcp/cache/` - Cache de respostas
- `~/.aexos/credentials/` - Armazenamento seguro de credenciais
...
O AEXOS inclui templates para servidores MCP populares:
...
Credenciais são armazenadas em `~/.aexos/credentials/` com um `.gitignore` para prevenir commits acidentais.
...
| Diretório já existe    | Use `aexos mcp setup --force` para recriar                                 |
...
_Synkra AEXOS v4 Guia de Configuração Global MCP_
