<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/guides/security-hardening.md
...
-->
# Guia de Hardening de Segurança do AEXOS

> [EN](../../guides/security-hardening.md) | **PT** | [ES](../../es/guides/security-hardening.md)

---

> Guia completo para fortalecer a segurança de implantações do AEXOS (Cyryx) - do desenvolvimento à produção.
...
O AEXOS (Cyryx) opera em uma camada privilegiada entre modelos de IA e seu sistema. Este guia cobre estratégias de hardening específicas para ambientes de desenvolvimento orquestrados por IA.
...
### Preocupações de Segurança Específicas do AEXOS
...
O AEXOS implementa múltiplas camadas de proteção:
...
Chaves de API são os segredos mais críticos no AEXOS. Chaves comprometidas podem levar a uso não autorizado, violações de dados e impacto financeiro significativo.
...
```bash
# ============================================================
# CONFIGURAÇÃO DE AMBIENTE DO AEXOS
# ============================================================
...
### Permissões da Estrutura de Diretórios do AEXOS
...
# Diretórios de configuração do AEXOS
chmod 700 .aexos/              # Apenas o proprietário pode acessar
chmod 700 .aexos-core/         # Fonte do framework
chmod 700 .claude/            # Configuração do Claude

# Arquivos de configuração sensíveis
chmod 600 .env                # Variáveis de ambiente
chmod 600 .aexos/config.yaml   # Configuração principal
chmod 600 .aexos/users.json    # Banco de dados de usuários
chmod 600 .aexos/sessions.json # Sessões ativas
...
```yaml
# .aexos/config.yaml - Configuração de diretórios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificação de Permissões de Segurança do AEXOS"
echo "==============================================="
...
O AEXOS usa containers Docker para isolar servidores MCP do sistema host:
...
```yaml
# .aexos/config.yaml - Configuração de auditoria
audit:
  enabled: true
...
function validateProductionSecurity() {
  ...
  if (process.env.AEXOS_DEBUG === 'true') {
    errors.push('AEXOS_DEBUG deve ser false em produção');
  }
...
Se você descobrir uma vulnerabilidade de segurança no AEXOS (Cyryx), por favor siga práticas de divulgação responsável:

### Processo de Relatório

1. **NÃO** crie uma issue pública no GitHub para vulnerabilidades de segurança
2. Reporte preocupações de segurança via [GitHub Security Advisories](https://github.com/CyryxLabs/aexos-engine/security/advisories)
...
Atualmente, o AEXOS (Cyryx) não possui um programa formal de bug bounty. No entanto, contribuições significativas de segurança são reconhecidas e podem receber licenças AEXOS Pro ou outro reconhecimento.
...
_Guia de Hardening de Segurança do AEXOS (Cyryx) v4.0_
