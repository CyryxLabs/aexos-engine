<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/guides/project-status-feature.md
...
A funcionalidade de Status do Projeto exibe automaticamente seu contexto de trabalho atual quando qualquer agente AEXOS é ativado. Isso inclui:
...
- **Framework AEXOS-FullStack** instalado
...
3. Criar arquivo de cache `.aexos/project-status.yaml`
...
1. Edite `.aexos-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Localização: `.aexos-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .aexos/project-status.yaml  # Localização do arquivo de cache
...
1. **Verifica cache** - Procura por `.aexos/project-status.yaml`
...
- **Localização do Cache:** `.aexos/project-status.yaml`
...
Todos os 11 agentes AEXOS exibem status do projeto:
...
11. **@aexos-master** (Orion - Orchestrator)
...
3. O arquivo `.aexos-core/infrastructure/scripts/project-status-loader.js` existe?
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.aexos-core/infrastructure/scripts/project-status-loader.js');
...
const { clearCache } = require('./.aexos-core/infrastructure/scripts/project-status-loader.js');
await clearCache();
```
...
```bash
# Remove script
rm .aexos-core/infrastructure/scripts/project-status-loader.js

# Remove task
rm .aexos-core/tasks/init-project-status.md

# Remove cache
rm .aexos/project-status.yaml

# Remove tests
rm .aexos-core/infrastructure/scripts/__tests__/project-status-loader.test.js
...
R: Sim, todos os agentes usam o mesmo arquivo de cache (`.aexos/project-status.yaml`).

---

## Documentação Relacionada

- **Story:** `docs/stories/aexos migration/story-6.1.2.4-project-status-context.md`
- **Config:** `.aexos-core/core-config.yaml` (seção projectStatus)
- **Script:** `.aexos-core/infrastructure/scripts/project-status-loader.js`
- **Task de Init:** `.aexos-core/tasks/init-project-status.md`
- **Testes:** `.aexos-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
