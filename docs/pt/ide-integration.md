<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/pt/ide-integration.md
...
Guia para integrar o AEXOS com IDEs e plataformas de desenvolvimento com IA suportadas.
...
O AEXOS suporta 6 plataformas de desenvolvimento com IA. Escolha a que melhor se adapta ao seu fluxo de trabalho.
...
| IDE | Paridade de Hooks vs Claude | O que degrada sem hooks completos | Mitigação no AEXOS |
| --- | --- | --- | --- |
| Claude Code | Completa | Nenhum (comportamento de referência) | Hooks nativos + pipeline completo do AEXOS |
...
**Nível de Recomendação:** Melhor integração com AEXOS
...
1. AEXOS cria automaticamente o diretório `.claude/` durante a inicialização
...
4. Use `/skills` e selecione `aexos-<agent-id>`
...
1. AEXOS cria o diretório `.cursor/` durante a inicialização
...
2. AEXOS cria `.github/copilot-instructions.md`
...
1. AEXOS cria o diretório `.antigravity/`
...
O AEXOS mantém uma única fonte de verdade para definições de agentes e as sincroniza com todas as IDEs configuradas:
...
O AEXOS pode ser configurado para sincronizar automaticamente quando houver mudanças nos agentes:
...
├── Sim --> Claude Code (Melhor integração com AEXOS)
└── Não
...
_Guia de Integração com IDEs do AEXOS (Cyryx) v4.0_
