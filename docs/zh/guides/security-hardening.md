<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/security-hardening.md
...
> **安全报告说明 — availability boundary**
>
> 源文档中的安全邮箱属于上游项目，不能作为 AEXOS 联系方式重新标记。
>
> **Portable content retained:** 请使用此仓库规范安全政策中列出的当前报告渠道。
>
> **Unsupported prerequisite:** 此本地化页面不会虚构电子邮件地址或响应承诺。
# AEXOS安全加固指南
...
> 完整指南，用于加固AEXOS (Cyryx)部署的安全性 - 从开发到生产。
...
AEXOS (Cyryx)在AI模型和您的系统之间的特权层运行。本指南涵盖特定于AI编排开发环境的加固策略。
...
API密钥是AEXOS中最关键的机密。受损密钥可能导致未授权使用、数据泄露和重大财务影响。
...
# AEXOS配置目录
chmod 700 .aexos/              # 仅所有者可访问
chmod 700 .aexos-core/         # 框架源
chmod 700 .claude/            # Claude配置

# 敏感配置文件
chmod 600 .env                # 环境变量
chmod 600 .aexos/config.yaml   # 主配置
chmod 600 .aexos/users.json    # 用户数据库
chmod 600 .aexos/sessions.json # 活跃会话
...
如果您在AEXOS (Cyryx)中发现安全漏洞，请遵循负责任的披露做法：

### 报告流程

1. **不要**为安全漏洞创建公共GitHub问题
2. 将安全问题发送至: **UPSTREAM_SECURITY_CONTACT_UNAVAILABLE**
3. 在报告中包括以下内容:
   - 漏洞描述
   - 复现步骤
   - 潜在影响评估
...
目前，AEXOS (Cyryx)没有正式的Bug赏金计划。但是，重大安全贡献会被认可，可能会获得AEXOS Pro许可或其他认可。
