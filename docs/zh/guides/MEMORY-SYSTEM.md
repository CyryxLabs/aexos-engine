<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/MEMORY-SYSTEM.md
...
AEXOS 的记忆系统在**两个独立的层**上运行，它们共存但**彼此不通信**：

| 层 | 管理者 | 范围 |
|--------|---------------|--------|
| **第一层: Claude Code 原生** | Claude Code CLI | 自动记忆, CLAUDE.md, 会话转录 |
| **第二层: AEXOS 框架** | `.aexos-core/` 中的 JS 脚本 | Gotchas, 会话状态, 上下文快照, Timeline |
...
flowchart TB
    subgraph CLAUDE_NATIVE["第一层: Claude Code 原生"]
        ...
        CLAUDE_MD_P["CLAUDE.md 项目<br/>aexos-core/.claude/CLAUDE.md"]
    ...
    subgraph AEXOS_FRAMEWORK["第二层: AEXOS 框架"]
        direction TB
        GOTCHAS["Gotchas 记忆<br/>.aexos-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["上下文快照<br/>.aexos-core/core/memory/context-snapshot.js"]
        FILE_EVO["文件演化追踪器<br/>.aexos-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.aexos-core/core/memory/timeline-manager.js"]
        CTX_LOADER["会话上下文加载器<br/>.aexos-core/core/session/context-loader.js"]
        CTX_DETECT["上下文检测器<br/>.aexos-core/core/session/context-detector.js"]
        SESS_STATE["会话状态<br/>.aexos-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.aexos-core/hooks/gemini/"]
        HOOKS_UNI["统一 Hooks<br/>.aexos-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_AEXOS["AEXOS 存储 (.aexos/)"]
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["记忆层 (.aexos-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["会话层 (.aexos-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["编排 (.aexos-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["激活管道 (.aexos-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks 系统 (.aexos-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["自动加载 — 固定顺序"]
        ...
        LOAD_PROJECT["3. aexos-core/.claude/CLAUDE.md<br/>项目级指令"]
        LOAD_RULES["4. aexos-core/.claude/rules/*.md<br/>5 个详细规则文件"]
...
flowchart LR
    ...
    NOTE["注意: 只有定义在<br/>.claude/agents/ 中的代理<br/>使用此记忆。<br/>AEXOS 代理 (.aexos-core/<br/>development/agents/)<br/>不使用。"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("持久化到:<br/>.aexos/gotchas.json<br/>.aexos/gotchas.md<br/>.aexos/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("持久化到:<br/>.aexos/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("持久化到:<br/>.aexos/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — 统一门面"]
        ...
        T4[("持久化到:<br/>.aexos/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD2{"有对话<br/>历史?"}
        CD3["分析最近命令"]
        CD4["读取 .aexos/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("持久化到:<br/>.aexos/session-state.json<br/>TTL: 1 小时")]
...
sequenceDiagram
    ...
    Note over CC: 没有 AEXOS 记忆<br/>自动加载<br/>到 system prompt。<br/>仅会话上下文<br/>出现在 greeting 中。
```
...
flowchart TD
    ...
    subgraph SAVES["保存的内容"]
        S1[".aexos/session-state.json<br/>当前代理, 命令, 工作流"]
        S2[".aexos/error-tracking.json<br/>错误发生"]
        S3[".aexos/gotchas.json<br/>新自动捕获的 gotcha"]
        S4[".aexos/gotchas.json<br/>添加的手动 gotcha"]
        S5["docs/stories/.session-state.yaml<br/>更新的阶段"]
        S6["docs/stories/.session-state.yaml<br/>Story 标记完成"]
        S7[".aexos/snapshots/{id}.json<br/>捕获的上下文"]
        S8[".aexos/file-evolution/<br/>记录的演化"]
    end
...
sequenceDiagram
    ...
    CL->>FS: 读取 .aexos/session-state.json
    FS-->>CL: 当前状态 (或空)
    ...
    CL->>FS: 写入 .aexos/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["自动捕获流程"]
        ...
        TRACK --> READ_ET["读取 .aexos/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["保存 .aexos/gotchas.json"]
        SAVE_JSON --> SAVE_MD["生成 .aexos/gotchas.md"]
...
flowchart TD
    subgraph CAPTURE["捕获 — context-snapshot.js"]
        ...
        C4 --> C5["保存 .aexos/snapshots/{id}.json"]
        C5 --> C6{"autoCleanup?"}
        C6 -->|"是"| C7["移除快照:<br/>- > 50 个存在<br/>- > 7 天"]
    end

    subgraph RESTORE["恢复 — 恢复"]
        R1(["snapshot.restore(id)"]) --> R2["读取 .aexos/snapshots/{id}.json"]
        R2 --> R3["返回完整上下文"]
        R3 --> R4["代理可以从<br/>上次位置恢复"]
    end

    subgraph LIST["列出 — 查询"]
        L1(["snapshot.list()"]) --> L2["列出 .aexos/snapshots/ 中所有"]
        L2 --> L3["按时间戳排序"]
        L3 --> L4["返回元数据数组"]
    end
...
```mermaid
flowchart LR
    subgraph AEXOS_EVENTS["AEXOS 事件"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
| 文件 | 模块 | Story/Epic | 功能 |
|---------|--------|------------|--------|
| `.aexos-core/core/memory/gotchas-memory.js` | 记忆 | Epic 9, Story 9.4 | 自动捕获重复错误, 手动 gotchas, 注入任务 |
| `.aexos-core/core/memory/context-snapshot.js` | 记忆 | Story 12.6 | 捕获和恢复开发上下文 |
| `.aexos-core/core/memory/file-evolution-tracker.js` | 记忆 | Gap impl | 追踪文件演化, 检测漂移 |
| `.aexos-core/core/memory/timeline-manager.js` | 记忆 | Gap impl | 跨会话统一 timeline 门面 |
| `.aexos-core/core/session/context-loader.js` | 会话 | Story 2.2, 6.1.2.5 | 代理间连续性, 上下文交接 |
| `.aexos-core/core/session/context-detector.js` | 会话 | Story 2.2 | 混合会话类型检测 (new/existing/workflow) |
| `.aexos-core/core/orchestration/session-state.js` | 编排 | Story 11.5 | Epic/story 持久状态, 崩溃恢复 |
| `.aexos-core/core/orchestration/context-manager.js` | 编排 | Legacy | 阶段间工作流状态 (迁移到 session-state) |
| `.aexos-core/core/elicitation/session-manager.js` | Elicitation | — | Elicitation 会话保存/加载 |

### 激活脚本 (消费记忆)

| 文件 | 功能 |
|---------|--------|
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | 主编排器 — 在 Tier 3 加载会话 |
| `.aexos-core/development/scripts/greeting-builder.js` | 构建带会话/记忆上下文的 greeting |

### Hooks (会话持久化)

| 文件 | CLI | 功能 |
|---------|-----|--------|
| `.aexos-core/hooks/gemini/session-start.js` | Gemini | 会话开始时加载 AEXOS 上下文 |
| `.aexos-core/hooks/gemini/session-end.js` | Gemini | 将会话摘要持久化到 `.aexos/sessions/` |
| `.aexos-core/hooks/gemini/before-agent.js` | Gemini | 代理前预处理 |
| `.aexos-core/hooks/gemini/before-tool.js` | 两者 | 工具前预处理 |
| `.aexos-core/hooks/gemini/after-tool.js` | 两者 | 工具后后处理 |
| `.aexos-core/hooks/unified/hook-interface.js` | 两者 | UnifiedHook 基类 + EVENT_MAPPING |
| `.aexos-core/hooks/unified/hook-registry.js` | 两者 | hooks 中央注册表 |
| `.aexos-core/hooks/unified/index.js` | 两者 | 统一系统入口点 |
...
| `aexos-core/.claude/CLAUDE.md` | 项目指令 (始终加载) |
| `aexos-core/.claude/rules/*.md` | 5 个规则文件 (始终加载) |
...
AEXOS 模块 (gotchas, snapshots, timeline) 在会话结束时**不会自动 flush**。如果进程突然终止，内存中的数据可能丢失。
...
Claude 自动记忆 (`MEMORY.md`) 和 AEXOS 记忆 (`.aexos/`) 从不同步。AEXOS 捕获的 Gotchas 不会出现在 MEMORY.md 中，反之亦然。
...
| Gotchas 记忆脚本 | `.aexos-core/core/memory/gotchas-memory.js` |
| 上下文快照脚本 | `.aexos-core/core/memory/context-snapshot.js` |
| 文件演化追踪器 | `.aexos-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.aexos-core/core/memory/timeline-manager.js` |
| 会话上下文加载器 | `.aexos-core/core/session/context-loader.js` |
| 上下文检测器 | `.aexos-core/core/session/context-detector.js` |
| 会话状态 | `.aexos-core/core/orchestration/session-state.js` |
| 统一 Hook 接口 | `.aexos-core/hooks/unified/hook-interface.js` |
| Hook 注册表 | `.aexos-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.aexos-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.aexos-core/hooks/gemini/session-end.js` |
| Core 配置 | `.aexos-core/core-config.yaml` |
