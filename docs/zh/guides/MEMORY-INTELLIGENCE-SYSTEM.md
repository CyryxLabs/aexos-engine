<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/MEMORY-INTELLIGENCE-SYSTEM.md
...
> **内存系统可用性说明 — availability boundary**
>
> 源文档依赖无法访问的私有 Pro 运行时，因此不能将其描述为此开源仓库中可执行的功能。
>
> **Portable content retained:** 公开的 Core/Pro 扩展边界仍保留；当前核心内存行为请参阅本地内存系统文档。
>
> **Unsupported prerequisite:** 私有运行时的实现、安装和运行行为不在此仓库中，也未在本次恢复中验证。
# 记忆智能系统 - 架构愿景
...
**最后更新:** 2026-02-09
**作者:** @architect (Aria)
**Epic:** MIS — 记忆智能系统 (`UPSTREAM_REFERENCE_UNAVAILABLE`)
**基于:** MIS-1 调研 (`UPSTREAM_REFERENCE_UNAVAILABLE`)
...
> **架构:** MIS 遵循 AEXOS 的 **Open Core** 模型。所有记忆智能都位于私有仓库 `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE`（子模块 `pro/`）中。`aexos-core`（开源）仅提供扩展点，并且在没有智能记忆的情况下 100% 正常运行 — 与现在完全一样。
...
当前记忆系统在**两个断开的层**（Claude Code 原生 + AEXOS 框架）上运行，存在关键缺口：
...
| 层 | 功能 | 仓库 | Story |
|--------|--------|-------------|-------|
| **清理** | 移除死代码，准备基础 | **aexos-core** | MIS-2 |
| **捕获** | 通过 hooks 捕获会话知识 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-3 |
| **存储** | 使用带 YAML frontmatter 的 Markdown 存储 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-3, MIS-4 |
| **检索** | 按相关性渐进式检索 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** + **aexos-core** 扩展点 | MIS-4, MIS-6 |
| **演化** | 自动学习和演化规则 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-5, MIS-7 |

### 关键原则 (vs 当前状态)

| 方面 | 当前状态 | 使用 MIS (AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE 激活) | 没有 MIS (仅 aexos-core) |
|---------|-------------|--------------------------|--------------------------|
...
> **aexos-core 在没有 MIS 的情况下 100% 正常运行** — 与现在的运行方式完全相同。当 `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` 存在时（子模块 `pro/`），MIS 通过扩展点自动连接。无需手动配置。

### Open Core 模型

```mermaid
flowchart LR
    subgraph CORE["aexos-core (开源, MIT)"]
        direction TB
    ...
    subgraph PRO["AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE (私有, 子模块 pro/)"]
        direction TB
...
aexos-core 已有模块 `bin/utils/pro-detector.js` (Story PRO-5):
...
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | 修改 | 在 Tier 2 添加 `if (isProAvailable())` 用于记忆加载 |
| `.aexos-core/development/scripts/greeting-builder.js` | 修改 | 如果 pro 可用，在 greeting 中添加记忆统计 |
| `.aexos-core/core/memory/gotchas-memory.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
| `.aexos-core/core/session/context-loader.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
| `.claude/settings.json` | 修改 | 调用 `pro/` 脚本的 Hook 桩（如果不存在则优雅失败） |

#### AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE (私有) — 所有智能
...
Claude Code 的 hooks 在 `.claude/settings.json` (aexos-core) 中配置。指向的脚本位于 `pro/`:
...
```json
{
  "hooks": {
    "PreCompact": [
      {
        "type": "command",
        "command": "node .aexos-core/hooks/pro-hook-runner.js PreCompact",
        "timeout": 10000
      }
    ]
  }
...
```javascript
// .aexos-core/hooks/pro-hook-runner.js (在 core 中, ~20 行)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["aexos-core (开源)"]
        direction TB
        EXT["管道中的<br/>扩展点"]
        HOOK_RUNNER[".aexos-core/hooks/<br/>pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js<br/>(独立)"]
        CTX_LOADER["context-loader.js<br/>(独立)"]
    end

    subgraph PRO_BOUNDARY["AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE (私有 — pro/)"]
        ...
        subgraph STORAGE["第二层: 存储"]
            ST1[".aexos/memories/shared/"]
            ST2[".aexos/memories/{agent}/"]
            ST3[".aexos/memories/index.json"]
        end
...
```mermaid
flowchart LR
    subgraph CORE["aexos-core"]
        HR[".aexos-core/hooks/<br/>pro-hook-runner.js"]
        UAP["unified-activation-<br/>pipeline.js"]
        GM["gotchas-memory.js"]
        PD["bin/utils/<br/>pro-detector.js"]
    end

    subgraph PRO["AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE (pro/)"]
...
> **仓库:** `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` (pro/memory/)
> **core 中的扩展点:** `.aexos-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (aexos-core — 已提交)
{
  "hooks": {
    "PreCompact": [
      {
    ...
    "Stop": [
      {
        "type": "command",
        "command": "node .aexos-core/hooks/pro-hook-runner.js Stop",
        "timeout": 5000
      }
    ],
    "PostToolUseFailure": [
      {
        "type": "command",
        "command": "node .aexos-core/hooks/pro-hook-runner.js PostToolUseFailure",
        "timeout": 3000
      }
    ]
  }
...
```mermaid
flowchart TD
    subgraph CORE_HOOKS["aexos-core"]
        HR["pro-hook-runner.js"]
    end

    subgraph PRO_CAPTURE["AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("持久化到<br/>.aexos/memories/")]
    end
...
> **仓库:** `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` (pro/memory/) 管理存储
> **存储位置:** `.aexos/memories/` (gitignored, 由 pro 创建)
...
```
.aexos/memories/                          # 由 pro/memory/ 创建和管理
  shared/                                # 跨代理记忆
    durable/                             # 永久知识
...
> **仓库:** `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **仓库:** `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` (pro/memory/self-learner.js, rule-proposer.js)
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js<br/>(aexos-core)
    participant PD as pro-detector.js<br/>(aexos-core)
    participant SD as session-digest.js<br/>(AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE)
    participant FS as .aexos/memories/
...
> **仓库:** `AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE` (pro/memory/agent-memory-api.js)
...
每个代理访问：
1. 自己的私有记忆 (`.aexos/memories/{agent}/`)
2. 共享记忆 (`.aexos/memories/shared/`)
3. **绝不** 其他代理的私有记忆
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline<br/>(aexos-core)
    participant PD as pro-detector.js<br/>(aexos-core)
    participant ML as memory-loader.js<br/>(AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE)
    participant MR as memory-retriever.js<br/>(AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE)
    participant GB as GreetingBuilder<br/>(aexos-core)
...
> **100% 在 AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE 中** — core 中没有自学习组件
...
**没有 pro:** 仅 `[CORE]` 和 `[原生]` 存在。`[PRO]` (.aexos/memories/) 永远不会创建。
...
| `.aexos-core/hooks/pro-hook-runner.js` | **新建** | MIS-3 | 包装器: 检查 pro, 委托 hook (~20 行) |
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | **修改** | MIS-6 | 在 Tier 2 添加 `if (isProAvailable())` |
| `.aexos-core/development/scripts/greeting-builder.js` | **修改** | MIS-6 | 如果 pro 可用，在 greeting 中显示记忆统计 |
| `.aexos-core/core/memory/gotchas-memory.js` | **路径修复** | MIS-2 | 修正 `.aexos/error-tracking.json` |
| `.aexos-core/core/session/context-loader.js` | **路径修复** | MIS-2 | 修正 `.aexos/session-state.json` |
...
### 删除的文件 (MIS-2, aexos-core)

| 文件 | 行数 | 原因 |
|---------|--------|-------|
| `.aexos-core/core/memory/timeline-manager.js` | 746 | 0 个消费者 (孤立) |
| `.aexos-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 个消费者 (孤立) |
| `.aexos-core/core/memory/context-snapshot.js` | 648 | 0 个消费者 (孤立) |
| `.aexos-core/elicitation/session-manager.js` | — | 死代码 |
| `.aexos/compound-analysis/` | ~14K | 0 个读取者 (孤立数据) |

---

## 差异对比: 当前状态 vs MIS

| 方面 | 当前 aexos-core | aexos-core + AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE (MIS) | 没有 pro 的 aexos-core |
|---------|---------------|---------------------------|------------------|
...
gantt
    ...
    section aexos-core + AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE
    MIS-3 会话摘要                 :mis3, after mis1, 3d
    MIS-6 管道集成           :mis6, after mis4, 2d

    section AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE
...
| Story | 标题 | 仓库 | 依赖 | 工时 |
|-------|--------|-------------|------------|-------|
| MIS-1 | 调研与架构设计 | docs (aexos-core) | — | 12h (完成) |
| MIS-2 | 死代码清理与路径修复 | **aexos-core** | MIS-1 | 4h |
| MIS-3 | 会话摘要 (PreCompact Hook) | **aexos-core** (hook runner) + **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** (digest) | MIS-1 | 14h |
| MIS-4 | 渐进式记忆检索 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-3 | 16h |
| MIS-5 | 自学习引擎 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-3, MIS-4 | 14h |
| MIS-6 | 管道集成与代理记忆 API | **aexos-core** (扩展点) + **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md 与规则自动演化 | **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE** | MIS-5 | 8h |
| **总计** | | | | **~78h** |
...
| Epic MIS 索引 | EPIC-MIS-INDEX.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| MIS-1 调研 | story-mis-1-investigation.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| MIS-2 死代码清理 | story-mis-2-dead-code-cleanup.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| Epic PRO 架构 | EPIC-PRO-INDEX.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| ADR-PRO-001 仓库策略 | adr-pro-001-repository-strategy.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| ADR-PRO-003 Feature Gating | adr-pro-003-feature-gating-licensing.md (`UPSTREAM_REFERENCE_UNAVAILABLE`) |
| Pro Detector | [bin/utils/pro-detector.js](../../../bin/utils/pro-detector.js) |
| Feature Registry | pro/feature-registry.yaml (`UPSTREAM_REFERENCE_UNAVAILABLE`) |

---

*AEXOS 记忆智能系统 — 架构愿景 v4.0.4 (Core/Pro 分离)*
*Epic MIS 完成后的目标状态 (7 个 stories, ~78 小时)*
*aexos-core: 扩展点 + 死代码清理*
*AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE: 所有记忆智能 (捕获, 存储, 检索, 演化)*
*@architect (Aria) — 架构未来*
