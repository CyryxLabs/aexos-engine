<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/api-reference.md
...
AEXOS (Cyryx) 完整 API 参考 - AI 编排全栈开发系统。
...
AEXOS 通过两个主要机制提供统一 API 来与专业 AI 代理交互：
...
| `@aexos-master`   | Orion  | Orchestrator| 框架编排、元操作             |

### 激活行为

激活代理时：

1. 从 `.aexos-core/development/agents/{id}.md` 加载代理定义文件
2. 采用角色（语气、词汇、问候）
3. 基于会话类型显示上下文问候
4. 代理停止并等待用户输入
...
| 类别                 | 描述                         | 解决方案                           |
| -------------------- | ---------------------------- | ---------------------------------- |
| `AGENT_NOT_FOUND`    | 缺少代理定义                 | 检查 `.aexos-core/development/agents/` |
...
/aexos-master  → 激活 @aexos-master 代理
```
