<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/MEMORY-INTEGRATION.md
...
集成遵循 AEXOS Open Core 模型:
- **aexos-core:** UnifiedActivationPipeline 中的扩展点 (本指南)
- **AEXOS_PRO_PRIVATE_RUNTIME_UNAVAILABLE:** 记忆智能实现 (检索, 评分, 学习)
...
```yaml
# .aexos-core/development/agents/dev.md
agent:
  id: dev
  config:
...
```javascript
isProAvailable() === true
// 且 .aexos/session-digests/ 包含记忆摘要
```
...
// 检查摘要目录
const fs = require('fs');
const digests = fs.readdirSync('.aexos/session-digests');
...
**解决方案:**
1. 减少摘要数量 (将旧摘要归档到 `.aexos/session-digests/archive/`)
