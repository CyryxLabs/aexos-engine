<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/guides/project-status-feature.md
...
项目状态功能在任何AEXOS Agent激活时自动显示你当前的工作上下文。这包括：
...
3. 创建 `.aexos/project-status.yaml` 缓存文件
...
1. 编辑 `.aexos-core/core-config.yaml`：
   ```yaml
   projectStatus:
     enabled: true
...
位置: `.aexos-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .aexos/project-status.yaml  # 缓存文件位置
...
1. **检查缓存** - 查找 `.aexos/project-status.yaml`
2. **验证TTL** - 缓存 < 60秒？
...
- **缓存TTL:** 60秒（可配置）
- **缓存位置:** `.aexos/project-status.yaml`
...
3. `.aexos-core/infrastructure/scripts/project-status-loader.js` 存在吗？
4. Agent激活输出中有错误吗？
...
**Q: 缓存在agent之间共享吗?**
A: 是的，所有agent使用相同的缓存文件 (`.aexos/project-status.yaml`)。
...
- **配置:** `.aexos-core/core-config.yaml` (projectStatus部分)
- **脚本:** `.aexos-core/infrastructure/scripts/project-status-loader.js`
- **Init Task:** `.aexos-core/tasks/init-project-status.md`
- **测试:** `.aexos-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
