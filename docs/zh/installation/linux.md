<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/installation/linux.md
...
```bash
# 克隆仓库
git clone https://github.com/CyryxLabs/aexos-engine.git ~/.aexos-core-source
...
# 为您的项目运行安装程序
node bin/aexos-init.js ~/projects/my-project
```
...
2. 命令安装到 `.claude/commands/AEXOS/`
3. 使用 `/agent-name` 激活代理

### VS Code（使用 Continue 扩展）

1. 安装 Continue 扩展
2. 在 `.continue/` 中配置 AEXOS 规则
...
# AEXOS 配置
export AEXOS_HOME="$HOME/.aexos-core"
export PATH="$AEXOS_HOME/bin:$PATH"
...
AEXOS (Cyryx) 遵守 XDG 目录：

```bash
# 数据文件：~/.local/share/aexos/
# 配置文件：~/.config/aexos/
# 缓存：~/.cache/aexos/
# 状态：~/.local/state/aexos/
```
...
git pull
npm install
node bin/aexos-init.js ~/projects/my-project --update
```

更新程序将：

- 检测现有安装
- 将任何自定义备份到 `.aexos-backup/`
- 仅更新已更改的文件
- 保留您的配置
...
```bash
# 从项目中删除 AEXOS
rm -rf .aexos-core .claude/commands/AEXOS

# 删除全局安装
rm -rf ~/.aexos-core-source ~/.npm-global/lib/node_modules/@synkra
```
...
4. 加入我们的 [Cyryx support](https://cyryxlabs.com/contact) 获取帮助
