<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/CHANGELOG.md
...
- **AEXOS 自主开发引擎 (ADE)**：完整的自主开发系统，包含 7 个 Epic：
...
- **代理同步**：所有代理现在在 `.aexos-core/development/agents/` 和 `.claude/commands/AEXOS/agents/` 之间同步
...
- **目录结构**：`.legacy-core/` 重命名为 `.aexos-core/`
...
- **IDE 配置**：Claude Code 代理移至 `.claude/commands/AEXOS/agents/`
- **文件位置**：
...
- 手动安装流程（改用 `npx @aexos/core install`）
...
1. 目录重命名：`.legacy-core/` → `.aexos-core/`
...
- [GitHub 仓库](https://github.com/CyryxLabs/aexos-engine)
- [Issue 追踪器](https://github.com/CyryxLabs/aexos-engine/issues)
