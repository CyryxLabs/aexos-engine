<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/aiox-agent-flows/squad-creator-system.md
...
<!--
  翻译：zh-CN（简体中文）
  原文：/docs/aexos-agent-flows/squad-creator-system.md
  最后同步：2026-02-22
-->
...
**小队创建者** (Craft) 是 AEXOS 用于创建、验证、发布和管理小队的专业代理。小队是代理、任务、工作流和资源的模块化包，可在项目之间重用。

该系统实现了 AEXOS 的**任务优先架构**，其中任务是执行的主要入口点，代理编排这些任务。
...
- **列出小队**项目本地
- **分发小队**在 3 个级别（本地、aexos-squads、Cyryx Labs API）
- **迁移小队**到带编排和技能的 v2 格式
- **分析和扩展**现有小队
...
4. **3 级分发**：本地、公共（aexos-squads）、市场（Cyryx Labs API）
5. **与 aexos-core 集成**：小队与框架协同工作
...
| 文件 | 目的 |
|---------|-----------|
| `.aexos-core/development/agents/squad-creator.md` | 小队创建者代理核心定义 |
| `.claude/commands/AEXOS/agents/squad-creator.md` | 用于激活 @squad-creator 的 Claude Code 命令 |

### @squad-creator 任务文件

| 文件 | 命令 | 目的 | 状态 |
|---------|---------|-----------|--------|
| `.aexos-core/development/tasks/squad-creator-create.md` | `*create-squad` | 创建带完整结构的新小队 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-design.md` | `*design-squad` | 分析文档并生成蓝图 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-validate.md` | `*validate-squad` | 验证小队针对 schema 和模式 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-list.md` | `*list-squads` | 列出本地小队 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-analyze.md` | `*analyze-squad` | 分析结构并建议改进 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-extend.md` | `*extend-squad` | 使用新组件扩展小队 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-migrate.md` | `*migrate-to-v2` | 迁移小队到 v2 格式 | 活跃 |
| `.aexos-core/development/tasks/squad-generate-skills.md` | `*generate-skills` | 生成小队知识技能 | 活跃 |
| `.aexos-core/development/tasks/squad-generate-workflow.md` | `*generate-workflow` | 生成 YAML 编排工作流 | 活跃 |
| `.aexos-core/development/tasks/squad-creator-download.md` | `*download-squad` | 从公共仓库下载小队 | 占位符（Sprint 8） |
| `.aexos-core/development/tasks/squad-creator-publish.md` | `*publish-squad` | 发布小队到 aexos-squads | 占位符（Sprint 8） |
| `.aexos-core/development/tasks/squad-creator-sync-synkra.md` | `*sync-squad-synkra` | 同步小队到 Cyryx Labs API | 占位符（Sprint 8） |

### 相关任务文件

| 文件 | 命令 | 目的 |
|---------|---------|-----------|
| `.aexos-core/development/tasks/create-agent.md` | `*create-agent` | 创建单个代理定义 |
| `.aexos-core/development/tasks/create-task.md` | `*create-task` | 创建单个任务文件 |
| `.aexos-core/development/tasks/create-workflow.md` | `*create-workflow` | 创建编排工作流 |

### 支持脚本

| 文件 | 类/函数 | 目的 |
|---------|---------------|-----------|
| `.aexos-core/development/scripts/squad/squad-generator.js` | `SquadGenerator` | 生成完整小队结构 |
| `.aexos-core/development/scripts/squad/squad-validator.js` | `SquadValidator` | 验证小队针对 schema 和模式 |
| `.aexos-core/development/scripts/squad/squad-loader.js` | `SquadLoader` | 加载和解析小队 |
| `.aexos-core/development/scripts/squad/squad-designer.js` | `SquadDesigner` | 分析文档并生成蓝图 |
| `.aexos-core/development/scripts/squad/squad-analyzer.js` | `SquadAnalyzer` | 分析小队结构 |
| `.aexos-core/development/scripts/squad/squad-extender.js` | `SquadExtender` | 扩展现有小队 |
| `.aexos-core/development/scripts/squad/squad-migrator.js` | `SquadMigrator` | 迁移小队到 v2 |
| `.aexos-core/development/scripts/squad/squad-downloader.js` | `SquadDownloader` | 从仓库下载小队 |
| `.aexos-core/development/scripts/squad/squad-publisher.js` | `SquadPublisher` | 发布小队 |

### JSON Schema

| 文件 | 目的 |
|---------|-----------|
| `.aexos-core/schemas/squad-schema.json` | squad.yaml 验证 schema |
| `.aexos-core/schemas/squad-design-schema.json` | 蓝图验证 schema |
...
flowchart TB
    ...
    subgraph DISTRIBUTE["分发"]
        LOCAL["本地<br/>./squads/"]
        PUBLIC["公共<br/>github.com/Cyryx Labs/aexos-squads"]
...
| 命令 | 任务文件 | 操作 |
|---------|-----------|----------|
| `*download-squad` | `squad-creator-download.md` | 从 aexos-squads 下载小队 |
| `*publish-squad` | `squad-creator-publish.md` | 发布小队到 aexos-squads |
| `*sync-squad-synkra` | `squad-creator-sync-synkra.md` | 同步小队到 Cyryx Labs API |
...
flowchart LR
    ...
    subgraph PUBLIC["级别 2：公共"]
        P_REPO["github.com/Cyryx Labs/aexos-squads"]
        P_DESC["社区小队（免费）"]
        P_CMD["*publish-squad"]
    end
    ...
    subgraph MARKET["级别 3：市场"]
        ...
        M_DESC["通过 Cyryx Labs API 的高级小队"]
...
- [任务：squad-creator-create.md](.aexos-core/development/tasks/squad-creator-create.md)
- [任务：squad-creator-validate.md](.aexos-core/development/tasks/squad-creator-validate.md)
- [任务：squad-creator-design.md](.aexos-core/development/tasks/squad-creator-design.md)
- [脚本：squad-generator.js](.aexos-core/development/scripts/squad/squad-generator.js)
- [脚本：squad-validator.js](.aexos-core/development/scripts/squad/squad-validator.js)
- [Schema：squad-schema.json](.aexos-core/schemas/squad-schema.json)
- [代理：squad-creator.md](.aexos-core/development/agents/squad-creator.md)
- [命令：squad-creator.md](.claude/commands/AEXOS/agents/squad-creator.md)
...
| **分发级别** | 3 个（本地、aexos-squads、Cyryx Labs API） |
