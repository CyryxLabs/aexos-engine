<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/aiox-workflows/greenfield-fullstack-workflow.md
...
**类型:** 绿地项目
**最后更新:** 2026-02-04
**源文件:** `.aexos-core/development/workflows/greenfield-fullstack.yaml`

---

## 概述

**绿地全栈工作流** 是 AEXOS 从概念到开发的主要工作流,用于构建全栈应用。本工作流既支持复杂项目的全面规划,也支持简单项目的快速原型设计。
...
flowchart TD
    subgraph FASE0["第 0 阶段: 环境引导"]
        ...
        CHECK -->|验证| ENV_REPORT[".aexos/environment-report.json 存在?"]
...
| 步骤 | 代理 | 任务 | 输入 | 输出 | 必需 |
|------|------|------|------|------|------|
| 1 | @devops (Gage) | `environment-bootstrap.md` | `project_name`, `project_path`, `github_org` | `.aexos/config.yaml`, `.aexos/environment-report.json`, `.gitignore`, `README.md`, `package.json` | 是 |

### 创建的工件

| 文件 | 描述 |
|------|------|
| `.aexos/config.yaml` | AEXOS 项目配置 |
| `.aexos/environment-report.json` | 完整的环境报告 |
| `.gitignore` | Git 忽略规则 |
| `README.md` | 初始项目文档 |
| `package.json` | NPM 配置 |
...
- 仅在项目已有 `.aexos/environment-report.json` 时跳过
- 切换机器或新团队成员加入时重新执行
...
| 阶段 | 输入 | 输出 |
|------|------|------|
| 0 | 项目名、GitHub 组织 | AEXOS 配置、Git 仓库、文件夹结构 |
...
| 阶段 | 决策点 | 选项 | 标准 |
|------|--------|------|------|
| 0 | 环境就绪? | 跳过/执行引导 | `.aexos/environment-report.json` 存在 |
...
flowchart TD
    D1{环境就绪?}
    D1 -->|检查 .aexos/environment-report.json| D1_CHECK
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 模板未找到 | 路径错误 | 检查 `.aexos-core/development/templates/` |
...
| 类型 | 文件 | 描述 |
|------|------|------|
| 工作流 | `.aexos-core/development/workflows/greenfield-fullstack.yaml` | 工作流定义 |
| 任务 | `.aexos-core/development/tasks/environment-bootstrap.md` | 环境引导 |
| 任务 | `.aexos-core/development/tasks/shard-doc.md` | 文档分片 |
| 任务 | `.aexos-core/development/tasks/sm-create-next-story.md` | 故事创建 |
| 代理 | `.aexos-core/development/agents/*.md` | 代理定义 |
| 模板 | `.aexos-core/development/templates/*.yaml` | 文档模板 |
| 检查列表 | `.aexos-core/development/checklists/*.md` | 验证检查列表 |
