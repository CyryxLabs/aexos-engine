<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/getting-started.md
...
欢迎使用 AEXOS (Cyryx)。本指南针对 10 分钟内实现首次价值进行了优化。
...
- Gemini CLI：`/aexos-menu` 然后 `/aexos-<agent>`
- Codex CLI：`/skills` 然后 `aexos-<agent-id>`
...
AEXOS 代理通过您的 IDE 激活。激活后，代理响应以 `*` 为前缀的命令：
...
并非所有 IDE 都同等支持 AEXOS 功能。请参阅 [`docs/ide-integration.md`](./ide-integration.md) 中的完整比较。
...
| Gemini CLI | 可用 | `/aexos-menu` 然后 `/aexos-<agent>` |
| Codex CLI | 有限 | `/skills` 然后 `aexos-<agent-id>` |
...
当您首次在现有项目中激活 AEXOS 代理时：

1. **检测**：AEXOS 检测到代码但没有 AEXOS 文档
...
- **非破坏性**：AEXOS 创建文件，从不覆盖现有文件
- **回滚**：`git checkout HEAD~1 -- .` 恢复 AEXOS 之前的状态
- **配置保留**：您的 `.eslintrc`、`tsconfig.json` 等保持不变

### 资源

- **[棕地项目工作指南](.aexos-core/working-in-the-brownfield.md)** - 完整的棕地项目文档
- **[兼容性检查清单](.aexos-core/development/checklists/brownfield-compatibility-checklist.md)** - 迁移前/后检查
- **[风险报告模板](.aexos-core/product/templates/brownfield-risk-report-tmpl.yaml)** - 分阶段风险评估
...
所有 AEXOS 开发都遵循 `docs/stories/` 中的故事。每个故事包含：
- 带复选框的验收标准
- 映射到特定 AC 的任务
- CodeRabbit 集成用于自动审查
- 质量门控分配
...
小队将 AEXOS 扩展到软件开发之外的任何领域。请参阅[小队指南](./guides/squads-guide.md)。
...
- **[小队指南](./guides/squads-guide.md)** - 将 AEXOS 扩展到任何领域
- **[故障排除](./troubleshooting.md)** - 常见问题和解决方案

---

_Synkra AEXOS 入门指南 v4.2.11_
