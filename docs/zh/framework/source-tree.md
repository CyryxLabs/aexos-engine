<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/framework/source-tree.md
...
- [概述](#概述)
- [模块化架构](#模块化架构)
- [框架核心 (.aexos-core/)](#框架核心-aexos-core)
- [模块详情](#模块详情)
- [文档 (docs/)](#文档-docs)
- [Squads 系统](#squads-系统)
...
AEXOS 使用**模块化架构**，具有清晰的关注点分离：

1. **框架核心** (`.aexos-core/`) - 按领域组织的可移植框架组件
...
│
├── data/                              # 共享数据
│   ├── aexos-kb.md                     # AEXOS 知识库（@aexos-master，延迟加载）
...
### 共享数据文件 (.aexos-core/data/)

| 文件 | 所有者 | 填充规则 | 更新触发器 | 使用者 |
|------|--------|----------|------------|--------|
| `aexos-kb.md` | @aexos-master | 框架重大变更时更新 | 手动编辑 | @aexos-master (延迟) |
...
### 产品数据文件 (.aexos-core/product/data/)
...
Squads 是为 AEXOS 添加专门功能的模块化扩展。与已弃用的 Squads 不同，Squads 遵循标准化的模板结构。
...
### 仓库1: Cyryx Labs/aexos-core (MIT)

```
aexos-core/
├── .aexos-core/                        # 框架资产（模块化 v2.0）
│   ├── cli/                           # CLI 命令和实用工具
│   ├── core/                          # 框架基础
│   │   ├── config/                    # 配置系统
│   │   ├── quality-gates/             # 质量验证器
...
### 仓库2: Cyryx Labs/squads (MIT)

```
squads/
├── verified/                          # AEXOS 认证的 squads
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### 仓库3: Cyryx Labs/mcp-ecosystem (Apache 2.0)
...
### 仓库4: Cyryx Labs/certified-partners (私有)
...
ADE 通过 `.aexos-core/core-config.yaml` 配置：
...
| 1.1 | 2025-12-14 | 更新组织为 Cyryx Labs，用 Squads 系统替换 Squads [Story 6.10] | Dex (dev) |
...
| 3.1 | 2026-02-06 | 添加数据文件治理部分：记录7个缺失的数据文件及其所有者、填充规则和更新触发器。扩展 .aexos-core/data/ 和 product/data/ 树列表。[Story ACT-8] | Dex (dev) |

---

_这是官方 AEXOS 框架标准。所有文件放置必须遵循此结构。_
