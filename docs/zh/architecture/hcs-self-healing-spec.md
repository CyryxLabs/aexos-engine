<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/zh/architecture/hcs-self-healing-spec.md
...
本文档指定了 AEXOS 健康检查系统 (HCS) 的自动恢复能力。自动恢复允许系统自动纠正某些问题,同时保持安全性和用户控制。
...
| 操作                | 描述                                      | 备份 |
| ------------------- | ----------------------------------------- | ---- |
| `recreate_config`   | 从模板重新创建缺失的 `.aexos/config.yaml` | 是   |
| `clear_cache`       | 清除 `.aexos/cache/` 中的过期缓存文件     | 是   |
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # 用户配置(out of .aexos/)
    - '.env*' # 环境文件
    - '**/secrets/**' # 密钥
    - '**/credentials*' # 凭证
    - '.git/**' # Git 内部
