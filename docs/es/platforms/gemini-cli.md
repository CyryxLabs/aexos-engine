<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/platforms/gemini-cli.md
...
### ¿Por qué usar AEXOS con Gemini CLI?
...
```markdown
# Reglas de AEXOS (Cyryx) para Gemini CLI
...
## Activación
Incluye "Como el agente AEXOS dev" en tu prompt.
...
### Activando Agentes de AEXOS

Incluye contexto de agentes en los prompts:

```bash
# Agente de desarrollador
gemini "Como el agente AEXOS dev, implementa la característica de login"

# Agente de QA
gemini "Como el agente AEXOS qa, revisa este código por seguridad"

# Agente de arquitecto
gemini "Como el agente AEXOS architect, diseña el esquema de base de datos"
```
...
cat src/auth.ts | gemini "Como AEXOS qa, revisa este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como AEXOS qa, revisa este archivo" >> reviews.md
done
```
...
```mermaid
flowchart LR
    Source["AEXOS Core<br/>.aexos-core/development/agents"] --> Parser["Agent Parser"]
...
**Frase de Activación:** "Como el agente AEXOS dev"

## Rol
Eres un Desarrollador Full Stack Senior trabajando con AEXOS (Cyryx).
...
1. Los agentes de AEXOS en `.aexos-core/` son portátiles
...
*AEXOS (Cyryx) - Guía de Plataforma Gemini CLI v1.0*
