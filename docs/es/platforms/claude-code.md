<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/platforms/claude-code.md
...
# Guía AEXOS para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | [PT](../../pt/platforms/claude-code.md) | **ES**

---

> **CLI Oficial de Anthropic** - Plataforma Recomendada para AEXOS
...
### ¿Por qué usar AEXOS con Claude Code?

Claude Code ofrece la **mejor integración** con AEXOS debido a:
...
# Verifica que los archivos AEXOS fueron creados
ls -la .claude/
```
...
export AEXOS_DEBUG="true"                        # Habilitar registro de depuración
```
...
```mermaid
flowchart LR
    Source["AEXOS Core<br/>.aexos-core/development/agents"] --> Parser["Analizador de Agentes"]
    Parser --> Transform["Transformador Claude"]
    Transform --> Output[".claude/commands/AEXOS/agents"]
```
...
1. Los cambios locales se respaldan en `.claude/commands/AEXOS/agents/.backup/`
...
ls .claude/commands/AEXOS/agents/
```
...
# Verificar registros de AEXOS
cat .aexos-core/logs/sync.log
```
...
### Preguntas Específicas de AEXOS
...
R: En `.claude/commands/AEXOS/agents/` después de la sincronización.

**P: ¿Con qué frecuencia debo sincronizar agentes?**
R: Después de actualizar AEXOS o cuando los agentes se modifiquen en el núcleo.
...
2. Inicializa AEXOS con Claude Code:
   ```bash
...
1. Tus agentes AEXOS se almacenan en `.aexos-core/development/agents/`
2. Inicializa AEXOS para el IDE objetivo
...
*AEXOS (Cyryx) - Guía de Plataforma Claude Code v1.0*
