<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/guides/quality-gates.md
...
# Guía del Sistema de Quality Gates de AEXOS

> 🌐 [EN](../../guides/quality-gates.md) | [PT](../../pt/guides/quality-gates.md) | **ES**

---

> Guía completa del sistema de quality gates de 3 capas para AEXOS (Cyryx).
...
El Sistema de Quality Gates de AEXOS proporciona aseguramiento de calidad automatizado a través de tres capas progresivas de validación. Cada capa detecta diferentes tipos de problemas en la etapa apropiada del desarrollo.
...
```yaml
# .aexos-core/core/quality-gates/quality-gate-config.yaml
layer1:
  enabled: true
...
aexos qa run --layer=1

# Ejecutar verificación específica
aexos qa run --layer=1 --check=lint
aexos qa run --layer=1 --check=test
aexos qa run --layer=1 --check=typecheck

# Ejecutar con salida detallada
aexos qa run --layer=1 --verbose
```
...
layer2:
  ...
  quinn:
    enabled: true
    autoReview: true
    agentPath: '.claude/commands/AEXOS/agents/qa.md'
    severity:
      block: ['CRITICAL']
      warn: ['HIGH', 'MEDIUM']
```
...
aexos qa run --layer=2

# Ejecutar solo CodeRabbit
aexos qa run --layer=2 --tool=coderabbit

# Ejecutar revisión de Quinn (@qa)
aexos qa run --layer=2 --tool=quinn
```
...
aexos qa request-review --pr=123

# Aprobar la revisión
aexos qa signoff --pr=123 --reviewer="@architect"

# Verificar estado de aprobación
aexos qa signoff-status --pr=123
```
...
aexos qa run --layer=1
aexos qa run --layer=2
aexos qa run --layer=3

# Ejecutar con opciones
aexos qa run --verbose          # Salida detallada
aexos qa run --fail-fast        # Detener en primera falla
aexos qa run --continue-on-fail # Continuar a pesar de fallas
```
...
aexos qa status --layer=1

# Obtener estado para PR
aexos qa status --pr=123
```
...
# Exportar a archivo
aexos qa report --output=qa-report.json
aexos qa report --format=markdown --output=qa-report.md
```
...
aexos qa configure --layer1.coverage.minimum=90
aexos qa configure --layer2.coderabbit.enabled=false
aexos qa configure --layer3.requireSignoff=true
```
...
jobs:
  ...
  layer3:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: aexos qa request-review --pr=${{ github.event.pull_request.number }}
```
...
aexos qa run --layer=1 --fail-fast
```
...
_Guía del Sistema de Quality Gates de AEXOS (Cyryx) v4_
