<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/guides/service-discovery.md
...
# Guía de Descubrimiento de Servicios de AEXOS

> 🌐 [EN](../../guides/service-discovery.md) | [PT](../../pt/guides/service-discovery.md) | **ES**

---

> Cómo descubrir, consultar y usar workers en el framework AEXOS.
...
El sistema de Descubrimiento de Servicios permite encontrar y usar workers (tareas, plantillas, scripts, workflows) en todo el framework AEXOS. El **Registro de Servicios** es el catálogo central que contiene metadatos sobre todos los workers disponibles.
...
```javascript
const { getRegistry, loadRegistry } = require('./.aexos-core/core/registry/registry-loader');
...
//   name: 'Create Story',
//   category: 'task',
//   path: '.aexos-core/development/tasks/po-create-story.md',
...
```bash
# Buscar por texto
aexos discover "create story"

# Buscar por categoría
aexos discover --category task

# Buscar por etiqueta
aexos discover --tag testing

# Buscar para agente
aexos discover --agent dev

# Combinar filtros
aexos discover --category task --tag development --agent dev
```
...
Encontrados 5 workers que coinciden con "create story":
  ...
  [task] po-create-story
         Ruta: .aexos-core/development/tasks/po-create-story.md
         Etiquetas: task, creation, story, product
         Agentes: po

  [task] dev-create-brownfield-story
         Ruta: .aexos-core/development/tasks/dev-create-brownfield-story.md
...
aexos info create-story

# Obtener info del worker con ruta completa
aexos info --path .aexos-core/development/tasks/po-create-story.md
```
...
aexos list templates

# Listar workers para agente
aexos list --agent dev

# Listar con paginación
aexos list tasks --page 1 --limit 20
```
...
**Ubicación:** `.aexos-core/development/tasks/`
...
**Ubicación:** `.aexos-core/product/templates/`
...
**Ubicación:** `.aexos-core/infrastructure/scripts/`
...
**Ubicación:** `.aexos-core/development/workflows/`
...
**Ubicación:** `.aexos-core/product/checklists/`
...
node .aexos-core/core/registry/build-registry.js
```

El constructor escanea:

- `.aexos-core/development/tasks/**/*.md`
- `.aexos-core/product/templates/**/*`
- `.aexos-core/infrastructure/scripts/**/*.js`
- `.aexos-core/product/checklists/**/*.md`
- `.aexos-core/development/workflows/**/*.yaml`
- `.aexos-core/core/data/**/*`
...
```javascript
const { getRegistry } = require('./.aexos-core/core/registry/registry-loader');
...
```javascript
const { getRegistry } = require('./.aexos-core/core/registry/registry-loader');
const { TaskExecutor } = require('./.aexos-core/development/scripts/task-executor');
...
ls .aexos-core/core/registry/service-registry.json

# Reconstruir registro
node .aexos-core/core/registry/build-registry.js

# Validar registro
node .aexos-core/core/registry/validate-registry.js
```
...
_Guía de Descubrimiento de Servicios de AEXOS (Cyryx) v4_
