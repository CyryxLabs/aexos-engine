<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/guides/contextual-greeting-system-guide.md
...
El Sistema de Saludo Contextual es una mejora de UX que hace que los saludos de los agentes AEXOS sean inteligentes y adaptativos, mostrando información y comandos relevantes basados en el contexto de la sesión.
...
### Scripts Core
- `.aexos-core/core/session/context-detector.js` - Detección de tipo de sesión
- `.aexos-core/infrastructure/scripts/git-config-detector.js` - Detección de git config
- `.aexos-core/development/scripts/greeting-builder.js` - Construcción del saludo
- `.aexos-core/development/scripts/workflow-navigator.js` - Navegación de workflow
- `.aexos-core/development/scripts/agent-exit-hooks.js` - Hooks de salida (para persistencia)

### Archivos de Datos
- `.aexos-core/data/workflow-patterns.yaml` - Definiciones de workflows
...
### Configuración
- `.aexos-core/core-config.yaml` - Configuración global (secciones git + agentIdentity)

### Agentes (Actualizados)
- `.aexos-core/agents/dev.md` - Metadatos de visibilidad de comandos
- `.aexos-core/agents/po.md` - Metadatos de visibilidad de comandos
- `.aexos-core/agents/*.md` - 9 agentes restantes (actualización pendiente)
...
R: Usa el script de prueba: `node .aexos-core/development/scripts/test-greeting-system.js`
