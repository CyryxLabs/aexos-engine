<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/guides/security-hardening.md
...
-->
# Guía de Hardening de Seguridad de AEXOS

> [EN](../../guides/security-hardening.md) | [PT](../../pt/guides/security-hardening.md) | **ES**

---

> Guía completa para fortalecer la seguridad en despliegues de AEXOS (Cyryx) - desde desarrollo hasta producción.
...
AEXOS (Cyryx) opera en una capa privilegiada entre los modelos de IA y tu sistema. Esta guía cubre estrategias de hardening específicas para entornos de desarrollo orquestados por IA.
...
### Preocupaciones de Seguridad Específicas de AEXOS
...
AEXOS implementa múltiples capas de protección:
...
Las claves API son los secretos más críticos en AEXOS. Las claves comprometidas pueden llevar a uso no autorizado, brechas de datos e impacto financiero significativo.
...
const secrets = await SecretManager.loadSecrets({
  ...
  secretName: 'aexos/production/api-keys',
...
```javascript
// .aexos-core/core/security/key-validator.js
const requiredKeys = [
...
```bash
# ============================================================
# CONFIGURACIÓN DE ENTORNO AEXOS
# ============================================================
...
AUDIT_LOG_PATH=/var/log/aexos/audit.log
...
mkdir -p ~/.aexos/secrets
chmod 700 ~/.aexos/secrets

# Crear archivo de secretos cifrado
# Nunca almacenar secretos en texto plano
openssl enc -aes-256-cbc -salt -pbkdf2 \
  -in secrets.txt \
  -out ~/.aexos/secrets/encrypted.dat

# Establecer permisos apropiados
chmod 600 ~/.aexos/secrets/*
...
function validateEnvironment() {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (process.env.AEXOS_DEBUG === 'true') {
...
### Permisos de Estructura de Directorios AEXOS
...
# Directorios de configuración AEXOS
chmod 700 .aexos/              # Solo el propietario puede acceder
chmod 700 .aexos-core/         # Fuente del framework
chmod 700 .claude/            # Configuración de Claude

# Archivos de configuración sensibles
chmod 600 .env                # Variables de entorno
chmod 600 .aexos/config.yaml   # Configuración principal
chmod 600 .aexos/users.json    # Base de datos de usuarios
chmod 600 .aexos/sessions.json # Sesiones activas
...
# Archivos temporales
chmod 700 .aexos/temp/
chmod 600 .aexos/temp/*
```

### Control de Acceso a Directorios

```yaml
# .aexos/config.yaml - Configuración de directorios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificación de Permisos de Seguridad AEXOS"
echo "==========================================="
...
check_permission ".aexos" "700"
check_permission ".aexos/config.yaml" "600"
...
AEXOS usa contenedores Docker para aislar los servidores MCP del sistema host:
...
const { OperationGuard } = require('./.aexos-core/core/permissions');
...
```javascript
// .aexos-core/core/security/input-sanitizer.js
...
```yaml
# .aexos/config.yaml - Configuración de auditoría
audit:
  enabled: true
  ...
  retention:
    days: 90
    archivePath: .aexos/logs/archive
```
...
```javascript
// .aexos-core/core/security/audit-logger.js
...
function validateProductionSecurity() {
  ...
  if (process.env.AEXOS_DEBUG === 'true') {
    errors.push('AEXOS_DEBUG debe ser false en producción');
  }
...
Si descubres una vulnerabilidad de seguridad en AEXOS (Cyryx), por favor sigue las prácticas de divulgación responsable:

### Proceso de Reporte

1. **NO** crees un issue público en GitHub para vulnerabilidades de seguridad
2. Reporta preocupaciones de seguridad via [GitHub Security Advisories](https://github.com/CyryxLabs/aexos-engine/security/advisories)
...
Actualmente, AEXOS (Cyryx) no tiene un programa formal de bug bounty. Sin embargo, las contribuciones significativas de seguridad son reconocidas y pueden recibir licencias de AEXOS Pro u otro reconocimiento.
...
_Guía de Hardening de Seguridad de AEXOS (Cyryx) v4.0_
