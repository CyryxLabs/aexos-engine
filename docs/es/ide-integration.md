<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/ide-integration.md
...
Guía para integrar AEXOS con IDEs compatibles y plataformas de desarrollo con IA.
...
AEXOS es compatible con 6 plataformas de desarrollo potenciadas por IA. Elige la que mejor se adapte a tu flujo de trabajo.
...
| IDE | Paridad de Hooks vs Claude | Qué se degrada sin hooks completos | Mitigación en AEXOS |
| --- | --- | --- | --- |
| Claude Code | Completa | Ninguno (comportamiento de referencia) | Hooks nativos + pipeline completo de AEXOS |
...
**Nivel de Recomendación:** Mejor integración con AEXOS
...
1. AEXOS crea automáticamente el directorio `.claude/` al inicializar
...
4. Usa `/skills` y selecciona `aexos-<agent-id>`
...
1. AEXOS crea el directorio `.cursor/` al inicializar
...
2. AEXOS crea `.github/copilot-instructions.md`
...
1. AEXOS crea el directorio `.antigravity/`
...
AEXOS mantiene una única fuente de verdad para las definiciones de agentes y las sincroniza con todos los IDEs configurados:

```
┌─────────────────────────────────────────────────────┐
│                    AEXOS Core                         │
│  .aexos-core/development/agents/  (Fuente de Verdad) │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
AEXOS puede configurarse para sincronizar automáticamente cuando hay cambios en los agentes:
...
ls .claude/commands/AEXOS/agents/  # Para Claude Code
```
...
├── Sí --> Claude Code (Mejor integración con AEXOS)
└── No
    └── ¿Prefieres VS Code?
...
_Guía de Integración con IDEs de AEXOS (Cyryx) v4.0_
