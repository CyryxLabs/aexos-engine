<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/architecture/hcs-check-specifications.md
...
**Propósito:** Verificar que los archivos del framework AEXOS están correctamente configurados y son consistentes.

### Verificaciones

| ID     | Nombre                      | Severidad | Nivel | Modo  | Timeout | Descripción                                         |
| ------ | --------------------------- | --------- | ----- | ----- | ------- | --------------------------------------------------- |
| PC-001 | Config existe               | CRITICAL  | 1     | quick | 100ms   | `.aexos/config.yaml` existe y es YAML válido         |
...
| PC-006 | Sin archivos huérfanos      | LOW       | 3     | full  | 5s      | Todos archivos en `.aexos-core/` están referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    const configPath = path.join(context.projectRoot, '.aexos', 'config.yaml');

    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.aexos/config.yaml no encontrado',
  ...
  async heal(context) {
    const templatePath = '.aexos-core/infrastructure/templates/core-config/config-template.yaml';
    const configPath = path.join(context.projectRoot, '.aexos', 'config.yaml');
...
const requiredSections = [
  'Project Overview', // o 'AEXOS-FULLSTACK Development Rules'
...
```yaml
# .aexos-core/health-check/checks/ide-checks.yaml
checks:
  - id: IDE-VSCODE
    ...
    validation:
      type: json-schema
      schema: '.aexos-core/schemas/vscode-settings.json'
    autoFix:
      tier: 2
      action: 'create-from-template'
      template: '.aexos-core/infrastructure/templates/ide/vscode-settings.json'
...
Los usuarios pueden definir verificaciones específicas del proyecto en `.aexos/custom-checks.yaml`:

```yaml
# .aexos/custom-checks.yaml
version: 1.0
...
checks:
  ...
  - id: CUSTOM-001
    ...
    autoFix:
      action: create-from-template
      template: '.aexos-core/infrastructure/templates/project-docs/readme-template.md'
  ...
  - id: CUSTOM-005
    ...
    type: json-schema
    path: 'package.json'
    schema: '.aexos-core/schemas/package-json.json'
    severity: CRITICAL
    tier: 3
    mode: quick
```
