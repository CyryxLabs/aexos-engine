<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/architecture/dashboard-architecture.md
...
-->
# 🏛️ AEXOS Dashboard - Arquitectura Completa
...
El AEXOS Dashboard es una aplicación Next.js que proporciona una interfaz visual para monitorear y gestionar el sistema AEXOS. Se comunica con el CLI/AEXOS a través de archivos de estado en el sistema de archivos y Server-Sent Events (SSE).

### Principios Arquitectónicos

1. **CLI-First**: Dashboard es complementario al CLI, no sustituto
2. **File-Based Communication**: Estado a través de `.aexos/dashboard/status.json`
...
│   │   │   ├── status/route.ts       # Polling de estado AEXOS
...
│   │   ├── use-aexos-status.ts        # Estado con SWR
...
| ---------------- | ----------------------------- | -------------------------------- |
| `story-store`    | `aexos-stories`            | `storyOrder` (orden de las columnas) |
| `ui-store`       | `aexos-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `aexos-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `aexos-dashboard-settings` | Todo el objeto `settings`         |
...
```typescript
// Retorna estado actual de AEXOS
// Lee de: .aexos/dashboard/status.json

interface AexosStatus {
  version: string;
  updatedAt: string;
  connected: boolean;
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agente activado                         │
       │  ─────────────────────────────────────────▶  │
       │     Escribe .aexos/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
const { status, isLoading, isConnected, statusError, mutate } = useAexosStatus({
...
_Documentación generada por @architect (Aria) - AEXOS Core v2.0_
