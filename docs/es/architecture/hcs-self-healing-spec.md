<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/architecture/hcs-self-healing-spec.md
...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de AEXOS (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.aexos/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.aexos/cache/` | Sí           |
...
- id: PC-001
  description: '.aexos/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .aexos/config.yaml (respaldo: .aexos/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .aexos/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .aexos-core/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx aexos task validate deploy'
...
Pasos para resolver:
  1. Abrir .aexos-core/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx aexos task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .aexos/)
...
safeToAutoFix:
  paths:
    - '.aexos/**' # Archivos del workspace AEXOS
    - '.aexos-core/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "rollbackCommand": "npx aexos health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.aexos', 'backups', backupId);
...
npx aexos health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx aexos health-check --list-backups

# Revertir último respaldo
npx aexos health-check --rollback-last
```
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.pathExists('.aexos/config.yaml'));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.aexos-core/templates/config-template.yaml');
    await fs.writeFile('.aexos/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.pathExists('.aexos/config.yaml');
  }
}
```
...
```javascript
// .aexos/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.aexos/logs/self-healing.log';
    const logEntry = {
