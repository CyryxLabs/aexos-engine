<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/architecture/dashboard-realtime.md
...
-->
# 🔴 AEXOS Dashboard - Arquitectura de Observabilidad en Tiempo Real
...
Este documento describe la arquitectura para **observabilidad en tiempo real** del AEXOS Dashboard, permitiendo que los usuarios monitoreen comandos ejecutados en la CLI con máximo detalle visual.
...
│                                   ▼                                      │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │               .aexos/dashboard/events.jsonl (append-only)         │    │
...
```
.aexos-core/core/events/dashboard-emitter.ts
```

### Interfaz

```typescript
// .aexos-core/core/events/types.ts
...
const EVENTS_DIR = '.aexos/dashboard';
...
```typescript
// .aexos-core/integrations/claude-code/hooks.ts
...
const AEXOS_DIR = process.env.AEXOS_PROJECT_ROOT || process.cwd();
const STATUS_FILE = join(AEXOS_DIR, '.aexos/dashboard/status.json');
const EVENTS_FILE = join(AEXOS_DIR, '.aexos/dashboard/events.jsonl');
...
│                                                                              │
│  .aexos/dashboard/events.jsonl (SOLO ALTO NIVEL)                             │
│  ─────────────────────────────────────────────────                          │
