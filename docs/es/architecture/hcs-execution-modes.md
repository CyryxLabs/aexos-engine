<!--
  AEXOS localization provenance: adapted from https://github.com/SynkraAI/aiox-core/blob/4ef6530ff03b83aea953e4a426f95e012b8b70c5/docs/es/architecture/hcs-execution-modes.md
...
Este documento define los modos de ejecución para el Sistema de Verificación de Salud de AEXOS (HCS), basado en investigación de mejores prácticas de la industria de Kubernetes, VS Code, Terraform, npm/yarn, y patrones CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Al instalar/bootstrap**    | `npx aexos install` | 60-120s  | Esperado                 | Validación de setup         | ✅ **Requerido**  |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx aexos health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path: .aexos/reports/health-check-*.md
```
...
**Veredicto:** Requerido. Parte de `npx aexos install` y `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Post to Slack (on failure)
        ...
        with:
          payload: |
            {
              "text": "⚠️ AEXOS Health Check Falló",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx aexos health-check --mode=quick
```
...
{
  "aexos.healthCheck.enabled": true,
  "aexos.healthCheck.interval": 300,
  "aexos.healthCheck.mode": "quick",
  "aexos.healthCheck.showNotifications": true
}
```
