...
This document explains how to set up and use the cryptographic signing system for AEXOS-Core install manifests.

## Overview

AEXOS-Core uses **Ed25519 digital signatures** (via minisign format) to verify the integrity and authenticity of the `install-manifest.yaml` file. This ensures that:
...
│  │ (aexos-core.key)  │    │         -s aexos-core.key          │  │
│  │ NEVER SHARE!     │    └───────────────────────────────────┘  │
│  └──────────────────┘                    │                       │
│                                          ▼                       │
│                          ┌───────────────────────────────────┐  │
...
minisign -G -p aexos-core.pub -s aexos-core.key

# You will be prompted for a password to protect the secret key
# CHOOSE A STRONG PASSWORD!

# Output:
#   aexos-core.pub  - PUBLIC key (safe to share, will be hardcoded)
#   aexos-core.key  - SECRET key (NEVER share, store securely)
```
...
cat aexos-core.pub
# Output example:
# untrusted comment: minisign public key CYRYX0001
...
const PINNED_PUBLIC_KEY = {
  ...
  keyId: 'CYRYX0001',
  // Base64 public key (the second line of aexos-core.pub)
...
- Store `aexos-core.key` in a secure location (password manager, HSM, etc.)
...
1. **Generate/Update Manifest**
   ...
   ```bash
   node bin/aexos.js manifest:generate
   # Creates .aexos-core/install-manifest.yaml with all file hashes
   ```
...
2. **Sign the Manifest**
   ...
   ```bash
   cd .aexos-core
   minisign -Sm install-manifest.yaml -s /path/to/aexos-core.key
...
3. **Verify Signature (Optional but Recommended)**
   ...
   ```bash
   minisign -Vm install-manifest.yaml -p /path/to/aexos-core.pub
...
4. **Commit Both Files**
   ...
   ```bash
   git add .aexos-core/install-manifest.yaml
   git add .aexos-core/install-manifest.yaml.minisig
...
The `aexos validate` command uses a compatibility-safe default:

- If `.aexos-core/install-manifest.yaml.minisig` exists in the target or repair source, the signature is verified automatically.
...
- Strict enforcement is available with `aexos validate --require-signature` or `AEXOS_REQUIRE_SIGNATURE=1`.
- Emergency recovery can explicitly bypass verification with `aexos validate --no-signature`.
...
This option exists **exclusively** for local development, recovery, and unsigned package compatibility. Production builds that distribute `.minisig` artifacts **MUST** enforce signature verification (`requireSignature: true`) or run `aexos validate --require-signature`. Any deployment with signature verification disabled should be considered unsigned.
...
```bash
minisign -Sm .aexos-core/install-manifest.yaml -s /path/to/aexos-core.key
```
