...
| `bin/aexos-init.js`                               | **Modified** | Added `requireSignature` option          |
...
1. Generate a key pair: `minisign -G -p aexos-core.pub -s aexos-core.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s aexos-core.key`
...
3. **Sign manifest** before each release:
   ...
   ```bash
   minisign -Sm .aexos-core/install-manifest.yaml -s aexos-core.key
   ```
