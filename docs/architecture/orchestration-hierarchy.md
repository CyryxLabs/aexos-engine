...
This document defines the current ownership boundaries for AEXOS orchestration
modules and the lifecycle status of legacy workflow scripts.

## Decision Summary

New durable story and epic orchestration state must use
`.aexos-core/core/orchestration/session-state.js`.

Lean Full SDC and Wave Execute may keep synchronous, recoverable execution
journals under `.aexos/sdc/` and `.aexos/waves/`. These checkpoints are local
...
`.aexos-core/development/scripts/workflow-state-manager.js` is deprecated for new
...
`.aexos-core/development/scripts/workflow-navigator.js` is not deprecated. It is
...
| Lean SDC execution checkpoint | Lean SDC progress helper | `.aexos/sdc/{story-id}/state.json` | Gitignored, reconstructible local journal; never wins a canonical conflict |
| Wave execution checkpoint | Wave progress helper | `.aexos/waves/{wave-id}/state.json` | Gitignored, reconstructible local journal; never wins a canonical conflict |
| Legacy guided workflow state | `WorkflowStateManager` | `.aexos/{instance-id}-state.yaml` | Deprecated compatibility surface |
| Legacy workflow-state directory | `SessionState` migrator | `.aexos/workflow-state/` | Migration input for ADR-011 compatibility |
...
graph TD
    ...
    FullSDC[Full SDC] --> SDCCheckpoint[".aexos/sdc checkpoint"]
    WaveExecute[Wave Execute] --> WaveCheckpoint[".aexos/waves checkpoint"]
    ...
    SessionState -. migrates .-> LegacyDir[.aexos/workflow-state]
```
