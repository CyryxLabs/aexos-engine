...
`CYRYXError` or `ErrorRegistry` surface.
...
Introduce `.aexos-core/core/errors/` as the canonical core error-governance
surface.

The initial contract contains:

- `CYRYXError`: an `Error` subclass with stable `code`, `category`, `severity`,
