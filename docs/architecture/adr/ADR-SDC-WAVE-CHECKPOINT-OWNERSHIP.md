...
- `.aexos/sdc/{story-id}/state.json`
- `.aexos/waves/{wave-id}/state.json`
...
- `.aexos/sdc` and `.aexos/waves` contain local, recoverable execution journals.
...
5. Deleting `.aexos/sdc` or `.aexos/waves` may lose local execution convenience,
...
- Checkpoints are local runtime artifacts under the gitignored `.aexos/`
