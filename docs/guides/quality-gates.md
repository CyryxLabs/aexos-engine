...
The AEXOS Quality Gate System provides automated quality assurance through three progressive layers of validation. Each layer catches different types of issues at the appropriate stage of development.
...
| **Argus (@qa)** | Automated QA review | CRITICAL          |
...
# Run with options
aexos qa run --verbose          # Detailed output
aexos qa run --fail-fast        # Stop on first failure
aexos qa run --continue-on-fail # Continue despite failures
```
...
| Argus blocked       | Review @qa feedback, update code        |
