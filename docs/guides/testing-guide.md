...
AEXOS follows a comprehensive testing strategy that ensures code quality across all layers of the framework. Our testing philosophy is built on:
 ...
 * Agent Backward Compatibility Tests
 *
 * Ensures agents from previous AEXOS versions continue to work.
 */
...
AEXOS supports Windows, macOS, and Linux. Cross-platform testing ensures consistent behavior.
...
│   ├── index.html        # Overview
│   └── .aexos-core/       # Per-module coverage
...
The AEXOS Quality Gate System (see [Quality Gates Guide](./quality-gates.md)) integrates testing at multiple layers:
