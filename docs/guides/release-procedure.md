...
`validate-aexos-core-namespace.js` (wired into `validate:publish`).
...
AEXOS_ACTIVE_AGENT=devops git commit -m "chore(release): bump to X.Y.Z — <summary>"
AEXOS_ACTIVE_AGENT=devops git push -u origin chore/release-X.Y.Z
...
2. **Legacy branch protection** — `repos/CyryxLabs/AEXOS/branches/main/protection`
...
gh api repos/CyryxLabs/AEXOS/rulesets/13330052 \
  > /tmp/aexos-core-ruleset-original.json
gh api repos/CyryxLabs/AEXOS/branches/main/protection \
  > /tmp/aexos-core-branch-protection-original.json

# Sanitized restore payload for the ruleset (only writable fields):
jq '{name, target, enforcement, conditions, bypass_actors, rules}' \
  /tmp/aexos-core-ruleset-original.json \
  > /tmp/aexos-core-ruleset-restore.json
...
jq '{name, target, enforcement, conditions, bypass_actors,
     rules: (.rules | map(if .type=="pull_request"
       ...
       else . end))}' /tmp/aexos-core-ruleset-original.json \
  > /tmp/aexos-core-ruleset-bypass.json
...
jq '.required_pull_request_reviews | {dismiss_stale_reviews, require_code_owner_reviews,
   ...
   /tmp/aexos-core-branch-protection-original.json \
   > /tmp/aexos-core-prr-restore.json
```
...
test -s /tmp/aexos-core-ruleset-restore.json || { echo "Sanitize payloads first."; exit 1; }
test -s /tmp/aexos-core-prr-restore.json     || { echo "Sanitize payloads first."; exit 1; }
...
restore_protections() {
  ...
  gh api -X PUT repos/CyryxLabs/AEXOS/rulesets/13330052 \
    --input /tmp/aexos-core-ruleset-restore.json > /dev/null \
    || echo "::error::Ruleset restore FAILED — manual recovery required."
  gh api -X PATCH repos/CyryxLabs/AEXOS/branches/main/protection/required_pull_request_reviews \
    --input /tmp/aexos-core-prr-restore.json > /dev/null \
...
gh api -X PUT repos/CyryxLabs/AEXOS/rulesets/13330052 \
  --input /tmp/aexos-core-ruleset-bypass.json > /dev/null
gh api -X DELETE repos/CyryxLabs/AEXOS/branches/main/protection/required_pull_request_reviews

# 4. Merge. If this fails, the EXIT trap restores anyway.
AEXOS_ACTIVE_AGENT=devops gh pr merge "$PR_NUMBER" --squash --admin --delete-branch
...
```bash
diff <(jq -S '.rules[0].parameters' /tmp/aexos-core-ruleset-original.json) \
     <(gh api repos/CyryxLabs/AEXOS/rulesets/13330052 | jq -S '.rules[0].parameters')

diff <(jq -S '.required_pull_request_reviews | {dismiss_stale_reviews, require_code_owner_reviews, require_last_push_approval, required_approving_review_count}' /tmp/aexos-core-branch-protection-original.json) \
     <(gh api repos/CyryxLabs/AEXOS/branches/main/protection | jq -S '.required_pull_request_reviews | {dismiss_stale_reviews, require_code_owner_reviews, require_last_push_approval, required_approving_review_count}')
```
...
AEXOS_ACTIVE_AGENT=devops git tag -a -m "<release notes>" vX.Y.Z origin/main
AEXOS_ACTIVE_AGENT=devops git push origin vX.Y.Z
```
...
- **Race condition between `publish` and `publish_legacy_cyryx_core`** (fixed
  ...
  the CDN. Fix: serialize `publish_legacy_cyryx_core` after `publish`.
