...
The Project Status feature automatically displays your current work context when any AEXOS agent activates. This includes:
...
- **AEXOS-FullStack** framework installed
...
3. Create `.aexos/project-status.yaml` cache file
...
1. Edit `.aexos-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Location: `.aexos-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .aexos/project-status.yaml  # Cache file location
...
1. **Checks cache** - Looks for `.aexos/project-status.yaml`
...
- **Cache Location:** `.aexos/project-status.yaml`
...
All 11 AEXOS agents display project status:

1. **@dev** (Vulcan - Builder)
2. **@po** (Themis - Balancer)
3. **@qa** (Argus - Guardian)
4. **@sm** (Chronos - Facilitator)
5. **@pm** (Janus - Strategist)
6. **@architect** (Vega - Visionary)
7. **@analyst** (Sirius - Decoder)
8. **@devops** (Polaris - Operator)
9. **@data-engineer** (Ceres - Sage)
10. **@ux-design-expert** (Iris - Empathizer)
11. **@aexos-master** (Zeus - Orchestrator)
...
3. Does `.aexos-core/infrastructure/scripts/project-status-loader.js` exist?
...
A: Yes, all agents use the same cache file (`.aexos/project-status.yaml`).

---

## Related Documentation

- **Story:** `docs/stories/cyryx migration/story-6.1.2.4-project-status-context.md`
