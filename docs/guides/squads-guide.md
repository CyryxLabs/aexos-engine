...
> **AEXOS Squads:** Equipes de AI agents trabalhando com você
...
Squads are modular teams of AI agents that extend AEXOS functionality. Each squad is a self-contained package containing:
...
│  Level 2: CYRYX-SQUADS  → github.com/CyryxLabs (Public/Free)  │
│  Level 3: AEXOS API   → api.cyryx.dev      (Marketplace)  │
└─────────────────────────────────────────────────────────────┘
```
...
|-------|---------|-------------|
| [etl-squad](https://github.com/CyryxLabs/aexos-squads/tree/main/etl) | 2.0.0 | Data collection and transformation |
| [creator-squad](https://github.com/CyryxLabs/aexos-squads/tree/main/creator) | 1.0.0 | Content generation utilities |
...
- Node.js 18+
- AEXOS project initialized (`.aexos-core/` exists)
...
Tasks must follow [TASK-FORMAT-SPECIFICATION-V1](../../.aexos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Mode | Behavior |
|------|----------|
| `extend` | Add squad rules to core AEXOS rules |
...
```javascript
const { SquadAnalyzer } = require('./.aexos-core/development/scripts/squad/squad-analyzer');
const { SquadExtender } = require('./.aexos-core/development/scripts/squad/squad-extender');
...
```javascript
const { SquadValidator } = require('./.aexos-core/development/scripts/squad');
...
### Level 2: aexos-squads Repository (Public)
...
This creates a PR to [CyryxLabs/aexos-squads](https://github.com/CyryxLabs/aexos-squads).

### Level 3: Cyryx Marketplace

```bash
# Set up authentication
export AEXOS_API_TOKEN="your-token"

# Sync to marketplace
*sync-squad-cyryx ./squads/my-squad --public
```
...
3. **Add Fields** - `cyryx.type`, `cyryx.minVersion`
...
```javascript
const { SquadLoader } = require('./.aexos-core/development/scripts/squad');
...
```javascript
const { SquadLoader, SquadLoaderError } = require('./.aexos-core/development/scripts/squad');
...
# - cyryx.type: must be "squad"
# - cyryx.minVersion: must be valid semver
```
...
### What's the difference between a Squad and legacy AEXOS squad formats?

**Squads** are the standard format in AEXOS 2.1+ with:
- Task-first architecture
...
3. Re-publish: `*publish-squad` or `*sync-squad-cyryx`
...
- **Level 3**: Sync with `--private` flag: `*sync-squad-cyryx my-squad --private`

### What's the minimum AEXOS version for Squads?

Squads require AEXOS 2.1.0+. Set in manifest:
...
- [TASK-FORMAT-SPECIFICATION-V1](../../.aexos-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md)
...
- [@squad-creator Agent](../../.aexos-core/development/agents/squad-creator.md)
- [aexos-squads Repository](https://github.com/CyryxLabs/aexos-squads)

---

## Getting Help

- [GitHub Discussions](https://github.com/CyryxLabs/AEXOS/discussions)
- [Issue Tracker](https://github.com/CyryxLabs/AEXOS/issues)
