...
> **ARQUITETURA:** O MIS segue o modelo **Open Core** do AEXOS. Toda a inteligencia de memoria reside no repositorio privado `aexos-pro` (submodule `pro/`). O `aexos-core` (open source) fornece apenas extension points e funciona 100% sem memoria inteligente — exatamente como hoje.
...
O sistema de memoria atual opera em **duas camadas desconectadas** (Claude Code nativo + AEXOS Framework) com gaps criticos:
...
|--------|--------|-------------|-------|
| **Cleanup** | Remover dead code, preparar base | **aexos-core** | MIS-2 |
| **Capture** | Captura conhecimento de sessao via hooks | **aexos-pro** | MIS-3 |
| **Storage** | Armazena em Markdown com frontmatter YAML | **aexos-pro** | MIS-3, MIS-4 |
| **Retrieval** | Recupera por relevancia com progressive disclosure | **aexos-pro** + extension points em **aexos-core** | MIS-4, MIS-6 |
| **Evolution** | Aprende e evolui regras automaticamente | **aexos-pro** | MIS-5, MIS-7 |

### Principios Chave (vs Estado Atual)

| Aspecto | Estado Atual | Com MIS (aexos-pro ativo) | Sem MIS (aexos-core only) |
|---------|-------------|--------------------------|--------------------------|
...
> **aexos-core funciona 100% sem o MIS** — exatamente como funciona hoje. Quando `aexos-pro` esta presente (submodule `pro/`), o MIS se conecta automaticamente via extension points. Zero configuracao manual.

### Modelo Open Core

```mermaid
flowchart LR
    subgraph CORE["aexos-core (Open Source, MIT)"]
        direction TB
    ...
    subgraph PRO["aexos-pro (Privado, Submodule pro/)"]
        direction TB
...
O aexos-core ja possui o modulo `bin/utils/pro-detector.js` (Story PRO-5):
...
#### aexos-core (open source) — Extension Points Only

| Arquivo | Tipo | Descricao |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | Existente | `isProAvailable()`, `loadProModule()` |
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | Modificacao | Adiciona `if (isProAvailable())` no Tier 2 para memory loading |
| `.aexos-core/development/scripts/greeting-builder.js` | Modificacao | Adiciona memory stats no greeting se pro disponivel |
| `.aexos-core/core/memory/gotchas-memory.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.aexos-core/core/session/context-loader.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.claude/settings.json` | Modificacao | Hook stubs que chamam scripts em `pro/` (graceful fail se ausente) |

#### aexos-pro (privado) — Toda a Inteligencia
...
Os hooks do Claude Code sao configurados em `.claude/settings.json` (aexos-core). O script apontado reside em `pro/`:
...
```javascript
// .aexos-core/hooks/pro-hook-runner.js (no core, ~20 linhas)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["aexos-core (Open Source)"]
        direction TB
    ...
    subgraph PRO_BOUNDARY["aexos-pro (Privado — pro/)"]
...
### Fluxo Sem Pro (aexos-core standalone)
...
> **Repositorio:** `aexos-pro` (pro/memory/)
> **Extension point no core:** `.aexos-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (aexos-core — committed)
{
  "hooks": {
    "PreCompact": [
      {
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["aexos-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to<br/>.aexos/memories/")]
    end
...
> **Repositorio:** `aexos-pro` (pro/memory/) gerencia o storage
> **Storage location:** `.aexos/memories/` (gitignored, criado pelo pro)
...
```
.aexos/memories/                          # Criado e gerenciado por pro/memory/
...
> **Repositorio:** `aexos-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **Repositorio:** `aexos-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
sequenceDiagram
    ...
    participant SD as session-digest.js<br/>(aexos-pro)
...
> **Repositorio:** `aexos-pro` (pro/memory/agent-memory-api.js)
...
Cada agente acessa:
1. Suas proprias memorias privadas (`.aexos/memories/{agent}/`)
2. Memorias compartilhadas (`.aexos/memories/shared/`)
...
sequenceDiagram
    ...
    participant PD as pro-detector.js<br/>(aexos-core)
    participant ML as memory-loader.js<br/>(aexos-pro)
    participant MR as memory-retriever.js<br/>(aexos-pro)
...
### Especificacao (arquivos em .aexos/memories/)
...
# Always Use Absolute Imports in AEXOS
...
> **100% em aexos-pro** — nenhum componente de self-learning no core
...
→ PROPOSTA: Adicionar a rules/: "Warning: .aexos/ permissions"
...
**Sem pro:** Apenas `[CORE]` e `[NATIVO]` existem. `[PRO]` (.aexos/memories/) nunca e criado.

---

## Mapa de Arquivos do Sistema

### aexos-core (Open Source) — Extension Points

| Arquivo | Tipo | Story | Descricao |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | Existente (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.aexos-core/hooks/pro-hook-runner.js` | **Novo** | MIS-3 | Wrapper: verifica pro, delega hook (~20 linhas) |
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | **Modificacao** | MIS-6 | Adiciona `if (isProAvailable())` no Tier 2 |
| `.aexos-core/development/scripts/greeting-builder.js` | **Modificacao** | MIS-6 | Memory stats no greeting se pro disponivel |
| `.aexos-core/core/memory/gotchas-memory.js` | **Path fix** | MIS-2 | Corrige `.aexos/error-tracking.json` |
| `.aexos-core/core/session/context-loader.js` | **Path fix** | MIS-2 | Corrige `.aexos/session-state.json` |
...
### Arquivos Removidos (MIS-2, aexos-core)

| Arquivo | Linhas | Razao |
|---------|--------|-------|
| `.aexos-core/core/memory/timeline-manager.js` | 746 | 0 consumers (orphan) |
| `.aexos-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 consumers (orphan) |
| `.aexos-core/core/memory/context-snapshot.js` | 648 | 0 consumers (orphan) |
| `.aexos-core/elicitation/session-manager.js` | — | Dead code |
| `.aexos/compound-analysis/` | ~14K | 0 readers (orphan data) |

---

## Diferencas: Estado Atual vs MIS

| Aspecto | aexos-core hoje | aexos-core + aexos-pro (MIS) | aexos-core sem pro |
|---------|---------------|---------------------------|------------------|
...
|-------|--------|-------------|------------|-------|
| MIS-1 | Investigation & Architecture Design | docs (aexos-core) | — | 12h (Done) |
| MIS-2 | Dead Code Cleanup & Path Repair | **aexos-core** | MIS-1 | 4h |
| MIS-3 | Session Digest (PreCompact Hook) | **aexos-core** (hook runner) + **aexos-pro** (digest) | MIS-1 | 14h |
| MIS-4 | Progressive Memory Retrieval | **aexos-pro** | MIS-3 | 16h |
| MIS-5 | Self-Learning Engine | **aexos-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | Pipeline Integration & Agent Memory API | **aexos-core** (ext points) + **aexos-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md & Rules Auto-Evolution | **aexos-pro** | MIS-5 | 8h |
| **Total** | | | | **~78h** |
...
| Epic PRO Architecture | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-aexos-pro-architecture/EPIC-PRO-INDEX.md) |
...
*AEXOS Memory Intelligence System — Architecture Vision v4.0.4 (Core/Pro Split)*
*Target state apos Epic MIS completo (7 stories, ~78 horas)*
*aexos-core: extension points + dead code cleanup*
*aexos-pro: toda a inteligencia de memoria (Capture, Storage, Retrieval, Evolution)*
