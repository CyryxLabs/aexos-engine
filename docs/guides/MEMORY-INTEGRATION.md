...
The integration follows the AEXOS Open Core model:
- **aexos-core:** Extension point in UnifiedActivationPipeline (this guide)
- **aexos-pro:** Memory intelligence implementation (retrieval, scoring, learning)
...
```javascript
isProAvailable() === true
// but .aexos/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .aexos/session-digests/ contains memory digests
```
...
**Solutions:**
1. Reduce digest count (archive old digests to `.aexos/session-digests/archive/`)
