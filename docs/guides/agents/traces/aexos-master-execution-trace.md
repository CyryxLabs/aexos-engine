# @aexos-master (Zeus) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.aexos-core/development/agents/aexos-master.md`
...
|-------|------|--------|---------|
| 1 | `.aexos-core/development/agents/aexos-master.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.aexos-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.aexos-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: dataLocation, registry |
| 4 | `.aexos-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |
| 5 | `.aexos/session-state.json` | ContextDetector._detectFromFile() | Session type detection (if no conversation history) |
| 6 | `.aexos/project-status.yaml` | ProjectStatusLoader.loadCache() | Cached project status (60s TTL) |

**NOT loaded on activation:**

| File | Condition | Size |
|------|-----------|------|
| `.aexos-core/data/aexos-kb.md` | ONLY when user types `*kb` | 35KB |
...
sequenceDiagram
    ...
    participant AMd as aexos-master.md
...
aexos-master:
  config_sections:
    ...
    - registry
  files_loaded:
    - path: .aexos-core/data/aexos-kb.md
      lazy: true
      condition: kb_command
      size: 35KB
  lazy_loading:
    registry: false      # Always load (15KB, frequently used)
    aexos-kb: true        # Load only on *kb command
...
| Data | Source | Value |
|------|--------|-------|
| Greeting level | `persona_profile.greeting_levels.archetypal` | `👑 Zeus the Orchestrator ready to lead!` |
| Signature | `persona_profile.communication.signature_closing` | `— Zeus, orquestrando o sistema 🎯` |
| Role | `persona.role` | Master Orchestrator, Framework Developer & AEXOS Method Expert |
...
| `*list-components` | (built-in, scans .aexos-core/) | full | No |
...
**Task file:** `.aexos-core/development/tasks/kb-mode-interaction.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `kb-mode-interaction.md` | Task | EXISTS |
| `.aexos-core/data/aexos-kb.md` | Data | EXISTS (lazy, 35KB) |
...
flowchart TD
    ...
    B --> C[Load aexos-kb.md into context]
    C --> D[Toggle KB mode ON]
    D --> E[Agent now answers with full AEXOS Method knowledge]
    E --> F[User types *kb again to toggle OFF]
```

**CRITICAL:** aexos-kb.md is NEVER loaded unless user explicitly types `*kb`.

---

### `*create`

**Task files:** `.aexos-core/development/tasks/create-agent.md`, `create-task.md`, `create-workflow.md`
...
| `agent-template.yaml` | Template | EXISTS (in .aexos-core/product/templates/) |
| `task-template.md` | Template | EXISTS (in .aexos-core/product/templates/) |
| `workflow-template.yaml` | Template | EXISTS (in .aexos-core/product/templates/) |
| `security-checker.js` | Util | EXISTS (in .aexos-core/infrastructure/scripts/) |
| `yaml-validator.js` | Util | EXISTS (in .aexos-core/infrastructure/scripts/) |
...
flowchart TD
    ...
    M --> N{Validation pass?}
    N -->|yes| O[Write component to .aexos-core/development/{type}/]
    N -->|no| P[Report validation errors]
    O --> Q[Update manifest if applicable]
```

**Expected output:** New AEXOS component file written to appropriate directory.

---

### `*modify`

**Task files:** `.aexos-core/development/tasks/modify-agent.md`, `modify-task.md`, `modify-workflow.md`
...
**Task file:** `.aexos-core/development/tasks/update-manifest.md`
...
**Task file:** `.aexos-core/development/tasks/deprecate-component.md`
...
**Task file:** `.aexos-core/development/tasks/propose-modification.md`
...
**Task file:** `.aexos-core/development/tasks/undo-last.md`
...
**Task file:** `.aexos-core/development/tasks/validate-workflow.md`
...
| `yaml-validator.js` | Util | EXISTS |
| `.aexos-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task files:** `.aexos-core/development/tasks/run-workflow.md`, `run-workflow-engine.md`
**Template:** `.aexos-core/development/templates/subagent-step-prompt.md`
...
| `subagent-step-prompt.md` | Template | EXISTS (in .aexos-core/development/templates/) |
| `workflow-management.md` | Util | EXISTS (in .aexos-core/scripts/) |
| `.aexos-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task file:** `.aexos-core/development/tasks/analyze-framework.md`
...
flowchart TD
    ...
    B --> C[Scan .aexos-core/ directory structure]
...
**Task file:** `.aexos-core/development/tasks/execute-checklist.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `execute-checklist.md` | Task | EXISTS |
| `.aexos-core/product/checklists/{checklist}.md` | Checklist | Dynamically loaded |
...
flowchart TD
    ...
    B --> C{Name provided?}
    C -->|yes| D[Resolve checklist from .aexos-core/product/checklists/]
...
**Task file:** `.aexos-core/development/tasks/create-doc.md`
**Templates:** All 15 templates from `.aexos-core/product/templates/`
...
| `.aexos-core/product/data/elicitation-methods.md` | Data | EXISTS |
...
flowchart TD
    ...
    C -->|yes| D[Load template from .aexos-core/product/templates/]
...
**Task file:** `.aexos-core/development/tasks/add-tech-doc.md` (MISSING)
...
**Task file:** `.aexos-core/development/tasks/create-next-story.md`
...
**Task file:** `.aexos-core/development/tasks/advanced-elicitation.md`
...
**Task file:** `.aexos-core/development/tasks/correct-course.md`
...
**Task file:** `.aexos-core/development/tasks/index-docs.md`
...
| `*guide` | Renders the `## AEXOS Master Guide` section from agent .md |
...
| `*exit` | Exits aexos-master mode, returns to base Claude Code |
| `*agent {name}` | Reads and displays info about a specialized agent (use `@` to transform into it) |
| `*list-components` | Scans .aexos-core/ and lists all framework components (agents, tasks, workflows, templates, checklists) |
...
|-------------|-----------|---------|
| @aexos-master -> @pm | Delegate | Epic/story creation (brownfield-create-epic, brownfield-create-story) |
| @aexos-master -> @analyst | Delegate | Brainstorming sessions (*brainstorm) |
| @aexos-master -> @qa | Delegate | Test suite creation (*create-suite) |
| @aexos-master -> @architect | Delegate | AI prompt generation (*generate-ai-prompt) |
| @aexos-master -> @dev | Handoff | Story implementation after story creation |
| @aexos-master -> @devops | Delegate | Git push operations, PR creation |
| @aexos-master -> any agent | Execute | Can execute any task from any agent directly |
| Any agent -> @aexos-master | Escalate | Cross-agent coordination, framework operations |
...
|------------|---------------|-------------|
| `security-checker.js` | `.aexos-core/development/utils/` | `.aexos-core/infrastructure/scripts/security-checker.js` |
| `yaml-validator.js` | `.aexos-core/development/utils/` | `.aexos-core/infrastructure/scripts/yaml-validator.js` |
| `workflow-management.md` | `.aexos-core/development/utils/` | `.aexos-core/scripts/workflow-management.md` |
| `subagent-step-prompt.md` | `.aexos-core/product/templates/` | `.aexos-core/development/templates/subagent-step-prompt.md` |
| `elicitation-methods.md` | `.aexos-core/data/` | `.aexos-core/product/data/elicitation-methods.md` |
| `brainstorming-techniques.md` | `.aexos-core/data/` | `.aexos-core/product/data/brainstorming-techniques.md` |
