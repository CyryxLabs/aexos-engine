# @sm (Chronos) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.aexos-core/development/agents/sm.md`
...
|-------|------|--------|---------|
| 1 | `.aexos-core/development/agents/sm.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.aexos-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.aexos-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: devStoryLocation, storyBacklog, dataLocation |
| 4 | `.aexos-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |
| 5 | `.aexos-core/product/data/mode-selection-best-practices.md` | AgentConfigLoader.loadFile() | Mode selection best practices (always loaded, 10KB) |
...
sequenceDiagram
    ...
    ACL-->>CC: { agent: { name: 'Chronos', icon: '🌊' }, commands: [...6], ... }
...
sm:
  config_sections:
    ...
    - dataLocation
  files_loaded:
    - path: .aexos-core/product/data/mode-selection-best-practices.md
      lazy: false
      size: 10KB
    - path: .aexos-core/data/workflow-patterns.yaml
      lazy: false
      size: 8KB
...
| Greeting level | `persona_profile.greeting_levels.named` | `🌊 Chronos (Facilitator) ready. Let's flow!` |
| Greeting level | `persona_profile.greeting_levels.archetypal` | `🌊 Chronos the Facilitator ready to flow!` |
| Signature | `persona_profile.communication.signature_closing` | `— Chronos, fluindo com o time 🌊` |
...
**Task file:** `.aexos-core/development/tasks/sm-create-next-story.md`
**Template:** `.aexos-core/development/templates/story-tmpl.yaml` (MISSING in development/, EXISTS in `.aexos-core/product/templates/story-tmpl.yaml`)
...
| `.aexos-core/product/data/mode-selection-best-practices.md` | Data | Referenced by sm config |
...
**Checklist:** `.aexos-core/development/checklists/story-draft-checklist.md` (MISSING in development/, EXISTS in `.aexos-core/product/checklists/story-draft-checklist.md`)
...
- Course corrections needed (`@aexos-master *correct-course`)
...
|------|------|---------------|--------|
| `story-tmpl.yaml` | Template | `*draft` (sm-create-next-story.md) | MISSING in `.aexos-core/development/templates/`, EXISTS in `.aexos-core/product/templates/story-tmpl.yaml` |
| `story-draft-checklist.md` | Checklist | `*story-checklist` (execute-checklist.md) | MISSING in `.aexos-core/development/checklists/`, EXISTS in `.aexos-core/product/checklists/story-draft-checklist.md` |
