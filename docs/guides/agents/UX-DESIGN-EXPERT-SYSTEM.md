...
This document describes the complete system of the **UX-Design Expert (Iris)** agent, including all the files involved, workflows, available commands, integrations with other agents and AEXOS workflows.
...
flowchart TB
    ...
    subgraph PHASE2["Phase 2: Design System Audit"]
        A1["*audit {path}<br/>Scan for Redundancy"]
    ...
    subgraph PHASE3["Phase 3: Design Tokens"]
        ...
        T4["*upgrade-tailwind<br/>Tailwind v4 Upgrade"]
...
| File | Purpose |
|---------|-----------|
| `.aexos-core/development/agents/ux-design-expert.md` | Full agent definition (persona, commands, workflows) |
...
| File | Command | Purpose |
|---------|---------|-----------|
| `.aexos-core/development/tasks/audit-codebase.md` | `*audit {path}` | Scan the codebase for UI redundancy |
...
flowchart TB
    ...
    C --> D["*consolidate<br/>HSL 5% Clustering"]
...
|---------|-----------|---------|-------|
| `*research` | `ux-user-research.md` | Objectives, methods | personas.md, user-journeys.md, insights.md |
...
| `*guide` | N/A | N/A | Full agent guide |
...
flowchart TB
    subgraph AGENTS["Agent Collaboration"]
        ...
        subgraph DEV["Dev (Vulcan)"]
            DEV_DESC["Implements components<br/>Integrates the design system<br/>Code quality"]
        end

        subgraph QA["QA (Argus)"]
            QA_DESC["Component review<br/>Accessibility testing<br/>Visual validation"]
        end
...
graph TD
    ...
    D --> F{Docs complete?}
    F -->|Yes| G["ux-design-expert: WCAG AA a11y audit"]
    F -->|No| H["Complete the docs"]
    H --> D
...
- Use HSL 5% clustering for colors (perceptual similarity)
- Document manual overrides when necessary

### 3. Design Tokens

- tokens.yaml is the source of truth - all exports derive from it
- Use semantic naming (primary, not blue-500)
- OKLCH for modern colors with a hex fallback
...
- WCAG AA is the minimum, AAA is ideal
...
**Cause:** Very distinct patterns or a threshold that is too low
**Solution:**
- Review the patterns manually
- Adjust the HSL threshold (default 5%)
- Accept a smaller reduction with a justification
