...
This system implements the AEXOS **task-first architecture**, in which tasks are the main entry point for execution and agents orchestrate those tasks.

### System Purposes

- **Create squads** following AEXOS standards and structure
- **Validate squads** against JSON Schema and task specifications
...
### Agent Definition Core Files

| File | Purpose |
|---------|-----------|
| `.aexos-core/development/agents/squad-creator.md` | Core definition of the Squad Creator agent |
| `.claude/commands/AEXOS/agents/squad-creator.md` | Claude Code command that activates @squad-creator |

### @squad-creator Task Files

| File | Command | Purpose | Status |
|---------|---------|-----------|--------|
| `.aexos-core/development/tasks/squad-creator-create.md` | `*create-squad` | Creates a new squad with a complete structure | Active |
...
| `.aexos-core/development/tasks/squad-creator-validate.md` | `*validate-squad` | Validates a squad against schema and standards | Active |
...
| `.aexos-core/development/tasks/squad-generate-skills.md` | `*generate-skills` | Generates knowledge skills from the squad | Active |
...
| `.aexos-core/development/tasks/squad-creator-sync-aexos.md` | `*sync-squad-cyryx` | Synchronizes a squad with the AEXOS API | Placeholder (Sprint 8) |
...
| `.aexos-core/development/scripts/squad/squad-validator.js` | `SquadValidator` | Validates a squad against schema and standards |
...
| Directory | Purpose |
|-----------|-----------|
| `./squads/{squad-name}/` | Squad root directory |
...
|---------|-----------|----------|
| `*create-squad` | `squad-creator-create.md` | CREATE squad with a complete structure |
| `*create-squad --from-design` | `squad-creator-create.md` | CREATE squad from a blueprint |
| `*design-squad` | `squad-creator-design.md` | DESIGN squad through document analysis |
| `*validate-squad` | `squad-creator-validate.md` | VALIDATE squad against the schema |
...
| `*extend-squad` | `squad-creator-extend.md` | EXTEND squad with new components |

### Orchestration and Skills Commands (v2)

| Command | Task File | Operation |
|---------|-----------|----------|
| `*generate-skills` | `squad-generate-skills.md` | GENERATE skills from the squad knowledge |
| `*generate-workflow` | `squad-generate-workflow.md` | GENERATE orchestration workflow YAML |
| `*migrate-to-v2` | `squad-creator-migrate.md` | MIGRATE squad to the v2 format |

### Distribution Commands (Sprint 8 - Placeholders)

| Command | Task File | Operation |
|---------|-----------|----------|
| `*download-squad` | `squad-creator-download.md` | DOWNLOAD squad from aexos-squads |
| `*publish-squad` | `squad-creator-publish.md` | PUBLISH squad to aexos-squads |
| `*sync-squad-cyryx` | `squad-creator-sync-aexos.md` | SYNC squad to the AEXOS API |

### Individual Component Commands

| Command | Task File | Operation |
|---------|-----------|----------|
| `*create-agent` | `create-agent.md` | CREATE agent definition |
| `*create-task` | `create-task.md` | CREATE task file |
| `*create-workflow` | `create-workflow.md` | CREATE orchestration workflow |
...
| `SCHEMA_ERROR` | Error | Manifest does not match the JSON Schema |
| `FILE_NOT_FOUND` | Error | Referenced file does not exist |
...
| `AGENT_INVALID_FORMAT` | Warning | Agent file may not follow the format |
| `INVALID_NAMING` | Warning | File name is not kebab-case |
...
flowchart LR
    ...
    subgraph MARKET["💰 Level 3: Marketplace"]
        ...
        M_DESC["Premium squads via AEXOS API"]
...
1. **Always start with design** - Use `*design-squad` for complex projects
...
5. **Document thoroughly** - README.md and comments in YAML

### Component Organization

1. **Naming**: Always use kebab-case
2. **Tasks**: Include all required TASK-FORMAT-V1 fields
...
4. **Correction**: Address warnings for better quality

### Distribution

1. **Test locally** - Validate and use before publishing
2. **Documentation** - Complete README and clear description
...
### Squad does not show up in *list-squads

- Check that the directory exists under `./squads/`
- Check that `squad.yaml` or `config.yaml` exists
- Validate the manifest YAML syntax

### Validation fails with SCHEMA_ERROR

- Check the `name` field (must be kebab-case)
- Check the `version` field (must be semver: 1.0.0)
- Use a YAML linter to verify the syntax
...
### Task reports TASK_MISSING_FIELD
...
### Blueprint fails to generate

- Provide more detailed documentation
- Use `--verbose` to inspect the analysis
- Use `--domain` to give context

### *create-squad --from-design fails

- Check that the blueprint exists at the specified path
- Validate the blueprint YAML syntax
...
| **Support Scripts** | 9 scripts in squad/ |
