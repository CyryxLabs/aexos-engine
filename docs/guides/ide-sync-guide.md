...
Synchronize AEXOS agents, tasks, workflows, and checklists across multiple IDE configurations.

## Overview

The `*command` task automates the synchronization of AEXOS components to all configured IDE directories (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminating manual copy operations.
...
Edit `.aexos-sync.yaml` to enable your IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Read .aexos-sync.yaml configuration               │
...
```
your-project/
├── .aexos-sync.yaml           # Sync configuration
...
**Solution**: Add the alias to `.aexos-sync.yaml`:
...
- [AEXOS Architecture](../core-architecture.md)
