...
The MCP Global System allows you to configure MCP servers once and share them across all AEXOS projects. This eliminates the need to configure the same servers in every project.
...
- `~/.aexos/mcp/cache/` - Response cache
...
AEXOS includes templates for popular MCP servers:
...
Credentials are stored in `~/.aexos/credentials/` with a `.gitignore` to prevent accidental commits.
...
| Directory exists  | Use `aexos mcp setup --force` to recreate                          |
