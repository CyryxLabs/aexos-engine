...
Before using AEXOS, ensure you have:
...
AEXOS provides orchestrated structure while allowing flexibility in communication. This means:
...
### The AEXOS Difference

| Traditional AI Dev            | AEXOS                                    |
| ----------------------------- | --------------------------------------- |
...
AEXOS includes 11 specialized agents, each with a distinct role and personality:
...
@dev                # Activate Vulcan (Developer)
@qa                 # Activate Argus (QA)
@architect          # Activate Vega (Architect)
@aexos-master        # Activate Themis (Orchestrator)
...
Tasks are the primary entry point in AEXOS. Everything is a task.
...
```bash
# Start a workflow
cyryx workflow greenfield-fullstack

# With parameters
cyryx workflow brownfield-integration --target=./existing-project
```
...
Squads are modular teams of AI agents that extend AEXOS functionality.
...
# Download a squad
cyryx squads download etl-squad
...
```
my-project/
├── .aexos-core/                # Framework configuration
...
The primary configuration is in `.aexos-core/core/config/`:
...
AEXOS supports multiple IDEs. Configuration is synchronized across:
...
AEXOS implements 3-layer quality gates:
