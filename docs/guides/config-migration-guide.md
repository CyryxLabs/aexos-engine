...
AEXOS uses a 5-level configuration hierarchy that replaces the single `core-config.yaml` file:
...
| **L5** User | `~/.aexos/user-config.yaml` | Outside project | Cross-project user preferences |

Resolution order: L1 → L2 → Pro → L3 → L4 → L5 (last wins for scalars, deep merge for objects).
...
Migration preserves the original configuration in the ignored L4 file, including unknown extensions, falsy values and environment placeholders. It compares both raw retained values and resolved behavior before reporting success. The original bytes remain in `core-config.yaml.backup`; a failed migration restores the previous files and reports any incomplete recovery.

Legacy monolithic configuration treats `null` as a literal value. To preserve that behavior, migration may add reserved `_aexos_migration` metadata to L4 with `version: 1` and `literal_null_paths`, an array of key-segment arrays. These exceptions apply only to listed paths whose current L4 value is still `null`; the metadata is omitted from resolved configuration. Remove a path from the list to use the ordinary null-as-delete rule for that key. New unlisted nulls and L5 nulls still delete normally. A legacy collision with the reserved metadata key stops migration before writes.
