...
AEXOS provides a unified API for interacting with specialized AI agents through two primary mechanisms:
...
| ---------------- | ------ | ------------ | ---------------------------------------- |
| `@dev`           | Vulcan    | Builder      | Code implementation, debugging, testing  |
| `@qa`            | Argus  | Guardian     | Quality assurance, code review, testing  |
| `@architect`     | Vega   | Visionary    | System architecture, API design          |
| `@pm`            | Janus | Strategist   | Product requirements, epics, strategy    |
| `@po`            | Themis    | Champion     | Backlog management, acceptance criteria  |
| `@sm`            | Chronos  | Facilitator  | Sprint planning, story creation          |
...
1. Agent definition file is loaded from `.aexos-core/development/agents/{id}.md`
...
| Command                | Arguments       | Description                                 |
| ---------------------- | --------------- | ------------------------------------------- |
| `*create`              | `{type} {name}` | Create AEXOS component (agent/task/workflow) |
...
| `*kb`   | -                          | Toggle KB mode (AEXOS Method knowledge) |
...
| -------------------- | ------------------------------- | -------------------------------------- |
| `AGENT_NOT_FOUND`    | Agent definition missing        | Check `.aexos-core/development/agents/` |
...
/aexos-master  → Activates @aexos-master agent
```
...
- [User Guide](./user-guide.md) - Getting started with AEXOS
...
- [Squads Guide](./squads-guide.md) - Extending AEXOS with squads
