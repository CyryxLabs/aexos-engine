...
The Service Discovery system enables finding and using workers (tasks, templates, scripts, workflows) across the AEXOS framework. The **Service Registry** is the central catalog containing metadata about all available workers.
...
```bash
# Search by text
cyryx discover "create story"

# Search by category
cyryx discover --category task

# Search by tag
cyryx discover --tag testing

# Search for agent
cyryx discover --agent dev

# Combine filters
cyryx discover --category task --tag development --agent dev
```
...
# List all templates
cyryx list templates

# List workers for agent
cyryx list --agent dev

# List with pagination
cyryx list tasks --page 1 --limit 20
```
