...
AEXOS 2.1 introduced a new squad format with:
...
| AEXOS type field | Missing | `cyryx.type: squad` |
| Min version | Missing | `cyryx.minVersion: "2.1.0"` |
...
**Result:**
- Adds missing `cyryx` fields to manifest
...
- [ ] `cyryx.type` is `"squad"`
- [ ] `cyryx.minVersion` is `"2.1.0"` or higher
