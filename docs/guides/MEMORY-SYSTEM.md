...
4. [Camada 2: AEXOS Framework](#camada-2-aexos-framework)
...
O sistema de memoria do AEXOS opera em **duas camadas independentes** que coexistem mas **nao se comunicam entre si**:

| Camada | Gerenciado Por | Escopo |
|--------|---------------|--------|
| **Camada 1: Claude Code Nativo** | Claude Code CLI | Auto Memory, CLAUDE.md, Session Transcripts |
| **Camada 2: AEXOS Framework** | Scripts JS em `.aexos-core/` | Gotchas, Session State, Context Snapshots, Timeline |
...
- **Cada camada persiste de forma independente** — `.claude/` vs `.aexos/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>aexos-core/.claude/CLAUDE.md"]
    ...
    subgraph AEXOS_FRAMEWORK["Camada 2: AEXOS Framework"]
        direction TB
        GOTCHAS["Gotchas Memory<br/>.aexos-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot<br/>.aexos-core/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker<br/>.aexos-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.aexos-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.aexos-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.aexos-core/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.aexos-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.aexos-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.aexos-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_AEXOS["Storage AEXOS (.aexos/)"]
        ...
        S_TIMELINE[".aexos/timeline/"]
        S_FILE_EVO[".aexos/file-evolution/"]
        S_SESSION[".aexos/session-state.json"]
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.aexos-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.aexos-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.aexos-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.aexos-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.aexos-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. aexos-core/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. aexos-core/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart LR
    subgraph AGENT_MEM[".claude/agent-memory/"]
        AM1["aexos-architect/MEMORY.md"]
        AM2["aexos-dev/MEMORY.md"]
    ...
    NOTE["NOTA: Apenas agentes<br/>definidos em .claude/agents/<br/>usam esta memoria.<br/>Agentes AEXOS (.aexos-core/<br/>development/agents/)<br/>NAO usam."]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.aexos/gotchas.json<br/>.aexos/gotchas.md<br/>.aexos/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.aexos/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.aexos/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.aexos/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .aexos/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.aexos/session-state.json<br/>TTL: 1 hora")]
...
sequenceDiagram
    ...
    Note over CC: NENHUMA memoria AEXOS<br/>e carregada automaticamente<br/>no system prompt.<br/>Apenas contexto de sessao<br/>aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".aexos/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".aexos/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".aexos/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".aexos/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".aexos/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".aexos/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .aexos/session-state.json
    ...
    CL->>FS: Escreve .aexos/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .aexos/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .aexos/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .aexos/gotchas.md"]
...
classDiagram
    class GotchasStore {
        +string schema = "aexos-gotchas-memory-v1"
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.aexos/file-evolution/"]
        SRC2["ContextSnapshot<br/>.aexos/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".aexos/timeline/<br/>unified-timeline.json")]
    end
...
```mermaid
flowchart LR
    subgraph AEXOS_EVENTS["Eventos AEXOS"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
|---------|--------|------------|--------|
| `.aexos-core/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.aexos-core/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.aexos-core/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.aexos-core/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.aexos-core/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.aexos-core/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.aexos-core/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.aexos-core/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.aexos-core/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.aexos-core/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.aexos-core/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |

### Hooks (persistencia de sessao)

| Arquivo | CLI | Funcao |
|---------|-----|--------|
| `.aexos-core/hooks/gemini/session-start.js` | Gemini | Carrega contexto AEXOS no inicio da sessao |
| `.aexos-core/hooks/gemini/session-end.js` | Gemini | Persiste sumario da sessao em `.aexos/sessions/` |
| `.aexos-core/hooks/gemini/before-agent.js` | Gemini | Pre-processamento antes de agente |
| `.aexos-core/hooks/gemini/before-tool.js` | Ambos | Pre-processamento antes de tool |
| `.aexos-core/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.aexos-core/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.aexos-core/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.aexos-core/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `aexos-core/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `aexos-core/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
```
.aexos/                                        # Runtime state (gitignored)
...
Os modulos AEXOS (gotchas, snapshots, timeline) **nao fazem flush** automatico ao final da sessao. Dados em memoria podem se perder se o processo terminar abruptamente.
...
Claude auto-memory (`MEMORY.md`) e AEXOS memory (`.aexos/`) nunca se sincronizam. Gotchas capturadas pelo AEXOS nao aparecem no MEMORY.md e vice-versa.
...
| Gotchas Memory Script | `.aexos-core/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.aexos-core/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.aexos-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.aexos-core/core/memory/timeline-manager.js` |
| Session Context Loader | `.aexos-core/core/session/context-loader.js` |
| Context Detector | `.aexos-core/core/session/context-detector.js` |
| Session State | `.aexos-core/core/orchestration/session-state.js` |
| Unified Hook Interface | `.aexos-core/hooks/unified/hook-interface.js` |
| Hook Registry | `.aexos-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.aexos-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.aexos-core/hooks/gemini/session-end.js` |
...
*AEXOS Memory System Architecture Guide v1.0*
*Traced from source code, not documentation.*
*@architect (Vega) — arquitetando o futuro*
