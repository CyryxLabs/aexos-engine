...
The **@aexos-master** (Zeus - The Orchestrator) is the main meta-agent of the AEXOS-FULLSTACK framework. It acts as a universal orchestrator, framework developer and specialist in the AEXOS method. Its main responsibilities include:
...
- **Knowledge Base**: Access to the complete AEXOS Method knowledge
...
| `.aexos-core/development/tasks/modify-workflow.md` | `*modify workflow` | Modifies an existing workflow while keeping integrity |
...
| `.aexos-core/development/tasks/shard-doc.md` | `*shard-doc` | Breaks a document into smaller parts |
...
| File | Purpose |
|---------|-----------|
| `.aexos-core/development/agents/aexos-master.md` | Complete agent definition (persona, commands, dependencies) |
...
| `.aexos-core/development/templates/prd-tmpl.yaml` | Template for PRD |
| `.aexos-core/development/templates/story-tmpl.yaml` | Template for stories |
| `.aexos-core/development/templates/architecture-tmpl.yaml` | Template for architecture |
...
flowchart TB
    subgraph ACTIVATION["🚀 ACTIVATION"]
        ...
        GREETING["UnifiedActivationPipeline<br/>Smart Greeting"]
    end
...
|---------|-----------|----------|
| `*create agent {name}` | `create-agent.md` | Creates a new agent via progressive elicitation |
...
| `*modify workflow {name}` | `modify-workflow.md` | Modifies a workflow while keeping integrity |
...
| Command | Task File | Operation |
|---------|-----------|----------|
| `*analyze-framework` | `analyze-framework.md` | Complete framework analysis |
...
| `*shard-doc` | `shard-doc.md` | Breaks a document into parts |
...
- **Validate before using**: Run `*validate-component` after creation
- **Document dependencies**: List all dependencies in the component

### 2. Component Modification

- **Backup first**: The system creates a backup automatically, but verify it
...
- **Enable KB when needed**: Use `*kb` for queries about the framework
- **Do not load automatically**: The KB is only loaded on demand
...
2. Use `*list-components` to see the available components
...
**Solution:**
1. List the available templates: check `.aexos-core/development/templates/`
...
**Symptom:** Error when creating a backup or reverting
...
3. Continue from where it stopped or restart
...
|------|-------|-----------|
| 2026-02-04 | @aexos-master | Initial document created with complete mapping |
