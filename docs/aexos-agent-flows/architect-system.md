...
The **@architect** (Vega) agent is the **Holistic System Architect & Full-Stack Technical Leader** of AEXOS. This agent is responsible for:
...
2. **User Experience Drives Architecture** - Start with user journeys and work backwards
3. **Pragmatic Technology Selection** - Choose "boring" technology where possible, "exciting" where necessary
4. **Progressive Complexity** - Design systems that are simple at first but scalable
...
| `.aexos-core/development/tasks/document-project.md` | `*document-project` | Generates brownfield documentation of an existing project |
...
| `.claude/commands/AEXOS/agents/architect.md` | Claude Code command to activate @architect |
...
| `*execute-checklist` | `execute-checklist.md` | Runs an architecture checklist |
...
| `*guide` | Shows the agent's usage guide |
| `*yolo` | Toggle to skip confirmations |
| `*exit` | Exits architect mode |

---

## Workflows That Involve @architect
...
- Creates brownfield-architecture.md if necessary
...
- **Phase 6 (Plan):** Creates the implementation plan after approval
...
**@architect's role:**
- Receives escalations of specs BLOCKED by QA
...
collaboration_pattern:
  ...
  - question: "How should the data layer be integrated?"
...
graph TD
    subgraph DIMENSIONS["5 COMPLEXITY DIMENSIONS"]
        SCOPE["SCOPE\nHow many files are affected?\n1-5: 1-2 files up to 20+"]
        INTEG["INTEGRATION\nHow many external integrations?\n1-5: None up to orchestration"]
        INFRA["INFRASTRUCTURE\nInfra changes?\n1-5: None up to a new server"]
        KNOW["KNOWLEDGE\nKnowledge required?\n1-5: Existing patterns up to an unknown domain"]
        RISK["RISK\nImpact risk?\n1-5: Low up to critical"]
    end
...
```json
{
  ...
  "dimensions": {
    ...
    "knowledge": { "score": 2, "notes": "OAuth pattern exists in the codebase" },
...
1. **Use the Right Depth** - `shallow` for quick checks, `deep` for critical changes
...
- Break the analysis down by module
...
2. Use `*create-doc` without a template and pick one from the list
3. Create a custom template if necessary
...
- "How should the schema be modeled?" -> @db-sage
- "How should the data layer be integrated?" -> @architect designs, @db-sage implements the schema
```
...
### Problem: I cannot do a git push
