...
Guide for integrating AEXOS with supported IDEs and AI development platforms.
...
## Compatibility Contract (AEXOS 4.2.11)

The IDE matrix is enforced by a versioned contract:

- Contract file: `.aexos-core/infrastructure/contracts/compatibility/aexos-4.2.11.yaml`
...
AEXOS supports multiple AI-powered development platforms. Choose the one that best fits your workflow.

### Quick Status Matrix (AEXOS 4.2.11)
...
| Gemini CLI | Works | `/aexos-menu` then `/aexos-<agent>` | Works (minor differences in event handling) | -- |
| Codex CLI | Limited | `/skills` then `aexos-<agent-id>` | Limited (some checks need manual sync) | Run `npm run sync:ide:codex` and follow `/skills` flow |
...
Legend:
- `Works`: fully recommended for new users in AEXOS 4.2.11.
...
**Recommendation Level:** Best AEXOS integration

```yaml
config_file: .claude/CLAUDE.md
agent_folder: .claude/commands/AEXOS/agents
...
1. AEXOS automatically creates `.claude/` directory on init
...
# Verify setup
ls -la .claude/commands/AEXOS/agents/
```
...
special_features:
  ...
  - /skills activators (aexos-<agent-id>)
...
4. Use `/skills` and choose `aexos-architect`, `aexos-dev`, etc.
...
1. AEXOS creates `.cursor/` directory on init
...
2. AEXOS creates `.github/copilot-instructions.md`
...
1. AEXOS creates `.antigravity/` directory
...
agent_folder: .gemini/rules/AEXOS/agents
...
2. AEXOS creates:
   - `.gemini/rules.md`
   - `.gemini/rules/AEXOS/agents/*.md`
   - `.gemini/commands/*.toml` (`/aexos-menu`, `/aexos-<agent>`)
   - `.gemini/hooks/*.js`
...
4. Quick agent activation (recommended):
   - `/aexos-menu` to list shortcuts
   - `/aexos-dev`, `/aexos-architect`, `/aexos-qa`, etc.
   - `/aexos-agent <agent-id>` for generic launcher
...
AEXOS maintains a single source of truth for agent definitions and synchronizes them to all configured IDEs:

```
┌─────────────────────────────────────────────────────┐
│                    AEXOS Core                         │
│  .aexos-core/development/agents/  (Source of Truth)  │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
AEXOS can be configured to automatically sync on agent changes:

```yaml
# .aexos-core/core/config/sync.yaml
auto_sync:
  enabled: true
  watch_paths:
...
ls .claude/commands/AEXOS/agents/       # Claude Code
ls .gemini/rules/AEXOS/agents/          # Gemini CLI
```
...
├── Yes --> Claude Code (Best AEXOS integration)
└── No
...
# Verify migration
diff -r ./rules-backup/ .claude/commands/AEXOS/agents/
```
