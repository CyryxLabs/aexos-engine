...
### Why it matters for AEXOS

The AEXOS Unified Activation Pipeline runs `git status` during agent activation to detect modified files and project state. On large repositories, this can cause the `projectStatus` loader to timeout (~60% timeout rate observed). Enabling fsmonitor dramatically reduces this latency.
...
_This is an official AEXOS framework standard._
