...
Reference documentation for the AEXOS layered config system (ADR-PRO-002).
...
```
L1 Framework (.aexos-core/framework-config.yaml)   — Read-only, shipped with npm
  ↓ deep merge
L2 Project (.aexos-core/project-config.yaml)        — Team-shared, committed
  ↓ deep merge
Pro Extension (pro/pro-config.yaml)                 — Optional Pro submodule
  ↓ deep merge
L3 App ({appDir}/aexos-app.config.yaml)             — Monorepo app-specific
  ↓ deep merge
L4 Local (.aexos-core/local-config.yaml)            — Machine-specific, gitignored
  ↓ deep merge
L5 User (~/.aexos/user-config.yaml)                 — Cross-project user prefs
```
...
## SYNAPSE pipeline timeout (CORE-SU.A1 / #798)

SynapseEngine total layer budget (soft-fail: remaining layers skipped with **visible** `console.warn`).

| Precedence (high → low) | Source |
|-------------------------|--------|
| 1 | Env `AEXOS_SYNAPSE_PIPELINE_TIMEOUT_MS` |
| 2 | `synapse.pipelineTimeoutMs` in `.aexos-core/core-config.yaml` (or merged config) |
| 3 | Default **100** ms |

| Constraint | Value |
|------------|--------|
| Valid range | integer **1–30000** ms |
| Invalid / out of range | fall back to default + warn |
| Not aliased | `AEXOS_PIPELINE_TIMEOUT` is for **unified-activation-pipeline** only |
...
synapse:
  pipelineTimeoutMs: 500
```

```bash
export AEXOS_SYNAPSE_PIPELINE_TIMEOUT_MS=2000
```

Implementation: `.aexos-core/core/synapse/engine.js` (`resolvePipelineTimeoutMs`).
...
| `resource_locations.templates_dir` | string | `.aexos-core/development/templates` | L2+ | Templates path |
| `resource_locations.checklists_dir` | string | `.aexos-core/development/checklists` | L2+ | Checklists path |
| `resource_locations.tools_dir` | string | `.aexos-core/tools` | L2+ | Tools path |
| `resource_locations.scripts.*` | string | various | L2+ | Script paths by category |
| `resource_locations.data_dir` | string | `.aexos-core/data` | L2+ | Data directory |
...
### L5 User Keys (~/.aexos/user-config.yaml)
...
Schema files: `.aexos-core/core/config/schemas/`
