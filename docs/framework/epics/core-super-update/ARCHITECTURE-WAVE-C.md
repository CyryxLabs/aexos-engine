...
| Depends | ARCH-B (done), `aexos sdc` / `aexos wave` runtime (B7/B8) |
...
**Headline:** `aexos wave plan` → dispatch full-sdc per story → durable batch completion → devops handoff.

## 2. Already shipped (do not reimplement)

| Capability | Where |
|------------|--------|
| full-sdc EXECUTE loop | skills + `aexos sdc` |
| wave plan DAG + file partition | `.aexos-core/core/sdc/wave-plan.js` |
| Progress files | `.aexos/sdc/`, `.aexos/waves/` |
| Atomic SDC skills | validate/develop/review/apply-qa-fixes/close |
| Legacy WaveExecutor (task waves) | `.aexos-core/core/execution/wave-executor.js` — **workflow tasks**, not story SDC |
...
User / skill wave-execute
        ...
        ▼
┌───────────────────┐
│ aexos wave plan    │  DAG + partition (exists)
└─────────┬─────────┘
          │
          ▼
┌───────────────────┐
...
| ID | Title | Scope |
|----|-------|--------|
| **C1** | Wave run controller | Mark batch complete/failed; cascade-block dependents; `aexos wave advance`; resume mid-wave |
...
| **C3** | Epic orchestration glue | Read epic story table or `--stories` list; write wave summary report under `.aexos/waves/{id}/report.md` |
...
4. Poll / accept `aexos sdc status completed` per story
...
- CLI: extend `aexos wave` with `advance` / `report` (C1/C3)
