...
| Depends | Read-only access to peer hub tree (`AEXOS_HUB_ROOT`) and peer enterprise tree (`AEXOS_ENTERPRISE_ROOT`) |

## 1. Goal

Make framework drift visible before every future harvest. The super-update must not be a one-off manual comparison; it needs a repeatable diff harness for `.aexos-core` across OSS, hub, and enterprise.
...
- exact `.aexos-core` file counts per repo;
...
```bash
AEXOS_HUB_ROOT=../sinkra-hub AEXOS_ENTERPRISE_ROOT=../CYRYX-enterprise npm run diff:framework-3way
```
...
- [x] Script exists and is deterministic: `.aexos-core/infrastructure/scripts/framework-3way-diff.js`.
