...
Ship **invocable SDC skills** for OSS: validate → develop → review → close, composed by a **thin** `full-sdc` orchestrator. Tasks under `.aexos-core/development/tasks/` remain **source of truth**.
...
**Skill SOT path (framework):** `.aexos-core/development/skills/<name>/SKILL.md`  
...
```
SKILL.md body MUST:
  1. Name the task file path under .aexos-core/development/tasks/
...
| Surface | Path |
|---------|------|
| Runtime lib | `.aexos-core/core/sdc/` (`story-meta`, `progress`, `phase-verify`, `wave-plan`) |
| CLI | `aexos sdc …`, `aexos wave …` |
| Durable state | `.aexos/sdc/{story-id}/state.json`, `.aexos/waves/{wave-id}/state.json` |
...
| Grok | `.grok/skills/aexos-<name>/` via `grok-skills-sync` WORKFLOW_SKILLS or copy |
...
- [x] This document  
- [x] Skills land under `.aexos-core/development/skills/`  
- [x] full-sdc lean references only OSS tasks  
- [x] No workspace/ in skills  
- [x] Grok sync copies development/skills → `.grok/skills/aexos-*`  
- [x] `aexos sdc` + `aexos wave` execute/plan surfaces  
