# Epic CORE-SUPER-UPDATE: AEXOS Core Harvest from Hub & Enterprise
...
Verified `.aexos-core` file counts on 2026-07-09: OSS `1180`, hub `1272`, enterprise `1192`.
...
2. No `services/*`, Supabase Cyryx, `policy/cards`, **`workspace/`**, journey-log product
...
|-------|--------|-------|--------|
| CORE-SU.0 | 3-way `.aexos-core` diff harness | `npm run diff:framework-3way` + advisory doctor integration | ✅ Done |
...
See [ARCHITECTURE-WAVE-E.md](./ARCHITECTURE-WAVE-E.md). **E1 Done** — Articles XI+XII in `.aexos-core/constitution.md` v1.1.0 (no Workspace Bus). E3 cross-links optional. E4 **DEFERRED**.
...
| Generated local Codex/Claude artifacts | `.codex/` stays ignored; `.claude/skills/aexos-*` stay untracked unless hardened and intentionally versioned |
