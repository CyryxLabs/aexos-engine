...
`.aexos-core/core/synapse/engine.js` hardcodes:
...
2. **Given** config resolution order **env `AEXOS_SYNAPSE_PIPELINE_TIMEOUT_MS` > core-config `synapse.pipelineTimeoutMs` > default**, **when** values conflict, **then** higher-precedence wins.  
   - Do **not** alias `AEXOS_PIPELINE_TIMEOUT` (used by unified-activation-pipeline) without documenting both.
...
- `.aexos-core/core/synapse/engine.js` — resolvePipelineTimeoutMs + warn on timeout
- `.aexos-core/core/synapse/runtime/hook-runtime.js` — loadCoreConfig + pass `synapse` to SynapseEngine
- `.aexos-core/core-config.yaml` — `synapse.pipelineTimeoutMs`
