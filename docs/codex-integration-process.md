# Integracao AEXOS com Codex CLI (Estado Atual)

Este documento descreve o estado operacional atual da integracao AEXOS + Codex CLI no AEXOS `4.2.11`.
O foco aqui nao e historico: e operacao pratica, compatibilidade real e como manter sem regressao.

## Resumo Executivo

O Codex hoje e alvo de primeira classe no AEXOS:
...
Em termos praticos: Codex esta no mesmo trilho arquitetural das integracoes principais do AEXOS, sem caminho paralelo legado.

## Status de Compatibilidade (AEXOS 4.2)

| O que você quer fazer | Funciona no Codex? | Como fazer |
| --- | --- | --- |
| Ativar agentes AEXOS | Works | `/skills` depois escolha `aexos-<agent-id>` |
| Sincronizar e validar arquivos AEXOS | Works | `npm run sync:ide:codex` e `npm run validate:codex-sync` |
...
- Agentes canonicos: `.aexos-core/development/agents/*.md`
- Regras de projeto Codex: `AGENTS.md`

### Pipeline de ativacao

- Runtime: `.aexos-core/development/scripts/activation-runtime.js`
- Entrada de greeting: `.aexos-core/development/scripts/generate-greeting.js`
...
- Skills locais: `.codex/skills/aexos-*/SKILL.md`
...
4. No Codex, usar `/skills` e escolher `aexos-<agent-id>` como ativacao padrao.
...
- Integracao estrutural: `.aexos-core/infrastructure/scripts/validate-codex-integration.js`
- Skills: `.aexos-core/infrastructure/scripts/codex-skills-sync/validate.js`
- Paths/contratos: `.aexos-core/infrastructure/scripts/validate-paths.js`
...
- `.codex/skills/aexos-*/SKILL.md` existente
- validadores sem erro

## Recursos Recentes do Codex (AEXOS 4.0)
...
No AEXOS, o caminho recomendado continua: `AGENTS.md` + `/skills` + MCP + scripts de sync/validacao.
...
- menor automacao de eventos de ciclo de sessao (`SessionStart/SessionEnd`) no padrao AEXOS
...
Mitigacao operacional no AEXOS:
...
- garantir skill apontando para agente canonico em `.aexos-core/development/agents/`
