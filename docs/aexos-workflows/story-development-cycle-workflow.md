...
| `feature-development` | Development of new functionality |
| `bug-fix` | Bug fixing |
...
The Scrum Master (Chronos) identifies and creates the next story from the backlog using the sharded PRD or the project documentation as the source.
...
| # | Check | Description |
|---|-------|-----------|
| 1 | Clear and objective title | The title describes precisely what will be done |
...
| 5 | Dependencies mapped | Prerequisite stories or resources identified |
| 6 | Complexity estimate | Complexity points or T-shirt sizing |
| 7 | Business value | Clear benefit for the user/business |
...
| **Rejected** | Returns to the SM with detailed feedback |

#### Story Status

- **Before:** `Draft`
- **After (Success):** `Ready`
- **After (Failure):** `Draft` (returns to the SM)
...
| Mode | Description | Prompts to the User |
|------|-----------|-------------------|
...
**Relevant Commands:**
- `*validate-story-draft {story}` - Validate the story quality
...
| **Focus** | Execute story tasks with precision and comprehensive tests |
...
- `*gate {story}` - Create the quality gate decision
...
| QA Gate | `qa-gate.md` | @qa | Create the quality gate decision |
...
1. **core-config.yaml** - Mandatory AEXOS configuration file
...
3. **Checklists** - Mandatory checklists available:
...
|-----------|-------|-------------|
| PRD (sharded or monolithic) | According to the `prd.*` config | Yes |
| Epic file | `docs/stories/epic-X/` | Yes |
| Architecture docs | According to the `architecture.*` config | Yes |
...
| PRD | Document | According to the config | Product Requirements |
...
| Implementation files | Code | According to the story tasks | Implemented code |
| Test files | Code | According to the project standard | Unit tests |
...
|-------|------|---------|----------|-------------------|-------------------|
| PO validation | 2 | @po | The 10 checks pass | Status: Ready | Returns to SM |
...
1. **Unapproved dependencies required** - A new library or resource is required
2. **Ambiguity after checking the story** - Requirements are not clear
3. **3 consecutive failures** - Implementation or correction attempts
4. **Configuration missing** - core-config.yaml or templates absent
5. **Regression failing** - Existing tests broke
...
- Generates `decision-log-{story-id}.md` with all the decisions
...
- Complete analysis of ambiguities before starting
- Comprehensive upfront questionnaire
- Execution without ambiguities after planning
...
- The epic file does not exist
- The PRD is not available
...
# Check the PRD
cat docs/prd/PRD.md  # or located according to the config
```

#### 2. PO validation fails repeatedly

**Symptom:** The story returns to the SM multiple times
...
3. Fill in the Dev Notes with architecture references
...
2. Run `*apply-qa-fixes` in @dev
3. Re-submit for review

#### 5. Workflow stalls at a handoff
...
- A dependency from the previous step is not satisfied
...
- When a complete cycle with validation and quality gate is needed
- When process traceability is needed
