---
task: auto-discover-ecosystem
owner: '@dispatch-chief'
owner_type: agent
atomic_layer: task
...
| `.aexos-core/` agents changed | Run full discovery |
...
- [ ] **1.2** Run domain registry builder:
  ...
  ```
  - Expected: domain-registry.yaml generated with all domains from squads + .aexos-core
  - Verify: File exists and is non-empty
...
- [ ] **3.1** Generate discovery summary:
  ...
  **Command Registry:**
  ...
  - Total agents: {count}
  - Total tasks: {count}
  - Sources scanned: squads/, .claude/commands/, .aexos-core/

  **Domain Registry:**
  - Total domains: {count}
  - From squads: {count}
  - From .aexos-core: {count}
  - Built-in: {count}
...
| 2 | command-registry.yaml contains .aexos-core agents (if .aexos-core exists) | AEXOS section present with agents |
| 3 | domain-registry.yaml contains domains for each squad (except dispatch itself) | Domain count >= squad count - 1 |
| 4 | domain-registry.yaml contains .aexos-core domains (if .aexos-core exists) | @dev, @qa, @architect etc. present |
...
| 7 | Works without .aexos-core | Run with --project-root on dir without .aexos-core, no errors |
