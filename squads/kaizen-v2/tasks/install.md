...
### Acceptance Criteria
- [ ] Auto-detect AEXOS or AEXOS project (check for .aexos-core/ or .aexos/)
- [ ] Hooks registered in `.claude/settings.json` (Stop and SessionStart)
- [ ] `.aexos/logs/` directory created
...
```bash
# Check for AEXOS or AEXOS
if [ -d .aexos-core/ ]; then PROJECT_TYPE="AEXOS"; fi
if [ -d .aexos/ ]; then PROJECT_TYPE="AEXOS"; fi
```
If neither found: ABORT with error "Not an AEXOS or AEXOS project".
...
1. Project type detected (AEXOS/AEXOS)
