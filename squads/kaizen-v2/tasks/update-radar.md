...
task:
  ...
  status: ready
  responsible_executor: kaizen-v2-tech-radar
  ...
  input:
    ...
    - "Capability gaps from kaizen-v2-capability-mapper"
  ...
  output:
    ...
    - artifact: radar_report
      format: "markdown"
      template: "templates/kaizen-v2-tech-radar-tmpl.md"
  ...
  veto_conditions:
    ...
    - "Nova tool em Assess sem gap correspondente no kaizen-v2-capability-mapper → REMOVER"
    - "Consolidação recomendada sem grep de referências no codebase → BLOQUEAR (GATE-RD-VERIFY)"

  action_items:
    - "Run kaizen-v2-tech-radar *radar"
    - "Save updated radar to data/radar/radar-{date}.yaml"
    - "Hand off tool gap recommendations to kaizen-v2-chief"
