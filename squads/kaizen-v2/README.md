...
- **v1 base:** análise semanal do ecossistema via 7 agentes (topology, performance, bottleneck, capability, radar, cost, kaizen-v2-chief)
...
│
├── agents/
│   ├── kaizen-v2-chief.md                  # Orchestrator (atualizado para v2)
│   ├── kaizen-v2-memory-keeper.md                 # Tier 0 Sensorial [NOVO]
...
| `*install` | Auto-detecção AEXOS/AEXOS + merge hooks + init dirs |
...
Periodicamente (overnight via cron ou manual via `*reflect`), o kaizen-v2-memory-keeper analisa os dailies e extrai patterns verificados.
...
- **AEXOS 2.1.0+** (detecção automática)
- **AEXOS 1.0.0+** (detecção automática)
...
| **Agentes** | 7 | 8 (+ kaizen-v2-memory-keeper) |
| **Ativação** | `/kaizen:*analyze` | `/kaizen-v2:*capture`, `/kaizen-v2:*report` |

## Logs & Debug

- Hook logs: `.aexos/logs/kaizen-*.log`
