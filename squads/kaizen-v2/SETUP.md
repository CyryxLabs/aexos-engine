...
# Check logs
tail -f .aexos/logs/kaizen-stop.log
...
4. Check `.aexos/logs/kaizen-stop.log` for error details
...
# Check logs
tail -f .aexos/logs/kaizen-session-briefing.log
```
...
- **Stop hook:** `.aexos/logs/kaizen-stop.log`
- **SessionStart hook:** `.aexos/logs/kaizen-session-briefing.log`
...
## Integration with AEXOS/AEXOS

### Auto-Detection

`tasks/install.md` detects:
```bash
if [ -d .aexos-core/ ]; then PROJECT_TYPE="AEXOS"
elif [ -d .aexos/ ]; then PROJECT_TYPE="AEXOS"
else echo "ERROR: Not AEXOS or AEXOS"; fi
```
...
- Review `.aexos/logs/kaizen-*.log` for WARN/ERROR
