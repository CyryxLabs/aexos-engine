...
O Squad Creator gerencia os squads da sua instalação AEXOS. Use o comando `*refresh-registry` para ver estatísticas atualizadas do seu ecossistema.
...
Regras mínimas:
- `read_only|read_write|workspace_first`: precisa ter referências explícitas a `.aexos/project-context/`.
...
> Os agentes [PRO] são ativados automaticamente quando o AEXOS Pro está instalado.
...
| `*clone-mind {name}` | Extrair Voice + Thinking DNA | `.aexos/squad-runtime/minds/{slug}/` |
...
| `*validate-agent {file}` | AEXOS 6-level structure | SC_AGT_001 |
...
1. Copie a pasta `squads/squad-creator/` para o seu projeto AEXOS
...
3. Blueprint de Squad AEXOS pronto para criação imediata
...
> Esta funcionalidade requer o AEXOS Pro (Cohort Avançado).
...
- Verificação de conformidade com padrões AEXOS
...
## Integração com AEXOS Core

O Squad Architect integra perfeitamente com:

1. **AEXOS Developer Agent** - Pode usar AEXOS-developer para modificações avançadas de componentes
...
O AEXOS Pro possui 31+ squads em produção, incluindo:
...
> Estes squads foram criados usando o squad-creator e estão disponíveis no AEXOS Pro (Cohort Avançado).
...
- Framework AEXOS-FULLSTACK core
- AEXOS Developer agent (opcional, para modificações avançadas)
...
**Nota:** Pesquisa automática (mind-research-loop) é uma funcionalidade do AEXOS Pro. No modo base, o agent coleta informações do domínio através de perguntas estruturadas.
...
- Squads gerados seguem padrões AEXOS-FULLSTACK automaticamente
