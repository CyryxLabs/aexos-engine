...
Usage:
    ...
    python scripts/squad-analytics.py --registry-path .aexos/squad-runtime/ecosystem-registry.yaml
"""
...
REGISTRY_PATH_ENV = "AEXOS_ECOSYSTEM_REGISTRY_PATH"
DEFAULT_REGISTRY_REL_PATH = Path(".AEXOS") / "squad-runtime" / "ecosystem-registry.yaml"
...
def quality_audit(squad_data: Dict[str, Any]) -> Dict[str, Any]:
    """Run quality audit against AEXOS standards."""
    audit = {
...
def format_table(results: Dict[str, Any], detailed: bool = False) -> str:
    ...
    lines.append("=" * 100)
    lines.append("📊 AEXOS SQUAD ANALYTICS")
...
def format_single_squad(
    ...
    if quality_audit_flag and squad_data.get("line_counts"):
        ...
        lines.append("")
        lines.append("-" * 80)
        lines.append("🔍 QUALITY AUDIT (AEXOS Standards)")
        lines.append("-" * 80)
...
def main() -> None:
    ...
    parser.add_argument("--quality-audit", "-q", action="store_true", help="Run quality audit against AEXOS standards (requires --squad)")
