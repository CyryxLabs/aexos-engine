...
check_structure() {
  ...
  if [[ -n "$workspace_level" && "$workspace_level" != "none" ]]; then
    if rg -n "\.aexos/project-context/" "$SQUAD_DIR/agents" "$SQUAD_DIR/tasks" "$SQUAD_DIR/workflows" "$SQUAD_DIR/squad.yaml" >/dev/null 2>&1; then
...
check_production() {
  ...
  # Canonical runtime evidence must live in .aexos/squad-runtime
  local has_outputs=false
  local runtime_root="${AEXOS_RUNTIME_ROOT:-./.aexos/squad-runtime}"
  if [ -d "$runtime_root" ]; then
    ...
    if [ -n "$squad_runtime_dir" ] && [ -d "$squad_runtime_dir" ]; then
      ...
      if [ "$output_count" -gt 0 ]; then
        log_pass "Found runtime evidence in .aexos/squad-runtime ($output_count files)"
        has_outputs=true
  ...
  if [ "$has_outputs" = false ]; then
    log_warn "No runtime evidence found in .aexos/squad-runtime for squad '$SQUAD_NAME'"
  fi
...
analyze_with_claude() {
  ...
  local ANALYSIS_PROMPT="You are evaluating the quality of an AEXOS squad. Analyze and provide a JSON response.
