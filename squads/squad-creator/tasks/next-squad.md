...
- [ ] `{registry_path}` resolved via `AEXOS_ECOSYSTEM_REGISTRY_PATH` (fallback: `.aexos/squad-runtime/ecosystem-registry.yaml`)
...
scan_locations:
  ...
  - pattern: ".aexos/squad-runtime/**/*"
    look_for: "Ad-hoc outputs in domains without squads"
    signal: ".aexos/squad-runtime/legal-analysis/ exists but no legal squad → organic demand"

  - pattern: ".aexos/project-status.yaml"
...
removed_squads_triage:
  ...
  check:
    ...
    - "Are there residual artifacts in .aexos/squad-runtime/ or docs/?"
  classify:
...
| Candidate scores | YAML (optional) | `.aexos/squad-runtime/next-squad/scores-{date}.yaml` |
