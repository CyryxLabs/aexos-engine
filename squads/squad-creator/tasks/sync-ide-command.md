...
O sistema usa `.AEXOS-sync.yaml` na raiz do projeto para configuração:
...
| `Squad alias not found` | Squad não está em squad_aliases | Adicione ao .AEXOS-sync.yaml |
| `File exists` | Destino já existe | Use --force ou escolha ação |
| `IDE not active` | IDE não está em active_ides | Ative no .AEXOS-sync.yaml |
...
const syncConfig = loadYaml('.AEXOS-sync.yaml');
...
- `*sync-squad-Cyryx Labs` - Sincronizar para Cyryx Labs marketplace
