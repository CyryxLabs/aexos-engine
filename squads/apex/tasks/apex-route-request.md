...
apex_lead_escalation:
  ...
  action: "Escalate to @AEXOS-master for framework-level triage"
  message: |
    ...
    Escalating to @AEXOS-master for cross-squad routing or new capability assessment.
...
| Backend API implementation | `@dev` (Dex) — AEXOS core agent |
| Database schema changes | `@data-engineer` (Dara) — AEXOS core agent |
| Product requirements | `@pm` (Morgan) — AEXOS core agent |
| Story creation | `@sm` (River) — AEXOS core agent |
| Epic management | `@pm` (Morgan) — AEXOS core agent |
