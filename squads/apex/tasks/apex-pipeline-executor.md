...
outputs:
  - Pipeline state file at .aexos/apex-pipeline/{pipeline-id}.yaml
...
5. Create state file at `.aexos/apex-pipeline/{pipeline-id}.yaml`
...
1. Look for pipeline state files in `.aexos/apex-pipeline/`
...
1. Load current pipeline state from `.aexos/apex-pipeline/`
...
Artifacts preserved at: .aexos/apex-pipeline/{pipeline-id}.yaml
...
2. Stores artifact at `.aexos/handoffs/handoff-{from}-to-{to}-{timestamp}.yaml`
...
4. If ANY fail → gate FAIL:
   ...
   - If max reached → escalate: agent → apex-lead → AEXOS-master
...
State file location: `.aexos/apex-pipeline/{pipeline-id}.yaml`
...
Gate FAIL → Create FIX sub-task for responsible agent
         ...
         → apex-lead cannot resolve → escalate to AEXOS-master
```
...
Agent fails → Retry 1x
           ...
           → apex-lead assigns alternative approach or escalates to AEXOS-master
```
...
ship_to_devops:
  description: >
    ...
    .aexos/handoffs/ for @devops (Gage). This handoff is MANUAL — the user
    must activate @devops and run *push. Apex CANNOT push to remote.
  artifact_path: ".aexos/handoffs/handoff-apex-lead-to-devops-{timestamp}.yaml"
  artifact_contents:
