...
**Prerequisite:** At least one prior `*scrape` or `*extract-tokens` extraction must exist (cached in `.aexos/apex-context/extractions/`).
...
```yaml
extraction_history:
  cache_location: ".aexos/apex-context/extractions/"
  scan:
    - "*.extraction.yaml"
    - "*.extraction.json"
    - "*.fusion-ready.yaml"
