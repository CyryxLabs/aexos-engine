...
```yaml
action: Read pipeline state from .aexos/apex-pipeline/{pipeline-id}.yaml
...
actions:
  ...
  - "Copy current pipeline state to .aexos/apex-pipeline/{pipeline-id}.rollback-backup.yaml"
```
