...
const EXCLUDE_PATTERNS = [
  ...
  /docs\/standards\/AEXOS-LIVRO-DE-OURO/,
  /docs\/standards\/CYRYX-FRAMEWORK-MASTER\.md$/,
...
async function generateManifest() {
  ...
  if (!fs.existsSync(cyryxCoreDir)) {
    throw new Error(`.aexos-core directory not found at ${cyryxCoreDir}`);
  }
  ...
  console.log(`Generating manifest for aexos-core v${version}...`);

  const allFiles = [];

  // Scan folders
  for (const folder of FOLDERS_TO_COPY) {
    const folderPath = path.join(cyryxCoreDir, folder);
    if (fs.existsSync(folderPath)) {
      scanDirectory(folderPath, cyryxCoreDir, allFiles);
    }
  }

  // Add root files
  for (const file of ROOT_FILES_TO_COPY) {
    const filePath = path.join(cyryxCoreDir, file);
  ...
  for (const fullPath of allFiles) {
    try {
      const relativePath = path.relative(cyryxCoreDir, fullPath).replace(/\\/g, '/');
...
async function writeManifest(manifest) {
  const manifestPath = path.join(__dirname, '..', '.aexos-core', 'install-manifest.yaml');
  ...
  // Add header comment
  const header = `# CYRYX-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('AEXOS Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
