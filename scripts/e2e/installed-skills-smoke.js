...
const { resolveNpmInvocation } = require('../../packages/installer/src/wizard/pro-setup')._testing;
const npmInvocation = resolveNpmInvocation({
  env: { ...process.env, npm_execpath: process.env.AEXOS_PARITY_NPM_CLI || process.env.npm_execpath },
});
...
function run(command, args, options = {}) {
  let useWindowsNpmShim = false;
  if (command === 'npm') {
    args = [...npmInvocation.prefixArgs, ...args];
    command = npmInvocation.command;
    useWindowsNpmShim = process.platform === 'win32' && command === 'npm.cmd';
  ...
  let result;
  if (useWindowsNpmShim) {
    // execa's Windows command-shim handling retains argument boundaries.
    try {
      const executed = require('execa').sync(command, args, { ...executionOptions, reject: false });
      result = { ...executed, status: executed.exitCode };
    } catch (error) { result = { error }; }
  } else result = spawnSync(command, args, executionOptions);
...
function runInstalledCli(args, options = {}) {
  return run('npm', ['exec', '--offline', '--', 'aexos-core', ...args], options);
}
...
async function main() {
  ...
  let tarballPath = process.env.AEXOS_E2E_TARBALL && path.resolve(process.env.AEXOS_E2E_TARBALL);
  if (tarballPath) {
    assertAbsolutePathExists(tarballPath, 'file');
    log('Using the supplied packaged artifact');
  } else {
  ...
  console.log(JSON.stringify({ status: 'passed', isolated_consumer: projectRoot,
    package_root: path.join(projectRoot, 'node_modules', '@aexos', 'core') }));
}
