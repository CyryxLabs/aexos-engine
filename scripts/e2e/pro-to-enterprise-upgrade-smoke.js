...
const cliPath = path.join(repoRoot, 'bin', 'aexos.js');
const keepTemp = process.env.AEXOS_E2E_KEEP_TEMP === '1';
const verbose = process.env.AEXOS_E2E_VERBOSE === '1';
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'aexos-pro-enterprise-e2e-'));
...
function runCli(args, options = {}) {
  ...
  if (result.error) {
    fail(`Command failed to start: cyryx ${args.join(' ')}`, result.error.message);
  }

  if (result.status !== 0) {
    fail(
      `Command failed (${result.status}): cyryx ${args.join(' ')}`,
...
function setupTarget() {
  ...
  writeFile('.aexos-core/.installed-manifest.yaml', 'installedVersion: 5.1.15\n');
  ...
  writeFile('bin/aexos.js', 'console.log(JSON.stringify({ summary: { fail: 0 } }));\n');
...
function setupEnterpriseFixture() {
  ...
  writeFile('.aexos-sync.yaml', 'active_ides:\n  - claude\n  - codex\n', enterpriseSource);
  writeFile('package.json', '{"name":"aexos-enterprise","version":"1.0.0"}\n', enterpriseSource);
...
function main() {
  try {
    ...
    const executionManifestPath = path.join(targetDir, '.aexos', 'enterprise-upgrade-manifest.yaml');
