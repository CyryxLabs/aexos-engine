...
function shouldCheckManifest(stagedFiles) {
  ...
  return stagedFiles.some((file) => {
    if (file === '.aexos-core/install-manifest.yaml') return false;
    return file.startsWith('.aexos-core/');
  });
}
...
async function main() {
  ...
  if (!shouldCheckManifest(stagedFiles)) {
    console.log('ℹ️ manifest: no .aexos-core changes staged, skipping check');
    return;
  }
  ...
  execSync('git add .aexos-core/install-manifest.yaml', { stdio: 'inherit' });
