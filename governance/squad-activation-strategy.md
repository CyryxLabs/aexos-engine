...
**Status:** Draft v1.0
**Date:** 2026-05-07
**Author:** @aexos-master (Zeus)
**Approval gate:** Paulo Petruff
**Source finding:** [`AF-20260507-aexos-master-routing-gap`](../audits/promoted/AF-20260507-aexos-master-routing-gap.yaml)

---

## Problem statement

`@aexos-master` (the framework's primary orchestrator) currently has **no routing logic to consult specialized squads first** before delegating to itself or to YOLO terminals. Concrete consequences observed during EPIC-MVP-CDM-CADASTRO Wave 1.A:

1. Voice-clone squad (Akita, Montano, Kira) **never auto-consulted** even when their domains were directly relevant. Paulo Petruff had to explicitly invoke them.
2. `claude-code-mastery` squad (which exists in `aexos-core/squads/`) **never invoked** despite Claude Code questions arising naturally.
...
Paulo Petruff's diagnostic: *"todos os plans criados cogitaram em nenhum momento a perfeita integração desses squads, por isso criamos o business-ai-first baseado no cyryx e departamentos da minha empresa."*

---

## Design principles (Paulo Petruff-stated, Akita-aligned)
...
3. **Heuristic-driven.** Use keyword/intent matching to suggest a squad. The orchestrator suggests; Paulo Petruff can override.
...
5. **Backward compatible.** Existing `@aexos-master` invocations keep working. The squad routing is additive.

---

## Squad inventory (current)

### Framework-bound squads (in `aexos-core/squads/`)
...
## Routing matrix for `@aexos-master`

The matrix below extends `@aexos-master`'s YAML definition. It mirrors the pattern found in `claude-mastery-chief` (`triage.routing_matrix`) and adapts it for cross-squad orchestration.
...
`@aexos-master` answers directly without invoking squads when the request is:
...
- Meta-framework operations explicitly within @aexos-master scope (`*create agent`, `*modify task`)
...
## Patch for `@aexos-master.md` (proposed extension)

The following YAML block should be added to `aexos-core/.aexos-core/development/agents/aexos-master.md` after the `core_principles` section. (Mirrored to all `aexos-master.md` instances in `.antigravity/`, `.claude/commands/`, `.codex/`, `.cursor/`.)
...
triage:
  ...
  direct_answer_domains:
    ...
    - "Meta-framework ops within @aexos-master scope"
    - "Trivial questions (1-2 sentences)"
...
When a project-local agent (e.g., `@aexos-master` in `business-ai-first`) discovers a Claude Code-related improvement (hook pattern, skill convention, settings.json structure), it should propose enhancement to `claude-code-mastery` squad in `aexos-core`.
...
[business-ai-first agent learns X about Claude Code]
   ...
   Proposer drafts FrameworkProposal
        ↓
   Paulo Petruff approves
        ↓
   PR in aexos-core/squads/claude-code-mastery/
        (e.g., adding new tactic to hooks-architect agent)
        ↓
   business-ai-first pulls aexos-core update
        ↓
   Knowledge propagated to all consumer projects
```

This closes the loop Paulo Petruff pointed out: *"o agente deve se juntar ao time do aexos-core para contribuir."*
...
Companion to `migration-dictionary-guard.cjs` (already shipping). Lives at `aexos-core/hooks/squad-suggestion-advisor.cjs`.
...
| Step | Owner | When |
|---|---|---|
| 1. PR in aexos-core with: rule + agent patch + hook + this strategy doc | @aexos-master | After Paulo Petruff approves |
| 2. business-ai-first pulls aexos-core | Paulo Petruff (manual git pull or npm update) | Next session of business-ai-first |
| 3. Verify @aexos-master in business-ai-first now exhibits squad-suggesting behavior | Paulo Petruff + @aexos-master | First task of next session |
| 4. Iterate keyword list based on observed misses | @aexos-master (audit findings → proposals) | Continuous |
...
| Hardcode squad list in `@aexos-master` (no extensibility) | Squads come and go; matrix must be data-driven |
...
- [ ] FrameworkProposal `PROP-20260507-squad-routing-strategy.yaml` written and approved by Paulo Petruff
- [ ] PR in aexos-core merging:
  - [ ] `aexos-core/.claude/rules/squad-activation.md` (synthesizes this doc as rule)
  - [ ] `aexos-core/hooks/squad-suggestion-advisor.cjs` (advisory implementation)
  - [ ] Patch to all `aexos-master.md` instances adding `triage` block
  - [ ] This strategy doc at `aexos-core/governance/squad-activation-strategy.md`
- [ ] business-ai-first pulls aexos-core update
- [ ] First-session validation: @aexos-master suggests `db-inventory` consult before next migration
- [ ] First-session validation: @aexos-master suggests voice-clone when "parecer" keyword appears
...
- A 2026-05-07 workspace audit informed this work. It was removed from the
  repository: it inventoried a third party's local directory and private
  projects and had no bearing on the framework.
- `business-ai-first/docs/decisions/ADR-DBOPS-V1-vocabulary-contract-failure.md` — original triggering ADR
- `business-ai-first/.aexos/handoffs/handoff-2026-05-07-phase1-to-phase2-data-contract.yaml` — Phase 2 contract referencing this strategy

---

— Zeus (`@aexos-master`), respondendo ao gap arquitetural sinalizado por Paulo Petruff: o orquestrador principal precisa consultar o squad certo antes de delegar
