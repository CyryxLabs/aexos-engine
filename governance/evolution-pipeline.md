# AEXOS Framework Evolution Pipeline

**Status:** Draft v1.0
**Date:** 2026-05-07
**Author:** @aexos-master (Zeus)
**Approval gate:** Paulo Petruff (sole orchestrator approver)
...
Define how the AEXOS framework (`aexos-core`) evolves from real-world findings discovered in consumer projects (e.g., `business-ai-first`). The framework is not static — it grows via **audited evidence** from production usage. This document specifies the formal pipeline:

```text
Project audit → Finding → Proposal → Paulo Petruff approval → aexos-core PR → Distribution
```
...
- Paulo Petruff as orchestrator decides what is generic enough to promote
...
### Auditor (any agent or Paulo Petruff)
...
### Proposer (typically @aexos-master or domain specialist)
...
### Approver (Paulo Petruff — sole authority)
- Reviews FrameworkProposal
- Decides: APPROVED / REJECTED / NEEDS_REVISION
- If APPROVED: signs the proposal with `approver_approval` field + timestamp
- Approval triggers PR creation against `aexos-core`

### Implementer (@aexos-master + relevant specialist)
- Opens PR in `aexos-core` with the changes specified in approved proposal
...
- Consumer project pulls latest `aexos-core` (via npm install / git submodule / vendored copy)
...
| Existing framework artifact is missing or insufficient | `@aexos-master` lacks `triage.routing_matrix` for squad consultation |
...
Lives in: `<project>/docs/audits/<finding-id>.yaml` initially. If approved as framework-candidate, copy to `aexos-core/audits/promoted/<finding-id>.yaml`.
...
audit_finding:
  ...
  date: "<ISO-8601>"
  auditor: "<agent-id or orchestrator>"
...
Lives in: `aexos-core/governance/proposals/PROP-<YYYYMMDD>-<slug>.yaml`.
...
framework_proposal:
  ...
  target:
    ...
    artifact_path: "<intended path in aexos-core>"
  ...
  approval:
    approver_decision: "PENDING|APPROVED|REJECTED|NEEDS_REVISION"
    approver_decision_at: "<ISO-8601 or null>"
    approver_decision_rationale: |
...
│    (auditor agent or Paulo Petruff)                                 │
└────────────────────────────┬────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────┐
...
└────────────────────────────┬────────────────────────────────┘
        ...
        false (scope-bound)          true (framework candidate)
              ...
              ▼                             ▼
       Project handles it          ┌─────────────────────┐
       internally; finding         │ 3. @aexos-master or   │
       archived in project.        │    domain specialist │
              ...
              │    aexos-core with │    with revision     close finding.
              │    changes        │    notes
              └────────┬─────────┘
                       │
                       ▼
...
audit_finding:
  ...
  framework_candidate_rationale: |
    Any DB-touching project under AEXOS faces this risk. The fix is not project-specific:
    1. Pre-flight hook on Edit/Write of supabase/migrations/*.sql
    2. Vocabulary contract pattern as framework convention
    3. aexos-master triage matrix to consult db-inventory before delegating
  ...
  proposed_disposition:
    - "Promote migration-dictionary-guard.cjs to aexos-core/hooks/ as framework hook"
    - "Add Article VII (Vocabulary Contract) to .aexos-core/constitution.md"
    - "Extend @aexos-master with triage.routing_matrix consulting db-inventory before migration tasks"
    - "Document Vocabulary Contract pattern in aexos-core/governance/patterns/"
```
...
framework_proposal:
  ...
  target:
    ...
    artifact_path: |
      Multiple:
      - aexos-core/.claude/rules/vocabulary-contract.md (NEW)
      - aexos-core/hooks/migration-dictionary-guard.cjs (NEW, copied + generalized)
      - aexos-core/.aexos-core/development/agents/aexos-master.md (MODIFY: add triage.routing_matrix)
      - aexos-core/governance/patterns/vocabulary-store.md (NEW pattern doc)
  ...
  migration_path:
    ...
    rollout_plan: |
      1. PR adds files to aexos-core (no existing files broken)
      2. business-ai-first pulls aexos-core update on next sync
      3. business-ai-first migration-dictionary-guard.cjs (project-local) is replaced by aexos-core version (vendor or symlink)
  ...
  cost_benefit:
    cost: "~3-4 hours engineering to generalize hook + write pattern doc + extend aexos-master"
    benefit: "Prevents the same failure mode in any future AEXOS project. Akita-quality fundamental."
    risk: "low — additive changes; no existing behavior modified"

  approval:
    approver_decision: "PENDING"
...
This is the artifact Paulo Petruff reviews. On `APPROVED`, the PR opens in `aexos-core`.

---

## Operational rules

1. **No framework change without an approved proposal.** Even if @aexos-master spots an opportunity, it MUST go through the pipeline. Paulo Petruff is the gate.
...
3. **Voice-clone opinions are evidence, not authority.** Akita/Montano/Kira are inputs to the proposal but cannot bypass Paulo Petruff.
...
- Multi-orchestrator approval (currently Paulo Petruff is sole approver; future may add @architect or @qa as co-approvers for non-strategic changes)
- Cross-organization adoption (this is internal to Paulo Petruff's projects for now)
- Versioning aexos-core releases (already handled by `CHANGELOG.md` and `package.json` version)

---

— Zeus (`@aexos-master`), formalizando o pipeline R&D contínuo do framework AEXOS por solicitação de Paulo Petruff
