...
COMMANDS:
  install           Install AEXOS in the current directory
  uninstall         Remove AEXOS from the current project
  init              Create and initialize a project directory
  info              Show system information
  doctor            Run installation diagnostics
  validate          Validate installation integrity
  update            Check or apply framework updates
  enterprise        Plan or apply Enterprise upgrades
  workers           Manage and discover workers
  manifest          Validate or regenerate manifests
  qa                Run and inspect quality gates
  mcp               Manage global MCP configuration
  migrate           Migrate earlier project structures
  generate          Generate documents from templates
  metrics           Record and inspect quality metrics
  config            Manage layered configuration
  pro               Manage AEXOS Pro licensing
  sdc               Run the lean full-SDC workflow
  wave              Plan and inspect wave execution
...
async function runUninstall(options = {}) {
  ...
  const existingItems = itemsToRemove.filter(item =>
    pathExistsWithoutFollowingLinks(path.join(cwd, item.path)),
  );
  ...
  // Dry run - stop here
  if (dryRun) {
    const preview = removeFootprint(cwd, existingItems, { dryRun: true });
    if (preview.failed.length > 0) {
      for (const failure of preview.failed) console.error(`  ✗ Cannot safely remove ${failure.path}: ${failure.message}`);
      process.exitCode = 1;
    }
    if (!quiet) {
      for (const preserved of preview.preserved) console.log(`  ↳ Preserving customized ${preserved}`);
    }
  ...
  const removal = removeFootprint(cwd, existingItems);
  if (!quiet) {
    for (const removed of removal.removed) console.log(`  ✓ Removed ${removed}`);
    for (const preserved of removal.preserved) console.log(`  ↳ Preserved customized ${preserved}`);
  }
  for (const failure of removal.failed) {
    console.error(`  ✗ Failed to remove ${failure.path}: ${failure.message}`);
  }
  ...
  // Summary
  if (!quiet) {
    if (removal.failed.length === 0) console.log('\n✅ AEXOS managed components have been uninstalled.');
    else console.log('\n⚠️  AEXOS uninstall is incomplete; see failures above.');
    if (keepData) {
      console.log('   Your project data in .aexos/ has been preserved.');
    }
    console.log('\n   To reinstall: npx @aexos/core install');
  }
  if (removal.failed.length > 0) process.exitCode = 1;
}
...
async function main() {
  switch (command) {
    ...
    case 'metrics':
    case 'migrate':
      // Commands implemented by the canonical Commander CLI.
      try {
        const { run } = require('../.aexos-core/cli/index.js');
        await run(process.argv);
      } catch (error) {
