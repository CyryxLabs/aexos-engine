 ...
 * 3. (INS-4.12) .aexos-core/package.json dependency completeness
 *
...
const parsedPackTimeoutMs = Number.parseInt(
  process.env.AEXOS_VALIDATE_PUBLISH_PACK_TIMEOUT_MS || '',
  10,
);
...
// Check 4 (INS-4.12): .aexos-core dependency completeness
console.log('');
console.log('--- Dependency Completeness (INS-4.12) ---\n');
try {
  const depValidatorPath = path.join(PROJECT_ROOT, 'scripts', 'validate-aexos-core-deps.js');
  if (fs.existsSync(depValidatorPath)) {
    ...
    console.log('PASS: .aexos-core dependency completeness validated');
  } else {
    console.log('SKIP: scripts/validate-aexos-core-deps.js not found');
  }
} catch (_depErr) {
  console.error('FAIL: .aexos-core dependency completeness check failed');
  console.error('  Fix: Run "node scripts/validate-aexos-core-deps.js" to see details');
  passed = false;
}

// Check 5 (#739 Bug 2 follow-up): .aexos-core/package.json namespace + version sync
//
...
const nsValidatorPath = path.join(PROJECT_ROOT, 'scripts', 'validate-aexos-core-namespace.js');
if (!fs.existsSync(nsValidatorPath)) {
  console.error('FAIL: scripts/validate-aexos-core-namespace.js is required but not found');
  ...
  try {
    ...
    console.error('FAIL: .aexos-core/package.json namespace/version drift detected');
    console.error('  Fix: Run "node scripts/validate-aexos-core-namespace.js" to see details');
    passed = false;
  }
}
