...
/**
 * AEXOS IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   cyryx ids:query {intent}     — Query registry for matching artifacts
 *   cyryx ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   cyryx ids:health             — Run registry health check (self-healing)
 *   cyryx ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   cyryx ids:impact {entity-id} — Impact analysis for modifications
 *   cyryx ids:stats              — Registry statistics
 *   cyryx ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.aexos-core', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.aexos-core', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.aexos-core', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.aexos-core', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.aexos-core', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
AEXOS IDS — Incremental Development System
...
Examples:
  cyryx ids:query "validate story drafts"
  cyryx ids:query "template rendering engine" --json
  cyryx ids:query "database migration" --type script
  cyryx ids:create-review
  cyryx ids:create-review --json
  cyryx ids:health
  cyryx ids:health --fix
  cyryx ids:check "validate yaml schema" --type task
  cyryx ids:impact create-doc
  cyryx ids:stats
  cyryx ids:stats --json
  cyryx ids:register .aexos-core/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: cyryx ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: cyryx ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: cyryx ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: cyryx ids:register {file-path}');
    process.exit(1);
  }
