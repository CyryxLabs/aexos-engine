...
/**
 * AEXOS Installation Wizard v5 (LEGACY)
...
/**
 * Smart path resolution for AEXOS Core modules
 */
function resolveCyryxCoreModule(modulePath) {
  const cyryxCoreModule = path.join(__dirname, '..', '.aexos-core', modulePath);

  const moduleExists =
    fs.existsSync(cyryxCoreModule + '.js') ||
    fs.existsSync(cyryxCoreModule + '/index.js') ||
    fs.existsSync(cyryxCoreModule);

  if (!moduleExists) {
    throw new Error(
      `Cannot find AEXOS Core module: ${modulePath}\n` +
        `Searched: ${cyryxCoreModule}\n` +
        'Please ensure aexos-core is installed correctly.'
    );
  }

  return require(cyryxCoreModule);
}

// Load AEXOS Core modules
const { detectRepositoryContext } = resolveCyryxCoreModule(
  'infrastructure/scripts/repository-detector'
);
// PM adapters imported but not used directly (loaded dynamically)
// const { ClickUpAdapter } = resolveCyryxCoreModule('utils/pm-adapters/clickup-adapter');
// const { GitHubProjectsAdapter } = resolveCyryxCoreModule('utils/pm-adapters/github-adapter');
// const { JiraAdapter } = resolveCyryxCoreModule('utils/pm-adapters/jira-adapter');
...
async function writeCursorMdcAgent(sourcePath, targetPath) {
  try {
    const agentParser = resolveCyryxCoreModule('infrastructure/scripts/ide-sync/agent-parser');
    const cursorTransformer = resolveCyryxCoreModule('infrastructure/scripts/ide-sync/transformers/cursor');
...
async function main() {
  ...
  const installedManifestPath = path.join(projectRoot, '.aexos-core', '.installed-manifest.yaml');
  const hasExistingInstall = fs.existsSync(installedManifestPath);

  if (hasExistingInstall && brownfieldUpgrader) {
    console.log(chalk.yellow('🔄 Existing AEXOS installation detected!'));
    console.log('');

    const sourceDir = path.join(context.frameworkLocation, '.aexos-core');
      ...
      if (upgradeChoice === 'upgrade') {
        ...
        if (result.success) {
          ...
          brownfieldUpgrader.updateInstalledManifest(
            projectRoot,
            sourceManifest,
            `aexos-core@${packageJson.version}`
          );
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using AEXOS?'),
      choices: [
        {
          name: '  Using AEXOS in a project ' + chalk.gray('(Framework files added to .gitignore)'),
          value: 'project-development',
        },
        {
          name:
            '  Developing AEXOS framework itself ' + chalk.gray('(Framework files are source code)'),
  ...
  const configPath = path.join(context.projectRoot, '.aexos-installation-config.yaml');
  // INS-2 Performance: Use async write instead of sync
  ...
  // Step 4b: Copy AEXOS Core files
  console.log('');

  const sourceCoreDir = path.join(context.frameworkLocation, '.aexos-core');
  const targetCoreDir = path.join(context.projectRoot, '.aexos-core');

  if (fs.existsSync(sourceCoreDir)) {
    // INS-2 Performance: Add spinner for file copy (AC9)
    const copySpinner = ora('Installing AEXOS Core files...').start();
    await fse.copy(sourceCoreDir, targetCoreDir);
    copySpinner.succeed(
      'AEXOS Core files installed ' +
    ...
    if (ensureProjectNodeModulesLink) {
      const linkResult = await ensureProjectNodeModulesLink({
        ...
        targetCyryxCore: targetCoreDir,
      });
      if (linkResult.linked) {
        console.log(chalk.green('✓') + ' node_modules linked to .aexos-core/node_modules');
    ...
    console.error(chalk.red('✗') + ' AEXOS Core files not found');
    process.exit(1);
  }
  ...
  if (!ides.includes('none')) {
    ...
    const ideRulesMap = {
      ...
      cursor: { source: 'cursor-rules.md', target: '.cursor/rules/aexos-global.mdc' },
      gemini: { source: 'gemini-rules.md', target: '.gemini/rules.md' },
      'github-copilot': { source: 'copilot-rules.md', target: '.github/chatmodes/aexos-agent.md' },
    ...
    // Step 2: Install AEXOS Core agents and tasks for Claude Code
    ...
    if (ides.includes('claude')) {
      ...
      // Create AEXOS README for Claude Code
      const cyryxsReadme = path.join(
        context.projectRoot,
        '.claude',
        'commands',
        'CYRYX',
      ...
      await fse.ensureDir(path.dirname(cyryxsReadme));
      await fse.writeFile(
        cyryxsReadme,
        `# AEXOS Core Commands

This directory contains the core AEXOS agents and tasks.
...
## Documentation
See .aexos-core/user-guide.md for complete documentation.
`
      );
    ...
    // Step 3: Install AEXOS Core agents for Cursor
    ...
    if (ides.includes('cursor')) {
      ...
      // Create AEXOS README for Cursor
...
This directory contains the core AEXOS agent rules for Cursor.
...
`
    ...
    // Step 4: Install AEXOS Core agents for other IDEs (Gemini, AntiGravity)
    // v4: Agents are in development/ module
    // INS-2 Performance: Uses cached agent files list
    ...
    if (ides.includes('github-copilot')) {
      ...
      if (cachedAgentFiles.length > 0) {
        ...
        for (const agentFile of cachedAgentFiles) {
          ...
          const targetPath = path.join(copilotModesDir, `aexos-${agentName}.md`);
          await fse.copy(sourcePath, targetPath);
        }
  ...
  const possibleSquadsDirs = [
    ...
    path.join(context.projectRoot, 'node_modules', 'aexos-core', 'squads'),
    path.join(context.projectRoot, 'node_modules', '@cyryx', 'fullstack', 'squads'),
  ];
  ...
  try {
    ...
    const validator = new PostInstallValidator(context.projectRoot, context.frameworkLocation, {
      ...
      // For production integrity checks, users should run `aexos validate` which
    ...
    if (
      ...
      console.log(
        chalk.yellow('   Run ') +
          chalk.cyan('aexos validate --repair') +
    ...
    if (process.env.DEBUG || process.env.AEXOS_DEBUG) {
      console.log(chalk.dim(`   Stack: ${validationError.stack}`));
    }
    console.log(chalk.dim('   Run `aexos validate` to check installation integrity'));
  }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ AEXOS installation complete! 🎉'));
  console.log('');
  ...
  console.log('  ' + chalk.dim('.aexos-core/') + '           - Framework core files');
  ...
  if (ides.includes('cursor')) {
    ...
    console.log('    ' + chalk.dim('├─ rules/aexos-global.mdc') + ' - Main configuration');
    console.log('    ' + chalk.dim('└─ rules/'));
    console.log('      ' + chalk.dim('  ├─ CYRYX/') + '         - Core agent rules');
    if (selectedSquads && selectedSquads.length > 0) {
      selectedSquads.forEach((squad) => {
  ...
  if (ides.includes('claude')) {
    ...
    console.log('    • Browse: .claude/commands/AEXOS/agents/ for all available agents');
  }
  ...
  if (ides.includes('antigravity')) {
    ...
    console.log('    • Browse: .antigravity/rules/AEXOS/agents/ for all available agents');
  }

  console.log('  ' + chalk.yellow('General:'));
  console.log('    • Run ' + chalk.yellow('aexos validate') + ' to verify installation integrity');
  console.log('    • Run ' + chalk.yellow('aexos validate --repair') + ' to fix any missing files');
  console.log('    • Check .aexos-core/user-guide.md for complete documentation');
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# CYRYX-FullStack Framework Files (auto-managed - do not edit)',
      '.aexos-core/',
      'node_modules/@cyryx/',
      'outputs/minds/',
      '.aexos-installation-config.yaml',
      '# End CYRYX-FullStack auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# CYRYX-FullStack Framework Files');
...
async function savePMConfig(pmTool, config, projectRoot) {
  ...
  const configPath = path.join(projectRoot, '.aexos-pm-config.yaml');
  // INS-2 Performance: Use async write
 ...
 * @param {string} sourceCoreDir - Path to installed .aexos-core directory
 */
