...
async function installProjectMCPs(options = {}) {
  ...
  // Create .aexos directory for logs
  const cyryxDir = path.join(projectPath, '.aexos');
  await fse.ensureDir(cyryxDir);

  const logPath = path.join(cyryxDir, 'install-log.txt');
  const errorLogPath = path.join(cyryxDir, 'install-errors.log');
...
function displayInstallationStatus(results) {
  ...
  console.log(chalk.gray('📋 Installation log: .aexos/install-log.txt'));
  console.log('');
}
