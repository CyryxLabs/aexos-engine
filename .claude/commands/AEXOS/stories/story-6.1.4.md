...
- STEP 3: |
    ...
    1. Execute: node .aexos-core/scripts/generate-greeting.js {agent-id}
...
Add to `.aexos-core/scripts/agent-config-loader.js`:
...
async loadAgentDefinition(options = {}) {
  ...
  // Load from file
  const agentPath = path.join(process.cwd(), '.aexos-core', 'agents', `${this.agentId}.md`);
...
1. Add deprecation warning to `.aexos-core/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .aexos-core/
grep -r "from.*config-loader" .aexos-core/
```
...
1. `.aexos-core/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.aexos-core/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.aexos-core/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.aexos-core/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .aexos-core/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aexos-core/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aexos-core/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aexos-core/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .aexos-core/scripts/batch-integrate-greeting-builder.js ] && rm .aexos-core/scripts/batch-integrate-greeting-builder.js
   [ -f .aexos-core/scripts/update-activation-instructions.js ] && rm .aexos-core/scripts/update-activation-instructions.js
   ```
...
Modify `.aexos-core/scripts/greeting-builder.js`:
...
Create `.aexos-core/scripts/generate-greeting.js`:
...
async function generateGreeting(agentId) {
  ...
  try {
    // Load core config
    const coreConfigPath = path.join(process.cwd(), '.aexos-core', 'core-config.yaml');
...
**Pilot Agent:**
- `qa.md` (Argus - Test Architect & Quality Advisor)
...
- STEP 3: |
    ...
    1. Execute: node .aexos-core/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
2. **Existing Session Test:**
   ...
   node -e "const fs = require('fs'); const path = require('path'); const sessionPath = path.join(process.cwd(), '.aexos', 'session-state.json'); fs.mkdirSync(path.dirname(sessionPath), { recursive: true }); fs.writeFileSync(sessionPath, JSON.stringify({ sessionId: 'test-123', startTime: new Date().toISOString(), lastActivity: new Date().toISOString(), agentSequence: [{ id: 'po', name: 'Themis' }], lastCommands: ['create-story'], workflowActive: null }), 'utf8');"
...
3. **Direct Script Test:**
   ...
   node .aexos-core/scripts/generate-greeting.js qa
...
4. **Error Handling Test:**
   ...
   node .aexos-core/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .aexos-core/scripts/generate-greeting.js qa
...
Create script `.aexos/temp-update-remaining-agents.js`:

```javascript
const fs = require('fs');
const path = require('path');

const AGENTS_DIR = path.join(process.cwd(), '.aexos-core', 'agents');
const REMAINING_AGENTS = [
  'dev', 'po', 'sm', 'pm', 'architect', 'analyst',
  'data-engineer', 'devops', 'aexos-master', 'ux-design-expert'
];
...
for agent in dev po sm pm architect analyst data-engineer devops aexos-master ux-design-expert; do
  echo "Testing $agent..."
  node .aexos-core/scripts/generate-greeting.js $agent
  echo "---"
done
```
...
Create `.aexos-core/scripts/command-execution-hook.js`:
 ...
 * 
 * @example
 * updateSessionAfterCommand('qa', 'Argus', 'analyze-framework');
 */
...
Create `.aexos-core/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.aexos-core/scripts/generate-greeting');
...
- `.aexos-core/scripts/generate-greeting.js` (Unified greeting generator)
- `.aexos-core/scripts/command-execution-hook.js` (Session update hook)
- `.aexos-core/docs/session-update-pattern.md` (Documentation)
- `tests/unit/generate-greeting.test.js` (Unit tests)
- `tests/integration/greeting-system-integration.test.js` (Integration tests)

### Files Modified

- `.aexos-core/scripts/agent-config-loader.js` (Expanded with `loadAgentDefinition()` and `loadComplete()`)
- `.aexos-core/scripts/greeting-builder.js` (Modified to accept pre-loaded context)
- `.aexos-core/scripts/config-loader.js` (Deprecated warning added)
- `.aexos-core/agents/qa.md` (Pilot - STEP 3 updated first)
- `.aexos-core/agents/*.md` (Remaining 10 agents - STEP 3 updated after QA validation)

### Files Deleted

- `.aexos-core/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.aexos-core/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.aexos-core/scripts/update-activation-instructions.js` (Temporary migration script)
- `.aexos-core/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.aexos-core/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.aexos-core/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.aexos-core/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.aexos-core/scripts/context-detector.js` (Used by greeting-builder.js)
- `.aexos-core/scripts/workflow-navigator.js` (Used by greeting-builder.js)
...
**Output:**
```
✅ Argus (Guardian) ready. Let's ensure quality!
...
|------|---------|---------|--------|
| 2025-01-17 | 4.1 | Added CodeRabbit Integration section, clarified migration scripts deletion process, verified test directory structure | Themis (po) |
| 2025-01-17 | 4.0 | Expanded to unified system integration, integrated all analyses, added script consolidation, added session updates | Themis (po) + Vega (architect) |
| 2025-01-16 | 3.0 | Validation improvements, integration tests added | Themis (po) |
| 2025-01-15 | 2.0 | Major rewrite to integrate with Story 6.1.2.5 | Argus (qa) |
